...
select *
 ...
 order by 2 desc, 3 desc, 1 desc;');
    ...
    BEGIN
        ...
        /* Data validation */
        SELECT 'DT_CARE_PLAN_TASK_HIST = ' || mig_data.dt_care_plan_task_hist || 'ID_CARE_PLAN_TASK = ' ||
               mig_data.id_care_plan_task || ' ID_ITEM = ' || mig_data.id_item BULK COLLECT
          INTO l_not_mig_care_plan_task
          FROM (SELECT cpth.dt_care_plan_task_hist,
                       cpth.id_care_plan_task,
                       cpth.id_item,
                       to_number(substr(cpth.id_item, 1, instr(cpth.id_item, '|') - 1)) AS id_analysis,
                       to_number(substr(cpth.id_item, instr(cpth.id_item, '|') + 1)) AS id_sample_type
