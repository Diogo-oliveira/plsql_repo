...
BEGIN
    FOR rec IN (SELECT erd.id_exam_req_det,
                       ...
                       erd.id_prof_order,
                       erd.dt_order,
                       nvl(erd.id_order_type, l_def_order_type) id_order_type,
                       erd.flg_co_sign,
                       erd.id_prof_co_sign,
                       erd.dt_co_sign,
                       erd.notes_co_sign
                  ...
                  JOIN episode e
                    ON (e.id_episode = nvl(er.id_episode, er.id_episode_origin))
                  JOIN epis_info ei
                    ON ei.id_episode = e.id_episode
                 WHERE erd.id_prof_order IS NOT NULL
                   AND erd.dt_order IS NOT NULL
                   AND erd.id_co_sign_order IS NULL
                   AND erd.id_co_sign_cancel IS NULL)
    LOOP
        l_co_sign := NULL;
    
        --CREATE NEW CO_SIGN
        IF NOT pk_co_sign_api.set_pending_co_sign_task(i_lang                   => l_lang,
                                                       i_prof                   => profissional(rec.id_prof_req,
                                                                                                rec.id_institution,
                                                                                                rec.id_software),
                                                       i_episode                => rec.id_episode,
                                                       i_id_task_type           => CASE
                                                                                       WHEN rec.flg_type = 'I' THEN
                                                                                        7
                                                                                       ELSE
                                                                                        8
                                                                                   END,
                                                       i_cosign_def_action_type => 'NEEDS_COSIGN_ORDER',
                                                       i_id_task                => rec.id_exam_req_det,
                                                       i_id_task_group          => rec.id_exam_req_det,
                                                       i_id_order_type          => rec.id_order_type,
                                                       i_id_prof_created        => rec.id_prof_req,
        ...
        THEN
            handle_error(i_msg   => 'ERROR CALLING PK_CO_SIGN.SET_COSIGN_TASK(' || l_lang || --
                                    ', profissional(' || rec.id_prof_req || ', ' || rec.id_institution || ', ' ||
                                    rec.id_software || '), ' || --
                                    rec.id_episode || ', ' || --
                                    rec.flg_type || ', ' || --
                                    'NEEDS_COSIGN_ORDER' || ', ' || --
        ...
        UPDATE exam_req_det erd
           SET erd.id_co_sign_order = l_co_sign
...
BEGIN
    FOR rec IN (SELECT erd.id_exam_req_det,
                  ...
                  JOIN epis_info ei
                    ON ei.id_episode = e.id_episode
                 WHERE erd.id_co_sign_order IS NOT NULL
                   AND erd.id_prof_order IS NOT NULL
                   AND erd.dt_order IS NOT NULL
                   AND (erd.id_order_type IS NOT NULL OR
                       (erd.id_prof_order != er.id_prof_req AND erd.id_order_type IS NULL)))
    LOOP
        ...
        --Update Transactional table with id_co_sign
        UPDATE exam_req_det erd
           SET erd.id_co_sign_order = l_co_sign_hist
         WHERE erd.id_exam_req_det = rec.id_exam_req_det;
...
BEGIN
    FOR rec IN (SELECT erd.id_exam_req_det,
                  ...
                  JOIN epis_info ei
                    ON ei.id_episode = e.id_episode
                 WHERE erd.id_co_sign_order IS NULL
                   AND erd.id_prof_order IS NOT NULL
                   AND erd.dt_order IS NOT NULL
                   AND (erd.id_order_type IS NOT NULL OR
                       (erd.id_prof_order != er.id_prof_req AND erd.id_order_type IS NULL)))
