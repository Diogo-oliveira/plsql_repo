 ...
 order by 2 desc, 3 desc, 1 desc;');
        ...
        LOOP
            ...
            THEN
                SELECT COUNT(1)
                  ...
                  FROM bed_hist bh
                 WHERE bh.id_bed_hist = l_tab_bed_hist(rec).id_bed_hist;
                
                IF (l_count_dcs_ids = 0)
                THEN
                   l_not_migrated_ids_dcs := l_not_migrated_ids_dcs || to_char(l_tab_bed_hist(rec).id_bed_hist) || ' ';
                END IF;
            END IF;
    ...
    EXCEPTION
        WHEN e_has_findings THEN
            log_error('ERROR ON BED HIST MIGRATION. NOT migrated dcs_ids: ' || l_not_migrated_ids_dcs);
