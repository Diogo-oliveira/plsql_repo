...
DECLARE
    ...
    CURSOR c_get_records IS
        SELECT ed.id_epis_documentation,
         ...
         WHERE ed.id_doc_area IN (39, 5592, 6607, 6608, 6680, 6681, 280077)
    ...
    BEGIN
        ...
        SELECT CASE
                    WHEN template_title IS NOT NULL THEN
                     template_title || chr(10) || CASE
                         WHEN desc_hcn IS NOT NULL THEN
                          desc_hcn || chr(10)
                         ELSE
                          ''
                     END
          ...
          FROM (
               ...
               -- Touch-option entries
                   SELECT tot.id_epis_documentation,
                          ...
                          CASE
                               WHEN e.id_doc_area = 39 THEN
                                (SELECT pk_message.get_message(l_id_language, 'HCN_LABEL_T005') || ': ' || he.total_points || CASE
                                             WHEN hdp.num_hcn IS NOT NULL THEN
                                              chr(10) || pk_message.get_message(l_id_language, 'HCN_LABEL_T006') || ' ' ||
                                              hdp.num_hcn
                                         END desc_class
                                   FROM epis_documentation ed
                                  INNER JOIN hcn_eval he
                                   ...
                                   LEFT JOIN hcn_def_points hdp
                                    ...
                                    AND nvl(pk_hcn.get_eval_total_points(ed.id_epis_documentation), 0) BETWEEN
                                        hdp.points_min_value AND hdp.points_max_value
                                 
                                  WHERE ed.id_epis_documentation = tot.id_epis_documentation
                                    AND rownum = 1)
                               ELSE
                                ''
                           END desc_hcn,
...
BEGIN
    ...
    UPDATE epis_documentation ed
     ...
     WHERE ed.id_doc_area = 6607;

    UPDATE task_timeline_ea t
       SET t.id_doc_area = 5592
     WHERE t.id_doc_area = 6607;

    --breast-feading assessment
    UPDATE epis_documentation ed
       SET ed.id_doc_area = 5592
     WHERE ed.id_doc_area = 6608;

    UPDATE task_timeline_ea t
       SET t.id_doc_area = 5592
     WHERE t.id_doc_area = 6608;

    --shift assessment
    UPDATE epis_documentation ed
       SET ed.id_doc_area = 5592
     WHERE ed.id_doc_area = 6680;

    UPDATE task_timeline_ea t
       SET t.id_doc_area = 5592
     WHERE t.id_doc_area = 6680;

    --neonatal assessment
    UPDATE documentation d
       SET d.id_doc_area = 5592
     WHERE d.id_doc_area = 6607;

    INSERT INTO doc_template_area
        (id_doc_template, id_doc_area, action_subject)
        SELECT dta.id_doc_template, 5592 id_doc_area, dta.action_subject
          FROM doc_template_area dta
         WHERE dta.id_doc_area = 6607
           AND NOT EXISTS (SELECT 1
                  FROM doc_template_area d
                 WHERE d.id_doc_area = 5592
                   AND d.id_doc_template = dta.id_doc_template);

    UPDATE doc_template_area_doc dtad
       SET dtad.id_doc_area = 5592
     WHERE dtad.id_doc_area = 6607;

    DELETE FROM doc_template_area dta
     WHERE dta.id_doc_area = 6607;

    --Breast-feeding assessment   
    UPDATE documentation d
       SET d.id_doc_area = 5592
     WHERE d.id_doc_area = 6608;

    INSERT INTO doc_template_area
        (id_doc_template, id_doc_area, action_subject)
        SELECT dta.id_doc_template, 5592 id_doc_area, dta.action_subject
          FROM doc_template_area dta
         WHERE dta.id_doc_area = 6608
           AND NOT EXISTS (SELECT 1
                  FROM doc_template_area d
                 WHERE d.id_doc_area = 5592
                   AND d.id_doc_template = dta.id_doc_template);
    ...
    UPDATE documentation d
       SET d.id_doc_area = 5592
     WHERE d.id_doc_area = 6680;

    INSERT INTO doc_template_area
        (id_doc_template, id_doc_area, action_subject)
        SELECT dta.id_doc_template, 5592 id_doc_area, dta.action_subject
          FROM doc_template_area dta
         WHERE dta.id_doc_area = 6680
           AND NOT EXISTS (SELECT 1
                  FROM doc_template_area d
                 WHERE d.id_doc_area = 5592
                   AND d.id_doc_template = dta.id_doc_template);
    ...
    UPDATE doc_template_area_doc dtad
       SET dtad.id_doc_area = 5592
     WHERE dtad.id_doc_area = 6680;

    DELETE FROM doc_template_area dta
     WHERE dta.id_doc_area = 6680;

    --shift assessment
    UPDATE doc_template_context d
       SET d.id_context = 5592
     WHERE d.id_context = 6608
       AND d.flg_type = 'D';
