/*-- Last Change Revision: $Rev: 2028016 $*/
/*-- Last Change by: $Author: mario.fernandes $*/
/*-- Date of last change: $Date: 2022-08-02 18:43:57 +0100 (ter, 02 ago 2022) $*/

CREATE OR REPLACE PACKAGE BODY TS_DONOR_CONTAG_DISEASE
/*
| Generated by or retrieved from QCGU - DO NOT MODIFY!
| QCGU - "Get it right, do it fast" - www.ToadWorld.com
| QCGU Universal ID: {657BD35E-2E48-4CD4-B14E-41420B8433B3}
| Created On: Junho 10, 2010 16:2:57
| Created By: ALERT
*/
IS
   ...
   BEGIN
     ...
     INSERT INTO DONOR_CONTAG_DISEASE (
        ...
        BEGIN
           ...
           THEN
              ...
              pk_alert_exceptions.add_context (
                 err_instance_id_in => l_err_instance_id
                ,NAME_IN => 'TABLE_NAME'
                ,value_in => 'DONOR_CONTAG_DISEASE'
                ,validate_in => FALSE
                );
              pk_alert_exceptions.add_context (
                 err_instance_id_in => l_err_instance_id
                ,NAME_IN => 'ID_DONOR_CONTAG_DISEASE'
                ,value_in => id_donor_contag_disease_in
                ,validate_in => FALSE
                );
           ...
           ELSE
              pk_alert_exceptions.raise_error (
                    ...
                    ,value2_in => l_name
                    ,name3_in => 'TABLE_NAME'
                    ,value3_in => 'DONOR_CONTAG_DISEASE');
           END IF;
        END;
        END IF;
      ...
      THEN
        ...
        BEGIN
           ...
           pk_alert_exceptions.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'DONOR_CONTAG_DISEASE');
           IF l_name = 'DCD_OD_FK'
           THEN
   ...
   IS
      l_rec DONOR_CONTAG_DISEASE%ROWTYPE := rec_in;
   BEGIN
      ins (
   ...
   BEGIN
      ...
      ELSE
         FORALL indx IN rows_in.FIRST .. rows_in.LAST
            SAVE EXCEPTIONS
            INSERT INTO DONOR_CONTAG_DISEASE VALUES rows_in (indx) RETURNING ROWID BULK COLLECT INTO rows_out;
      END IF;
   EXCEPTION
     WHEN e_forall_error
     THEN
     ...
     THEN
       ...
       pk_alert_exceptions.raise_error(
          error_name_in => 'FORALL-INSERT-FAILURE'
          ,name1_in => 'TABLE_NAME'
          ,value1_in => 'DONOR_CONTAG_DISEASE'
          ,name2_in => 'ROW_COUNT'
...
PROCEDURE upd (
   ...
   l_UPDATE_INSTITUTION_n := sys.diutil.bool_to_int(UPDATE_INSTITUTION_nin);
         ...
         UPDATE DONOR_CONTAG_DISEASE SET
...
PROCEDURE upd (
   ...
   l_UPDATE_TIME_n NUMBER(1);
   l_UPDATE_INSTITUTION_n NUMBER(1);
      id_organ_donor_in DONOR_CONTAG_DISEASE.ID_ORGAN_DONOR%TYPE;
      id_pat_history_diagnosis_in DONOR_CONTAG_DISEASE.ID_PAT_HISTORY_DIAGNOSIS%TYPE;
   BEGIN
...
l_sql := 'UPDATE DONOR_CONTAG_DISEASE SET '
     || ' ID_DONOR_CONTAG_DISEASE = decode (' || l_ID_DONOR_CONTAG_DISEASE_n || ',0,:id_donor_contag_disease_in, NVL (:id_donor_contag_disease_in, ID_DONOR_CONTAG_DISEASE)) '|| ','
     || ' CREATE_USER = decode (' || l_CREATE_USER_n || ',0,:create_user_in, NVL (:create_user_in, CREATE_USER)) '|| ','
     || ' CREATE_TIME = decode (' || l_CREATE_TIME_n || ',0,:create_time_in, NVL (:create_time_in, CREATE_TIME)) '|| ','
     || ' CREATE_INSTITUTION = decode (' || l_CREATE_INSTITUTION_n || ',0,:create_institution_in, NVL (:create_institution_in, CREATE_INSTITUTION)) '|| ','
...
PROCEDURE upd (
   ...
   BEGIN
      RETURN
         'BEGIN UPDATE DONOR_CONTAG_DISEASE
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE DONOR_CONTAG_DISEASE set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE DONOR_CONTAG_DISEASE set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
   ...
   IS
   BEGIN
      DELETE FROM DONOR_CONTAG_DISEASE
       WHERE
          ID_ORGAN_DONOR = id_organ_donor_in AND
          ID_PAT_HISTORY_DIAGNOSIS = id_pat_history_diagnosis_in
       RETURNING ROWID BULK COLLECT INTO rows_out
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM DONOR_CONTAG_DISEASE
       WHERE
          ID_DONOR_CONTAG_DISEASE = id_donor_contag_disease_in
      RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_DCD_DCD_UK (
   ...
   BEGIN
      DELETE FROM DONOR_CONTAG_DISEASE
       WHERE
          ID_ORGAN_DONOR = del_DCD_OD_FK.id_organ_donor_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_DCD_OD_FK (
   ...
   BEGIN
      DELETE FROM DONOR_CONTAG_DISEASE
       WHERE
          ID_PAT_HISTORY_DIAGNOSIS = del_DCD_PHD_FK.id_pat_history_diagnosis_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_DCD_PHD_FK (
   ...
   BEGIN
      IF where_in IS NULL
      THEN
         RETURN 'DELETE FROM DONOR_CONTAG_DISEASE';
      ELSE
         RETURN
            'DELETE FROM DONOR_CONTAG_DISEASE WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   PROCEDURE initrec (
      donor_contag_disease_inout IN OUT DONOR_CONTAG_DISEASE%ROWTYPE
   )
   IS
   BEGIN
      donor_contag_disease_inout.ID_DONOR_CONTAG_DISEASE := NULL;
      donor_contag_disease_inout.ID_ORGAN_DONOR := NULL;
      donor_contag_disease_inout.ID_PAT_HISTORY_DIAGNOSIS := NULL;
      donor_contag_disease_inout.CREATE_USER := NULL;
      donor_contag_disease_inout.CREATE_TIME := NULL;
      donor_contag_disease_inout.CREATE_INSTITUTION := NULL;
      donor_contag_disease_inout.UPDATE_USER := NULL;
      donor_contag_disease_inout.UPDATE_TIME := NULL;
      donor_contag_disease_inout.UPDATE_INSTITUTION := NULL;
   END initrec;

   FUNCTION initrec RETURN DONOR_CONTAG_DISEASE%ROWTYPE
   IS
      l_donor_contag_disease DONOR_CONTAG_DISEASE%ROWTYPE;
   BEGIN
   ...
   FUNCTION get_data_rowid(
        ...
        ) RETURN DONOR_CONTAG_DISEASE_tc
   IS
        data DONOR_CONTAG_DISEASE_tc;
   BEGIN
        select * bulk collect into data from DONOR_CONTAG_DISEASE where rowid in (select /*+opt_estimate(table,t,scale_rows=0.0000001))*/ * from table(rows_in) t);
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
