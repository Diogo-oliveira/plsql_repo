...
CREATE OR REPLACE PACKAGE BODY TS_DOC_MACRO_PROF
/*
| Generated by or retrieved from QCGU - DO NOT MODIFY!
| QCGU - "Get it right, do it fast" - www.ToadWorld.com
| QCGU Universal ID: {657BD35E-2E48-4CD4-B14E-41420B8433B3}
| Created On: Março 30, 2012 17:1:5
| Created By: ALERT
*/
IS
      ...
      THEN
        ...
        BEGIN
           ...
           pk_alert_exceptions.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'DOC_MACRO_PROF');
           IF l_name = 'DCMP_DCM_FK'
           THEN
   ...
   IS
      l_rec DOC_MACRO_PROF%ROWTYPE := rec_in;
   BEGIN
      IF gen_pky_in THEN
         l_rec.ID_DOC_MACRO_PROF := next_key (sequence_in);
      END IF;
      ins (
  ...
  ins (
   ...
   IS
     retval DOC_MACRO_PROF.ID_DOC_MACRO_PROF%TYPE;

   BEGIN
      IF sequence_in IS NULL
      THEN
         SELECT seq_DOC_MACRO_PROF.NEXTVAL INTO retval FROM dual;
      ELSE
         EXECUTE IMMEDIATE
   ...
   EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
           ...
           ,name1_in => 'SEQUENCE'
           ,value1_in => NVL (sequence_in, 'seq_DOC_MACRO_PROF')
           );
   END next_key;
   ...
   IS
        l_pky DOC_MACRO_PROF.ID_DOC_MACRO_PROF%TYPE := next_key;
   BEGIN
      ins (
         id_doc_macro_prof_in => l_pky,
         id_doc_macro_in => id_doc_macro_in,
         id_professional_in => id_professional_in,
         flg_status_in => flg_status_in,
         dt_creation_in => dt_creation_in,
   ...
   BEGIN
      ...
      update_time_in => update_time_in,
      update_institution_in => update_institution_in,
      id_doc_macro_prof_out => id_doc_macro_prof_out
      ,handle_error_in => handle_error_in
      , rows_out => rows_out
   );
   END ins;
   ...
   BEGIN
      ...
      ELSE
         FORALL indx IN rows_in.FIRST .. rows_in.LAST
            SAVE EXCEPTIONS
            INSERT INTO DOC_MACRO_PROF VALUES rows_in (indx) RETURNING ROWID BULK COLLECT INTO rows_out;
      END IF;
   EXCEPTION
     WHEN e_forall_error
     THEN
...
PROCEDURE upd (
   ...
   l_UPDATE_INSTITUTION_n := sys.diutil.bool_to_int(UPDATE_INSTITUTION_nin);
     ...
     UPDATE_INSTITUTION = decode (l_UPDATE_INSTITUTION_n,0,update_institution_in, NVL (update_institution_in, UPDATE_INSTITUTION))
          WHERE
             ID_DOC_MACRO_PROF = id_doc_macro_prof_in
...
PROCEDURE upd (
   ...
   l_UPDATE_TIME_n NUMBER(1);
   l_UPDATE_INSTITUTION_n NUMBER(1);
      id_doc_macro_prof_in DOC_MACRO_PROF.ID_DOC_MACRO_PROF%TYPE;
   BEGIN
...
l_sql := 'UPDATE DOC_MACRO_PROF SET '
     || ' ID_DOC_MACRO = decode (' || l_ID_DOC_MACRO_n || ',0,:id_doc_macro_in, NVL (:id_doc_macro_in, ID_DOC_MACRO)) '|| ','
     || ' ID_PROFESSIONAL = decode (' || l_ID_PROFESSIONAL_n || ',0,:id_professional_in, NVL (:id_professional_in, ID_PROFESSIONAL)) '|| ','
     || ' FLG_STATUS = decode (' || l_FLG_STATUS_n || ',0,:flg_status_in, NVL (:flg_status_in, FLG_STATUS)) '|| ','
     || ' DT_CREATION = decode (' || l_DT_CREATION_n || ',0,:dt_creation_in, NVL (:dt_creation_in, DT_CREATION)) '|| ','
...
PROCEDURE upd (
      ...
      THEN
         ...
         FORALL i IN col_in.FIRST .. col_in.LAST
            UPDATE DOC_MACRO_PROF SET
             ...
             WHERE
                ID_DOC_MACRO_PROF = l_ID_DOC_MACRO_PROF(i)
          returning rowid bulk collect into rows_out;
      ELSE
         FORALL i IN col_in.FIRST .. col_in.LAST
            UPDATE DOC_MACRO_PROF SET
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE DOC_MACRO_PROF set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE DOC_MACRO_PROF set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
...
del_DCMP_UK_IDX (
   ...
   BEGIN
      DELETE FROM DOC_MACRO_PROF
       WHERE
          ID_DOC_MACRO = del_DCMP_DCM_FK.id_doc_macro_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_DCMP_DCM_FK (
   ...
   BEGIN
      DELETE FROM DOC_MACRO_PROF
       WHERE
          ID_PROFESSIONAL = del_DCMP_PROF_FK.id_professional_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_DCMP_PROF_FK (
   ...
   BEGIN
      IF where_in IS NULL
      THEN
         RETURN 'DELETE FROM DOC_MACRO_PROF';
      ELSE
         RETURN
            'DELETE FROM DOC_MACRO_PROF WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   IS
   BEGIN
      doc_macro_prof_inout.ID_DOC_MACRO_PROF := NULL;
      doc_macro_prof_inout.ID_DOC_MACRO := NULL;
      doc_macro_prof_inout.ID_PROFESSIONAL := NULL;
      doc_macro_prof_inout.FLG_STATUS := NULL;
      doc_macro_prof_inout.DT_CREATION := NULL;
      doc_macro_prof_inout.CREATE_USER := NULL;
      doc_macro_prof_inout.CREATE_TIME := NULL;
      doc_macro_prof_inout.CREATE_INSTITUTION := NULL;
      doc_macro_prof_inout.UPDATE_USER := NULL;
      doc_macro_prof_inout.UPDATE_TIME := NULL;
      doc_macro_prof_inout.UPDATE_INSTITUTION := NULL;
   END initrec;

   FUNCTION initrec RETURN DOC_MACRO_PROF%ROWTYPE
   IS
      l_doc_macro_prof DOC_MACRO_PROF%ROWTYPE;
   BEGIN
      RETURN l_doc_macro_prof;
   END initrec;
   ...
   IS
        data DOC_MACRO_PROF_tc;
   BEGIN
        select * bulk collect into data from DOC_MACRO_PROF where rowid in (select /*+opt_estimate(table,t,scale_rows=0.0000001))*/ * from table(rows_in) t);
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
