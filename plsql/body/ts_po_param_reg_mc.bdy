...
CREATE OR REPLACE PACKAGE BODY TS_PO_PARAM_REG_MC
/*
| Generated by or retrieved from QCGU - DO NOT MODIFY!
| QCGU - "Get it right, do it fast" - www.ToadWorld.com
| QCGU Universal ID: {7A44F965-7C77-42EE-9A04-269EF006213B}
| Created On: Agosto 16, 2013 16:12:17
| Created By: ALERT
*/
IS
      ...
      THEN
        ...
        BEGIN
           ...
           pk_alert_exceptions.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'PO_PARAM_REG_MC');
           IF l_name = 'POPRMC_POPMC_FK'
           THEN
              -- Add a context value for each column
              pk_alert_exceptions.add_context (
                 err_instance_id_in => l_id
               , name_in => 'ID_PO_PARAM_MC'
               , value_in => id_po_param_mc_in);
           END IF;
           IF l_name = 'POPRMC_POPR_FK'
           THEN
              -- Add a context value for each column
              pk_alert_exceptions.add_context (
                 err_instance_id_in => l_id
               , name_in => 'ID_PO_PARAM_REG'
               , value_in => id_po_param_reg_in);
           END IF;
   ...
   BEGIN
     ...
     ins (
      ...
      id_po_param_mc_in => id_po_param_mc_in
      ,
   ...
   IS
      l_rec PO_PARAM_REG_MC%ROWTYPE := rec_in;
   BEGIN
      ins (
         id_po_param_reg_in => l_rec.ID_PO_PARAM_REG,
         id_po_param_mc_in => l_rec.ID_PO_PARAM_MC
         ,
   ...
   BEGIN
      ...
      ELSE
         FORALL indx IN rows_in.FIRST .. rows_in.LAST
            SAVE EXCEPTIONS
            INSERT INTO PO_PARAM_REG_MC VALUES rows_in (indx) RETURNING ROWID BULK COLLECT INTO rows_out;
      END IF;
   EXCEPTION
     WHEN e_forall_error
     THEN
       ...
       ELSE
          ...
          BEGIN
             ...
             LOOP
                ...
                pk_alert_exceptions.add_context (
                   err_instance_id_in => l_err_instance_id
                  ,NAME_IN => 'ID_PO_PARAM_MC _' || indx
                  ,value_in => rows_in(SQL%BULK_EXCEPTIONS (indx).ERROR_INDEX).ID_PO_PARAM_MC
                  ,validate_in => FALSE
                );
...
PROCEDURE upd (
   ...
   l_UPDATE_INSTITUTION_n := sys.diutil.bool_to_int(UPDATE_INSTITUTION_nin);
     ...
     UPDATE_INSTITUTION = decode (l_UPDATE_INSTITUTION_n,0,update_institution_in, NVL (update_institution_in, UPDATE_INSTITUTION))
          WHERE
             ID_PO_PARAM_REG = id_po_param_reg_in AND
             ID_PO_PARAM_MC = id_po_param_mc_in
...
PROCEDURE upd (
   ...
   l_UPDATE_TIME_n NUMBER(1);
   l_UPDATE_INSTITUTION_n NUMBER(1);
      id_po_param_reg_in PO_PARAM_REG_MC.ID_PO_PARAM_REG%TYPE;
      id_po_param_mc_in PO_PARAM_REG_MC.ID_PO_PARAM_MC%TYPE;
   BEGIN
...
l_sql := 'UPDATE PO_PARAM_REG_MC SET '
     || ' CREATE_USER = decode (' || l_CREATE_USER_n || ',0,:create_user_in, NVL (:create_user_in, CREATE_USER)) '|| ','
     || ' CREATE_TIME = decode (' || l_CREATE_TIME_n || ',0,:create_time_in, NVL (:create_time_in, CREATE_TIME)) '|| ','
     || ' CREATE_INSTITUTION = decode (' || l_CREATE_INSTITUTION_n || ',0,:create_institution_in, NVL (:create_institution_in, CREATE_INSTITUTION)) '|| ','
     || ' UPDATE_USER = decode (' || l_UPDATE_USER_n || ',0,:update_user_in, NVL (:update_user_in, UPDATE_USER)) '|| ','
...
PROCEDURE upd (
   ...
   BEGIN
      upd (
         ...
         id_po_param_mc_in => rec_in.ID_PO_PARAM_MC,
   ...
   IS
      ...
      l_ID_PO_PARAM_MC ID_PO_PARAM_MC_cc;
   ...
   BEGIN
      FOR i IN col_in.FIRST .. col_in.LAST loop
         ...
         l_ID_PO_PARAM_MC(i) := col_in(i).ID_PO_PARAM_MC;
      ...
      THEN
         ...
         FORALL i IN col_in.FIRST .. col_in.LAST
            UPDATE PO_PARAM_REG_MC SET
             ...
             WHERE
                ID_PO_PARAM_REG = l_ID_PO_PARAM_REG(i) AND
                ID_PO_PARAM_MC = l_ID_PO_PARAM_MC(i)
          returning rowid bulk collect into rows_out;
      ELSE
         FORALL i IN col_in.FIRST .. col_in.LAST
            UPDATE PO_PARAM_REG_MC SET
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE PO_PARAM_REG_MC set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE PO_PARAM_REG_MC set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM PO_PARAM_REG_MC
       WHERE ID_PO_PARAM_MC = id_po_param_mc_in
      RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM PO_PARAM_REG_MC
       WHERE
          ID_PO_PARAM_MC = del_POPRMC_POPMC_FK.id_po_param_mc_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_POPRMC_POPMC_FK (
   ...
   BEGIN
      DELETE FROM PO_PARAM_REG_MC
       WHERE
          ID_PO_PARAM_REG = del_POPRMC_POPR_FK.id_po_param_reg_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_POPRMC_POPR_FK (
   ...
   BEGIN
      IF where_in IS NULL
      THEN
         RETURN 'DELETE FROM PO_PARAM_REG_MC';
      ELSE
         RETURN
            'DELETE FROM PO_PARAM_REG_MC WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   IS
   BEGIN
      po_param_reg_mc_inout.ID_PO_PARAM_REG := NULL;
      po_param_reg_mc_inout.ID_PO_PARAM_MC := NULL;
      po_param_reg_mc_inout.CREATE_USER := NULL;
      po_param_reg_mc_inout.CREATE_TIME := NULL;
      po_param_reg_mc_inout.CREATE_INSTITUTION := NULL;
      po_param_reg_mc_inout.UPDATE_USER := NULL;
      po_param_reg_mc_inout.UPDATE_TIME := NULL;
      po_param_reg_mc_inout.UPDATE_INSTITUTION := NULL;
   END initrec;

   FUNCTION initrec RETURN PO_PARAM_REG_MC%ROWTYPE
   IS
      l_po_param_reg_mc PO_PARAM_REG_MC%ROWTYPE;
   BEGIN
      RETURN l_po_param_reg_mc;
   END initrec;
   ...
   IS
        data PO_PARAM_REG_MC_tc;
   BEGIN
        select * bulk collect into data from PO_PARAM_REG_MC where rowid in (select /*+opt_estimate(table,t,scale_rows=0.0000001))*/ * from table(rows_in) t);
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
