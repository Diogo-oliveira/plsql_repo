...
CREATE OR REPLACE PACKAGE BODY TS_SUSP_TASK
/*
| Generated by or retrieved from QCGU - DO NOT MODIFY!
| QCGU - "Get it right, do it fast" - www.ToadWorld.com
| QCGU Universal ID: {657BD35E-2E48-4CD4-B14E-41420B8433B3}
| Created On: Junho 4, 2010 15:53:41
| Created By: ALERT
*/
IS
      ...
      THEN
        ...
        BEGIN
           ...
           PK_ALERT_EXCEPTIONS.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'SUSP_TASK');
           IF l_name = 'WSKT_WSN_FK'
           THEN
              -- Add a context value for each column
              PK_ALERT_EXCEPTIONS.add_context (
                 err_instance_id_in => l_id
               , name_in => 'ID_SUSP_ACTION'
               , value_in => id_susp_action_in);
           END IF;
   ...
   IS
      l_rec SUSP_TASK%ROWTYPE := rec_in;
   BEGIN
      IF gen_pky_in THEN
         l_rec.ID_SUSP_TASK := next_key (sequence_in);
      END IF;
      ins (
  ...
  ins (
   ...
   IS
     retval SUSP_TASK.ID_SUSP_TASK%TYPE;

   BEGIN
      IF sequence_in IS NULL
      THEN
         SELECT seq_SUSP_TASK.NEXTVAL INTO retval FROM dual;
      ELSE
         EXECUTE IMMEDIATE
   ...
   EXCEPTION
      WHEN OTHERS THEN
        PK_ALERT_EXCEPTIONS.raise_error (
           ...
           ,name1_in => 'SEQUENCE'
           ,value1_in => NVL (sequence_in, 'seq_SUSP_TASK')
           );
   END next_key;
   ...
   IS
        l_pky SUSP_TASK.ID_SUSP_TASK%TYPE := next_key;
   BEGIN
      ins (
         id_susp_task_in => l_pky,
         id_susp_action_in => id_susp_action_in,
         flg_status_in => flg_status_in,
         create_user_in => create_user_in,
         create_time_in => create_time_in,
   ...
   BEGIN
      ...
      update_time_in => update_time_in,
      update_institution_in => update_institution_in,
      id_susp_task_out => id_susp_task_out
      ,handle_error_in => handle_error_in
      , rows_out => rows_out
   );
   END ins;
   ...
   BEGIN
      ...
      ELSE
         FORALL indx IN rows_in.FIRST .. rows_in.LAST
            SAVE EXCEPTIONS
            INSERT INTO SUSP_TASK VALUES rows_in (indx) RETURNING ROWID BULK COLLECT INTO rows_out;
      END IF;
   EXCEPTION
     WHEN e_forall_error
     THEN
...
PROCEDURE upd (
   ...
   l_UPDATE_INSTITUTION_n := sys.diutil.bool_to_int(UPDATE_INSTITUTION_nin);
         ...
         UPDATE SUSP_TASK SET
     ID_SUSP_ACTION = decode (l_ID_SUSP_ACTION_n,0,id_susp_action_in, NVL (id_susp_action_in, ID_SUSP_ACTION)),
...
PROCEDURE upd (
   ...
   l_UPDATE_TIME_n NUMBER(1);
   l_UPDATE_INSTITUTION_n NUMBER(1);
      id_susp_task_in SUSP_TASK.ID_SUSP_TASK%TYPE;
   BEGIN
...
PROCEDURE upd (
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE SUSP_TASK set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE SUSP_TASK set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM SUSP_TASK
       WHERE
          ID_SUSP_ACTION = del_WSKT_WSN_FK.id_susp_action_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_WSKT_WSN_FK (
   ...
   BEGIN
      IF where_in IS NULL
      THEN
         RETURN 'DELETE FROM SUSP_TASK';
      ELSE
         RETURN
            'DELETE FROM SUSP_TASK WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   IS
   BEGIN
      susp_task_inout.ID_SUSP_TASK := NULL;
      susp_task_inout.ID_SUSP_ACTION := NULL;
      susp_task_inout.FLG_STATUS := NULL;
      susp_task_inout.CREATE_USER := NULL;
      susp_task_inout.CREATE_TIME := NULL;
      susp_task_inout.CREATE_INSTITUTION := NULL;
      susp_task_inout.UPDATE_USER := NULL;
      susp_task_inout.UPDATE_TIME := NULL;
      susp_task_inout.UPDATE_INSTITUTION := NULL;
   END initrec;

   FUNCTION initrec RETURN SUSP_TASK%ROWTYPE
   IS
      l_susp_task SUSP_TASK%ROWTYPE;
   BEGIN
      RETURN l_susp_task;
   END initrec;
   ...
   IS
        data SUSP_TASK_tc;
   BEGIN
        select * bulk collect into data from SUSP_TASK where rowid in (select /*+opt_estimate(table,t,scale_rows=0.0000001))*/ * from table(rows_in) t);
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        PK_ALERT_EXCEPTIONS.raise_error (
