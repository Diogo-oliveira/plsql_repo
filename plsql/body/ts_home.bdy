...
IS
      ...
      THEN
        ...
        BEGIN
           ...
           pk_alert_exceptions.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'HOME');
           IF l_name = 'HOME_PROF_FK'
           THEN
   ...
   IS
      l_rec HOME%ROWTYPE := rec_in;
   BEGIN
      IF gen_pky_in THEN
         l_rec.ID_HOME := next_key (sequence_in);
      END IF;
      ins (
  ...
  ins (
   ...
   IS
     retval HOME.ID_HOME%TYPE;

   BEGIN
      IF sequence_in IS NULL
      THEN
         SELECT seq_HOME.NEXTVAL INTO retval FROM dual;
      ELSE
         EXECUTE IMMEDIATE
   ...
   EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
           ...
           ,name1_in => 'SEQUENCE'
           ,value1_in => NVL (sequence_in, 'seq_HOME')
           );
   END next_key;
   ...
   IS
        l_pky HOME.ID_HOME%TYPE := next_key;
   BEGIN
      ins (
         id_home_in => l_pky,
         id_pat_family_in => id_pat_family_in,
   ...
   BEGIN
      ...
      ELSE
         FORALL indx IN rows_in.FIRST .. rows_in.LAST
            SAVE EXCEPTIONS
            INSERT INTO HOME VALUES rows_in (indx) RETURNING ROWID BULK COLLECT INTO rows_out;
      END IF;
   EXCEPTION
     WHEN e_forall_error
     THEN
...
PROCEDURE upd (
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE HOME set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE HOME set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM HOME
       WHERE
          ID_PROFESSIONAL = del_HOME_PROF_FK.id_professional_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_HOME_PROF_FK (
   ...
   BEGIN
      DELETE FROM HOME
       WHERE
          ID_PAT_FAMILY = del_HOME_PTFAM_FK.id_pat_family_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_HOME_PTFAM_FK (
   ...
   BEGIN
      ...
      ELSE
         RETURN
            'DELETE FROM HOME WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   IS
   BEGIN
      home_inout.ID_HOME := NULL;
      home_inout.ID_PAT_FAMILY := NULL;
      home_inout.ID_PROFESSIONAL := NULL;
      home_inout.NUM_ROOMS := NULL;
      home_inout.NUM_BEDROOMS := NULL;
      home_inout.NUM_PERSON_ROOM := NULL;
      home_inout.FLG_WC_TYPE := NULL;
      home_inout.FLG_WC_LOCATION := NULL;
      home_inout.FLG_WC_OUT := NULL;
      home_inout.FLG_WATER_DISTRIB := NULL;
      home_inout.FLG_WATER_ORIGIN := NULL;
      home_inout.FLG_CONSERV := NULL;
      home_inout.FLG_OWNER := NULL;
      home_inout.FLG_HAB_TYPE := NULL;
      home_inout.FLG_LIGHT := NULL;
      home_inout.FLG_HEAT := NULL;
      home_inout.ARQUITECT_BARRIER := NULL;
      home_inout.DT_REGISTRY_TSTZ := NULL;
      home_inout.FLG_HAB_LOCATION := NULL;
      home_inout.NOTES := NULL;
      home_inout.CREATE_USER := NULL;
      home_inout.CREATE_TIME := NULL;
      home_inout.CREATE_INSTITUTION := NULL;
      home_inout.UPDATE_USER := NULL;
      home_inout.UPDATE_TIME := NULL;
      home_inout.UPDATE_INSTITUTION := NULL;
      home_inout.FLG_STATUS := NULL;
      home_inout.ID_CANCEL_INFO_DET := NULL;
      home_inout.FLG_WATER_TREATMENT := NULL;
      home_inout.FLG_GARBAGE_DEST := NULL;
      home_inout.FLG_BATH := NULL;
      home_inout.FT_BATH := NULL;
      home_inout.FT_WC_TYPE := NULL;
      home_inout.FT_WC_LOCATION := NULL;
      home_inout.FT_WC_OUT := NULL;
      home_inout.FT_WATER_DISTRIB := NULL;
      home_inout.FT_WATER_ORIGIN := NULL;
      home_inout.FT_CONSERV := NULL;
      home_inout.FT_OWNER := NULL;
      home_inout.FT_GARBAGE_DEST := NULL;
      home_inout.FT_HAB_TYPE := NULL;
      home_inout.FT_WATER_TREATMENT := NULL;
      home_inout.FT_LIGHT := NULL;
      home_inout.FT_HEAT := NULL;
      home_inout.FT_HAB_LOCATION := NULL;
   END initrec;
   ...
   IS
        data HOME_tc;
   BEGIN
        select * bulk collect into data from HOME where rowid in (select /*+opt_estimate(table,t,scale_rows=0.0000001))*/ * from table(rows_in) t);
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
