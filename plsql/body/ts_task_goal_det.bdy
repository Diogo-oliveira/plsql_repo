...
CREATE OR REPLACE PACKAGE BODY ts_task_goal_det IS
      ...
      THEN
        ...
        BEGIN
           ...
           pk_alert_exceptions.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'TASK_GOAL_DET');
           IF l_name = 'TGLD_TGL_FK'
           THEN
   ...
   */
   ...
   IS
      l_rec TASK_GOAL_DET%ROWTYPE := rec_in;
   BEGIN
      IF gen_pky_in THEN
         l_rec.ID_TASK_GOAL_DET := next_key (sequence_in);
      END IF;
      ins (
  ...
  ins (
   ...
   IS
     retval TASK_GOAL_DET.ID_TASK_GOAL_DET%TYPE;

   BEGIN
      IF sequence_in IS NULL
      THEN
         SELECT seq_TASK_GOAL_DET.NEXTVAL INTO retval FROM dual;
      ELSE
         EXECUTE IMMEDIATE
   ...
   EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
           ...
           ,name1_in => 'SEQUENCE'
           ,value1_in => NVL (sequence_in, 'seq_TASK_GOAL_DET')
           );
   END next_key;
   ...
   IS
        l_pky TASK_GOAL_DET.ID_TASK_GOAL_DET%TYPE := next_key;
   BEGIN
      ins (
         id_task_goal_det_in => l_pky,
         id_task_goal_in => id_task_goal_in,
         desc_task_goal_in => desc_task_goal_in,
         create_user_in => create_user_in,
         create_time_in => create_time_in,
   ...
   BEGIN
      ...
      update_time_in => update_time_in,
      update_institution_in => update_institution_in,
      id_task_goal_det_out => id_task_goal_det_out
      ,handle_error_in => handle_error_in
      , rows_out => rows_out
   );
   END ins;
   ...
   BEGIN
      ...
      ELSE
         FORALL indx IN rows_in.FIRST .. rows_in.LAST
            SAVE EXCEPTIONS
            INSERT INTO TASK_GOAL_DET VALUES rows_in (indx) RETURNING ROWID BULK COLLECT INTO rows_out;
      END IF;
   EXCEPTION
     WHEN e_forall_error
     THEN
...
PROCEDURE upd (
   ...
   l_UPDATE_INSTITUTION_n := sys.diutil.bool_to_int(UPDATE_INSTITUTION_nin);
     ...
     UPDATE_INSTITUTION = decode (l_UPDATE_INSTITUTION_n,0,update_institution_in, NVL (update_institution_in, UPDATE_INSTITUTION))
          WHERE
             ID_TASK_GOAL_DET = id_task_goal_det_in
...
PROCEDURE upd (
   ...
   l_UPDATE_TIME_n NUMBER(1);
   l_UPDATE_INSTITUTION_n NUMBER(1);
      id_task_goal_det_in TASK_GOAL_DET.ID_TASK_GOAL_DET%TYPE;
   BEGIN
...
l_sql := 'UPDATE TASK_GOAL_DET SET '
     || ' ID_TASK_GOAL = decode (' || l_ID_TASK_GOAL_n || ',0,:id_task_goal_in, NVL (:id_task_goal_in, ID_TASK_GOAL)) '|| ','
     || ' DESC_TASK_GOAL = decode (' || l_DESC_TASK_GOAL_n || ',0,:desc_task_goal_in, NVL (:desc_task_goal_in, DESC_TASK_GOAL)) '|| ','
     || ' CREATE_USER = decode (' || l_CREATE_USER_n || ',0,:create_user_in, NVL (:create_user_in, CREATE_USER)) '|| ','
     || ' CREATE_TIME = decode (' || l_CREATE_TIME_n || ',0,:create_time_in, NVL (:create_time_in, CREATE_TIME)) '|| ','
...
PROCEDURE upd (
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE TASK_GOAL_DET set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE TASK_GOAL_DET set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM TASK_GOAL_DET
       WHERE ID_TASK_GOAL_DET = id_task_goal_det_in
      RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM TASK_GOAL_DET
       WHERE
          ID_TASK_GOAL = del_TGLD_TGL_FK.id_task_goal_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_TGLD_TGL_FK (
   ...
   BEGIN
      IF where_in IS NULL
      THEN
         RETURN 'DELETE FROM TASK_GOAL_DET';
      ELSE
         RETURN
            'DELETE FROM TASK_GOAL_DET WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   IS
   BEGIN
      task_goal_det_inout.ID_TASK_GOAL_DET := NULL;
      task_goal_det_inout.ID_TASK_GOAL := NULL;
      task_goal_det_inout.DESC_TASK_GOAL := NULL;
      task_goal_det_inout.CREATE_USER := NULL;
      task_goal_det_inout.CREATE_TIME := NULL;
      task_goal_det_inout.CREATE_INSTITUTION := NULL;
      task_goal_det_inout.UPDATE_USER := NULL;
      task_goal_det_inout.UPDATE_TIME := NULL;
      task_goal_det_inout.UPDATE_INSTITUTION := NULL;
   END initrec;

   FUNCTION initrec RETURN TASK_GOAL_DET%ROWTYPE
   IS
      l_task_goal_det TASK_GOAL_DET%ROWTYPE;
   BEGIN
      RETURN l_task_goal_det;
   END initrec;
   ...
   IS
        data TASK_GOAL_DET_tc;
   BEGIN
        select * bulk collect into data from TASK_GOAL_DET where rowid in (select * from table(rows_in));
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
