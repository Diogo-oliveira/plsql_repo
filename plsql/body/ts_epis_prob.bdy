...
CREATE OR REPLACE PACKAGE BODY TS_EPIS_PROB
/*
| Generated by or retrieved from QCGU - DO NOT MODIFY!
| QCGU - "Get it right, do it fast" - www.ToadWorld.com
| QCGU Universal ID: {7A44F965-7C77-42EE-9A04-269EF006213B}
| Created On: 12ды 13, 2017 18:0:8
| Created By: ALERT
*/
IS
      ...
      THEN
        ...
        BEGIN
           ...
           pk_alert_exceptions.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'EPIS_PROB');
           IF l_name = 'EPB_CR_FK'
           THEN
   ...
   IS
      l_rec EPIS_PROB%ROWTYPE := rec_in;
   BEGIN
      IF gen_pky_in THEN
         l_rec.ID_EPIS_PROBLEM := next_key (sequence_in);
      END IF;
      ins (
  ...
  ins (
   ...
   IS
     retval EPIS_PROB.ID_EPIS_PROBLEM%TYPE;

   BEGIN
      IF sequence_in IS NULL
      THEN
         SELECT seq_EPIS_PROB.NEXTVAL INTO retval FROM dual;
      ELSE
         EXECUTE IMMEDIATE
   ...
   EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
           ...
           ,name1_in => 'SEQUENCE'
           ,value1_in => NVL (sequence_in, 'seq_EPIS_PROB')
           );
   END next_key;
   ...
   IS
        l_pky EPIS_PROB.ID_EPIS_PROBLEM%TYPE := next_key;
   BEGIN
      ins (
         id_epis_problem_in => l_pky,
         id_problem_in => id_problem_in,
         id_epis_prob_group_in => id_epis_prob_group_in,
         id_episode_in => id_episode_in,
         flg_type_in => flg_type_in,
   ...
   BEGIN
      ...
      id_professional_in => id_professional_in,
      dt_epis_prob_tstz_in => dt_epis_prob_tstz_in,
      id_epis_problem_out => id_epis_problem_out
      ,handle_error_in => handle_error_in
      , rows_out => rows_out
   );
   END ins;
...
PROCEDURE upd (
   ...
   l_DT_EPIS_PROB_TSTZ_n := sys.diutil.bool_to_int(DT_EPIS_PROB_TSTZ_nin);
     ...
     DT_EPIS_PROB_TSTZ = decode (l_DT_EPIS_PROB_TSTZ_n,0,dt_epis_prob_tstz_in, NVL (dt_epis_prob_tstz_in, DT_EPIS_PROB_TSTZ))
          WHERE
             ID_EPIS_PROBLEM = id_epis_problem_in
...
PROCEDURE upd (
   ...
   l_ID_PROFESSIONAL_n NUMBER(1);
   l_DT_EPIS_PROB_TSTZ_n NUMBER(1);
      id_epis_problem_in EPIS_PROB.ID_EPIS_PROBLEM%TYPE;
   BEGIN
...
PROCEDURE upd (
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE EPIS_PROB set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE EPIS_PROB set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM EPIS_PROB
       WHERE ID_EPIS_PROBLEM = id_epis_problem_in
      RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM EPIS_PROB
       WHERE
          ID_CANCEL_REASON = del_EPB_CR_FK.id_cancel_reason_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_EPB_CR_FK (
   ...
   BEGIN
      DELETE FROM EPIS_PROB
       WHERE
          ID_EPIS_PROB_GROUP = del_EPB_EPBG_FK.id_epis_prob_group_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_EPB_EPBG_FK (
   ...
   BEGIN
      DELETE FROM EPIS_PROB
       WHERE
          ID_PROF_CANCEL = del_EPB_PROFC_FK.id_prof_cancel_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_EPB_PROFC_FK (
   ...
   BEGIN
      DELETE FROM EPIS_PROB
       WHERE
          ID_PROFESSIONAL = del_EPB_PROF_FK.id_professional_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_EPB_PROF_FK (
   ...
   BEGIN
      IF where_in IS NULL
      THEN
         RETURN 'DELETE FROM EPIS_PROB';
      ELSE
         RETURN
            'DELETE FROM EPIS_PROB WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   IS
   BEGIN
      epis_prob_inout.ID_EPIS_PROBLEM := NULL;
      epis_prob_inout.ID_PROBLEM := NULL;
      epis_prob_inout.ID_EPIS_PROB_GROUP := NULL;
      epis_prob_inout.ID_EPISODE := NULL;
      epis_prob_inout.FLG_TYPE := NULL;
      epis_prob_inout.FLG_STATUS := NULL;
      epis_prob_inout.RANK := NULL;
      epis_prob_inout.ID_CANCEL_REASON := NULL;
      epis_prob_inout.CANCEL_NOTES := NULL;
      epis_prob_inout.ID_PROF_CANCEL := NULL;
      epis_prob_inout.DT_CANCEL := NULL;
      epis_prob_inout.CREATE_USER := NULL;
      epis_prob_inout.CREATE_TIME := NULL;
      epis_prob_inout.CREATE_INSTITUTION := NULL;
      epis_prob_inout.UPDATE_USER := NULL;
      epis_prob_inout.UPDATE_TIME := NULL;
      epis_prob_inout.UPDATE_INSTITUTION := NULL;
      epis_prob_inout.ID_PROBLEM_NEW := NULL;
      epis_prob_inout.ID_PROFESSIONAL := NULL;
      epis_prob_inout.DT_EPIS_PROB_TSTZ := NULL;
   END initrec;

   FUNCTION initrec RETURN EPIS_PROB%ROWTYPE
   IS
      l_epis_prob EPIS_PROB%ROWTYPE;
   BEGIN
      RETURN l_epis_prob;
   END initrec;
   ...
   IS
        data EPIS_PROB_tc;
   BEGIN
        select * bulk collect into data from EPIS_PROB where rowid in (select /*+opt_estimate(table,t,scale_rows=0.0000001))*/ * from table(rows_in) t);
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
