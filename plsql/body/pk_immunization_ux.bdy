/*-- Last Change Revision: $Rev: 1874685 $*/
/*-- Last Change by: $Author: pedro.henriques $*/
/*-- Date of last change: $Date: 2018-10-25 09:48:22 +0100 (qui, 25 out 2018) $*/
CREATE OR REPLACE PACKAGE BODY pk_immunization_ux IS

    /* CAN'T TOUCH THIS */
    g_error   VARCHAR2(1000 CHAR);
    g_owner   VARCHAR2(30 CHAR);
    g_package VARCHAR2(30 CHAR);
    g_retval  BOOLEAN;
    g_exception_np EXCEPTION;

    -- debug mode enabled/disabled
    g_debug BOOLEAN;

    FUNCTION get_vacc_summary_all
    (
        i_lang             IN language.id_language%TYPE,
        i_prof             IN profissional,
        i_patient          IN patient.id_patient%TYPE,
        i_episode          IN episode.id_episode%TYPE,
        o_oth_vaccine_time OUT pk_types.cursor_type,
        o_oth_vaccine_par  OUT pk_types.cursor_type,
        o_oth_vaccine_val  OUT pk_types.cursor_type,
        --PNV Vaccines
        o_vaccine_group_name OUT pk_types.cursor_type,
        o_vaccine_time       OUT pk_types.cursor_type,
        o_vaccine_par        OUT pk_types.cursor_type,
        o_vaccine_val        OUT pk_types.cursor_type,
        --Tuberculin tests
        o_tuberculin_group_name OUT VARCHAR2,
        o_tuberculin_time       OUT pk_types.cursor_type,
        o_tuberculin_par        OUT pk_types.cursor_type,
        o_tuberculin_val        OUT pk_types.cursor_type,
        o_review                OUT pk_types.cursor_type,
        o_create                OUT VARCHAR2,
        --ERROR
        o_error OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_SUMMARY_ALL';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
    
        g_error  := 'Call GET_VACC_SUMMARY_ALL / ' || l_params;
        g_retval := pk_immunization_core.get_vacc_summary_all(i_lang                  => i_lang,
                                                              i_prof                  => i_prof,
                                                              i_patient               => i_patient,
                                                              i_episode               => i_episode,
                                                              o_oth_vaccine_time      => o_oth_vaccine_time,
                                                              o_oth_vaccine_par       => o_oth_vaccine_par,
                                                              o_oth_vaccine_val       => o_oth_vaccine_val,
                                                              o_vaccine_group_name    => o_vaccine_group_name,
                                                              o_vaccine_time          => o_vaccine_time,
                                                              o_vaccine_par           => o_vaccine_par,
                                                              o_vaccine_val           => o_vaccine_val,
                                                              o_tuberculin_group_name => o_tuberculin_group_name,
                                                              o_tuberculin_time       => o_tuberculin_time,
                                                              o_tuberculin_par        => o_tuberculin_par,
                                                              o_tuberculin_val        => o_tuberculin_val,
                                                              o_review                => o_review,
                                                              o_create                => o_create,
                                                              o_error                 => o_error);
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_summary_all;

    FUNCTION get_vacc_add
    (
        i_lang       IN language.id_language%TYPE,
        i_prof       IN profissional,
        i_type       IN VARCHAR2,
        i_orig       IN VARCHAR2,
        i_patient    IN patient.id_patient%TYPE,
        i_episode    IN episode.id_episode%TYPE,
        i_vacc       IN vacc.id_vacc%TYPE,
        i_type_vacc  IN VARCHAR2,
        i_id_reg     IN NUMBER,
        i_flg_status IN VARCHAR2,
        o_val        OUT pk_types.cursor_type,
        o_error      OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_ADD';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_ADD / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_add(i_lang       => i_lang,
                                                      i_prof       => i_prof,
                                                      i_type       => i_type,
                                                      i_orig       => i_orig,
                                                      i_patient    => i_patient,
                                                      i_episode    => i_episode,
                                                      i_vacc       => i_vacc,
                                                      i_type_vacc  => i_type_vacc,
                                                      i_id_reg     => i_id_reg,
                                                      i_flg_status => i_flg_status,
                                                      o_val        => o_val,
                                                      o_error      => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_add;

    FUNCTION set_pnv_review
    (
        i_lang               IN language.id_language%TYPE,
        i_prof               IN profissional,
        i_patient            IN patient.id_patient%TYPE,
        i_episode            IN episode.id_episode%TYPE,
        i_review_notes       IN VARCHAR2,
        o_review             OUT pk_types.cursor_type,
        o_vaccine_group_name OUT pk_types.cursor_type,
        o_error              OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'SET_PNV_REVIEW';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call SET_PNV_REVIEW / ' || l_params;
    
        g_retval := pk_immunization_core.set_pnv_review(i_lang               => i_lang,
                                                        i_prof               => i_prof,
                                                        i_patient            => i_patient,
                                                        i_episode            => i_episode,
                                                        i_review_notes       => i_review_notes,
                                                        o_review             => o_review,
                                                        o_vaccine_group_name => o_vaccine_group_name,
                                                        o_error              => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END set_pnv_review;

    FUNCTION get_most_freq_all
    (
        i_lang           IN language.id_language%TYPE,
        i_prof           IN profissional,
        i_type           IN VARCHAR2,
        i_button         IN VARCHAR2,
        o_med_freq_label OUT VARCHAR2,
        o_med_sel_label  OUT VARCHAR2,
        o_search_label   OUT VARCHAR2,
        o_med_freq       OUT pk_types.cursor_type,
        o_error          OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_MOST_FREQ_ALL';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_MOST_FREQ_ALL / ' || l_params;
    
        g_retval := pk_immunization_core.get_most_freq_all(i_lang           => i_lang,
                                                           i_prof           => i_prof,
                                                           i_type           => i_type,
                                                           i_button         => i_button,
                                                           o_med_freq_label => o_med_freq_label,
                                                           o_med_sel_label  => o_med_sel_label,
                                                           o_search_label   => o_search_label,
                                                           o_med_freq       => o_med_freq,
                                                           o_error          => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_most_freq_all;

    FUNCTION set_tuberculin_test_presc
    (
        i_lang    IN language.id_language%TYPE,
        i_patient IN patient.id_patient%TYPE,
        i_episode IN episode.id_episode%TYPE,
        i_prof    IN profissional,
        --presc_det
        i_drug         IN drug_presc_det.id_drug%TYPE, --mi_med.id_drug%TYPE,
        i_dosage       IN drug_presc_det.dosage_description%TYPE,
        i_unit_measure IN drug_presc_det.id_unit_measure%TYPE,
        i_admin_via    IN drug_presc_det.route_id%TYPE,
        --
        i_prof_write          IN professional.id_professional%TYPE,
        i_notes_justif        IN drug_presc_det.notes_justif%TYPE,
        i_notes               IN drug_presc_det.notes%TYPE,
        i_presc_date          IN VARCHAR2,
        i_requested_by        IN VARCHAR2,
        i_vacc_manuf          IN vacc_manufacturer.id_vacc_manufacturer%TYPE DEFAULT NULL,
        code_mvx              IN vacc_manufacturer.code_mvx%TYPE DEFAULT NULL,
        i_flg_type_date       IN drug_presc_plan.flg_type_date%TYPE,
        i_dosage_admin        IN drug_presc_plan.dosage%TYPE,
        i_dosage_unit_measure IN drug_presc_plan.dosage_unit_measure%TYPE,
        --OUT
        o_test_id    OUT drug_prescription.id_drug_prescription%TYPE,
        o_id_admin   OUT drug_presc_plan.id_drug_presc_plan%TYPE,
        o_type_admin OUT VARCHAR2,
        --ERROR
        o_error OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'SET_TUBERCULIN_TEST_PRESC';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call SET_TUBERCULIN_TEST_PRESC / ' || l_params;
    
        g_retval := pk_immunization_core.set_tuberculin_test_presc(i_lang                => i_lang,
                                                                   i_patient             => i_patient,
                                                                   i_episode             => i_episode,
                                                                   i_prof                => i_prof,
                                                                   i_drug                => i_drug,
                                                                   i_dosage              => i_dosage,
                                                                   i_unit_measure        => i_unit_measure,
                                                                   i_admin_via           => i_admin_via,
                                                                   i_prof_write          => i_prof_write,
                                                                   i_notes_justif        => i_notes_justif,
                                                                   i_notes               => i_notes,
                                                                   i_presc_date          => i_presc_date,
                                                                   i_requested_by        => i_requested_by,
                                                                   i_vacc_manuf          => i_vacc_manuf,
                                                                   code_mvx              => code_mvx,
                                                                   i_flg_type_date       => i_flg_type_date,
                                                                   i_dosage_admin        => i_dosage_admin,
                                                                   i_dosage_unit_measure => i_dosage_unit_measure,
                                                                   o_test_id             => o_test_id,
                                                                   o_id_admin            => o_id_admin,
                                                                   o_type_admin          => o_type_admin,
                                                                   o_error               => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END set_tuberculin_test_presc;

    FUNCTION get_application_spot_list
    (
        i_lang    IN language.id_language%TYPE,
        o_ap_spot OUT pk_types.cursor_type,
        o_error   OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_APPLICATION_SPOT_LIST';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_APPLICATION_SPOT_LIST / ' || l_params;
    
        g_retval := pk_immunization_core.get_application_spot_list(i_lang    => i_lang,
                                                                   o_ap_spot => o_ap_spot,
                                                                   o_error   => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_application_spot_list;

    FUNCTION get_vacc_manufacturer
    (
        i_lang              IN language.id_language%TYPE,
        i_prof              IN profissional,
        i_id_drug           IN mi_med.id_drug%TYPE,
        o_vacc_manufacturer OUT pk_types.cursor_type,
        o_error             OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_MANUFACTURER';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_MANUFACTURER / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_manufacturer(i_lang              => i_lang,
                                                               i_prof              => i_prof,
                                                               i_id_drug           => i_id_drug,
                                                               o_vacc_manufacturer => o_vacc_manufacturer,
                                                               o_error             => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_manufacturer;

    FUNCTION get_vacc_manufacturer
    (
        i_lang              IN language.id_language%TYPE,
        i_prof              IN profissional,
        o_vacc_manufacturer OUT pk_types.cursor_type,
        o_error             OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_MANUFACTURER';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_MANUFACTURER / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_manufacturer(i_lang              => i_lang,
                                                               i_prof              => i_prof,
                                                               o_vacc_manufacturer => o_vacc_manufacturer,
                                                               o_error             => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_manufacturer;

    FUNCTION get_vacc_unit_measure
    (
        i_lang              IN language.id_language%TYPE,
        i_prof              IN profissional,
        o_vacc_unit_measure OUT pk_types.cursor_type,
        o_error             OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_UNIT_MEASURE';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_UNIT_MEASURE / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_unit_measure(i_lang              => i_lang,
                                                               i_prof              => i_prof,
                                                               o_vacc_unit_measure => o_vacc_unit_measure,
                                                               o_error             => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_unit_measure;

    FUNCTION get_vacc_adm_warnings
    (
        i_lang       IN language.id_language%TYPE,
        i_prof       IN profissional,
        i_vacc       IN vacc.id_vacc%TYPE,
        i_id_patient IN patient.id_patient%TYPE,
        --OUT
        o_flg_show       OUT VARCHAR2,
        o_message_title  OUT VARCHAR2,
        o_message_text   OUT VARCHAR2,
        o_forward_button OUT VARCHAR2,
        o_back_button    OUT VARCHAR2,
        o_error          OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_ADM_WARNINGS';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_ADM_WARNINGS / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_adm_warnings(i_lang       => i_lang,
                                                               i_prof       => i_prof,
                                                               i_vacc       => i_vacc,
                                                               i_id_patient => i_id_patient,
                                                               --OUT                           
                                                               o_flg_show       => o_flg_show,
                                                               o_message_title  => o_message_title,
                                                               o_message_text   => o_message_text,
                                                               o_forward_button => o_forward_button,
                                                               o_back_button    => o_back_button,
                                                               o_error          => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_adm_warnings;

    FUNCTION get_vacc_form_administration
    (
        i_lang     IN language.id_language%TYPE,
        i_prof     IN profissional,
        i_patient  IN patient.id_patient%TYPE,
        i_vacc     IN vacc.id_vacc%TYPE,
        i_drug     IN drug_prescription.id_drug_prescription%TYPE,
        o_form     OUT pk_types.cursor_type,
        o_doc_show OUT VARCHAR2,
        o_error    OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_FORM_ADMINISTRATION';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_FORM_ADMINISTRATION / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_form_administration(i_lang     => i_lang,
                                                                      i_prof     => i_prof,
                                                                      i_patient  => i_patient,
                                                                      i_vacc     => i_vacc,
                                                                      i_drug     => i_drug,
                                                                      o_form     => o_form,
                                                                      o_doc_show => o_doc_show,
                                                                      o_error    => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_form_administration;

    FUNCTION get_vacc_viewer_details
    (
        i_lang        IN language.id_language%TYPE,
        i_prof        IN profissional,
        i_vacc        IN vacc.id_vacc%TYPE,
        o_detail_info OUT VARCHAR2,
        o_error       OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_VIEWER_DETAILS';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_VIEWER_DETAILS / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_viewer_details(i_lang        => i_lang,
                                                                 i_prof        => i_prof,
                                                                 i_vacc        => i_vacc,
                                                                 o_detail_info => o_detail_info,
                                                                 o_error       => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_viewer_details;

    FUNCTION get_vacc_med_ext
    (
        i_lang         IN language.id_language%TYPE,
        i_prof         IN profissional,
        i_vacc         IN vacc.id_vacc%TYPE,
        i_orig         IN VARCHAR2,
        o_vacc_med_ext OUT pk_types.cursor_type,
        o_error        OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_MED_EXT';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_MED_EXT / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_med_ext(i_lang         => i_lang,
                                                          i_prof         => i_prof,
                                                          i_vacc         => i_vacc,
                                                          i_orig         => i_orig,
                                                          o_vacc_med_ext => o_vacc_med_ext,
                                                          o_error        => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_med_ext;

    FUNCTION get_vacc_dose_default
    (
        i_lang      IN language.id_language%TYPE,
        i_prof      IN profissional,
        i_id_drug   IN mi_med.id_drug%TYPE,
        o_vacc_dose OUT pk_types.cursor_type,
        o_error     OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_DOSE_DEFAULT';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_DOSE_DEFAULT / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_dose_default(i_lang      => i_lang,
                                                               i_prof      => i_prof,
                                                               i_id_drug   => i_id_drug,
                                                               o_vacc_dose => o_vacc_dose,
                                                               o_error     => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_dose_default;

    FUNCTION get_vacc_doc_list
    (
        i_lang     IN language.id_language%TYPE,
        i_prof     IN profissional,
        i_id_drug  IN mi_med.id_drug%TYPE,
        o_vacc_doc OUT pk_types.cursor_type,
        o_error    OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_DOC_LIST';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_DOC_LIST / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_doc_list(i_lang     => i_lang,
                                                           i_prof     => i_prof,
                                                           i_id_drug  => i_id_drug,
                                                           o_vacc_doc => o_vacc_doc,
                                                           o_error    => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_doc_list;

    FUNCTION get_vacc_route_list
    (
        i_lang       IN language.id_language%TYPE,
        i_prof       IN profissional,
        i_id_drug    IN mi_med.id_drug%TYPE,
        o_vacc_route OUT pk_types.cursor_type,
        o_error      OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_ROUTE_LIST';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_ROUTE_LIST / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_route_list(i_lang       => i_lang,
                                                             i_prof       => i_prof,
                                                             i_id_drug    => i_id_drug,
                                                             o_vacc_route => o_vacc_route,
                                                             o_error      => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_route_list;

    FUNCTION get_choice_type
    (
        i_lang  IN language.id_language%TYPE,
        o_type  OUT pk_types.cursor_type,
        o_error OUT t_error_out
    ) RETURN BOOLEAN IS
    
    BEGIN
    
        IF NOT pk_immunization_core.get_choice_type(i_lang => i_lang, o_type => o_type, o_error => o_error)
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => 'GET_CHOICE_TYPE',
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state;
            pk_types.open_my_cursor(o_type);
            RETURN FALSE;
    END get_choice_type;

    FUNCTION set_pat_administration
    (
        i_lang          IN language.id_language%TYPE,
        i_episode       IN drug_prescription.id_episode%TYPE,
        i_prof          IN profissional,
        i_pat           IN patient.id_patient%TYPE,
        i_drug_presc    IN drug_prescription.id_drug_prescription%TYPE,
        i_dt_begin      IN VARCHAR2,
        i_prof_cat_type IN category.flg_type%TYPE,
        i_id_drug       IN drug_presc_det.id_drug%TYPE,
        i_id_vacc       IN vacc.id_vacc%TYPE DEFAULT NULL,
        
        --adverse reaction
        i_advers_react       IN vacc_adverse_reaction.id_vacc_adverse_reaction%TYPE,
        i_notes_advers_react IN drug_presc_plan.notes_advers_react%TYPE,
        
        --Application_spot
        i_application_spot      IN drug_presc_plan.application_spot_code%TYPE DEFAULT '',
        i_application_spot_desc IN drug_presc_plan.application_spot%TYPE,
        
        i_lot_number IN drug_presc_plan.lot_number%TYPE,
        i_dt_exp     IN VARCHAR2,
        
        --Manufactured
        i_vacc_manuf      IN vacc_manufacturer.id_vacc_manufacturer%TYPE,
        i_vacc_manuf_desc IN VARCHAR2,
        
        i_dosage_admin        IN drug_presc_plan.dosage%TYPE,
        i_dosage_unit_measure IN drug_presc_plan.dosage_unit_measure%TYPE,
        
        --Administration route
        i_adm_route IN VARCHAR2,
        
        --Vaccine origin
        i_vacc_origin      IN vacc_origin.id_vacc_origin%TYPE,
        i_vacc_origin_desc IN VARCHAR2,
        
        --Docs
        i_doc_vis      IN vacc_doc_vis.id_vacc_doc_vis%TYPE,
        i_doc_vis_desc IN VARCHAR2,
        
        i_dt_doc_delivery IN VARCHAR2,
        i_doc_cat         IN vacc_funding_eligibility.id_vacc_funding_elig%TYPE,
        i_doc_source      IN vacc_funding_source.id_vacc_funding_source%TYPE,
        i_doc_source_desc IN drug_presc_plan.funding_source_desc%TYPE,
        
        --Ordered By
        i_order_by   IN professional.id_professional%TYPE,
        i_order_desc IN VARCHAR2,
        
        --Administer By
        i_administer_by   IN professional.id_professional%TYPE,
        i_administer_desc IN VARCHAR2,
        
        --Next dose schedule
        i_dt_predicted IN VARCHAR2,
        
        --Notes
        i_notes IN drug_presc_plan.notes%TYPE,
        
        o_drug_presc_plan OUT NUMBER,
        o_drug_presc_det  OUT NUMBER,
        o_flg_show        OUT VARCHAR2,
        o_msg             OUT VARCHAR2,
        o_msg_result      OUT VARCHAR2,
        o_msg_title       OUT VARCHAR2,
        o_type_admin      OUT VARCHAR2,
        o_error           OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'SET_PAT_ADMINISTRATION';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call SET_PAT_ADMINISTRATION / ' || l_params;
    
        g_retval := pk_immunization_core.set_pat_administration(i_lang                  => i_lang,
                                                                i_episode               => i_episode,
                                                                i_prof                  => i_prof,
                                                                i_pat                   => i_pat,
                                                                i_drug_presc            => i_drug_presc,
                                                                i_dt_begin              => i_dt_begin,
                                                                i_prof_cat_type         => i_prof_cat_type,
                                                                i_id_drug               => i_id_drug,
                                                                i_id_vacc               => i_id_vacc,
                                                                i_advers_react          => i_advers_react,
                                                                i_notes_advers_react    => i_notes_advers_react,
                                                                i_application_spot      => i_application_spot,
                                                                i_application_spot_desc => i_application_spot_desc,
                                                                i_lot_number            => i_lot_number,
                                                                i_dt_exp                => i_dt_exp,
                                                                i_vacc_manuf            => i_vacc_manuf,
                                                                i_vacc_manuf_desc       => i_vacc_manuf_desc,
                                                                i_dosage_admin          => i_dosage_admin,
                                                                i_dosage_unit_measure   => i_dosage_unit_measure,
                                                                i_adm_route             => i_adm_route,
                                                                i_vacc_origin           => i_vacc_origin,
                                                                i_vacc_origin_desc      => i_vacc_origin_desc,
                                                                i_doc_vis               => i_doc_vis,
                                                                i_doc_vis_desc          => i_doc_vis_desc,
                                                                i_dt_doc_delivery       => i_dt_doc_delivery,
                                                                i_doc_cat               => i_doc_cat,
                                                                i_doc_source            => i_doc_source,
                                                                i_doc_source_desc       => i_doc_source_desc,
                                                                i_order_by              => i_order_by,
                                                                i_order_desc            => i_order_desc,
                                                                i_administer_by         => i_administer_by,
                                                                i_administer_desc       => i_administer_desc,
                                                                i_dt_predicted          => i_dt_predicted,
                                                                i_notes                 => i_notes,
                                                                o_drug_presc_plan       => o_drug_presc_plan,
                                                                o_drug_presc_det        => o_drug_presc_det,
                                                                o_flg_show              => o_flg_show,
                                                                o_msg                   => o_msg,
                                                                o_msg_result            => o_msg_result,
                                                                o_msg_title             => o_msg_title,
                                                                o_type_admin            => o_type_admin,
                                                                o_error                 => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END set_pat_administration;

    /**************************************************************************
    * set_pat_administration_all
    *
    * @param i_lang                   Language ID
    * @param i_id_episode             Episode ID
    * @param i_prof                   Profissional ID
    * @param i_id_pat             Patient ID
    * @param i_dt_begin        array data for take date
    * @param i_prof_cat_type            category type
    * @param i_id_drug                      vaccination medication
    * @param i_vacc                            vaccination ID
    * @param o_drug_presc_plan
    * @param o_drug_presc_det        
    * @param o_flg_show
    * @param o_msg
    * @param o_msg_result
    * @param o_msg_title
    * @param o_type_admin          type of admin
    * @param o_error                  Error
    *
    * @author                         Lillian Lu
    * @version                        2.7.1.0
    * @since                          04/10/2019
    **************************************************************************/
    FUNCTION set_pat_administration_all
    (
        i_lang          IN language.id_language%TYPE,
        i_episode       IN drug_prescription.id_episode%TYPE,
        i_prof          IN profissional,
        i_pat           IN patient.id_patient%TYPE,
        i_drug_presc    IN table_number,
        i_dt_begin      IN table_varchar,
        i_prof_cat_type IN category.flg_type%TYPE,
        i_id_drug       IN table_varchar,
        
        i_vacc            IN table_number,
        o_drug_presc_plan OUT NOCOPY table_number,
        o_drug_presc_det  OUT NOCOPY table_number,
        o_flg_show        OUT VARCHAR2,
        o_msg             OUT VARCHAR2,
        o_msg_result      OUT VARCHAR2,
        o_msg_title       OUT VARCHAR2,
        o_type_admin      OUT VARCHAR2,
        o_error           OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'SET_PAT_ADMINISTRATION_ALL';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call SET_PAT_ADMINISTRATION_ALL / ' || l_params;
    
        g_retval := pk_immunization_core.set_pat_administration_all(i_lang            => i_lang,
                                                                    i_episode         => i_episode,
                                                                    i_prof            => i_prof,
                                                                    i_pat             => i_pat,
                                                                    i_drug_presc      => i_drug_presc,
                                                                    i_dt_begin        => i_dt_begin,
                                                                    i_prof_cat_type   => i_prof_cat_type,
                                                                    i_id_drug         => i_id_drug,
                                                                    i_vacc            => i_vacc,
                                                                    o_drug_presc_plan => o_drug_presc_plan,
                                                                    o_drug_presc_det  => o_drug_presc_det,
                                                                    o_flg_show        => o_flg_show,
                                                                    o_msg             => o_msg,
                                                                    o_msg_result      => o_msg_result,
                                                                    o_msg_title       => o_msg_title,
                                                                    o_type_admin      => o_type_admin,
                                                                    o_error           => o_error);
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END set_pat_administration_all;

    FUNCTION get_vacc_adm_take_desc
    (
        i_lang    IN language.id_language%TYPE,
        i_prof    IN profissional,
        i_id_vacc IN vacc.id_vacc%TYPE,
        i_drug    IN drug_prescription.id_drug_prescription%TYPE,
        o_desc    OUT VARCHAR2
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_ADM_TAKE_DESC';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_ADM_TAKE_DESC / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_adm_take_desc(i_lang    => i_lang,
                                                                i_prof    => i_prof,
                                                                i_id_vacc => i_id_vacc,
                                                                i_drug    => i_drug,
                                                                o_desc    => o_desc);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
        
            RETURN FALSE;
    END get_vacc_adm_take_desc;

    FUNCTION set_cancel_adm
    (
        i_lang              IN language.id_language%TYPE,
        i_prof              IN profissional,
        i_id_patient        IN patient.id_patient%TYPE,
        i_id_episode        IN episode.id_episode%TYPE,
        i_drug_prescription IN drug_prescription.id_drug_prescription%TYPE,
        i_id_cancel_reason  IN cancel_reason.id_cancel_reason%TYPE,
        i_notes_cancel      IN VARCHAR2,
        o_error             OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'SET_CANCEL_ADM';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call SET_CANCEL_ADM / ' || l_params;
    
        g_retval := pk_immunization_core.set_cancel_adm(i_lang              => i_lang,
                                                        i_prof              => i_prof,
                                                        i_id_patient        => i_id_patient,
                                                        i_id_episode        => i_id_episode,
                                                        i_drug_prescription => i_drug_prescription,
                                                        i_id_cancel_reason  => i_id_cancel_reason,
                                                        i_notes_cancel      => i_notes_cancel,
                                                        o_error             => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END set_cancel_adm;

    FUNCTION get_vacc_form_report
    (
        i_lang     IN language.id_language%TYPE,
        i_prof     IN profissional,
        i_patient  IN patient.id_patient%TYPE,
        i_vacc     IN vacc.id_vacc%TYPE,
        i_drug     IN pat_vacc_adm.id_pat_vacc_adm%TYPE,
        o_form     OUT pk_types.cursor_type,
        o_doc_show OUT VARCHAR2,
        o_error    OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_FORM_REPORT';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_FORM_REPORT / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_form_report(i_lang     => i_lang,
                                                              i_prof     => i_prof,
                                                              i_patient  => i_patient,
                                                              i_vacc     => i_vacc,
                                                              i_drug     => i_drug,
                                                              o_form     => o_form,
                                                              o_doc_show => o_doc_show,
                                                              o_error    => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_form_report;
    FUNCTION get_reported
    (
        i_lang    IN sys_domain.id_language%TYPE,
        i_prof    IN profissional,
        o_domains OUT pk_types.cursor_type,
        o_error   OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_REPORTED';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_REPORTED / ' || l_params;
    
        g_retval := pk_immunization_core.get_reported(i_lang    => i_lang,
                                                      i_prof    => i_prof,
                                                      o_domains => o_domains,
                                                      o_error   => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_reported;

    FUNCTION get_vacc_next_date
    (
        i_lang           IN language.id_language%TYPE,
        i_prof           IN profissional,
        i_patient        IN patient.id_patient%TYPE,
        i_vacc           IN vacc.id_vacc%TYPE,
        i_dt_adm_str     IN VARCHAR2,
        o_info_next_date OUT pk_types.cursor_type,
        o_error          OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_NEXT_DATE';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_NEXT_DATE / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_next_date(i_lang           => i_lang,
                                                            i_prof           => i_prof,
                                                            i_patient        => i_patient,
                                                            i_vacc           => i_vacc,
                                                            i_dt_adm_str     => i_dt_adm_str,
                                                            o_info_next_date => o_info_next_date,
                                                            o_error          => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_next_date;

    FUNCTION set_pat_report
    (
        i_lang               IN language.id_language%TYPE,
        i_id_episode         IN episode.id_episode%TYPE,
        i_prof               IN profissional,
        i_id_patient         IN patient.id_patient%TYPE,
        i_presc              IN pat_vacc_adm.id_pat_vacc_adm%TYPE,
        i_dt_begin_str       IN VARCHAR2,
        i_prof_cat_type      IN category.flg_type%TYPE,
        i_id_drug            IN mi_med.id_drug%TYPE,
        i_vacc               IN pat_vacc_adm.id_vacc%TYPE,
        i_desc_vaccine       IN pat_vacc_adm_det.desc_vaccine%TYPE,
        i_advers_react       IN vacc_adverse_reaction.id_vacc_adverse_reaction%TYPE,
        i_notes_advers_react IN drug_presc_plan.notes_advers_react%TYPE,
        
        i_application_spot_code IN pat_vacc_adm_det.application_spot_code%TYPE,
        i_application_spot      IN pat_vacc_adm_det.application_spot%TYPE DEFAULT '',
        
        i_lot_number        IN pat_vacc_adm_det.lot_number%TYPE,
        i_dt_expiration_str IN VARCHAR2,
        
        i_vacc_manuf      IN vacc_manufacturer.id_vacc_manufacturer%TYPE DEFAULT NULL,
        i_vacc_manuf_desc IN VARCHAR2,
        
        i_dosage_admin        IN pat_vacc_adm.dosage_admin%TYPE,
        i_dosage_unit_measure IN pat_vacc_adm.dosage_unit_measure%TYPE,
        
        i_adm_route IN VARCHAR2,
        
        i_vacc_origin      IN pat_vacc_adm_det.id_vacc_origin%TYPE DEFAULT NULL,
        i_vacc_origin_desc IN VARCHAR2,
        
        --Docs
        i_doc_vis      IN vacc_doc_vis.id_vacc_doc_vis%TYPE,
        i_doc_vis_desc IN VARCHAR2,
        
        i_dt_doc_delivery     IN VARCHAR2,
        i_vacc_funding_cat    IN pat_vacc_adm_det.id_vacc_funding_cat%TYPE DEFAULT NULL,
        i_vacc_funding_source IN pat_vacc_adm_det.id_vacc_funding_source%TYPE DEFAULT NULL,
        i_funding_source_desc IN pat_vacc_adm_det.funding_source_desc%TYPE DEFAULT NULL,
        
        i_information_source IN pat_vacc_adm_det.id_information_source%TYPE,
        i_report_orig        IN pat_vacc_adm_det.report_orig%TYPE,
        
        i_administred      IN pat_vacc_adm_det.id_administred%TYPE DEFAULT NULL,
        i_administred_desc IN VARCHAR2,
        
        --Next dose schedule
        i_dt_predicted IN VARCHAR2,
        
        i_notes IN pat_vacc_adm_det.notes%TYPE,
        
        o_id_admin   OUT NUMBER,
        o_type_admin OUT VARCHAR2,
        o_error      OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'SET_PAT_REPORT';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call SET_PAT_REPORT / ' || l_params;
    
        g_retval := pk_immunization_core.set_pat_report(i_lang                  => i_lang,
                                                        i_id_episode            => i_id_episode,
                                                        i_prof                  => i_prof,
                                                        i_id_patient            => i_id_patient,
                                                        i_presc                 => i_presc,
                                                        i_dt_begin_str          => i_dt_begin_str,
                                                        i_prof_cat_type         => i_prof_cat_type,
                                                        i_id_drug               => i_id_drug,
                                                        i_vacc                  => i_vacc,
                                                        i_desc_vaccine          => i_desc_vaccine,
                                                        i_advers_react          => i_advers_react,
                                                        i_notes_advers_react    => i_notes_advers_react,
                                                        i_application_spot_code => i_application_spot_code,
                                                        i_application_spot      => i_application_spot,
                                                        i_lot_number            => i_lot_number,
                                                        i_dt_expiration_str     => i_dt_expiration_str,
                                                        i_vacc_manuf            => i_vacc_manuf,
                                                        i_vacc_manuf_desc       => i_vacc_manuf_desc,
                                                        i_dosage_admin          => i_dosage_admin,
                                                        i_dosage_unit_measure   => i_dosage_unit_measure,
                                                        i_adm_route             => i_adm_route,
                                                        i_vacc_origin           => i_vacc_origin,
                                                        i_vacc_origin_desc      => i_vacc_origin_desc,
                                                        i_doc_vis               => i_doc_vis,
                                                        i_doc_vis_desc          => i_doc_vis_desc,
                                                        i_dt_doc_delivery       => i_dt_doc_delivery,
                                                        i_vacc_funding_cat      => i_vacc_funding_cat,
                                                        i_vacc_funding_source   => i_vacc_funding_source,
                                                        i_funding_source_desc   => i_funding_source_desc,
                                                        i_information_source    => i_information_source,
                                                        i_report_orig           => i_report_orig,
                                                        i_administred           => i_administred,
                                                        i_administred_desc      => i_administred_desc,
                                                        i_notes                 => i_notes,
                                                        i_flg_status            => pk_alert_constant.g_active,
                                                        i_suspended_notes       => '',
                                                        i_dt_predicted          => i_dt_predicted,
                                                        i_id_reason_sus         => NULL,
                                                        i_dt_suspended          => NULL,
                                                        o_id_admin              => o_id_admin,
                                                        o_type_admin            => o_type_admin,
                                                        o_error                 => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END set_pat_report;

    FUNCTION get_vacc_details
    
    (
        i_lang    IN language.id_language%TYPE,
        i_prof    IN profissional,
        i_patient IN patient.id_patient%TYPE,
        i_id_vacc IN vacc.id_vacc%TYPE,
        o_adm     OUT pk_types.cursor_type,
        o_desc    OUT pk_types.cursor_type,
        o_error   OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_DETAILS';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_DETAILS / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_details(i_lang    => i_lang,
                                                          i_prof    => i_prof,
                                                          i_patient => i_patient,
                                                          i_id_vacc => i_id_vacc,
                                                          o_adm     => o_adm,
                                                          o_desc    => o_desc,
                                                          o_error   => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_details;

    FUNCTION get_vacc_update
    (
        i_lang      IN language.id_language%TYPE,
        i_prof      IN profissional,
        i_patient   IN patient.id_patient%TYPE,
        i_vacc      IN vacc.id_vacc%TYPE,
        o_vacc_info OUT pk_types.cursor_type,
        o_error     OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_UPDATE';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_UPDATE / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_update(i_lang      => i_lang,
                                                         i_prof      => i_prof,
                                                         i_patient   => i_patient,
                                                         i_vacc      => i_vacc,
                                                         o_vacc_info => o_vacc_info,
                                                         o_error     => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_update;

    FUNCTION set_vacc_update
    (
        i_lang       IN language.id_language%TYPE,
        i_prof       IN profissional,
        i_patient    IN patient.id_patient%TYPE,
        i_id_episode IN episode.id_episode%TYPE,
        i_vacc       IN vacc.id_vacc%TYPE,
        i_reg        IN vacc_update_admin.id_reg%TYPE,
        i_type       IN vacc_update_admin.flg_type%TYPE,
        i_dt_admin   IN VARCHAR2,
        o_error      OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'SET_VACC_UPDATE';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call SET_VACC_UPDATE / ' || l_params;
    
        g_retval := pk_immunization_core.set_vacc_update(i_lang       => i_lang,
                                                         i_prof       => i_prof,
                                                         i_patient    => i_patient,
                                                         i_id_episode => i_id_episode,
                                                         i_vacc       => i_vacc,
                                                         i_reg        => i_reg,
                                                         i_type       => i_type,
                                                         i_dt_admin   => i_dt_admin,
                                                         o_error      => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END set_vacc_update;

    FUNCTION get_vacc_rep_take_desc
    (
        i_lang         IN language.id_language%TYPE,
        i_prof         IN profissional,
        i_id_vacc      IN vacc.id_vacc%TYPE,
        i_pat_vacc_adm IN pat_vacc_adm.id_pat_vacc_adm%TYPE,
        o_desc         OUT VARCHAR2
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_REP_TAKE_DESC';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_REP_TAKE_DESC / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_rep_take_desc(i_lang         => i_lang,
                                                                i_prof         => i_prof,
                                                                i_id_vacc      => i_id_vacc,
                                                                i_pat_vacc_adm => i_pat_vacc_adm,
                                                                o_desc         => o_desc);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
        
            RETURN FALSE;
    END get_vacc_rep_take_desc;

    FUNCTION set_cancel_report
    (
        i_lang             IN language.id_language%TYPE,
        i_prof             IN profissional,
        i_id_patient       IN patient.id_patient%TYPE,
        i_id_episode       IN episode.id_episode%TYPE,
        i_vacc_presc_id    IN pat_vacc_adm.id_pat_vacc_adm%TYPE,
        i_id_cancel_reason IN cancel_reason.id_cancel_reason%TYPE,
        i_notes_cancel     IN VARCHAR2,
        o_error            OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'SET_CANCEL_REPORT';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call SET_CANCEL_REPORT / ' || l_params;
    
        g_retval := pk_immunization_core.set_cancel_report(i_lang             => i_lang,
                                                           i_prof             => i_prof,
                                                           i_id_patient       => i_id_patient,
                                                           i_id_episode       => i_id_episode,
                                                           i_vacc_presc_id    => i_vacc_presc_id,
                                                           i_id_cancel_reason => i_id_cancel_reason,
                                                           i_notes_cancel     => i_notes_cancel,
                                                           o_error            => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END set_cancel_report;

    FUNCTION get_vacc_descontinue_desc
    (
        i_lang    IN language.id_language%TYPE,
        i_prof    IN profissional,
        i_pat     IN patient.id_patient%TYPE,
        i_id_vacc IN vacc.id_vacc%TYPE,
        i_dose    IN NUMBER,
        o_desc    OUT VARCHAR2,
        o_error   OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_DESCONTINUE_DESC';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_DESCONTINUE_DESC / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_descontinue_desc(i_lang    => i_lang,
                                                                   i_prof    => i_prof,
                                                                   i_pat     => i_pat,
                                                                   i_id_vacc => i_id_vacc,
                                                                   i_dose    => i_dose,
                                                                   o_desc    => o_desc,
                                                                   o_error   => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_descontinue_desc;

    FUNCTION set_discontinue_dose
    (
        i_lang            IN language.id_language%TYPE,
        i_prof            IN profissional,
        i_id_patient      IN patient.id_patient%TYPE,
        i_id_episode      IN episode.id_episode%TYPE,
        i_vacc            IN pat_vacc_adm.id_vacc%TYPE,
        i_id_reason_sus   IN pat_vacc_adm_det.id_reason_sus%TYPE,
        i_suspended_notes IN pat_vacc_adm_det.suspended_notes%TYPE,
        o_error           OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'SET_DISCONTINUE_DOSE';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call SET_DISCONTINUE_DOSE / ' || l_params;
    
        g_retval := pk_immunization_core.set_discontinue_dose(i_lang            => i_lang,
                                                              i_prof            => i_prof,
                                                              i_id_patient      => i_id_patient,
                                                              i_id_episode      => i_id_episode,
                                                              i_vacc            => i_vacc,
                                                              i_id_reason_sus   => i_id_reason_sus,
                                                              i_suspended_notes => i_suspended_notes,
                                                              o_error           => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END set_discontinue_dose;

    FUNCTION set_vacc_status
    (
        i_lang       IN language.id_language%TYPE,
        i_prof       IN profissional,
        i_id_patient IN patient.id_patient%TYPE,
        i_episode    IN episode.id_episode%TYPE,
        i_vacc       IN vacc.id_vacc%TYPE,
        i_status     IN pat_vacc.flg_status%TYPE,
        i_id_reason  IN NUMBER,
        i_notes      IN VARCHAR2,
        o_error      OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'SET_VACC_STATUS';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call SET_VACC_STATUS / ' || l_params;
    
        g_retval := pk_immunization_core.set_vacc_status(i_lang       => i_lang,
                                                         i_prof       => i_prof,
                                                         i_id_patient => i_id_patient,
                                                         i_episode    => i_episode,
                                                         i_vacc       => i_vacc,
                                                         i_status     => i_status,
                                                         i_id_reason  => i_id_reason,
                                                         i_notes      => i_notes,
                                                         o_error      => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END set_vacc_status;

    FUNCTION set_resume_dose
    (
        i_lang       IN language.id_language%TYPE,
        i_prof       IN profissional,
        i_id_patient IN patient.id_patient%TYPE,
        i_id_episode IN episode.id_episode%TYPE,
        i_vacc       IN pat_vacc_adm.id_vacc%TYPE,
        i_drug       IN pat_vacc_adm.id_pat_vacc_adm%TYPE,
        o_error      OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'SET_RESUME_DOSE';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call SET_RESUME_DOSE / ' || l_params;
    
        g_retval := pk_immunization_core.set_resume_dose(i_lang       => i_lang,
                                                         i_prof       => i_prof,
                                                         i_id_patient => i_id_patient,
                                                         i_id_episode => i_id_episode,
                                                         i_vacc       => i_vacc,
                                                         i_drug       => i_drug,
                                                         o_error      => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END set_resume_dose;

    FUNCTION get_advers_react
    (
        i_lang      IN language.id_language%TYPE,
        i_prof      IN profissional,
        i_value     IN NUMBER,
        i_type_vacc IN VARCHAR2,
        o_id_value  OUT NUMBER,
        o_notes     OUT VARCHAR2,
        --ERROR
        o_error OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_ADVERS_REACT';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_ADVERS_REACT / ' || l_params;
    
        g_retval := pk_immunization_core.get_advers_react(i_lang      => i_lang,
                                                          i_prof      => i_prof,
                                                          i_value     => i_value,
                                                          i_type_vacc => i_type_vacc,
                                                          o_id_value  => o_id_value,
                                                          o_notes     => o_notes,
                                                          o_error     => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_advers_react;

    FUNCTION set_advers_react
    (
        i_lang       IN language.id_language%TYPE,
        i_prof       IN profissional,
        i_id_patient IN patient.id_patient%TYPE,
        i_id_episode IN episode.id_episode%TYPE,
        i_id_reg     IN drug_prescription.id_drug_prescription%TYPE,
        i_value      IN vacc_advers_react.id_vacc_adver_reac%TYPE,
        i_notes      IN vacc_advers_react.notes_advers_react%TYPE,
        i_type_vacc  IN VARCHAR2,
        o_error      OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'SET_ADVERS_REACT';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call SET_ADVERS_REACT / ' || l_params;
    
        g_retval := pk_immunization_core.set_advers_react(i_lang       => i_lang,
                                                          i_prof       => i_prof,
                                                          i_id_patient => i_id_patient,
                                                          i_id_episode => i_id_episode,
                                                          i_id_reg     => i_id_reg,
                                                          i_value      => i_value,
                                                          i_notes      => i_notes,
                                                          i_type_vacc  => i_type_vacc,
                                                          o_error      => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END set_advers_react;

    FUNCTION set_tuberculin_test_adm
    (
        i_lang                IN language.id_language%TYPE,
        i_patient             IN patient.id_patient%TYPE,
        i_prof                IN profissional,
        i_episode             IN episode.id_episode%TYPE,
        i_test_id             IN NUMBER,
        i_dt_adm              IN VARCHAR2,
        i_lote_adm            IN VARCHAR2,
        i_dt_valid            IN VARCHAR2,
        i_app_place           IN VARCHAR2,
        i_prof_write          professional.id_professional%TYPE,
        i_notes               IN VARCHAR2,
        i_vacc_manuf          IN vacc_manufacturer.id_vacc_manufacturer%TYPE DEFAULT NULL,
        code_mvx              IN vacc_manufacturer.code_mvx%TYPE DEFAULT NULL,
        i_flg_type_date       IN drug_presc_plan.flg_type_date%TYPE,
        i_dosage_admin        IN drug_presc_plan.dosage%TYPE,
        i_dosage_unit_measure IN drug_presc_plan.dosage_unit_measure%TYPE,
        --ERROR
        o_error OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'SET_TUBERCULIN_TEST_ADM';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call SET_TUBERCULIN_TEST_ADM / ' || l_params;
    
        g_retval := pk_immunization_core.set_tuberculin_test_adm(i_lang                => i_lang,
                                                                 i_patient             => i_patient,
                                                                 i_prof                => i_prof,
                                                                 i_episode             => i_episode,
                                                                 i_test_id             => i_test_id,
                                                                 i_dt_adm              => i_dt_adm,
                                                                 i_lote_adm            => i_lote_adm,
                                                                 i_dt_valid            => i_dt_valid,
                                                                 i_app_place           => i_app_place,
                                                                 i_prof_write          => i_prof_write,
                                                                 i_notes               => i_notes,
                                                                 i_vacc_manuf          => i_vacc_manuf,
                                                                 code_mvx              => code_mvx,
                                                                 i_flg_type_date       => i_flg_type_date,
                                                                 i_dosage_admin        => i_dosage_admin,
                                                                 i_dosage_unit_measure => i_dosage_unit_measure,
                                                                 o_error               => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END set_tuberculin_test_adm;

    FUNCTION get_cancel_info
    (
        i_lang    IN language.id_language%TYPE,
        i_patient IN patient.id_patient%TYPE,
        i_prof    IN profissional,
        --presc_det
        i_cancel_id IN pat_vacc_adm.id_pat_vacc_adm%TYPE,
        i_cancel_op IN VARCHAR,
        --Out
        o_main_title  OUT VARCHAR2,
        o_notes_title OUT VARCHAR2,
        --Administration
        o_adm_title OUT VARCHAR2,
        o_adm_det   OUT pk_types.cursor_type,
        --Prescription
        o_presc_title OUT VARCHAR2,
        o_presc_det   OUT pk_types.cursor_type,
        --ERROR
        o_error OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_CANCEL_INFO';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_CANCEL_INFO / ' || l_params;
    
        g_retval := pk_immunization_core.get_cancel_info(i_lang        => i_lang,
                                                         i_patient     => i_patient,
                                                         i_prof        => i_prof,
                                                         i_cancel_id   => i_cancel_id,
                                                         i_cancel_op   => i_cancel_op,
                                                         o_main_title  => o_main_title,
                                                         o_notes_title => o_notes_title,
                                                         o_adm_title   => o_adm_title,
                                                         o_adm_det     => o_adm_det,
                                                         o_presc_title => o_presc_title,
                                                         o_presc_det   => o_presc_det,
                                                         o_error       => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_cancel_info;

    FUNCTION set_cancel_info
    (
        i_lang IN language.id_language%TYPE,
        i_prof IN profissional,
        --presc_det
        i_cancel_id    IN pat_vacc_adm.id_pat_vacc_adm%TYPE,
        i_cancel_op    IN VARCHAR,
        i_notes_cancel IN VARCHAR2,
        --ERROR
        o_error OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'SET_CANCEL_INFO';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call SET_CANCEL_INFO / ' || l_params;
    
        g_retval := pk_immunization_core.set_cancel_info(i_lang         => i_lang,
                                                         i_prof         => i_prof,
                                                         i_cancel_id    => i_cancel_id,
                                                         i_cancel_op    => i_cancel_op,
                                                         i_notes_cancel => i_notes_cancel,
                                                         o_error        => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END set_cancel_info;

    FUNCTION get_tuberculin_test_warnings
    (
        i_lang    IN language.id_language%TYPE,
        i_prof    IN profissional,
        i_test_id IN drug_prescription.id_drug_prescription%TYPE,
        --OUT
        o_flg_show       OUT VARCHAR2,
        o_message_title  OUT VARCHAR2,
        o_message_text   OUT VARCHAR2,
        o_forward_button OUT VARCHAR2,
        o_back_button    OUT VARCHAR2,
        o_error          OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_TUBERCULIN_TEST_WARNINGS';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_TUBERCULIN_TEST_WARNINGS / ' || l_params;
    
        g_retval := pk_immunization_core.get_tuberculin_test_warnings(i_lang           => i_lang,
                                                                      i_prof           => i_prof,
                                                                      i_test_id        => i_test_id,
                                                                      o_flg_show       => o_flg_show,
                                                                      o_message_title  => o_message_title,
                                                                      o_message_text   => o_message_text,
                                                                      o_forward_button => o_forward_button,
                                                                      o_back_button    => o_back_button,
                                                                      o_error          => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_tuberculin_test_warnings;

    FUNCTION get_tuberculin_test_add
    (
        i_lang    IN language.id_language%TYPE,
        i_patient IN patient.id_patient%TYPE,
        i_prof    IN profissional,
        i_test_id IN NUMBER,
        --OUT
        o_main_title OUT VARCHAR2,
        --test info
        o_test_info OUT pk_types.cursor_type,
        --Results
        o_res_title OUT VARCHAR2,
        o_res_det   OUT pk_types.cursor_type,
        --Administration
        o_adm_title OUT VARCHAR2,
        o_adm_det   OUT pk_types.cursor_type,
        --Prescription
        o_presc_title OUT VARCHAR2,
        o_presc_det   OUT pk_types.cursor_type,
        --ERROR
        o_error OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_TUBERCULIN_TEST_ADD';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_TUBERCULIN_TEST_ADD / ' || l_params;
    
        g_retval := pk_immunization_core.get_tuberculin_test_add(i_lang        => i_lang,
                                                                 i_patient     => i_patient,
                                                                 i_prof        => i_prof,
                                                                 i_test_id     => i_test_id,
                                                                 o_main_title  => o_main_title,
                                                                 o_test_info   => o_test_info,
                                                                 o_res_title   => o_res_title,
                                                                 o_res_det     => o_res_det,
                                                                 o_adm_title   => o_adm_title,
                                                                 o_adm_det     => o_adm_det,
                                                                 o_presc_title => o_presc_title,
                                                                 o_presc_det   => o_presc_det,
                                                                 o_error       => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_tuberculin_test_add;

    FUNCTION get_adm_prof_list
    (
        i_lang      IN language.id_language%TYPE,
        i_prof      IN profissional,
        o_prof_list OUT pk_types.cursor_type,
        o_error     OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_ADM_PROF_LIST';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_ADM_PROF_LIST / ' || l_params;
    
        g_retval := pk_immunization_core.get_adm_prof_list(i_lang      => i_lang,
                                                           i_prof      => i_prof,
                                                           o_prof_list => o_prof_list,
                                                           o_error     => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_adm_prof_list;
    FUNCTION get_notes_advers_react_list
    (
        i_lang    IN language.id_language%TYPE,
        o_ap_spot OUT pk_types.cursor_type,
        o_error   OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_NOTES_ADVERS_REACT_LIST';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_NOTES_ADVERS_REACT_LIST / ' || l_params;
    
        g_retval := pk_immunization_core.get_notes_advers_react_list(i_lang    => i_lang,
                                                                     o_ap_spot => o_ap_spot,
                                                                     o_error   => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_notes_advers_react_list;

    FUNCTION get_reading_unit_list
    (
        i_lang      IN language.id_language%TYPE,
        o_read_unit OUT pk_types.cursor_type,
        o_error     OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_READING_UNIT_LIST';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        --        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_READING_UNIT_LIST / ' || l_params;
    
        g_retval := pk_immunization_core.get_reading_unit_list(i_lang      => i_lang,
                                                               o_read_unit => o_read_unit,
                                                               o_error     => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_reading_unit_list;
    FUNCTION get_evaluation_values
    (
        i_lang  IN language.id_language%TYPE,
        i_prof  IN profissional,
        o_param OUT pk_types.cursor_type,
        o_error OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_EVALUATION_VALUES';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_EVALUATION_VALUES / ' || l_params;
    
        g_retval := pk_immunization_core.get_evaluation_values(i_lang  => i_lang,
                                                               i_prof  => i_prof,
                                                               o_param => o_param,
                                                               o_error => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_evaluation_values;

    FUNCTION set_tuberculin_test_res
    (
        i_lang          IN language.id_language%TYPE,
        i_patient       IN patient.id_patient%TYPE,
        i_prof          IN profissional,
        i_test_id       IN drug_prescription.id_drug_prescription%TYPE,
        i_dt_read       IN VARCHAR2,
        i_value         IN drug_presc_result.value%TYPE,
        i_evaluation    IN drug_presc_result.evaluation%TYPE,
        i_evaluation_id IN drug_presc_result.id_evaluation%TYPE,
        i_reactions     IN drug_presc_result.notes_advers_react%TYPE,
        i_prof_write    IN professional.id_professional%TYPE,
        i_notes         IN VARCHAR2,
        --ERROR
        o_error OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'SET_TUBERCULIN_TEST_RES';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call SET_TUBERCULIN_TEST_RES / ' || l_params;
    
        g_retval := pk_immunization_core.set_tuberculin_test_res(i_lang          => i_lang,
                                                                 i_patient       => i_patient,
                                                                 i_prof          => i_prof,
                                                                 i_test_id       => i_test_id,
                                                                 i_dt_read       => i_dt_read,
                                                                 i_value         => i_value,
                                                                 i_evaluation    => i_evaluation,
                                                                 i_evaluation_id => i_evaluation_id,
                                                                 i_reactions     => i_reactions,
                                                                 i_prof_write    => i_prof_write,
                                                                 i_notes         => i_notes,
                                                                 o_error         => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END set_tuberculin_test_res;

    FUNCTION get_care_dash_vacc
    (
        i_lang    IN language.id_language%TYPE,
        i_prof    IN profissional,
        i_patient IN patient.id_patient%TYPE,
        o_vacc    OUT pk_types.cursor_type,
        o_error   OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_CARE_DASH_VACC';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_CARE_DASH_VACC / ' || l_params;
    
        g_retval := pk_immunization_core.get_care_dash_vacc(i_lang    => i_lang,
                                                            i_prof    => i_prof,
                                                            i_patient => i_patient,
                                                            o_vacc    => o_vacc,
                                                            o_error   => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_care_dash_vacc;

    FUNCTION get_vacc_funding_type
    (
        i_lang      IN language.id_language%TYPE,
        i_prof      IN profissional,
        o_vacc_type OUT pk_types.cursor_type,
        o_error     OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_FUNDING_TYPE';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_FUNDING_TYPE / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_funding_type(i_lang      => i_lang,
                                                               i_prof      => i_prof,
                                                               o_vacc_type => o_vacc_type,
                                                               o_error     => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_funding_type;

    FUNCTION get_vacc_origin_list
    (
        i_lang        IN language.id_language%TYPE,
        i_prof        IN profissional,
        o_vacc_origin OUT pk_types.cursor_type,
        o_error       OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_ORIGIN_LIST';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_ORIGIN_LIST / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_origin_list(i_lang        => i_lang,
                                                              i_prof        => i_prof,
                                                              o_vacc_origin => o_vacc_origin,
                                                              o_error       => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_origin_list;

    FUNCTION get_vacc_funding_source
    (
        i_lang        IN language.id_language%TYPE,
        i_prof        IN profissional,
        o_vacc_source OUT pk_types.cursor_type,
        o_error       OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_FUNDING_SOURCE';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_FUNDING_SOURCE / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_funding_source(i_lang        => i_lang,
                                                                 i_prof        => i_prof,
                                                                 o_vacc_source => o_vacc_source,
                                                                 o_error       => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_funding_source;

    FUNCTION get_adverse_reaction_list
    (
        i_lang         IN language.id_language%TYPE,
        i_prof         IN profissional,
        o_adv_reaction OUT pk_types.cursor_type,
        o_error        OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_ADVERSE_REACTION_LIST';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_ADVERSE_REACTION_LIST / ' || l_params;
    
        g_retval := pk_immunization_core.get_adverse_reaction_list(i_lang         => i_lang,
                                                                   i_prof         => i_prof,
                                                                   o_adv_reaction => o_adv_reaction,
                                                                   o_error        => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_adverse_reaction_list;

    FUNCTION get_order_by_prof_list
    (
        i_lang      IN language.id_language%TYPE,
        i_prof      IN profissional,
        o_prof_list OUT pk_types.cursor_type,
        o_error     OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_ORDER_BY_PROF_LIST';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_ORDER_BY_PROF_LIST / ' || l_params;
    
        g_retval := pk_immunization_core.get_order_by_prof_list(i_lang      => i_lang,
                                                                i_prof      => i_prof,
                                                                o_prof_list => o_prof_list,
                                                                o_error     => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_order_by_prof_list;

    FUNCTION get_pnv_detail
    (
        i_lang       IN language.id_language%TYPE,
        i_prof       IN profissional,
        i_id_patient IN patient.id_patient%TYPE,
        i_id_episode IN episode.id_episode%TYPE,
        o_detail     OUT pk_types.cursor_type,
        o_error      OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_PNV_DETAIL';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_PNV_DETAIL / ' || l_params;
    
        g_retval := pk_immunization_core.get_pnv_detail(i_lang       => i_lang,
                                                        i_prof       => i_prof,
                                                        i_id_patient => i_id_patient,
                                                        i_id_episode => i_id_episode,
                                                        o_detail     => o_detail,
                                                        o_error      => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_pnv_detail;

    FUNCTION get_tuberculin_tests_detail
    (
        i_lang    IN language.id_language%TYPE,
        i_patient IN patient.id_patient%TYPE,
        i_prof    IN profissional,
        --OUT
        --titles
        o_main_title         OUT VARCHAR2,
        o_this_take_title    OUT VARCHAR2,
        o_history_take_title OUT VARCHAR2,
        o_detail_info        OUT VARCHAR2,
        --test info
        o_test_info OUT pk_types.cursor_type,
        --Cancel
        o_can_title OUT VARCHAR2,
        o_can_det   OUT pk_types.cursor_type,
        --Results
        o_res_title OUT VARCHAR2,
        o_res_det   OUT pk_types.cursor_type,
        --Administration
        o_adm_title OUT VARCHAR2,
        o_adm_det   OUT pk_types.cursor_type,
        --Prescription
        o_presc_title OUT VARCHAR2,
        o_presc_det   OUT pk_types.cursor_type,
        -- Adverses React
        o_advers_react_title OUT VARCHAR2,
        o_advers_react_det   OUT pk_types.cursor_type,
        --ERROR
        o_error OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_TUBERCULIN_TESTS_DETAIL';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_TUBERCULIN_TESTS_DETAIL / ' || l_params;
    
        g_retval := pk_immunization_core.get_tuberculin_tests_detail(i_lang               => i_lang,
                                                                     i_patient            => i_patient,
                                                                     i_prof               => i_prof,
                                                                     o_main_title         => o_main_title,
                                                                     o_this_take_title    => o_this_take_title,
                                                                     o_history_take_title => o_history_take_title,
                                                                     o_detail_info        => o_detail_info,
                                                                     o_test_info          => o_test_info,
                                                                     o_can_title          => o_can_title,
                                                                     o_can_det            => o_can_det,
                                                                     o_res_title          => o_res_title,
                                                                     o_res_det            => o_res_det,
                                                                     o_adm_title          => o_adm_title,
                                                                     o_adm_det            => o_adm_det,
                                                                     o_presc_title        => o_presc_title,
                                                                     o_presc_det          => o_presc_det,
                                                                     o_advers_react_title => o_advers_react_title,
                                                                     o_advers_react_det   => o_advers_react_det,
                                                                     o_error              => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_tuberculin_tests_detail;

    FUNCTION get_vacc_dose_info_detail_new
    (
        i_lang     IN language.id_language%TYPE,
        i_prof     IN profissional,
        i_vacc     IN vacc.id_vacc%TYPE,
        i_pat      IN patient.id_patient%TYPE,
        i_emb      IN me_med.emb_id%TYPE,
        o_info     OUT pk_types.cursor_type,
        o_info_age OUT pk_types.cursor_type,
        o_error    OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_DOSE_INFO_DETAIL_NEW';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_DOSE_INFO_DETAIL_NEW / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_dose_info_detail_new(i_lang     => i_lang,
                                                                       i_prof     => i_prof,
                                                                       i_vacc     => i_vacc,
                                                                       i_pat      => i_pat,
                                                                       i_emb      => i_emb,
                                                                       o_info     => o_info,
                                                                       o_info_age => o_info_age,
                                                                       o_error    => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_dose_info_detail_new;

    FUNCTION get_vacc_last_take_detail_new
    (
        i_lang            IN language.id_language%TYPE,
        i_vacc            IN vacc.id_vacc%TYPE,
        i_pat             IN patient.id_patient%TYPE,
        i_emb             IN me_med.emb_id%TYPE,
        i_prof            IN profissional,
        o_info_last_take  OUT pk_types.cursor_type,
        o_predicted_take  OUT pk_types.cursor_type,
        o_predicted_label OUT VARCHAR2,
        o_error           OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_LAST_TAKE_DETAIL_NEW';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_VACC_LAST_TAKE_DETAIL_NEW / ' || l_params;
    
        g_retval := pk_immunization_core.get_vacc_last_take_detail_new(i_lang            => i_lang,
                                                                       i_vacc            => i_vacc,
                                                                       i_pat             => i_pat,
                                                                       i_emb             => i_emb,
                                                                       i_prof            => i_prof,
                                                                       o_info_last_take  => o_info_last_take,
                                                                       o_predicted_take  => o_predicted_take,
                                                                       o_predicted_label => o_predicted_label,
                                                                       o_error           => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_last_take_detail_new;

    FUNCTION get_label_print
    (
        i_lang              IN language.id_language%TYPE,
        i_prof              IN profissional,
        i_episode           IN episode.id_episode%TYPE,
        i_patient           IN patient.id_patient%TYPE,
        i_id_admin          IN pat_vacc_adm.id_pat_vacc_adm%TYPE,
        i_type_admin        IN VARCHAR2,
        o_printer           OUT VARCHAR2,
        o_codification_type OUT VARCHAR2,
        o_barcode_pat       OUT VARCHAR2,
        o_error             OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_LABEL_PRINT';
        l_params VARCHAR2(1000 CHAR);
    BEGIN
        l_params := 'i_prof=' || pk_utils.to_string(i_prof);
    
        -- init
        g_error := 'Init ' || l_func_name || ' / ' || l_params;
        pk_alertlog.log_debug(text => g_error, object_name => g_package, sub_object_name => l_func_name);
        g_error := 'Call GET_LABEL_PRINT / ' || l_params;
    
        g_retval := pk_immunization_core.get_label_print(i_lang              => i_lang,
                                                         i_prof              => i_prof,
                                                         i_episode           => i_episode,
                                                         i_patient           => i_patient,
                                                         i_id_admin          => i_id_admin,
                                                         i_type_admin        => i_type_admin,
                                                         o_printer           => o_printer,
                                                         o_codification_type => o_codification_type,
                                                         o_barcode_pat       => o_barcode_pat,
                                                         o_error             => o_error);
    
        IF NOT g_retval
        THEN
            RAISE g_exception_np;
        END IF;
    
        RETURN TRUE;
    
    EXCEPTION
        WHEN g_exception_np THEN
            pk_alertlog.log_warn(g_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_label_print;

    FUNCTION get_vacc_doc_value
    (
        i_lang         IN language.id_language%TYPE,
        i_prof         IN profissional,
        i_barcode_desc IN VARCHAR2,
        o_vacc_doc     OUT pk_types.cursor_type,
        o_error        OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_VACC_DOC_VALUE';
    BEGIN
    
        IF NOT pk_vacc.get_vacc_doc_value(i_lang         => i_lang,
                                          i_prof         => i_prof,
                                          i_barcode_desc => i_barcode_desc,
                                          o_vacc_doc     => o_vacc_doc,
                                          o_error        => o_error)
        THEN
            RAISE g_exception_np;
        END IF;
        RETURN TRUE;
    EXCEPTION
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vacc_doc_value;

    FUNCTION get_vaccines_detail
    (
        i_lang    IN language.id_language%TYPE,
        i_patient IN patient.id_patient%TYPE,
        i_prof    IN profissional,
        i_vacc_id IN vacc.id_vacc%TYPE,
        --OUT
        --titles
        o_main_title         OUT VARCHAR2,
        o_this_take_title    OUT VARCHAR2,
        o_history_take_title OUT VARCHAR2,
        o_detail_info        OUT VARCHAR2,
        o_vacc_name          OUT VARCHAR2,
        --test info
        o_test_info OUT pk_types.cursor_type,
        --Cancel
        o_can_title OUT VARCHAR2,
        o_can_det   OUT pk_types.cursor_type,
        --Administration
        o_adm_title OUT VARCHAR2,
        o_adm_det   OUT pk_types.cursor_type,
        --Prescription
        o_presc_title OUT VARCHAR2,
        o_presc_det   OUT pk_types.cursor_type,
        --Advers React
        o_advers_react_title OUT VARCHAR2,
        o_advers_react_det   OUT pk_types.cursor_type,
        --ERROR
        o_error OUT t_error_out
    ) RETURN BOOLEAN IS
    BEGIN
        IF NOT pk_vacc.get_vaccines_detail(i_lang               => i_lang,
                                           i_patient            => i_patient,
                                           i_prof               => i_prof,
                                           i_vacc_id            => i_vacc_id,
                                           o_main_title         => o_main_title,
                                           o_this_take_title    => o_this_take_title,
                                           o_history_take_title => o_history_take_title,
                                           o_detail_info        => o_detail_info,
                                           o_vacc_name          => o_vacc_name,
                                           o_test_info          => o_test_info,
                                           o_can_title          => o_can_title,
                                           o_can_det            => o_can_det,
                                           o_adm_title          => o_adm_title,
                                           o_adm_det            => o_adm_det,
                                           o_presc_title        => o_presc_title,
                                           o_presc_det          => o_presc_det,
                                           o_advers_react_title => o_advers_react_title,
                                           o_advers_react_det   => o_advers_react_det,
                                           o_error              => o_error)
        THEN
            RAISE g_exception_np;
        END IF;
        RETURN TRUE;
    EXCEPTION
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => 'get_vaccines_detail',
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vaccines_detail;

    FUNCTION get_med_descr
    (
        i_lang      IN language.id_language%TYPE,
        i_prof      IN profissional,
        i_emb       IN me_med.emb_id%TYPE,
        i_med       IN VARCHAR2,
        o_med_descr OUT pk_types.cursor_type,
        o_error     OUT t_error_out
    ) RETURN BOOLEAN IS
        l_func_name CONSTANT VARCHAR2(100 CHAR) := 'GET_MED_DESCR';
    BEGIN
    
        IF NOT pk_vacc.get_med_descr(i_lang      => i_lang,
                                     i_prof      => i_prof,
                                     i_emb       => i_emb,
                                     i_med       => i_med,
                                     o_med_descr => o_med_descr,
                                     o_error     => o_error)
        THEN
            RAISE g_exception_np;
        END IF;
        RETURN TRUE;
    EXCEPTION
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => l_func_name,
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_med_descr;

    FUNCTION set_pat_vacc_adm
    (
        i_lang                IN language.id_language%TYPE,
        i_prof                IN profissional,
        i_vacc                IN table_number,
        i_emb                 IN table_varchar,
        i_id_patient          IN patient.id_patient%TYPE,
        i_id_episode          IN episode.id_episode%TYPE,
        i_dt_begin_str        IN VARCHAR2,
        i_presc               IN prescription.id_prescription%TYPE,
        i_flg_status          IN pat_vacc_adm.flg_status%TYPE,
        i_flg_orig            IN pat_vacc_adm.flg_orig%TYPE,
        i_desc_vaccine        IN pat_vacc_adm_det.desc_vaccine%TYPE,
        i_flg_advers_react    IN VARCHAR2,
        i_notes_advers_react  IN pat_vacc_adm_det.notes_advers_react%TYPE,
        i_application_spot    IN pat_vacc_adm_det.application_spot%TYPE,
        i_lot_number          IN pat_vacc_adm_det.lot_number%TYPE,
        i_dt_expiration_str   IN VARCHAR2,
        i_report_orig         IN pat_vacc_adm_det.report_orig%TYPE,
        i_notes               IN pat_vacc_adm_det.notes%TYPE,
        i_flg_time            IN table_varchar,
        i_takes               IN table_number,
        i_dosage              IN table_number,
        i_unit_measure        IN table_number,
        i_dt_presc            IN VARCHAR2,
        i_notes_presc         IN pat_vacc_adm.notes_presc%TYPE,
        i_prof_presc          IN pat_vacc_adm.prof_presc%TYPE,
        i_test                IN VARCHAR2,
        i_prof_cat_type       IN category.flg_type%TYPE,
        i_dt_predicted        IN VARCHAR2,
        i_flg_reported        IN VARCHAR2 DEFAULT NULL,
        i_vacc_manuf          IN vacc_manufacturer.id_vacc_manufacturer%TYPE DEFAULT NULL,
        code_mvx              IN vacc_manufacturer.code_mvx%TYPE DEFAULT NULL,
        i_flg_type_date       IN pat_vacc_adm.flg_type_date%TYPE,
        i_dosage_admin        IN table_number,
        i_dosage_unit_measure IN table_number,
        o_flg_show            OUT VARCHAR2,
        o_msg                 OUT VARCHAR2,
        o_msg_req             OUT VARCHAR2,
        o_msg_title           OUT VARCHAR2,
        o_button              OUT VARCHAR2,
        o_id_admin            OUT NUMBER,
        o_type_admin          OUT VARCHAR2,
        o_error               OUT t_error_out
    ) RETURN BOOLEAN IS
    BEGIN
    
        IF NOT pk_vacc.set_pat_vacc_adm(i_lang                => i_lang,
                                        i_prof                => i_prof,
                                        i_vacc                => i_vacc,
                                        i_emb                 => i_emb,
                                        i_id_patient          => i_id_patient,
                                        i_id_episode          => i_id_episode,
                                        i_dt_begin_str        => i_dt_begin_str,
                                        i_presc               => i_presc,
                                        i_flg_status          => i_flg_status,
                                        i_flg_orig            => i_flg_orig,
                                        i_desc_vaccine        => i_desc_vaccine,
                                        i_flg_advers_react    => i_flg_advers_react,
                                        i_notes_advers_react  => i_notes_advers_react,
                                        i_application_spot    => i_application_spot,
                                        i_lot_number          => i_lot_number,
                                        i_dt_expiration_str   => i_dt_expiration_str,
                                        i_report_orig         => i_report_orig,
                                        i_notes               => i_notes,
                                        i_flg_time            => i_flg_time,
                                        i_takes               => i_takes,
                                        i_dosage              => i_dosage,
                                        i_unit_measure        => i_unit_measure,
                                        i_dt_presc            => i_dt_presc,
                                        i_notes_presc         => i_notes_presc,
                                        i_prof_presc          => i_prof_presc,
                                        i_test                => i_test,
                                        i_prof_cat_type       => i_prof_cat_type,
                                        i_dt_predicted        => i_dt_predicted,
                                        i_flg_reported        => i_flg_reported,
                                        i_vacc_manuf          => i_vacc_manuf,
                                        code_mvx              => code_mvx,
                                        i_flg_type_date       => i_flg_type_date,
                                        i_dosage_admin        => i_dosage_admin,
                                        i_dosage_unit_measure => i_dosage_unit_measure,
                                        o_flg_show            => o_flg_show,
                                        o_msg                 => o_msg,
                                        o_msg_req             => o_msg_req,
                                        o_msg_title           => o_msg_title,
                                        o_button              => o_button,
                                        o_id_admin            => o_id_admin,
                                        o_type_admin          => o_type_admin,
                                        o_error               => o_error)
        THEN
            RAISE g_exception_np;
        END IF;
        RETURN TRUE;
    EXCEPTION
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => 'SET_PAT_VACC_ADM',
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END set_pat_vacc_adm;

    FUNCTION get_vaccines_detail_free_text
    (
        i_lang    IN language.id_language%TYPE,
        i_patient IN patient.id_patient%TYPE,
        i_prof    IN profissional,
        i_reg     IN pat_vacc_adm.id_pat_vacc_adm%TYPE,
        --OUT
        --titles
        o_main_title         OUT VARCHAR2,
        o_this_take_title    OUT VARCHAR2,
        o_history_take_title OUT VARCHAR2,
        o_detail_info        OUT VARCHAR2,
        o_vacc_name          OUT VARCHAR2,
        --test info
        o_test_info OUT pk_types.cursor_type,
        --Cancel
        o_can_title OUT VARCHAR2,
        o_can_det   OUT pk_types.cursor_type,
        --Administration
        o_adm_title OUT VARCHAR2,
        o_adm_det   OUT pk_types.cursor_type,
        --Prescription
        o_presc_title OUT VARCHAR2,
        o_presc_det   OUT pk_types.cursor_type,
        --Advers React
        o_advers_react_title OUT VARCHAR2,
        o_advers_react_det   OUT pk_types.cursor_type,
        --ERROR
        o_error OUT t_error_out
    ) RETURN BOOLEAN IS
    BEGIN
        IF NOT pk_vacc.get_vaccines_detail_free_text(i_lang               => i_lang,
                                                     i_patient            => i_patient,
                                                     i_prof               => i_prof,
                                                     i_reg                => i_reg,
                                                     o_main_title         => o_main_title,
                                                     o_this_take_title    => o_this_take_title,
                                                     o_history_take_title => o_history_take_title,
                                                     o_detail_info        => o_detail_info,
                                                     o_vacc_name          => o_vacc_name,
                                                     o_test_info          => o_test_info,
                                                     o_can_title          => o_can_title,
                                                     o_can_det            => o_can_det,
                                                     o_adm_title          => o_adm_title,
                                                     o_adm_det            => o_adm_det,
                                                     o_presc_title        => o_presc_title,
                                                     o_presc_det          => o_presc_det,
                                                     o_advers_react_title => o_advers_react_title,
                                                     o_advers_react_det   => o_advers_react_det,
                                                     o_error              => o_error)
        THEN
            RAISE g_exception_np;
        END IF;
        RETURN TRUE;
    EXCEPTION
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => 'GET_VACCINES_DETAIL_FREE_TEXT',
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END get_vaccines_detail_free_text;

    FUNCTION set_vacc_presc_det
    (
        i_lang               IN language.id_language%TYPE,
        i_vacc_adm           IN pat_vacc_adm.id_pat_vacc_adm%TYPE,
        i_patient            IN patient.id_patient%TYPE,
        i_notes              IN pat_vacc_adm_det.notes%TYPE,
        i_flg_take_type      IN VARCHAR2,
        i_prof               IN profissional,
        i_prof_cat_type      IN category.flg_type%TYPE,
        i_flg_advers_react   IN VARCHAR2,
        i_notes_advers_react IN pat_vacc_adm_det.notes_advers_react%TYPE,
        i_application_spot   IN pat_vacc_adm_det.application_spot%TYPE,
        i_lot_number         IN pat_vacc_adm_det.lot_number%TYPE,
        i_dt_expiration_str  IN VARCHAR2,
        i_dt_adm_str         IN VARCHAR2,
        i_vacc_manuf         IN vacc_manufacturer.id_vacc_manufacturer%TYPE,
        code_mvx             IN vacc_manufacturer.code_mvx%TYPE,
        i_flg_type_date      IN pat_vacc_adm_det.flg_type_date%TYPE,
        o_error              OUT t_error_out
    ) RETURN BOOLEAN IS
    BEGIN
        IF NOT pk_vacc.set_vacc_presc_det(i_lang               => i_lang,
                                          i_vacc_adm           => i_vacc_adm,
                                          i_patient            => i_patient,
                                          i_notes              => i_notes,
                                          i_flg_take_type      => i_flg_take_type,
                                          i_prof               => i_prof,
                                          i_prof_cat_type      => i_prof_cat_type,
                                          i_flg_advers_react   => i_flg_advers_react,
                                          i_notes_advers_react => i_notes_advers_react,
                                          i_application_spot   => i_application_spot,
                                          i_lot_number         => i_lot_number,
                                          i_dt_expiration_str  => i_dt_expiration_str,
                                          i_dt_adm_str         => i_dt_adm_str,
                                          i_vacc_manuf         => i_vacc_manuf,
                                          code_mvx             => code_mvx,
                                          i_flg_type_date      => i_flg_type_date,
                                          o_error              => o_error)
        THEN
            RAISE g_exception_np;
        END IF;
        RETURN TRUE;
    EXCEPTION
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang     => i_lang,
                                              i_sqlcode  => SQLCODE,
                                              i_sqlerrm  => SQLERRM,
                                              i_message  => g_error,
                                              i_owner    => g_owner,
                                              i_package  => g_package,
                                              i_function => 'SET_VACC_PRESC_DET',
                                              o_error    => o_error);
            pk_alert_exceptions.reset_error_state();
            RETURN FALSE;
    END set_vacc_presc_det;

BEGIN
    /* CAN'T TOUCH THIS */
    /* Who am I */
    pk_alertlog.who_am_i(owner => g_owner, name => g_package);
    /* Log init */
    pk_alertlog.log_init(object_name => g_package);
    g_debug := pk_alertlog.is_debug_enabled(i_object_name => g_package);
END pk_immunization_ux;
/
