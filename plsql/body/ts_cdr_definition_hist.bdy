...
CREATE OR REPLACE PACKAGE BODY TS_CDR_DEFINITION_HIST
/*
| Generated by or retrieved from QCGU - DO NOT MODIFY!
| QCGU - "Get it right, do it fast" - www.ToadWorld.com
| QCGU Universal ID: {7A44F965-7C77-42EE-9A04-269EF006213B}
| Created On: April 16, 2014 15:37:54
| Created By: ALERT
*/
IS
   ...
   BEGIN
     ...
     INSERT INTO CDR_DEFINITION_HIST (
         ID_CDR_DEFINITION,
         CREATE_USER,
         CREATE_TIME,
         CREATE_INSTITUTION,
      ...
      THEN
        ...
        BEGIN
           ...
           pk_alert_exceptions.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'CDR_DEFINITION_HIST');
           IF l_name = 'CDRDH_ID_CDR_DEFINITION'
           THEN
   ...
   BEGIN
     ...
     ins (
      ...
      id_links_new_in => id_links_new_in,
      id_links_old_in => id_links_old_in,
      status_new_in => status_new_in,
      status_old_in => status_old_in
   ...
   IS
      l_rec CDR_DEFINITION_HIST%ROWTYPE := rec_in;
   BEGIN
      ins (
         ...
         id_links_new_in => l_rec.ID_LINKS_NEW,
         id_links_old_in => l_rec.ID_LINKS_OLD,
         status_new_in => l_rec.STATUS_NEW,
         status_old_in => l_rec.STATUS_OLD
         ,handle_error_in => handle_error_in
         , rows_out => rows_out
         );
   END ins;
  ...
  ins (
   ...
   BEGIN
      ...
      ELSE
       FORALL indx IN rows_in.FIRST .. rows_in.LAST
            ...
            VALUES (
               rows_in(indx).ID_CDR_DEFINITION,
               ...
               rows_in(indx).ID_LINKS_NEW,
               rows_in(indx).ID_LINKS_OLD,
               rows_in(indx).STATUS_NEW,
               rows_in(indx).STATUS_OLD
               ) RETURNING ROWID BULK COLLECT INTO rows_out;
       ...
       ELSE
          ...
          BEGIN
             ...
             LOOP
                ...
                pk_alert_exceptions.add_context (
                   err_instance_id_in => l_err_instance_id
                  ,NAME_IN => 'ID_LINKS_NEW _' || indx
                  ,value_in => rows_in(SQL%BULK_EXCEPTIONS (indx).ERROR_INDEX).ID_LINKS_NEW
                  ,validate_in => FALSE
                );

                pk_alert_exceptions.add_context (
                   err_instance_id_in => l_err_instance_id
                  ,NAME_IN => 'ID_LINKS_OLD _' || indx
                  ,value_in => rows_in(SQL%BULK_EXCEPTIONS (indx).ERROR_INDEX).ID_LINKS_OLD
                  ,validate_in => FALSE
                );

                pk_alert_exceptions.add_context (
                   err_instance_id_in => l_err_instance_id
                  ,NAME_IN => 'STATUS_NEW _' || indx
                  ,value_in => rows_in(SQL%BULK_EXCEPTIONS (indx).ERROR_INDEX).STATUS_NEW
                  ,validate_in => FALSE
                );

                pk_alert_exceptions.add_context (
                   err_instance_id_in => l_err_instance_id
                  ,NAME_IN => 'STATUS_OLD _' || indx
                  ,value_in => rows_in(SQL%BULK_EXCEPTIONS (indx).ERROR_INDEX).STATUS_OLD
                  ,validate_in => FALSE
                );
   ...
   BEGIN
      RETURN
         'BEGIN UPDATE CDR_DEFINITION_HIST
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE CDR_DEFINITION_HIST set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE CDR_DEFINITION_HIST set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
   ...
   IS
   BEGIN
      DELETE FROM CDR_DEFINITION_HIST
       WHERE
          ID_CDR_DEFINITION = del_CDRDH_ID_CDR_DEFINITION.id_cdr_definition_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
      ...
      THEN
        ...
        END;
        END IF;
   END del_CDRDH_ID_CDR_DEFINITION;
...
del_CDRDH_ID_CDR_DEFINITION (
   ...
   BEGIN
      IF where_in IS NULL
      THEN
         RETURN 'DELETE FROM CDR_DEFINITION_HIST';
      ELSE
         RETURN
            'DELETE FROM CDR_DEFINITION_HIST WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   IS
   BEGIN
      cdr_definition_hist_inout.ID_CDR_DEFINITION := NULL;
      cdr_definition_hist_inout.CREATE_USER := NULL;
      cdr_definition_hist_inout.CREATE_TIME := NULL;
      cdr_definition_hist_inout.CREATE_INSTITUTION := NULL;
      cdr_definition_hist_inout.UPDATE_USER := NULL;
      cdr_definition_hist_inout.UPDATE_TIME := NULL;
      cdr_definition_hist_inout.UPDATE_INSTITUTION := NULL;
      cdr_definition_hist_inout.ID_LINKS_NEW := NULL;
      cdr_definition_hist_inout.ID_LINKS_OLD := NULL;
      cdr_definition_hist_inout.STATUS_NEW := NULL;
      cdr_definition_hist_inout.STATUS_OLD := NULL;
   END initrec;

   FUNCTION initrec RETURN CDR_DEFINITION_HIST%ROWTYPE
   IS
      l_cdr_definition_hist CDR_DEFINITION_HIST%ROWTYPE;
   BEGIN
      RETURN l_cdr_definition_hist;
   END initrec;
   ...
   IS
        data CDR_DEFINITION_HIST_tc;
   BEGIN
        select * bulk collect into data from CDR_DEFINITION_HIST where rowid in (select /*+opt_estimate(table,t,scale_rows=0.0000001))*/ * from table(rows_in) t);
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
