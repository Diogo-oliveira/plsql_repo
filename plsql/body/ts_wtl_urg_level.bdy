/*-- Last Change Revision: $Rev: 2028425 $*/
/*-- Last Change by: $Author: mario.fernandes $*/
/*-- Date of last change: $Date: 2022-08-02 18:45:41 +0100 (ter, 02 ago 2022) $*/

CREATE OR REPLACE PACKAGE BODY TS_WTL_URG_LEVEL
/*
| Generated by or retrieved from QCGU - DO NOT MODIFY!
| QCGU - "Get it right, do it fast" - www.ToadWorld.com
| QCGU Universal ID: {657BD35E-2E48-4CD4-B14E-41420B8433B3}
| Created On: Junho 4, 2010 15:36:19
| Created By: ALERT
*/
IS
      ...
      THEN
        ...
        BEGIN
           ...
           pk_alert_exceptions.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'WTL_URG_LEVEL');
   ...
   BEGIN
     ...
     ins (
      ...
      desc_wtl_urg_level_in => desc_wtl_urg_level_in
   ...
   IS
      l_rec WTL_URG_LEVEL%ROWTYPE := rec_in;
   BEGIN
      IF gen_pky_in THEN
         l_rec.ID_WTL_URG_LEVEL := next_key (sequence_in);
      END IF;
      ins (
         ...
         desc_wtl_urg_level_in => l_rec.DESC_WTL_URG_LEVEL
         ,handle_error_in => handle_error_in
         , rows_out => rows_out
         );
   END ins;
  ...
  ins (
   ...
   IS
     retval WTL_URG_LEVEL.ID_WTL_URG_LEVEL%TYPE;

   BEGIN
      IF sequence_in IS NULL
      THEN
         SELECT seq_WTL_URG_LEVEL.NEXTVAL INTO retval FROM dual;
      ELSE
         EXECUTE IMMEDIATE
   ...
   EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
           ...
           ,name1_in => 'SEQUENCE'
           ,value1_in => NVL (sequence_in, 'seq_WTL_URG_LEVEL')
           );
   END next_key;
   ...
   IS
        l_pky WTL_URG_LEVEL.ID_WTL_URG_LEVEL%TYPE := next_key;
   BEGIN
      ins (
         id_wtl_urg_level_in => l_pky,
         code_in => code_in,
         create_user_in => create_user_in,
         create_time_in => create_time_in,
         create_institution_in => create_institution_in,
   ...
   BEGIN
      ...
      dt_last_update_in => dt_last_update_in,
      desc_wtl_urg_level_in => desc_wtl_urg_level_in,
      id_wtl_urg_level_out => id_wtl_urg_level_out
      ,handle_error_in => handle_error_in
      , rows_out => rows_out
   );
   END ins;
   ...
   BEGIN
      ...
      ELSE
         FORALL indx IN rows_in.FIRST .. rows_in.LAST
            SAVE EXCEPTIONS
            INSERT INTO WTL_URG_LEVEL VALUES rows_in (indx) RETURNING ROWID BULK COLLECT INTO rows_out;
      END IF;
   EXCEPTION
     WHEN e_forall_error
     THEN
       ...
       ELSE
          ...
          BEGIN
             ...
             LOOP
                ...
                pk_alert_exceptions.add_context (
                   err_instance_id_in => l_err_instance_id
                  ,NAME_IN => 'DESC_WTL_URG_LEVEL _' || indx
                  ,value_in => rows_in(indx).DESC_WTL_URG_LEVEL
                  ,validate_in => FALSE
                );
...
PROCEDURE upd (
   ...
   l_DESC_WTL_URG_LEVEL_n NUMBER(1);
   BEGIN
   ...
   l_DESC_WTL_URG_LEVEL_n := sys.diutil.bool_to_int(DESC_WTL_URG_LEVEL_nin);


         UPDATE WTL_URG_LEVEL SET
     ...
     DESC_WTL_URG_LEVEL = decode (l_DESC_WTL_URG_LEVEL_n,0,desc_wtl_urg_level_in, NVL (desc_wtl_urg_level_in, DESC_WTL_URG_LEVEL))
          WHERE
             ID_WTL_URG_LEVEL = id_wtl_urg_level_in
...
rows_out :=  rows_out MULTISET UNION DISTINCT l_rows_out;
   ...
   BEGIN
     upd (
      ...
      desc_wtl_urg_level_in => desc_wtl_urg_level_in,
      DESC_WTL_URG_LEVEL_nin => DESC_WTL_URG_LEVEL_nin,
     handle_error_in => handle_error_in
     , rows_out => rows_out
      );
   END upd;
...
PROCEDURE upd (
   ...
   l_DT_LAST_UPDATE_n NUMBER(1);
   l_DESC_WTL_URG_LEVEL_n NUMBER(1);
      id_wtl_urg_level_in WTL_URG_LEVEL.ID_WTL_URG_LEVEL%TYPE;
   BEGIN
...
l_sql := 'UPDATE WTL_URG_LEVEL SET '
     || ' CODE = decode (' || l_CODE_n || ',0,:code_in, NVL (:code_in, CODE)) '|| ','
     || ' CREATE_USER = decode (' || l_CREATE_USER_n || ',0,:create_user_in, NVL (:create_user_in, CREATE_USER)) '|| ','
     || ' CREATE_TIME = decode (' || l_CREATE_TIME_n || ',0,:create_time_in, NVL (:create_time_in, CREATE_TIME)) '|| ','
     || ' CREATE_INSTITUTION = decode (' || l_CREATE_INSTITUTION_n || ',0,:create_institution_in, NVL (:create_institution_in, CREATE_INSTITUTION)) '|| ','
...
PROCEDURE upd (
   ...
   BEGIN
      upd (
         ...
         desc_wtl_urg_level_in => rec_in.DESC_WTL_URG_LEVEL
   ...
   IS
      ...
      l_DESC_WTL_URG_LEVEL DESC_WTL_URG_LEVEL_cc;
   BEGIN
      FOR i IN col_in.FIRST .. col_in.LAST loop
         ...
         l_DESC_WTL_URG_LEVEL(i) := col_in(i).DESC_WTL_URG_LEVEL;
      END LOOP;
      ...
      THEN
         ...
         FORALL i IN col_in.FIRST .. col_in.LAST
            UPDATE WTL_URG_LEVEL SET
               ...
               DESC_WTL_URG_LEVEL = NVL (l_DESC_WTL_URG_LEVEL(i), DESC_WTL_URG_LEVEL)
             WHERE
      ...
      ELSE
         FORALL i IN col_in.FIRST .. col_in.LAST
            UPDATE WTL_URG_LEVEL SET
               ...
               DESC_WTL_URG_LEVEL = l_DESC_WTL_URG_LEVEL(i)
             WHERE
                ID_WTL_URG_LEVEL = l_ID_WTL_URG_LEVEL(i)
         ;
      END IF;
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE WTL_URG_LEVEL set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE WTL_URG_LEVEL set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM WTL_URG_LEVEL
       WHERE ID_WTL_URG_LEVEL = id_wtl_urg_level_in
      RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
, rows_out => rows_out
   ...
   BEGIN
      IF where_in IS NULL
      THEN
         RETURN 'DELETE FROM WTL_URG_LEVEL';
      ELSE
         RETURN
            'DELETE FROM WTL_URG_LEVEL WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   IS
   BEGIN
      wtl_urg_level_inout.ID_WTL_URG_LEVEL := NULL;
      wtl_urg_level_inout.CODE := NULL;
      wtl_urg_level_inout.CREATE_USER := NULL;
      wtl_urg_level_inout.CREATE_TIME := NULL;
      wtl_urg_level_inout.CREATE_INSTITUTION := NULL;
      wtl_urg_level_inout.UPDATE_USER := NULL;
      wtl_urg_level_inout.UPDATE_TIME := NULL;
      wtl_urg_level_inout.UPDATE_INSTITUTION := NULL;
      wtl_urg_level_inout.FLG_AVAILABLE := NULL;
      wtl_urg_level_inout.DURATION := NULL;
      wtl_urg_level_inout.ID_GROUP := NULL;
      wtl_urg_level_inout.ID_INSTITUTION := NULL;
      wtl_urg_level_inout.ID_CONTENT := NULL;
      wtl_urg_level_inout.FLG_STATUS := NULL;
      wtl_urg_level_inout.FLG_PARAMETERIZATION_TYPE := 'C';
      wtl_urg_level_inout.ID_PROFESSIONAL := NULL;
      wtl_urg_level_inout.DT_CREATION := NULL;
      wtl_urg_level_inout.DT_LAST_UPDATE := NULL;
      wtl_urg_level_inout.DESC_WTL_URG_LEVEL := NULL;
   END initrec;

   FUNCTION initrec RETURN WTL_URG_LEVEL%ROWTYPE
   IS
      l_wtl_urg_level WTL_URG_LEVEL%ROWTYPE;
   BEGIN
      l_wtl_urg_level.FLG_PARAMETERIZATION_TYPE := 'C';
      RETURN l_wtl_urg_level;
   END initrec;
   ...
   IS
        data WTL_URG_LEVEL_tc;
   BEGIN
        select * bulk collect into data from WTL_URG_LEVEL where rowid in (select /*+opt_estimate(table,t,scale_rows=0.0000001))*/ * from table(rows_in) t);
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
