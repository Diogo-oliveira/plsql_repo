...
CREATE OR REPLACE PACKAGE BODY TS_SUSP_TASK_LAB
/*
| Generated by or retrieved from QCGU - DO NOT MODIFY!
| QCGU - "Get it right, do it fast" - www.ToadWorld.com
| QCGU Universal ID: {657BD35E-2E48-4CD4-B14E-41420B8433B3}
| Created On: Junho 1, 2010 15:33:9
| Created By: ALERT
*/
IS
      ...
      THEN
        ...
        BEGIN
           ...
           PK_ALERT_EXCEPTIONS.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'SUSP_TASK_LAB');
           IF l_name = 'STG_ARD_FK'
           THEN
   ...
   IS
      l_rec SUSP_TASK_LAB%ROWTYPE := rec_in;
   BEGIN
      ins (
   ...
   BEGIN
      ...
      ELSE
         FORALL indx IN rows_in.FIRST .. rows_in.LAST
            SAVE EXCEPTIONS
            INSERT INTO SUSP_TASK_LAB VALUES rows_in (indx) RETURNING ROWID BULK COLLECT INTO rows_out;
      END IF;
   EXCEPTION
     WHEN e_forall_error
     THEN
   ...
   l_UPDATE_INSTITUTION_n := sys.diutil.bool_to_int(UPDATE_INSTITUTION_nin);
     ...
     UPDATE_INSTITUTION = decode (l_UPDATE_INSTITUTION_n,0,update_institution_in, NVL (update_institution_in, UPDATE_INSTITUTION))
          WHERE
             ...
             ID_ANALYSIS_REQ_DET = id_analysis_req_det_in
...
PROCEDURE upd (
   ...
   l_UPDATE_TIME_n NUMBER(1);
   l_UPDATE_INSTITUTION_n NUMBER(1);
      id_susp_task_in SUSP_TASK_LAB.ID_SUSP_TASK%TYPE;
      id_analysis_req_det_in SUSP_TASK_LAB.ID_ANALYSIS_REQ_DET%TYPE;
   BEGIN
...
l_sql := 'UPDATE SUSP_TASK_LAB SET '
     || ' CREATE_USER = decode (' || l_CREATE_USER_n || ',0,:create_user_in, NVL (:create_user_in, CREATE_USER)) '|| ','
     || ' CREATE_TIME = decode (' || l_CREATE_TIME_n || ',0,:create_time_in, NVL (:create_time_in, CREATE_TIME)) '|| ','
     || ' CREATE_INSTITUTION = decode (' || l_CREATE_INSTITUTION_n || ',0,:create_institution_in, NVL (:create_institution_in, CREATE_INSTITUTION)) '|| ','
     || ' UPDATE_USER = decode (' || l_UPDATE_USER_n || ',0,:update_user_in, NVL (:update_user_in, UPDATE_USER)) '|| ','
...
PROCEDURE upd (
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE SUSP_TASK_LAB set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE SUSP_TASK_LAB set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM SUSP_TASK_LAB
       WHERE
          ID_ANALYSIS_REQ_DET = del_STG_ARD_FK.id_analysis_req_det_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_STG_ARD_FK (
   ...
   BEGIN
      DELETE FROM SUSP_TASK_LAB
       WHERE
          ID_SUSP_TASK = del_STG_WSK_FK.id_susp_task_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_STG_WSK_FK (
   ...
   BEGIN
      IF where_in IS NULL
      THEN
         RETURN 'DELETE FROM SUSP_TASK_LAB';
      ELSE
         RETURN
            'DELETE FROM SUSP_TASK_LAB WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   IS
   BEGIN
      susp_task_lab_inout.ID_SUSP_TASK := NULL;
      susp_task_lab_inout.ID_ANALYSIS_REQ_DET := NULL;
      susp_task_lab_inout.CREATE_USER := NULL;
      susp_task_lab_inout.CREATE_TIME := NULL;
      susp_task_lab_inout.CREATE_INSTITUTION := NULL;
      susp_task_lab_inout.UPDATE_USER := NULL;
      susp_task_lab_inout.UPDATE_TIME := NULL;
      susp_task_lab_inout.UPDATE_INSTITUTION := NULL;
   END initrec;

   FUNCTION initrec RETURN SUSP_TASK_LAB%ROWTYPE
   IS
      l_susp_task_lab SUSP_TASK_LAB%ROWTYPE;
   BEGIN
      RETURN l_susp_task_lab;
   END initrec;
   ...
   IS
        data SUSP_TASK_LAB_tc;
   BEGIN
        select * bulk collect into data from SUSP_TASK_LAB where rowid in (select /*+opt_estimate(table,t,scale_rows=0.0000001))*/ * from table(rows_in) t);
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        PK_ALERT_EXCEPTIONS.raise_error (
