/*-- Last Change Revision: $Rev: 2028344 $*/
/*-- Last Change by: $Author: mario.fernandes $*/
/*-- Date of last change: $Date: 2022-08-02 18:45:14 +0100 (ter, 02 ago 2022) $*/

CREATE OR REPLACE PACKAGE BODY TS_PROF_TEAM_DET
/*
| Generated by or retrieved from QCGU - DO NOT MODIFY!
| QCGU - "Get it right, do it fast" - www.ToadWorld.com
| QCGU Universal ID: {657BD35E-2E48-4CD4-B14E-41420B8433B3}
| Created On: July 5, 2009 17:25:31
| Created By: ALERT
*/
IS
   ...
   PROCEDURE ins (
      id_prof_team_det_in IN PROF_TEAM_DET.ID_PROF_TEAM_DET%TYPE
      ,
      id_prof_team_in IN PROF_TEAM_DET.ID_PROF_TEAM%TYPE DEFAULT NULL,
      id_professional_in IN PROF_TEAM_DET.ID_PROFESSIONAL%TYPE DEFAULT NULL,
      id_category_sub_in IN PROF_TEAM_DET.ID_CATEGORY_SUB%TYPE DEFAULT NULL,
      flg_available_in IN PROF_TEAM_DET.FLG_AVAILABLE%TYPE DEFAULT NULL,
      flg_status_in IN PROF_TEAM_DET.FLG_STATUS%TYPE DEFAULT NULL,
      id_prof_cancel_in IN PROF_TEAM_DET.ID_PROF_CANCEL%TYPE DEFAULT NULL,
      dt_cancel_tstz_in IN PROF_TEAM_DET.DT_CANCEL_TSTZ%TYPE DEFAULT NULL,
      adw_last_update_in IN PROF_TEAM_DET.ADW_LAST_UPDATE%TYPE DEFAULT NULL,
      notes_in IN PROF_TEAM_DET.NOTES%TYPE DEFAULT NULL,
      dt_begin_in IN PROF_TEAM_DET.DT_BEGIN%TYPE DEFAULT NULL,
      dt_end_in IN PROF_TEAM_DET.DT_END%TYPE DEFAULT NULL,
      create_user_in IN PROF_TEAM_DET.CREATE_USER%TYPE DEFAULT NULL,
      create_time_in IN PROF_TEAM_DET.CREATE_TIME%TYPE DEFAULT NULL,
      create_institution_in IN PROF_TEAM_DET.CREATE_INSTITUTION%TYPE DEFAULT NULL,
      update_user_in IN PROF_TEAM_DET.UPDATE_USER%TYPE DEFAULT NULL,
      update_time_in IN PROF_TEAM_DET.UPDATE_TIME%TYPE DEFAULT NULL,
      update_institution_in IN PROF_TEAM_DET.UPDATE_INSTITUTION%TYPE DEFAULT NULL,
      flg_leader_in IN PROF_TEAM_DET.FLG_LEADER%TYPE DEFAULT NULL
      ...
      THEN
        ...
        BEGIN
           ...
           pk_alert_exceptions.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'PROF_TEAM_DET');
           IF l_name = 'PRF_TEAM_D_CATS_FK'
           THEN
           ...
           END IF;
           IF l_name = 'PRF_TEAM_D_PROF_TEAM_FK'
           THEN
              -- Add a context value for each column
              pk_alert_exceptions.add_context (
                 err_instance_id_in => l_id
   ...
   BEGIN
     ...
     ins (
      id_prof_team_det_in => id_prof_team_det_in
      ,
   ...
   PROCEDURE ins (
      rec_in IN PROF_TEAM_DET%ROWTYPE
   ...
   IS
      l_rec PROF_TEAM_DET%ROWTYPE := rec_in;
   BEGIN
      IF gen_pky_in THEN
         l_rec.ID_PROF_TEAM_DET := next_key (sequence_in);
      END IF;
      ins (
         id_prof_team_det_in => l_rec.ID_PROF_TEAM_DET
         ,
  ...
  ins (
   ...
   FUNCTION next_key (sequence_in IN VARCHAR2 := NULL) RETURN PROF_TEAM_DET.ID_PROF_TEAM_DET%TYPE

   IS
     retval PROF_TEAM_DET.ID_PROF_TEAM_DET%TYPE;

   BEGIN
      IF sequence_in IS NULL
      THEN
         SELECT seq_PROF_TEAM_DET.NEXTVAL INTO retval FROM dual;
      ELSE
         EXECUTE IMMEDIATE
   ...
   EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
           ...
           ,name1_in => 'SEQUENCE'
           ,value1_in => NVL (sequence_in, 'seq_PROF_TEAM_DET')
           );
   END next_key;
   ...
   PROCEDURE ins (
      ...
      update_institution_in IN PROF_TEAM_DET.UPDATE_INSTITUTION%TYPE DEFAULT NULL,
      flg_leader_in IN PROF_TEAM_DET.FLG_LEADER%TYPE DEFAULT NULL,
      id_prof_team_det_out IN OUT PROF_TEAM_DET.ID_PROF_TEAM_DET%TYPE
      ,handle_error_in IN BOOLEAN := TRUE
      , rows_out OUT TABLE_VARCHAR
   )
   IS
        l_pky PROF_TEAM_DET.ID_PROF_TEAM_DET%TYPE := next_key;
   BEGIN
      ins (
         id_prof_team_det_in => l_pky,
         id_prof_team_in => id_prof_team_in,
         id_professional_in => id_professional_in,
         id_category_sub_in => id_category_sub_in,
         flg_available_in => flg_available_in,
   ...
   BEGIN
      ...
      update_institution_in => update_institution_in,
      flg_leader_in => flg_leader_in,
      id_prof_team_det_out => id_prof_team_det_out
      ,handle_error_in => handle_error_in
      , rows_out => rows_out
   );
   END ins;
   ...
   FUNCTION ins (
      ...
      RETURN
         PROF_TEAM_DET.ID_PROF_TEAM_DET%TYPE
   IS
        l_pky PROF_TEAM_DET.ID_PROF_TEAM_DET%TYPE := next_key;
   BEGIN
      ins (
   ...
   BEGIN
      ...
      ELSE
         FORALL indx IN rows_in.FIRST .. rows_in.LAST
            SAVE EXCEPTIONS
            INSERT INTO PROF_TEAM_DET VALUES rows_in (indx) RETURNING ROWID BULK COLLECT INTO rows_out;
      END IF;
   EXCEPTION
     WHEN e_forall_error
     THEN
       ...
       ELSE
          ...
          BEGIN
             ...
             LOOP
                ...
                pk_alert_exceptions.add_context (
                   err_instance_id_in => l_err_instance_id
                  ,NAME_IN => 'ID_PROF_TEAM_DET _' || indx
                  ,value_in => rows_in(indx).ID_PROF_TEAM_DET
                  ,validate_in => FALSE
                );
...
PROCEDURE upd (
   ...
   l_FLG_LEADER_n := sys.diutil.bool_to_int(FLG_LEADER_nin);
     ...
     FLG_LEADER = decode (l_FLG_LEADER_n,0,flg_leader_in, NVL (flg_leader_in, FLG_LEADER))
          WHERE
             ID_PROF_TEAM_DET = id_prof_team_det_in
...
PROCEDURE upd (
   ...
   l_UPDATE_INSTITUTION_n NUMBER(1);
   l_FLG_LEADER_n NUMBER(1);
      id_prof_team_det_in PROF_TEAM_DET.ID_PROF_TEAM_DET%TYPE;
   BEGIN
...
l_sql := 'UPDATE PROF_TEAM_DET SET '
     || ' ID_PROF_TEAM = decode (' || l_ID_PROF_TEAM_n || ',0,:id_prof_team_in, NVL (:id_prof_team_in, ID_PROF_TEAM)) '|| ','
     || ' ID_PROFESSIONAL = decode (' || l_ID_PROFESSIONAL_n || ',0,:id_professional_in, NVL (:id_professional_in, ID_PROFESSIONAL)) '|| ','
     || ' ID_CATEGORY_SUB = decode (' || l_ID_CATEGORY_SUB_n || ',0,:id_category_sub_in, NVL (:id_category_sub_in, ID_CATEGORY_SUB)) '|| ','
     || ' FLG_AVAILABLE = decode (' || l_FLG_AVAILABLE_n || ',0,:flg_available_in, NVL (:flg_available_in, FLG_AVAILABLE)) '|| ','
...
PROCEDURE upd (
   ...
   BEGIN
      upd (
         id_prof_team_det_in => rec_in.ID_PROF_TEAM_DET,
   ...
   PROCEDURE upd (
      col_in IN PROF_TEAM_DET_tc,
      ignore_if_null_in IN BOOLEAN := TRUE
     ,handle_error_in IN BOOLEAN := TRUE
     , rows_out IN OUT TABLE_VARCHAR
      )
   IS
      l_ID_PROF_TEAM_DET ID_PROF_TEAM_DET_cc;
   ...
   BEGIN
      FOR i IN col_in.FIRST .. col_in.LAST loop
         l_ID_PROF_TEAM_DET(i) := col_in(i).ID_PROF_TEAM_DET;
      ...
      THEN
         ...
         FORALL i IN col_in.FIRST .. col_in.LAST
            UPDATE PROF_TEAM_DET SET
             ...
             WHERE
                ID_PROF_TEAM_DET = l_ID_PROF_TEAM_DET(i)
         ;
      ELSE
         FORALL i IN col_in.FIRST .. col_in.LAST
            UPDATE PROF_TEAM_DET SET
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE PROF_TEAM_DET set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE PROF_TEAM_DET set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
...
, rows_out => rows_out
   ...
   -- Delete all rows for primary key column ID_PROF_TEAM_DET
   PROCEDURE del_ID_PROF_TEAM_DET (
      id_prof_team_det_in IN PROF_TEAM_DET.ID_PROF_TEAM_DET%TYPE
     ,handle_error_in IN BOOLEAN := TRUE
     , rows_out OUT TABLE_VARCHAR
     )
   ...
   BEGIN
      DELETE FROM PROF_TEAM_DET
       WHERE ID_PROF_TEAM_DET = id_prof_team_det_in
      RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
, rows_out => rows_out
   ...
   PROCEDURE del_PRF_TEAM_D_CATS_FK (
      id_category_sub_in IN PROF_TEAM_DET.ID_CATEGORY_SUB%TYPE
     ,handle_error_in IN BOOLEAN := TRUE
     , rows_out OUT TABLE_VARCHAR
      )
   IS
   BEGIN
      DELETE FROM PROF_TEAM_DET
       WHERE
          ID_CATEGORY_SUB = del_PRF_TEAM_D_CATS_FK.id_category_sub_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_PRF_TEAM_D_CATS_FK (
   ...
   PROCEDURE del_PRF_TEAM_D_PROF_FK (
      id_professional_in IN PROF_TEAM_DET.ID_PROFESSIONAL%TYPE
     ,handle_error_in IN BOOLEAN := TRUE
     , rows_out OUT TABLE_VARCHAR
      )
   IS
   BEGIN
      DELETE FROM PROF_TEAM_DET
       WHERE
          ID_PROFESSIONAL = del_PRF_TEAM_D_PROF_FK.id_professional_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_PRF_TEAM_D_PROF_FK (
   ...
   PROCEDURE del_PRF_TEAM_D_PROF_FK2 (
      id_prof_cancel_in IN PROF_TEAM_DET.ID_PROF_CANCEL%TYPE
     ,handle_error_in IN BOOLEAN := TRUE
     , rows_out OUT TABLE_VARCHAR
      )
   IS
   BEGIN
      DELETE FROM PROF_TEAM_DET
       WHERE
          ID_PROF_CANCEL = del_PRF_TEAM_D_PROF_FK2.id_prof_cancel_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_PRF_TEAM_D_PROF_FK2 (
   ...
   PROCEDURE del_PRF_TEAM_D_PROF_TEAM_FK (
      id_prof_team_in IN PROF_TEAM_DET.ID_PROF_TEAM%TYPE
     ,handle_error_in IN BOOLEAN := TRUE
     , rows_out OUT TABLE_VARCHAR
      )
   IS
   BEGIN
      DELETE FROM PROF_TEAM_DET
       WHERE
          ID_PROF_TEAM = del_PRF_TEAM_D_PROF_TEAM_FK.id_prof_team_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
      ...
      THEN
        ...
        END;
        END IF;
   END del_PRF_TEAM_D_PROF_TEAM_FK;
...
del_PRF_TEAM_D_PROF_TEAM_FK (
   ...
   BEGIN
      IF where_in IS NULL
      THEN
         RETURN 'DELETE FROM PROF_TEAM_DET';
      ELSE
         RETURN
            'DELETE FROM PROF_TEAM_DET WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   PROCEDURE initrec (
      prof_team_det_inout IN OUT PROF_TEAM_DET%ROWTYPE
   )
   IS
   BEGIN
      prof_team_det_inout.ID_PROF_TEAM_DET := NULL;
      prof_team_det_inout.ID_PROF_TEAM := NULL;
      prof_team_det_inout.ID_PROFESSIONAL := NULL;
      prof_team_det_inout.ID_CATEGORY_SUB := NULL;
      prof_team_det_inout.FLG_AVAILABLE := NULL;
      prof_team_det_inout.FLG_STATUS := NULL;
      prof_team_det_inout.ID_PROF_CANCEL := NULL;
      prof_team_det_inout.DT_CANCEL_TSTZ := NULL;
      prof_team_det_inout.ADW_LAST_UPDATE := NULL;
      prof_team_det_inout.NOTES := NULL;
      prof_team_det_inout.DT_BEGIN := NULL;
      prof_team_det_inout.DT_END := NULL;
      prof_team_det_inout.CREATE_USER := NULL;
      prof_team_det_inout.CREATE_TIME := NULL;
      prof_team_det_inout.CREATE_INSTITUTION := NULL;
      prof_team_det_inout.UPDATE_USER := NULL;
      prof_team_det_inout.UPDATE_TIME := NULL;
      prof_team_det_inout.UPDATE_INSTITUTION := NULL;
      prof_team_det_inout.FLG_LEADER := NULL;
   END initrec;

   FUNCTION initrec RETURN PROF_TEAM_DET%ROWTYPE
   IS
      l_prof_team_det PROF_TEAM_DET%ROWTYPE;
   BEGIN
      RETURN l_prof_team_det;
   END initrec;


   FUNCTION get_data_rowid(
        rows_in IN TABLE_VARCHAR
        ) RETURN PROF_TEAM_DET_tc
   IS
        data PROF_TEAM_DET_tc;
   BEGIN
        select * bulk collect into data from PROF_TEAM_DET where rowid in (select * from table(rows_in));
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
