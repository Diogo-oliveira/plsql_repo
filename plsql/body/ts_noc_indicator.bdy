...
CREATE OR REPLACE PACKAGE BODY TS_NOC_INDICATOR
/*
| Generated by or retrieved from QCGU - DO NOT MODIFY!
| QCGU - "Get it right, do it fast" - www.ToadWorld.com
| QCGU Universal ID: {7A44F965-7C77-42EE-9A04-269EF006213B}
| Created On: Outubro 3, 2013 16:37:11
| Created By: ALERT
*/
IS
   ...
   EXCEPTION
      ...
      THEN
        ...
        BEGIN
           ...
           ELSIF l_name = 'NOC_INDICATOR_UK'
           THEN
              pk_alert_exceptions.register_error (
                 error_name_in => 'DUPLICATE-VALUE'
                ,err_instance_id_out => l_err_instance_id
                ,text_in => 'Violation of unique index "NOC_INDICATOR_UK"'
                );
              ...
              pk_alert_exceptions.add_context (
                 err_instance_id_in => l_err_instance_id
                ,NAME_IN => 'INDICATOR_CODE'
                ,value_in => indicator_code_in
                ,validate_in => FALSE
                );
      ...
      THEN
        ...
        BEGIN
           ...
           pk_alert_exceptions.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'NOC_INDICATOR');
           IF l_name = 'NOCI_CNCPTTR_FK'
           THEN
   ...
   BEGIN
     ...
     ins (
      ...
      indicator_code_in => indicator_code_in,
   ...
   IS
      l_rec NOC_INDICATOR%ROWTYPE := rec_in;
   BEGIN
      IF gen_pky_in THEN
         l_rec.ID_NOC_INDICATOR := next_key (sequence_in);
      END IF;
      ins (
         id_noc_indicator_in => l_rec.ID_NOC_INDICATOR
         ,
         id_terminology_version_in => l_rec.ID_TERMINOLOGY_VERSION,
         indicator_code_in => l_rec.INDICATOR_CODE,
  ...
  ins (
   ...
   IS
     retval NOC_INDICATOR.ID_NOC_INDICATOR%TYPE;

   BEGIN
      IF sequence_in IS NULL
      THEN
         SELECT seq_NOC_INDICATOR.NEXTVAL INTO retval FROM dual;
      ELSE
         EXECUTE IMMEDIATE
   ...
   EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
           ...
           ,name1_in => 'SEQUENCE'
           ,value1_in => NVL (sequence_in, 'seq_NOC_INDICATOR')
           );
   END next_key;
   ...
   IS
        l_pky NOC_INDICATOR.ID_NOC_INDICATOR%TYPE := next_key;
   BEGIN
      ins (
         id_noc_indicator_in => l_pky,
         id_terminology_version_in => id_terminology_version_in,
         indicator_code_in => indicator_code_in,
         flg_other_in => flg_other_in,
         id_inst_owner_in => id_inst_owner_in,
   ...
   BEGIN
      ...
      update_time_in => update_time_in,
      update_institution_in => update_institution_in,
      id_noc_indicator_out => id_noc_indicator_out
      ,handle_error_in => handle_error_in
      , rows_out => rows_out
   );
   END ins;
   ...
   BEGIN
      ...
      ELSE
         FORALL indx IN rows_in.FIRST .. rows_in.LAST
            SAVE EXCEPTIONS
            INSERT INTO NOC_INDICATOR VALUES rows_in (indx) RETURNING ROWID BULK COLLECT INTO rows_out;
      END IF;
   EXCEPTION
     WHEN e_forall_error
     THEN
       ...
       ELSE
          ...
          BEGIN
             ...
             LOOP
                ...
                pk_alert_exceptions.add_context (
                   err_instance_id_in => l_err_instance_id
                  ,NAME_IN => 'INDICATOR_CODE _' || indx
                  ,value_in => rows_in(SQL%BULK_EXCEPTIONS (indx).ERROR_INDEX).INDICATOR_CODE
                  ,validate_in => FALSE
                );

                pk_alert_exceptions.add_context (
                   err_instance_id_in => l_err_instance_id
                  ,NAME_IN => 'FLG_OTHER _' || indx
                  ,value_in => rows_in(SQL%BULK_EXCEPTIONS (indx).ERROR_INDEX).FLG_OTHER
                  ,validate_in => FALSE
                );
...
PROCEDURE upd (
   ...
   l_INDICATOR_CODE_n NUMBER(1);
   l_FLG_OTHER_n NUMBER(1);
   ...
   l_INDICATOR_CODE_n := sys.diutil.bool_to_int(INDICATOR_CODE_nin);
   ...
   l_UPDATE_INSTITUTION_n := sys.diutil.bool_to_int(UPDATE_INSTITUTION_nin);
     ...
     INDICATOR_CODE = decode (l_INDICATOR_CODE_n,0,indicator_code_in, NVL (indicator_code_in, INDICATOR_CODE)),
...
rows_out :=  rows_out MULTISET UNION DISTINCT l_rows_out;
   ...
   BEGIN
     upd (
      ...
      indicator_code_in => indicator_code_in,
      INDICATOR_CODE_nin => INDICATOR_CODE_nin,
      flg_other_in => flg_other_in,
...
PROCEDURE upd (
   ...
   l_UPDATE_TIME_n NUMBER(1);
   l_UPDATE_INSTITUTION_n NUMBER(1);
      id_noc_indicator_in NOC_INDICATOR.ID_NOC_INDICATOR%TYPE;
   BEGIN
...
l_sql := 'UPDATE NOC_INDICATOR SET '
     || ' ID_TERMINOLOGY_VERSION = decode (' || l_ID_TERMINOLOGY_VERSION_n || ',0,:id_terminology_version_in, NVL (:id_terminology_version_in, ID_TERMINOLOGY_VERSION)) '|| ','
     || ' INDICATOR_CODE = decode (' || l_INDICATOR_CODE_n || ',0,:indicator_code_in, NVL (:indicator_code_in, INDICATOR_CODE)) '|| ','
     || ' FLG_OTHER = decode (' || l_FLG_OTHER_n || ',0,:flg_other_in, NVL (:flg_other_in, FLG_OTHER)) '|| ','
     || ' ID_INST_OWNER = decode (' || l_ID_INST_OWNER_n || ',0,:id_inst_owner_in, NVL (:id_inst_owner_in, ID_INST_OWNER)) '|| ','
...
PROCEDURE upd (
   ...
   BEGIN
      upd (
         ...
         indicator_code_in => rec_in.INDICATOR_CODE,
   ...
   IS
      ...
      l_INDICATOR_CODE INDICATOR_CODE_cc;
   ...
   BEGIN
      FOR i IN col_in.FIRST .. col_in.LAST loop
         ...
         l_INDICATOR_CODE(i) := col_in(i).INDICATOR_CODE;
      ...
      THEN
         ...
         FORALL i IN col_in.FIRST .. col_in.LAST
            UPDATE NOC_INDICATOR SET
               ...
               INDICATOR_CODE = NVL (l_INDICATOR_CODE(i), INDICATOR_CODE),
      ...
      ELSE
         FORALL i IN col_in.FIRST .. col_in.LAST
            UPDATE NOC_INDICATOR SET
               ...
               INDICATOR_CODE = l_INDICATOR_CODE(i),
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE NOC_INDICATOR set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE NOC_INDICATOR set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
   ...
   BEGIN
      DELETE FROM NOC_INDICATOR
       WHERE
          ...
          INDICATOR_CODE = indicator_code_in
      RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_NOC_INDICATOR_UK (
   ...
   BEGIN
      DELETE FROM NOC_INDICATOR
       WHERE
          ID_CONCEPT_TERM = del_NOCI_CNCPTTR_FK.id_concept_term_in AND
          ID_INST_OWNER = del_NOCI_CNCPTTR_FK.id_inst_owner_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_NOCI_CNCPTTR_FK (
   ...
   BEGIN
      DELETE FROM NOC_INDICATOR
       WHERE
          ID_CONCEPT_VERSION = del_NOCI_CNCPTV_FK.id_concept_version_in AND
          ID_INST_OWNER = del_NOCI_CNCPTV_FK.id_inst_owner_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_NOCI_CNCPTV_FK (
   ...
   BEGIN
      DELETE FROM NOC_INDICATOR
       WHERE
          ID_TERMINOLOGY_VERSION = del_NOCI_TERMV_FK.id_terminology_version_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_NOCI_TERMV_FK (
   ...
   BEGIN
      IF where_in IS NULL
      THEN
         RETURN 'DELETE FROM NOC_INDICATOR';
      ELSE
         RETURN
            'DELETE FROM NOC_INDICATOR WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   IS
   BEGIN
      noc_indicator_inout.ID_NOC_INDICATOR := NULL;
      noc_indicator_inout.ID_TERMINOLOGY_VERSION := NULL;
      noc_indicator_inout.INDICATOR_CODE := NULL;
      noc_indicator_inout.FLG_OTHER := NULL;
      noc_indicator_inout.ID_INST_OWNER := NULL;
      noc_indicator_inout.ID_CONCEPT_VERSION := NULL;
      noc_indicator_inout.ID_CONCEPT_TERM := NULL;
      noc_indicator_inout.CREATE_USER := NULL;
      noc_indicator_inout.CREATE_TIME := NULL;
      noc_indicator_inout.CREATE_INSTITUTION := NULL;
      noc_indicator_inout.UPDATE_USER := NULL;
      noc_indicator_inout.UPDATE_TIME := NULL;
      noc_indicator_inout.UPDATE_INSTITUTION := NULL;
   END initrec;

   FUNCTION initrec RETURN NOC_INDICATOR%ROWTYPE
   IS
      l_noc_indicator NOC_INDICATOR%ROWTYPE;
   BEGIN
      RETURN l_noc_indicator;
   END initrec;
   ...
   IS
        data NOC_INDICATOR_tc;
   BEGIN
        select * bulk collect into data from NOC_INDICATOR where rowid in (select /*+opt_estimate(table,t,scale_rows=0.0000001))*/ * from table(rows_in) t);
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
