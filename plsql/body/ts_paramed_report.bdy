...
CREATE OR REPLACE PACKAGE BODY TS_PARAMED_REPORT
/*
| Generated by or retrieved from QCGU - DO NOT MODIFY!
| QCGU - "Get it right, do it fast" - www.ToadWorld.com
| QCGU Universal ID: {657BD35E-2E48-4CD4-B14E-41420B8433B3}
| Created On: Março 19, 2010 13:37:47
| Created By: ALERT
*/
IS
      ...
      THEN
        ...
        BEGIN
           ...
           pk_alert_exceptions.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'PARAMED_REPORT');
           IF l_name = 'PMR_CID_FK'
           THEN
   ...
   */
   ...
   IS
      l_rec PARAMED_REPORT%ROWTYPE := rec_in;
   BEGIN
      IF gen_pky_in THEN
         l_rec.ID_PARAMED_REPORT := next_key (sequence_in);
      END IF;
      ins (
  ...
  ins (
   ...
   IS
     retval PARAMED_REPORT.ID_PARAMED_REPORT%TYPE;

   BEGIN
      IF sequence_in IS NULL
      THEN
         SELECT seq_PARAMED_REPORT.NEXTVAL INTO retval FROM dual;
      ELSE
         EXECUTE IMMEDIATE
   ...
   EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
           ...
           ,name1_in => 'SEQUENCE'
           ,value1_in => NVL (sequence_in, 'seq_PARAMED_REPORT')
           );
   END next_key;
   ...
   IS
        l_pky PARAMED_REPORT.ID_PARAMED_REPORT%TYPE := next_key;
   BEGIN
      ins (
         id_paramed_report_in => l_pky,
         flg_status_in => flg_status_in,
         text_in => text_in,
         id_episode_in => id_episode_in,
         dt_creation_in => dt_creation_in,
   ...
   BEGIN
      ...
      update_time_in => update_time_in,
      update_institution_in => update_institution_in,
      id_paramed_report_out => id_paramed_report_out
      ,handle_error_in => handle_error_in
      , rows_out => rows_out
   );
   END ins;
   ...
   BEGIN
      ...
      ELSE
         FORALL indx IN rows_in.FIRST .. rows_in.LAST
            SAVE EXCEPTIONS
            INSERT INTO PARAMED_REPORT VALUES rows_in (indx) RETURNING ROWID BULK COLLECT INTO rows_out;
      END IF;
   EXCEPTION
     WHEN e_forall_error
     THEN
...
PROCEDURE upd (
   ...
   l_UPDATE_INSTITUTION_n := sys.diutil.bool_to_int(UPDATE_INSTITUTION_nin);
     ...
     UPDATE_INSTITUTION = decode (l_UPDATE_INSTITUTION_n,0,update_institution_in, NVL (update_institution_in, UPDATE_INSTITUTION))
          WHERE
             ID_PARAMED_REPORT = id_paramed_report_in
...
PROCEDURE upd (
   ...
   l_UPDATE_TIME_n NUMBER(1);
   l_UPDATE_INSTITUTION_n NUMBER(1);
      id_paramed_report_in PARAMED_REPORT.ID_PARAMED_REPORT%TYPE;
   BEGIN
...
l_sql := 'UPDATE PARAMED_REPORT SET '
     || ' FLG_STATUS = decode (' || l_FLG_STATUS_n || ',0,:flg_status_in, NVL (:flg_status_in, FLG_STATUS)) '|| ','
     || ' TEXT = decode (' || l_TEXT_n || ',0,:text_in, NVL (:text_in, TEXT)) '|| ','
     || ' ID_EPISODE = decode (' || l_ID_EPISODE_n || ',0,:id_episode_in, NVL (:id_episode_in, ID_EPISODE)) '|| ','
     || ' DT_CREATION = decode (' || l_DT_CREATION_n || ',0,:dt_creation_in, NVL (:dt_creation_in, DT_CREATION)) '|| ','
...
PROCEDURE upd (
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE PARAMED_REPORT set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE PARAMED_REPORT set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM PARAMED_REPORT
       WHERE ID_PARAMED_REPORT = id_paramed_report_in
      RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM PARAMED_REPORT
       WHERE
          ID_CANCEL_INFO_DET = del_PMR_CID_FK.id_cancel_info_det_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_PMR_CID_FK (
   ...
   BEGIN
      DELETE FROM PARAMED_REPORT
       WHERE
          ID_EPISODE = del_PMR_EPIS_FK.id_episode_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_PMR_EPIS_FK (
   ...
   BEGIN
      DELETE FROM PARAMED_REPORT
       WHERE
          ID_PROFESSIONAL = del_PMR_PROF_FK.id_professional_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_PMR_PROF_FK (
   ...
   BEGIN
      IF where_in IS NULL
      THEN
         RETURN 'DELETE FROM PARAMED_REPORT';
      ELSE
         RETURN
            'DELETE FROM PARAMED_REPORT WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   IS
   BEGIN
      paramed_report_inout.ID_PARAMED_REPORT := NULL;
      paramed_report_inout.FLG_STATUS := NULL;
      paramed_report_inout.TEXT := NULL;
      paramed_report_inout.ID_EPISODE := NULL;
      paramed_report_inout.DT_CREATION := NULL;
      paramed_report_inout.DT_LAST_UPDATE := NULL;
      paramed_report_inout.ID_PROFESSIONAL := NULL;
      paramed_report_inout.ID_CANCEL_INFO_DET := NULL;
      paramed_report_inout.CREATE_USER := NULL;
      paramed_report_inout.CREATE_TIME := NULL;
      paramed_report_inout.CREATE_INSTITUTION := NULL;
      paramed_report_inout.UPDATE_USER := NULL;
      paramed_report_inout.UPDATE_TIME := NULL;
      paramed_report_inout.UPDATE_INSTITUTION := NULL;
   END initrec;

   FUNCTION initrec RETURN PARAMED_REPORT%ROWTYPE
   IS
      l_paramed_report PARAMED_REPORT%ROWTYPE;
   BEGIN
      RETURN l_paramed_report;
   END initrec;
   ...
   IS
        data PARAMED_REPORT_tc;
   BEGIN
        select * bulk collect into data from PARAMED_REPORT where rowid in (select * from table(rows_in));
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
