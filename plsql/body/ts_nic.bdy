      ...
      THEN
        ...
        BEGIN
           ...
           pk_alert_exceptions.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'NIC');
           IF l_name = 'NIC_CNCPT_FK'
           THEN
   ...
   BEGIN
      ...
      ELSE
         FORALL indx IN rows_in.FIRST .. rows_in.LAST
            SAVE EXCEPTIONS
            INSERT INTO NIC VALUES rows_in (indx) RETURNING ROWID BULK COLLECT INTO rows_out;
      END IF;
   EXCEPTION
     WHEN e_forall_error
     THEN
...
PROCEDURE upd (
   ...
   l_UPDATE_TIME_n NUMBER(1);
   l_UPDATE_INSTITUTION_n NUMBER(1);
      intervention_code_in NIC.INTERVENTION_CODE%TYPE;
   BEGIN
...
PROCEDURE upd (
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE NIC set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE NIC set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM NIC
       WHERE
          ID_CONCEPT = del_NIC_CNCPT_FK.id_concept_in AND
          ID_INST_OWNER = del_NIC_CNCPT_FK.id_inst_owner_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_NIC_CNCPT_FK (
   ...
   BEGIN
      ...
      ELSE
         RETURN
            'DELETE FROM NIC WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   IS
   BEGIN
      nic_inout.INTERVENTION_CODE := NULL;
      nic_inout.ID_INST_OWNER := NULL;
      nic_inout.ID_CONCEPT := NULL;
      nic_inout.CREATE_USER := NULL;
      nic_inout.CREATE_TIME := NULL;
      nic_inout.CREATE_INSTITUTION := NULL;
      nic_inout.UPDATE_USER := NULL;
      nic_inout.UPDATE_TIME := NULL;
      nic_inout.UPDATE_INSTITUTION := NULL;
   END initrec;
   ...
   IS
        data NIC_tc;
   BEGIN
        select * bulk collect into data from NIC where rowid in (select /*+opt_estimate(table,t,scale_rows=0.0000001))*/ * from table(rows_in) t);
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
