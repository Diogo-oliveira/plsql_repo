...
CREATE OR REPLACE PACKAGE BODY TS_TRIAL_PROF
/*
| Generated by or retrieved from QCGU - DO NOT MODIFY!
| QCGU - "Get it right, do it fast" - www.ToadWorld.com
| QCGU Universal ID: {657BD35E-2E48-4CD4-B14E-41420B8433B3}
| Created On: Janeiro 25, 2011 17:19:58
| Created By: ALERT
*/
IS
      ...
      THEN
        ...
        BEGIN
           ...
           pk_alert_exceptions.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'TRIAL_PROF');
           IF l_name = 'TP_PROF_FK'
           THEN
   ...
   IS
      l_rec TRIAL_PROF%ROWTYPE := rec_in;
   BEGIN
      ins (
   ...
   BEGIN
      ...
      ELSE
         FORALL indx IN rows_in.FIRST .. rows_in.LAST
            SAVE EXCEPTIONS
            INSERT INTO TRIAL_PROF VALUES rows_in (indx) RETURNING ROWID BULK COLLECT INTO rows_out;
      END IF;
   EXCEPTION
     WHEN e_forall_error
     THEN
...
PROCEDURE upd (
   ...
   l_UPDATE_TIME_n NUMBER(1);
   l_UPDATE_INSTITUTION_n NUMBER(1);
      id_trial_in TRIAL_PROF.ID_TRIAL%TYPE;
      id_professional_in TRIAL_PROF.ID_PROFESSIONAL%TYPE;
   BEGIN
...
PROCEDURE upd (
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE TRIAL_PROF set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE TRIAL_PROF set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM TRIAL_PROF
       WHERE
          ID_PROFESSIONAL = del_TP_PROF_FK.id_professional_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_TP_PROF_FK (
   ...
   BEGIN
      DELETE FROM TRIAL_PROF
       WHERE
          ID_TRIAL = del_TP_T_FK.id_trial_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_TP_T_FK (
   ...
   BEGIN
      IF where_in IS NULL
      THEN
         RETURN 'DELETE FROM TRIAL_PROF';
      ELSE
         RETURN
            'DELETE FROM TRIAL_PROF WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   IS
   BEGIN
      trial_prof_inout.ID_TRIAL := NULL;
      trial_prof_inout.ID_PROFESSIONAL := NULL;
      trial_prof_inout.CREATE_USER := NULL;
      trial_prof_inout.CREATE_TIME := NULL;
      trial_prof_inout.CREATE_INSTITUTION := NULL;
      trial_prof_inout.UPDATE_USER := NULL;
      trial_prof_inout.UPDATE_TIME := NULL;
      trial_prof_inout.UPDATE_INSTITUTION := NULL;
   END initrec;

   FUNCTION initrec RETURN TRIAL_PROF%ROWTYPE
   IS
      l_trial_prof TRIAL_PROF%ROWTYPE;
   BEGIN
      RETURN l_trial_prof;
   END initrec;
   ...
   IS
        data TRIAL_PROF_tc;
   BEGIN
        select * bulk collect into data from TRIAL_PROF where rowid in (select /*+opt_estimate(table,t,scale_rows=0.0000001))*/ * from table(rows_in) t);
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
