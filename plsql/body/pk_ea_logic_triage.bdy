...
CREATE OR REPLACE PACKAGE BODY pk_ea_logic_triage IS
    ...
    */
    ...
    (
        ...
        TYPE t_coll_triage IS TABLE OF c_triage%ROWTYPE;
    ...
    BEGIN
        ...
        IF NOT t_data_gov_mnt.validate_arguments(i_rowids                 => i_rowids,
                                                 ...
                                                 i_expected_table_name    => g_triage_src_table,
        ...
        THEN
            -- debug event
            g_error := 'processing insert or update event on ' || g_triage_src_table || ' into ' || l_ea_table;
            ...
            pk_alertlog.log_info(text => g_error, object_name => g_package, sub_object_name => l_func_name);
            OPEN c_triage(i_rowids => i_rowids);
            ...
            THEN
                ...
                l_ea_row.table_name        := g_triage_src_table;
                ...
                LOOP
                    ...
                    IF l_triage_rows(i).flg_status_epis = pk_alert_constant.g_epis_status_cancel
                    THEN
