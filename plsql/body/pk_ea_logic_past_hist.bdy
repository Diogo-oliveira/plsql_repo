/*-- Last Change Revision: $Rev: 2027043 $*/
/*-- Last Change by: $Author: mario.fernandes $*/
/*-- Date of last change: $Date: 2022-08-02 18:40:49 +0100 (ter, 02 ago 2022) $*/

CREATE OR REPLACE PACKAGE BODY pk_ea_logic_past_hist IS
    ...
    *******************************************************************************************************************************************/
    ...
    (
        ...
        l_func_proc_name VARCHAR2(30) := 'SET_TL_PAST_HIST_PHD';
        ...
        THEN
            ...
            THEN
                ...
                FOR r_cur IN (SELECT phd.id_pat_history_diagnosis,
                               ...
                               WHERE phd.rowid IN (SELECT vc_1
                                 ...
                                 AND (ad.flg_type = phd.flg_type OR ad.flg_type = pk_past_history.g_alert_diag_type_med OR
                                     ...
                                     (phd.id_alert_diagnosis IS NULL AND phd.id_diagnosis IS NULL))
                ...
                LOOP
                    ...
                    --
                    l_new_rec_row.id_tl_task        := CASE r_cur.flg_type
                                                           ...
                                                           WHEN pk_past_history.g_alert_type_treatments THEN
                                                            pk_prog_notes_constants.g_task_ph_treatments
                                                           WHEN pk_past_history.g_alert_diag_type_gyneco THEN
                                                            pk_prog_notes_constants.g_task_ph_gynec_diag
                                                           WHEN pk_past_history.g_alert_diag_type_family THEN
                    ...
                    l_new_rec_row.dt_dg_last_update := r_cur.dt_req;
                    --
                    ...
                    IF (r_cur.id_pat_history_diagnosis_new IS NULL)
                    THEN
                    ...
                    l_new_rec_row.id_parent_task_refid := r_cur.id_pat_history_diagnosis_new;
                    ...
                    IF ((r_cur.flg_status <> pk_past_history.g_pat_hist_diag_canceled OR r_cur.flg_status IS NULL) -- Active Data
                       AND (r_cur.flg_status_epis <> pk_alert_constant.g_epis_status_cancel) OR
                       r_cur.flg_status_epis IS NULL)
                       AND r_cur.id_pat_history_diagnosis_new IS NULL
                    THEN
                        ...
                        ELSE
                            ...
                            l_event_into_ea := 'I';
                        END IF;
                    ELSIF r_cur.flg_status = pk_past_history.g_pat_hist_diag_canceled
                          OR r_cur.flg_status_epis = pk_alert_constant.g_epis_status_cancel
                          OR r_cur.id_pat_history_diagnosis_new IS NOT NULL
                    THEN
                        -- Cancelled data
                        --
    ...
    EXCEPTION
        ...
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang,
                                              ...
                                              g_owner,
                                              g_package,
                                              'set_tl_past_hist_phd',
                                              l_error_out);
        
            pk_alert_exceptions.raise_error(error_code_in => SQLCODE);
        
    END set_tl_past_hist_phd;

    /*******************************************************************************************************************************************
    * Name:                           set_past_hist_phd
    * Description:                    Function that updates past history information in the Easy Access table (task_timeline_ea)
    * 
    * @param I_LANG                   Language ID
    * @param I_PROF                   Professional information Vector: (professional ID, institution ID, software ID)
    ...
    *******************************************************************************************************************************************/
    ...
    (
        ...
        l_new_rec_row    task_timeline_ea%ROWTYPE;
        l_func_proc_name VARCHAR2(30) := 'SET_TL_PAST_HIST_FTXT';
        l_name_table_ea  VARCHAR2(30) := 'TASK_TIMELINE_EA';
        l_process_name   VARCHAR2(30);
        l_event_into_ea  VARCHAR2(1);
        l_update_reg     NUMBER(24);
    ...
    BEGIN
        ...
        THEN
            ...
            THEN
                ...
                g_error := 'GET PAST HIST ROWIDS AND INFORMATION';
                pk_alertlog.log_debug(g_error);
                FOR r_cur IN (SELECT pphfth.id_pat_ph_ft,
                                     pphfth.id_episode,
                                     pphfth.dt_register                       dt_req,
                                     pphfth.flg_status,
                                     pphfth.id_professional                   id_prof_request,
                                     pphfth.id_patient,
                                     pphfth.id_doc_area,
                                     pphfth.id_visit,
                                     pphfth.flg_type,
                                     ...
                                     pk_alert_constant.g_yes                  flg_normal,
                                     pphfth.id_professional                   id_prof_exec,
                                     epi.flg_status                           flg_status_epis
                                FROM pat_past_hist_ft_hist pphfth
                               INNER JOIN episode epi
                                  ON (epi.id_episode = pphfth.id_episode)
                               WHERE pphfth.rowid IN (SELECT vc_1
                                                        FROM tbl_temp))
                ...
                LOOP
                    ...
                    l_new_rec_row.id_tl_task        := pk_prog_notes_constants.g_task_ph_free_txt;
                    l_new_rec_row.table_name        := pk_prog_notes_constants.g_tl_table_name_ph_ftxt;
                    l_new_rec_row.flg_show_method   := pk_alert_constant.g_tl_oriented_patient;
                    l_new_rec_row.dt_dg_last_update := current_timestamp;
                    --
                    l_new_rec_row.id_task_refid       := r_cur.id_pat_ph_ft;
                    ...
                    --
                    -- Events in task_timeline_ea table is dependent of l_new_rec_row.flg_status_req variable
                    IF r_cur.flg_status <> pk_past_history.g_pat_hist_diag_canceled -- Active Data
                       AND r_cur.flg_status_epis <> pk_alert_constant.g_epis_status_cancel
                    THEN
                        pk_alertlog.log_debug('Check for update id_task_refid: ' || l_new_rec_row.id_task_refid);
                        -- Search for updated registrie
                        SELECT COUNT(0)
                          INTO l_update_reg
                          FROM task_timeline_ea ttea
                        ...
                        IF l_update_reg > 0
                           AND
                           r_cur.flg_status NOT IN
                           (pk_past_history.g_pat_hist_diag_canceled, pk_past_history.g_flg_status_outdtd_free_text)
                        THEN
                            l_process_name  := 'UPDATE';
                            l_event_into_ea := 'U';
    ...
    EXCEPTION
        ...
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang,
                                              ...
                                              g_owner,
                                              g_package,
                                              'set_tl_past_hist_ftxt',
                                              l_error_out);
        
            pk_alert_exceptions.raise_error(error_code_in => SQLCODE);
        
    END set_tl_past_hist_ftxt;
