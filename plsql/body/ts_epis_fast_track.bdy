/*-- Last Change Revision: $Rev: 2028076 $*/
/*-- Last Change by: $Author: mario.fernandes $*/
/*-- Date of last change: $Date: 2022-08-02 18:44:07 +0100 (ter, 02 ago 2022) $*/
CREATE OR REPLACE PACKAGE BODY ts_epis_fast_track
/*
...
l_rec epis_fast_track%ROWTYPE := rec_in;
BEGIN
...
l_sql := 'UPDATE epis_fast_track SET' ||
' ID_FAST_TRACK = decode (' || l_ID_FAST_TRACK_n || ',0,:ID_FAST_TRACK_in, NVL (:ID_FAST_TRACK_in, ID_FAST_TRACK)) ' ||',' ||
' FLG_STATUS = decode (' || l_FLG_STATUS_n || ',0,:FLG_STATUS_in, NVL (:FLG_STATUS_in, FLG_STATUS)) ' ||',' ||
' ID_PROF_DISABLE = decode (' || l_ID_PROF_DISABLE_n || ',0,:ID_PROF_DISABLE_in, NVL (:ID_PROF_DISABLE_in, ID_PROF_DISABLE)) ' ||',' ||
' DT_DISABLE = decode (' || l_DT_DISABLE_n || ',0,:DT_DISABLE_in, NVL (:DT_DISABLE_in, DT_DISABLE)) ' ||',' ||
...
RETURN VARCHAR2 IS
BEGIN
RETURN 'BEGIN UPDATE epis_fast_track
...
is
begin
EXECUTE IMMEDIATE 'BEGIN UPDATE epis_fast_track
...
rows_out table_varchar;
begin
EXECUTE IMMEDIATE 'UPDATE epis_fast_track
SET ' || colname_in || '=' || colname_in || '+' || nvl(increment_value_in,1) ||
...
IF where_in IS NULL
THEN
RETURN 'DELETE FROM epis_fast_track';
ELSE
RETURN 'DELETE FROM epis_fast_track WHERE ' || where_in;
END IF;
end dyndelstr;
...
is
begin
epis_fast_track_inout.ID_EPIS_TRIAGE := NULL;
epis_fast_track_inout.ID_FAST_TRACK := NULL;
epis_fast_track_inout.FLG_STATUS := NULL;
epis_fast_track_inout.ID_PROF_DISABLE := NULL;
epis_fast_track_inout.DT_DISABLE := NULL;
epis_fast_track_inout.ID_FAST_TRACK_DISABLE := NULL;
epis_fast_track_inout.NOTES_DISABLE := NULL;
epis_fast_track_inout.CREATE_USER := NULL;
epis_fast_track_inout.CREATE_TIME := NULL;
epis_fast_track_inout.CREATE_INSTITUTION := NULL;
epis_fast_track_inout.UPDATE_USER := NULL;
epis_fast_track_inout.UPDATE_TIME := NULL;
epis_fast_track_inout.UPDATE_INSTITUTION := NULL;
epis_fast_track_inout.FLG_TYPE := NULL;
epis_fast_track_inout.FLG_ACTIVATION_TYPE := NULL;
epis_fast_track_inout.DT_ENABLE := NULL;
epis_fast_track_inout.ID_PROF_ENABLE := NULL;
epis_fast_track_inout.NOTES_ENABLE := NULL;
epis_fast_track_inout.DT_ACTIVATION := NULL;
end initrec;

-- Initialize a record with default values for columns in the table (fnc)
FUNCTION initrec RETURN epis_fast_track%ROWTYPE
is
l_epis_fast_track epis_fast_track%ROWTYPE;
begin
return l_epis_fast_track;
end initrec;

--get data from rowid
FUNCTION get_data_rowid(rows_in IN table_varchar) RETURN epis_fast_track_tc
is
data epis_fast_track_tc;
begin
select * bulk collect into data from epis_fast_track
