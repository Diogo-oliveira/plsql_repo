...
CREATE OR REPLACE PACKAGE BODY pk_vacc IS
    ...
    ***********************************************************************************************************/
    ...
    (
        ...
        /******************************************************************************
           OBJECTIVO:   Obter lista de Reacções adversas às vacinas administradas
           PARAMETROS:  Entrada: I_LANG - Língua registada como preferência do profissional
                  Saida:   O_SCHEDULE - TIPO
                     O_ERROR - erro
    ...
    * @param i_test                   indicação se testa a existência de exames com resultados ou já requisitados (se a msg O_MSG_REQ ou O_MSG_RESULT já foram apresentadas e o user continuou, I_TEST = pk_alert_constant.g_no)
    ...
    **********************************************************************************************/
    ...
    /***********************************************************************************************************
    *  Esta função permite criar um novo episodio genérico e de qualquer tipo. Como cada episodio está
    *   associado a uma visita, é necessário criar em primeiro lugar uma visita.
    *
    * @param      i_lang               Língua registada como preferência do profissional
    * @param      i_prof               ID do profissional
    ...
    *
    * @return     se a função termina com sucesso e FALSE caso contrário
    ...
    ***********************************************************************************************************/
    ...
    (
        ...
        --id para o novo episódio
        l_seq_epis NUMBER;
        --id do tipo de episódio que vai corresponder a uma consulta de enfermagem
    ...
    BEGIN
        ...
        --Criação da visita a que o novo episodio vai estar associado
        ...
        --Selecção do tipo de episódio: Consulta de Enfermagem
        SELECT et.id_epis_type
          INTO l_id_epis_type
          FROM epis_type et
         WHERE et.code_epis_type = 'EPIS_TYPE.CODE_EPIS_TYPE.14';
    
        --O episódio está inactivo na criação!
    ...
    /***********************************************************************************************************
    *  Esta função permite criar uma nova visita genérica que vai estar associada a uma determinada instituição.
    *
    * @param      i_lang               Língua registada como preferência do profissional
    * @param      i_prof               ID do profissional
    * @param      i_id_patient         ID do paciente
    ...
    ***********************************************************************************************************/
    ...
    BEGIN
        ...
        --A visita está activa na criação!
    ...
    * @param i_lang                   the id language
    * @param i_prof                   Profissional que requisita
    * @param i_type                   Se é para o multichoice de escolha entre a vacina e a prova à tuberculina ou a prescrição
    ...
    ***********************************************************************************************************/
    ...
    BEGIN
        ...
        THEN
            --formata os dados para o ecrã de detalhes da precrição de vacinas:
        ...
        THEN
            --formata os dados para o ecrã de detalhes da administração de vacinas:
    ...
    ***********************************************************************************************************/
    ...
    BEGIN
        ...
        --NOTA [OA]: A ordenação dos campos neste cursor é importante porque define a forma como os mesmos são
        -- apresentados no ecrã. Os códigos destes campos não podem ser alterados!
    ...
    ***********************************************************************************************************/
    ...
    BEGIN
        ...
        --if i_to_add is false indica que estamos num ecrã de resumo
        --if i_to_add is true indica que estamos nun ecrã de registo de informação.
        --se o estado é administração vamos adicionar resultados
        IF NOT i_to_add
        ...
        THEN
            ...
            THEN
                --já sabemos qual é a dose da vacina
                OPEN o_presc_det FOR
                    SELECT det_name, det_value, desc_resp, flg_show, id_test
                      FROM (SELECT rtrim(s.desc_message, ':') || ':' det_name,
                                   get_oth_vacc_value_det(i_lang, i_prof, i_vacc_take_id, s.code_message) det_value,
                                   '' desc_resp,
                                   --a label 'Notas da prescrição' apenas é apresentada para os vacinas trazidas pelo paciente...
                             ...
                             WHERE s.code_message = adm_info.desc_info
                               AND s.id_language = i_lang
                                  --apenas se o estado é Administrado
    ...
    ***********************************************************************************************************/
    ...
    BEGIN
        ...
        ELSE
            ...
            OPEN o_adm_det FOR
                ...
                UNION ALL -- o Union separado é para manter a ordem desejada
    ...
    (
        ...
        --procura todos os ids de vacinas fora do PNV ou relatos (gravados nas tabelas pat_vacc_adm*)
        --apenas para vacinas que tenham já sido administradas
        CURSOR c_tests IS
            SELECT pva.id_pat_vacc_adm id, '' desc_info
              FROM pat_vacc_adm pva, pat_vacc_adm_det pvad
    ...
    BEGIN
        ...
        --Explicação:
        -- A nested table 'l_tests_ids' tem os id dos vários registos nas tabelas pat_vacc_adm (relatos e vacinas fora do PNV)
        -- A nested table 'l_vacc_adm_ids' tem os id dos vários registos nas tabelas drug_prescription (vacinas do PNV)
        -- 1 é necessário fazer um union destes dois casos.
    
        -- A nested table 'l_adm_det_info' tem os uma lista com a informação que vai ser apresentada no ecrã, e com a sua ordenação
        -- É necessãrio cruzar com esta nested table para saber quais os campos a apresentar!
        OPEN o_advers_react_det FOR
            SELECT det_name, det_value, desc_resp, flg_show, id_test, dt, first_show
              FROM (
                    ...
                    SELECT pk_message.get_message(i_lang, 'TUBERCULIN_TEST_M005') det_name,
                            var.notes_advers_react det_value,
                            '' desc_resp,
                            --a label Origem do relato apenas é apresentada para os relatos...
                            pk_alert_constant.g_yes flg_show,
                            test_ids.id             id_test,
                            var.dt_prof_write       dt,
                            1                       first_show
                ...
                SELECT det_name, det_value, desc_resp, flg_show, id_test
                  FROM (
                        ...
                        SELECT rtrim(s.desc_message, ':') || ':' det_name, --garante que todas as labels terminam em :
                                ...
                                --a label Origem do relato apenas é apresentada para os relatos...
                                decode(s.code_message,
                                       'VACC_T036', -- 'Origem do relato', apenas disponível para relatos
                                       decode(pva.flg_reported,
                                              pk_alert_constant.g_yes,
                                              pk_alert_constant.g_yes,
                                              decode(pva.flg_orig, g_orig_r, pk_alert_constant.g_yes, pk_alert_constant.g_no)),
                                       'TUBERCULIN_TEST_T018', -- 'Responsável pela administração', nas importações do SINUS não há
    ...
    ***********************************************************************************************************/
    ...
    BEGIN
        ...
        l_test_state := get_tuberculin_test_state(i_test_id);
        --if false indica que estamos num ecrã de resumo
        --if true indica que estamos nun ecrã de registo de informação.
        IF NOT i_to_add
        THEN
            --title
            BEGIN
                ...
                SELECT rtrim(det_name, ':') det_name
                  FROM (SELECT s.desc_message det_name
                         ...
                         WHERE s.code_message IN ('TUBERCULIN_TEST_T010',
                           ...
                           AND s.id_language = i_lang
                         ORDER BY s.code_message)
                UNION ALL -- o Union separada é para manter a ordem desejada
                SELECT rtrim(s.desc_message, ':') det_name
                  FROM sys_message s
                 WHERE s.code_message IN ('TUBERCULIN_TEST_T025')
                   AND s.id_language = i_lang;
        ELSE
            --Neste caso estamos a adicionar valores de administração, pelo que não há dados para os resultados.
            --title
            o_res_title := '';
                    ...
                    SELECT det_name, det_value, desc_resp, flg_show, id_test
                      FROM (SELECT s.desc_message || ':' det_name,
                             ...
                             WHERE s.code_message IN
                                   ('TUBERCULIN_TEST_T056', 'TUBERCULIN_TEST_T057', 'TUBERCULIN_TEST_T058')
                               AND s.id_language = i_lang
                                  --apenas se o estado é Cancelado
    ...
    ***********************************************************************************************************/
    ...
    (
        ...
        --valor máximo do tempo para os testes à tuberculina
    ...
    ***********************************************************************************************************/
    ...
    BEGIN
        ...
        --apenas existe uma tipo de prova à tuberculina!
    ...
    ***********************************************************************************************************/
    ...
    BEGIN
        ...
        THEN
            ...
            -- para já retorna não existe distinção de query pelo i_button!
            --cursor com as tuberculinas (ainda estão a ser enviadas todas as tuberculinas!)
            OPEN o_med_freq FOR
                SELECT
                --Estes parâmetros são necessários para passar os dados para o ecrã da prescrição!
                 mim.id_drug         drug, --
                 mim.dosagem         dosage, --
    ...
    ***********************************************************************************************************/
    ...
    BEGIN
        ...
        --dados para actualizar "Actualizar datas de 1ª observação médica..."
        --Checklist - 16
    ...
    ***********************************************************************************************************/
    ...
    BEGIN
        --verifica qual é o tipo de registo para obter a informação!
        IF i_cancel_op = 'T'
        THEN
    ...
    ***********************************************************************************************************/
    ...
    BEGIN
        ...
        --grava as alterações
        --COMMIT; -- José Brito 12/03/2010 ALERT-26489 Avoid commit when cancelling reported medication.
    ...
    ***********************************************************************************************************/
    ...
    BEGIN
        ...
        --verifica qual é o tipo de regista a cancelar!
    ...
    ***********************************************************************************************************/
    ...
    BEGIN
        ...
        --[OA] DQ
        --A data da toma nunca pode ser inferior à data da prescrição já registada!
    ...
    BEGIN
        ...
        OPEN o_vacc_val FOR
            SELECT decode(pva.flg_orig, 'R', 'R', g_day) flg_orig,
                   ...
                   NULL icon_color, /*-- -> apenas necessário quando é uma cor diferente da normal*/
                   decode(pva.flg_status,
                          pk_alert_constant.g_active,
                          NULL,
                          pk_alert_constant.g_no,
    ...
    ***********************************************************************************************************/
    ...
    BEGIN
        ...
        --No caso das vacinas o cursor dos tempos vai ter as doses em que cada vacina é administrada.
        --Para preencher este cursor é calculado o nº da dose máxima de todas as vacinas e são retornados
        --os valores até essa dose como os ids dos tempos.
        ...
        OPEN o_vaccine_val FOR
            ...
            SELECT vd.n_dose time_var,
             ...
             WHERE vd.n_dose >= count_vacc_take(i_lang, i_patient, v.id_vacc, NULL, i_prof)
            UNION
            -- tomas da vacina do Tétano, é especifico porque tem num. de tomas ilimitado
            SELECT count_vacc_take(i_lang, i_patient, v.id_vacc, NULL, i_prof) time_var,
                   v.id_vacc par_var,
                   -1 id_value,
                   get_last_date(i_lang, i_prof, i_patient, v.id_vacc) dt_take,
    ...
    ***********************************************************************************************************/
    ...
    BEGIN
        ...
        THEN
            --nº da dose administrada na data
    ...
    */
    ...
    (
        ...
        /******************************************************************************
           ...
           PARAMETROS:  Entrada: I_LANG - Língua registada como preferência do profissional
                  ...
                  Saida: O_INFO - retorna se a vacina é do PNV e as contra-indicações.
    ...
    BEGIN
        ...
        g_error := 'OPEN O_INFO'; -- retorna se a vacina é do PNV e as contra-indicações.
    ...
    ***********************************************************************************************************/
    ...
    /********************************************************************************************
    * Informação sobre o profissional responsável
    *
    * @param i_lang                language id
    ...
    * @param i_flg_status             Estado da Administração (pk_alert_constant.g_active - Administrado)
    ...
    * @param i_test                   Indicação se testa a existência de vacinas administradas ou já requisitadas (se a msg O_MSG_REQ ou O_MSG_RESULT já foram apresentadas e o user continuou, I_TEST = pk_alert_constant.g_no)
    ...
    **********************************************************************************************/
    ...
    BEGIN
        ...
        ---------------------------------------
        -- validação da existência de um relato, importação ou administração da vacina para o paciente no dia indicado
        -- sou houver, a função retorna FALSE de modo a que os interfaces saibam que não foi inserido um registo novo
        ---------------------------------------
        SELECT SUM(counter)
          ...
          FROM (SELECT COUNT(1) AS counter -- relatos de vacinas e vacinas importadas do SINUS
                ...
                UNION ALL
                SELECT COUNT(1) AS counter -- prescrições de vacinas
    ...
    (
        ...
        l_id_pat_medic_list      pk_types.cursor_type; -- a utilizar em substituição da l_id_pat_medication_list nos parametros da função set_pat_medication
    ...
    BEGIN
        ...
        IF i_presc IS NOT NULL -- valida se a receita já foi impressa, de outra forma não é possivel avançar
        THEN
        ...
        THEN
            ...
            LOOP
                ...
                IF i_flg_time(i) = pk_alert_constant.g_no -- até ao próximo episódio
                THEN
                ...
                THEN
                    ...
                    -- se for um relato insere na medicação
                    ...
                    THEN
                        ...
                        -- no caso dos relatos terá de interagir com a medicação
                        THEN
                            ROLLBACK;
                            RETURN FALSE;
                        END IF;
    ...
    /**********************************************************************************************
    *   Verificar se a vacina já tinha sido administrada, ou requisitado recentemente  V.2.4.2
    *
    * @param i_lang                  The id language
    * @param i_id_episode            Episódio
    * @param i_id_patient            Id do paciente
    * @param i_vacc                  Id da vacina
    ...
    **********************************************************************************************/
    ...
    BEGIN
        ...
        THEN
            ...
            THEN
                -- Uma vacina requisitada para epis. seguinte ñ pode ser administrada, pq
                -- qd é requisitado p/ um epis. futuro, a criação de episódio provoca a replicação
                -- das requisições (em estado 'requisitado' e ñ 'pendente')
    ...
    ***********************************************************************************************************/
    ...
    BEGIN
        ...
        --vacc name
        --Esta informação serve o caso particular de administração de vacinas
        ...
        -- vacinas fora do PNV ou relatos
            ...
            SELECT dp.id_drug_prescription id_test,
                   ...
                   '' desc_other,
                   pk_message.get_message(i_lang, 'TUBERCULIN_TEST_T050') desc_state,
                   --as vacinas do PNV apenas têm o estado administrado
    ...
    --
    /**********************************************************************************************
    *   Verificar se a receita já foi impressa
    *
    * @param i_lang                  The id language
    * @param i_id_episode            Episódio
    * @param i_id_patient            Id do paciente
    * @param i_presc                 Id da prescrição
    *
    * @param o_flg_show              Y - existe msg para mostrar; N - ñ existe
    ...
    ***********************************************************************************************************/
    ...
    BEGIN
        ...
        -- retorna os registos que estão ocultos
        OPEN o_vaccine FOR
            SELECT v.id_vacc par_var,
                   pk_translation.get_translation(i_lang, v.code_vacc) par_desc,
    ...
    ***********************************************************************************************************/
    ...
    BEGIN
        ...
        IF i_val = 'OP'
        THEN
            -- Ocultar Vacina do Programa Nacional de Vacinação
            o_flg_show := pk_alert_constant.g_yes;
            --text message
            SELECT s.desc_message text
              INTO o_message_text
        ...
        ELSE
            -- OP Ocultar Programa Nacional de Vacinação
            o_flg_show := pk_alert_constant.g_yes;
            --text message
            SELECT s.desc_message text
              INTO o_message_text
    ...
    * @param OUT  o_vacc        cursor com as vacinas (para integrar com alertas de saúde)
    *
    ...
    **********************************************************************************************/
    ...
    (
        ...
        --vacinas já administradas(apenas as novas vacinas)
