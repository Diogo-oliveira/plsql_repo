...
CREATE OR REPLACE PACKAGE BODY TS_PAT_PO_PARAM
/*
| Generated by or retrieved from QCGU - DO NOT MODIFY!
| QCGU - "Get it right, do it fast" - www.ToadWorld.com
| QCGU Universal ID: {7A44F965-7C77-42EE-9A04-269EF006213B}
| Created On: Agosto 16, 2013 16:11:49
| Created By: ALERT
*/
IS
      ...
      THEN
        ...
        BEGIN
           ...
           pk_alert_exceptions.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'PAT_PO_PARAM');
           IF l_name = 'PATPOP_PAT_FK'
           THEN
   ...
   IS
      l_rec PAT_PO_PARAM%ROWTYPE := rec_in;
   BEGIN
      ins (
   ...
   BEGIN
      ...
      ELSE
         FORALL indx IN rows_in.FIRST .. rows_in.LAST
            SAVE EXCEPTIONS
            INSERT INTO PAT_PO_PARAM VALUES rows_in (indx) RETURNING ROWID BULK COLLECT INTO rows_out;
      END IF;
   EXCEPTION
     WHEN e_forall_error
     THEN
...
PROCEDURE upd (
   ...
   l_UPDATE_TIME_n NUMBER(1);
   l_UPDATE_INSTITUTION_n NUMBER(1);
      id_patient_in PAT_PO_PARAM.ID_PATIENT%TYPE;
      id_po_param_in PAT_PO_PARAM.ID_PO_PARAM%TYPE;
      id_inst_owner_in PAT_PO_PARAM.ID_INST_OWNER%TYPE;
   BEGIN
...
PROCEDURE upd (
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE PAT_PO_PARAM set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE PAT_PO_PARAM set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM PAT_PO_PARAM
       WHERE
          ID_PATIENT = del_PATPOP_PAT_FK.id_patient_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_PATPOP_PAT_FK (
   ...
   BEGIN
      DELETE FROM PAT_PO_PARAM
       WHERE
          ID_PO_PARAM = del_PATPOP_POP_FK.id_po_param_in AND
          ID_INST_OWNER = del_PATPOP_POP_FK.id_inst_owner_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_PATPOP_POP_FK (
   ...
   BEGIN
      IF where_in IS NULL
      THEN
         RETURN 'DELETE FROM PAT_PO_PARAM';
      ELSE
         RETURN
            'DELETE FROM PAT_PO_PARAM WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   IS
   BEGIN
      pat_po_param_inout.ID_PATIENT := NULL;
      pat_po_param_inout.ID_PO_PARAM := NULL;
      pat_po_param_inout.ID_INST_OWNER := NULL;
      pat_po_param_inout.FLG_VISIBLE := NULL;
      pat_po_param_inout.CREATE_USER := NULL;
      pat_po_param_inout.CREATE_TIME := NULL;
      pat_po_param_inout.CREATE_INSTITUTION := NULL;
      pat_po_param_inout.UPDATE_USER := NULL;
      pat_po_param_inout.UPDATE_TIME := NULL;
      pat_po_param_inout.UPDATE_INSTITUTION := NULL;
   END initrec;

   FUNCTION initrec RETURN PAT_PO_PARAM%ROWTYPE
   IS
      l_pat_po_param PAT_PO_PARAM%ROWTYPE;
   BEGIN
      RETURN l_pat_po_param;
   END initrec;
   ...
   IS
        data PAT_PO_PARAM_tc;
   BEGIN
        select * bulk collect into data from PAT_PO_PARAM where rowid in (select /*+opt_estimate(table,t,scale_rows=0.0000001))*/ * from table(rows_in) t);
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
