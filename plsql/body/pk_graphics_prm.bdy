...
CREATE OR REPLACE PACKAGE BODY pk_graphics_prm IS
    -- Package info
    g_package_owner t_low_char := 'alert';
    g_package_name  t_low_char := 'PK_graphics_prm';
    pos_soft        NUMBER := 1;
    ...
    FUNCTION set_graphic_soft_inst_search
    (
    ...
    BEGIN
        g_func_name := upper('set_graphic_soft_inst_search');
        INSERT INTO graphic_soft_inst
            (id_graphic_soft_inst, id_graphic, rank, id_institution, id_software)
            SELECT seq_graphic_soft_inst.nextval,
                   def_data.id_graphic,
                   def_data.rank,
                   i_institution,
                   i_software(pos_soft)
              FROM (SELECT temp_data.id_graphic,
                           temp_data.rank,
                           row_number() over(PARTITION BY temp_data.id_graphic
                           
                           ORDER BY temp_data.id_software DESC, temp_data.id_market DESC, decode(temp_data.version, 'DEFAULT', 0, 1) DESC) records_count
                      FROM (SELECT gsi.id_graphic, gsi.rank, gsi.id_software, gmv.id_market, gmv.version
                            -- decode FKS to dest_vals
                              FROM alert_default.graphic_mrk_vrs gmv
                             INNER JOIN alert_default.graphic_soft_inst gsi
                                ON gsi.id_graphic = gmv.id_graphic
                            
                             WHERE gsi.id_software IN
             ...
             WHERE def_data.records_count = 1
               AND NOT EXISTS (SELECT 0
                      FROM graphic_soft_inst gsi1
                     WHERE gsi1.id_graphic = def_data.id_graphic
                       AND gsi1.id_institution = i_institution
                       AND gsi1.id_software = i_software(pos_soft));
    ...
    EXCEPTION
        WHEN OTHERS THEN
            pk_alert_exceptions.process_error(i_lang,
                                              ...
                                              o_error);
            RETURN FALSE;
    END set_graphic_soft_inst_search;

    -- frequent loader method

    FUNCTION del_graphic_soft_inst_search
    (
        i_lang        IN language.id_language%TYPE,
        i_institution IN institution.id_institution%TYPE,
        i_software    IN table_number,
    ...
    BEGIN
        g_error     := 'delete graphic_soft_inst';
        g_func_name := upper('del_graphic_soft_inst_search');
        ...
        THEN
            DELETE FROM graphic_soft_inst gsi
             WHERE gsi.id_institution = i_institution
               AND gsi.id_software IN (SELECT /*+ dynamic_sampling(2)*/
                                        column_value
                                         FROM TABLE(CAST(i_software AS table_number)) p);
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ...
            RETURN FALSE;
    END del_graphic_soft_inst_search;
