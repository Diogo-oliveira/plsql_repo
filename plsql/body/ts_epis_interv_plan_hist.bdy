/*-- Last Change Revision: $Rev: 2028111 $*/
/*-- Last Change by: $Author: mario.fernandes $*/
/*-- Date of last change: $Date: 2022-08-02 18:44:13 +0100 (ter, 02 ago 2022) $*/

CREATE OR REPLACE PACKAGE BODY ts_epis_interv_plan_hist IS
  ...
  e_null_column_value EXCEPTION;
   ...
   BEGIN
     ...
     INSERT INTO EPIS_INTERV_PLAN_HIST (
        ...
        BEGIN
           ...
           ELSE
              pk_alert_exceptions.raise_error (
                    ...
                    ,value2_in => l_name
                    ,name3_in => 'TABLE_NAME'
                    ,value3_in => 'EPIS_INTERV_PLAN_HIST');
           END IF;
        END;
        END IF;
      ...
      THEN
        ...
        BEGIN
           ...
           pk_alert_exceptions.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'EPIS_INTERV_PLAN_HIST');
           IF l_name = 'EIPH_EIH_FK'
           THEN
              -- Add a context value for each column
              pk_alert_exceptions.add_context (
                 err_instance_id_in => l_id
               , name_in => 'ID_EPIS_INTERV_PLAN'
               , value_in => id_epis_interv_plan_in);
           END IF;
           IF l_name = 'EIPH_P_FK'
           THEN
              -- Add a context value for each column
   ...
   BEGIN
     ...
     ins (
      id_epis_interv_plan_hist_in => id_epis_interv_plan_hist_in
      ,
   ...
   IS
      l_rec EPIS_INTERV_PLAN_HIST%ROWTYPE := rec_in;
   BEGIN
      IF gen_pky_in THEN
         l_rec.ID_EPIS_INTERV_PLAN_HIST := next_key (sequence_in);
      END IF;
      ins (
         id_epis_interv_plan_hist_in => l_rec.ID_EPIS_INTERV_PLAN_HIST
         ,
  ...
  ins (
   ...
   IS
     retval EPIS_INTERV_PLAN_HIST.ID_EPIS_INTERV_PLAN_HIST%TYPE;

   BEGIN
      IF sequence_in IS NULL
      THEN
         SELECT seq_EPIS_INTERV_PLAN_HIST.NEXTVAL INTO retval FROM dual;
      ELSE
         EXECUTE IMMEDIATE
   ...
   EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
           ...
           ,name1_in => 'SEQUENCE'
           ,value1_in => NVL (sequence_in, 'seq_EPIS_INTERV_PLAN_HIST')
           );
   END next_key;
   ...
   IS
        l_pky EPIS_INTERV_PLAN_HIST.ID_EPIS_INTERV_PLAN_HIST%TYPE := next_key;
   BEGIN
      ins (
         id_epis_interv_plan_hist_in => l_pky,
         id_epis_interv_plan_in => id_epis_interv_plan_in,
         id_interv_plan_in => id_interv_plan_in,
         id_episode_in => id_episode_in,
         id_professional_in => id_professional_in,
      ...
      id_epis_interv_plan_hist_out := l_pky;
   END ins;
   ...
   BEGIN
      ...
      update_institution_in => update_institution_in,
      id_task_goal_det_in => id_task_goal_det_in,
      id_epis_interv_plan_hist_out => id_epis_interv_plan_hist_out
      ,handle_error_in => handle_error_in
      , rows_out => rows_out
   );
   END ins;
   ...
   BEGIN
      ...
      ELSE
         FORALL indx IN rows_in.FIRST .. rows_in.LAST
            SAVE EXCEPTIONS
            INSERT INTO EPIS_INTERV_PLAN_HIST VALUES rows_in (indx) RETURNING ROWID BULK COLLECT INTO rows_out;
      END IF;
   EXCEPTION
     WHEN e_forall_error
     THEN
       ...
       ELSE
          ...
          BEGIN
             ...
             LOOP
                ...
                pk_alert_exceptions.add_context (
                   err_instance_id_in => l_err_instance_id
                  ,NAME_IN => 'ID_EPIS_INTERV_PLAN_HIST _' || indx
                  ,value_in => rows_in(indx).ID_EPIS_INTERV_PLAN_HIST
                  ,validate_in => FALSE
                );
     ...
     THEN
       ...
       pk_alert_exceptions.raise_error(
          error_name_in => 'FORALL-INSERT-FAILURE'
          ,name1_in => 'TABLE_NAME'
          ,value1_in => 'EPIS_INTERV_PLAN_HIST'
          ,name2_in => 'ROW_COUNT'
...
PROCEDURE upd (
   ...
   l_ID_EPIS_INTERV_PLAN_n NUMBER(1);
   ...
   l_ID_EPIS_INTERV_PLAN_n := sys.diutil.bool_to_int(ID_EPIS_INTERV_PLAN_nin);
   ...
   l_ID_TASK_GOAL_DET_n := sys.diutil.bool_to_int(ID_TASK_GOAL_DET_nin);
         ...
         UPDATE EPIS_INTERV_PLAN_HIST SET
     ID_EPIS_INTERV_PLAN = decode (l_ID_EPIS_INTERV_PLAN_n,0,id_epis_interv_plan_in, NVL (id_epis_interv_plan_in, ID_EPIS_INTERV_PLAN)),
     ...
     ID_TASK_GOAL_DET = decode (l_ID_TASK_GOAL_DET_n,0,id_task_goal_det_in, NVL (id_task_goal_det_in, ID_TASK_GOAL_DET))
          WHERE
             ID_EPIS_INTERV_PLAN_HIST = id_epis_interv_plan_hist_in
...
rows_out :=  rows_out MULTISET UNION DISTINCT l_rows_out;
   ...
   BEGIN
     upd (
      id_epis_interv_plan_hist_in => id_epis_interv_plan_hist_in,
      id_epis_interv_plan_in => id_epis_interv_plan_in,
      ID_EPIS_INTERV_PLAN_nin => ID_EPIS_INTERV_PLAN_nin,
      id_interv_plan_in => id_interv_plan_in,
...
PROCEDURE upd (
   ...
   l_UPDATE_INSTITUTION_n NUMBER(1);
   l_ID_TASK_GOAL_DET_n NUMBER(1);
      id_epis_interv_plan_hist_in EPIS_INTERV_PLAN_HIST.ID_EPIS_INTERV_PLAN_HIST%TYPE;
   BEGIN
...
l_sql := 'UPDATE EPIS_INTERV_PLAN_HIST SET '
     || ' ID_EPIS_INTERV_PLAN = decode (' || l_ID_EPIS_INTERV_PLAN_n || ',0,:id_epis_interv_plan_in, NVL (:id_epis_interv_plan_in, ID_EPIS_INTERV_PLAN)) '|| ','
     || ' ID_INTERV_PLAN = decode (' || l_ID_INTERV_PLAN_n || ',0,:id_interv_plan_in, NVL (:id_interv_plan_in, ID_INTERV_PLAN)) '|| ','
     || ' ID_EPISODE = decode (' || l_ID_EPISODE_n || ',0,:id_episode_in, NVL (:id_episode_in, ID_EPISODE)) '|| ','
     || ' ID_PROFESSIONAL = decode (' || l_ID_PROFESSIONAL_n || ',0,:id_professional_in, NVL (:id_professional_in, ID_PROFESSIONAL)) '|| ','
...
PROCEDURE upd (
   ...
   BEGIN
      upd (
         id_epis_interv_plan_hist_in => rec_in.ID_EPIS_INTERV_PLAN_HIST,
   ...
   IS
      l_ID_EPIS_INTERV_PLAN_HIST ID_EPIS_INTERV_PLAN_HIST_cc;
   ...
   BEGIN
      FOR i IN col_in.FIRST .. col_in.LAST loop
         l_ID_EPIS_INTERV_PLAN_HIST(i) := col_in(i).ID_EPIS_INTERV_PLAN_HIST;
      ...
      THEN
         ...
         FORALL i IN col_in.FIRST .. col_in.LAST
            UPDATE EPIS_INTERV_PLAN_HIST SET
               ID_EPIS_INTERV_PLAN = NVL (l_ID_EPIS_INTERV_PLAN(i), ID_EPIS_INTERV_PLAN),
             ...
             WHERE
                ID_EPIS_INTERV_PLAN_HIST = l_ID_EPIS_INTERV_PLAN_HIST(i)
         ;
      ELSE
         FORALL i IN col_in.FIRST .. col_in.LAST
            UPDATE EPIS_INTERV_PLAN_HIST SET
   ...
   BEGIN
      RETURN
         'BEGIN UPDATE EPIS_INTERV_PLAN_HIST
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE EPIS_INTERV_PLAN_HIST set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE EPIS_INTERV_PLAN_HIST set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
   ...
   IS
   BEGIN
      DELETE FROM EPIS_INTERV_PLAN_HIST
       WHERE
          ID_EPIS_INTERV_PLAN_HIST = id_epis_interv_plan_hist_in
       RETURNING ROWID BULK COLLECT INTO rows_out
         ;
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM EPIS_INTERV_PLAN_HIST
       WHERE ID_EPIS_INTERV_PLAN_HIST = id_epis_interv_plan_hist_in
      RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
      ...
      THEN
        ...
        END;
        END IF;
   END del_ID_EPIS_INTERV_PLAN_HIST;
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM EPIS_INTERV_PLAN_HIST
       WHERE
          ID_EPIS_INTERV_PLAN = del_EIPH_EIH_FK.id_epis_interv_plan_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_EIPH_EIH_FK (
   ...
   BEGIN
      DELETE FROM EPIS_INTERV_PLAN_HIST
       WHERE
          ID_PROFESSIONAL = del_EIPH_P_FK.id_professional_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_EIPH_P_FK (
   ...
   BEGIN
      DELETE FROM EPIS_INTERV_PLAN_HIST
       WHERE
          ID_TASK_GOAL_DET = del_EIPH_TSKGD_FK.id_task_goal_det_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_EIPH_TSKGD_FK (
   ...
   BEGIN
      IF where_in IS NULL
      THEN
         RETURN 'DELETE FROM EPIS_INTERV_PLAN_HIST';
      ELSE
         RETURN
            'DELETE FROM EPIS_INTERV_PLAN_HIST WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   IS
   BEGIN
      epis_interv_plan_hist_inout.ID_EPIS_INTERV_PLAN_HIST := NULL;
      epis_interv_plan_hist_inout.ID_EPIS_INTERV_PLAN := NULL;
      epis_interv_plan_hist_inout.ID_INTERV_PLAN := NULL;
      epis_interv_plan_hist_inout.ID_EPISODE := NULL;
      epis_interv_plan_hist_inout.ID_PROFESSIONAL := NULL;
      epis_interv_plan_hist_inout.FLG_STATUS := NULL;
      epis_interv_plan_hist_inout.NOTES := NULL;
      epis_interv_plan_hist_inout.DT_CREATION := NULL;
      epis_interv_plan_hist_inout.DT_BEGIN := NULL;
      epis_interv_plan_hist_inout.DT_END := NULL;
      epis_interv_plan_hist_inout.ID_CANCEL_INFO_DET := NULL;
      epis_interv_plan_hist_inout.FLG_AVAILABLE := 'Y';
      epis_interv_plan_hist_inout.DESC_OTHER_INTERV_PLAN := NULL;
      epis_interv_plan_hist_inout.CREATE_USER := NULL;
      epis_interv_plan_hist_inout.CREATE_TIME := NULL;
      epis_interv_plan_hist_inout.CREATE_INSTITUTION := NULL;
      epis_interv_plan_hist_inout.UPDATE_USER := NULL;
      epis_interv_plan_hist_inout.UPDATE_TIME := NULL;
      epis_interv_plan_hist_inout.UPDATE_INSTITUTION := NULL;
      epis_interv_plan_hist_inout.ID_TASK_GOAL_DET := NULL;
   END initrec;

   FUNCTION initrec RETURN EPIS_INTERV_PLAN_HIST%ROWTYPE
   IS
      l_epis_interv_plan_hist EPIS_INTERV_PLAN_HIST%ROWTYPE;
   BEGIN
      l_epis_interv_plan_hist.FLG_AVAILABLE := 'Y';
      RETURN l_epis_interv_plan_hist;
   END initrec;
   ...
   IS
        data EPIS_INTERV_PLAN_HIST_tc;
   BEGIN
        select * bulk collect into data from EPIS_INTERV_PLAN_HIST where rowid in (select * from table(rows_in));
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
