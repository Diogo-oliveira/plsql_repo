...
CREATE OR REPLACE PACKAGE BODY TS_DOC_MACRO_PROF_HIST
/*
| Generated by or retrieved from QCGU - DO NOT MODIFY!
| QCGU - "Get it right, do it fast" - www.ToadWorld.com
| QCGU Universal ID: {657BD35E-2E48-4CD4-B14E-41420B8433B3}
| Created On: Março 30, 2012 17:1:13
| Created By: ALERT
*/
IS
      ...
      THEN
        ...
        BEGIN
           ...
           pk_alert_exceptions.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'DOC_MACRO_PROF_HIST');
           IF l_name = 'DCMPH_DCMP_FK'
           THEN
              -- Add a context value for each column
              pk_alert_exceptions.add_context (
                 err_instance_id_in => l_id
               , name_in => 'ID_DOC_MACRO_PROF'
               , value_in => id_doc_macro_prof_in);
           END IF;
   ...
   BEGIN
     ...
     ins (
      id_doc_macro_prof_in => id_doc_macro_prof_in,
   ...
   IS
      l_rec DOC_MACRO_PROF_HIST%ROWTYPE := rec_in;
   BEGIN
      ins (
         id_doc_macro_prof_in => l_rec.ID_DOC_MACRO_PROF,
   ...
   BEGIN
      ...
      ELSE
         FORALL indx IN rows_in.FIRST .. rows_in.LAST
            SAVE EXCEPTIONS
            INSERT INTO DOC_MACRO_PROF_HIST VALUES rows_in (indx) RETURNING ROWID BULK COLLECT INTO rows_out;
      END IF;
   EXCEPTION
     WHEN e_forall_error
     THEN
       ...
       ELSE
          ...
          BEGIN
             ...
             LOOP
                ...
                pk_alert_exceptions.add_context (
                   err_instance_id_in => l_err_instance_id
                  ,NAME_IN => 'ID_DOC_MACRO_PROF _' || indx
                  ,value_in => rows_in(SQL%BULK_EXCEPTIONS (indx).ERROR_INDEX).ID_DOC_MACRO_PROF
                  ,validate_in => FALSE
                );
...
PROCEDURE upd (
   ...
   l_UPDATE_INSTITUTION_n := sys.diutil.bool_to_int(UPDATE_INSTITUTION_nin);
         ...
         UPDATE DOC_MACRO_PROF_HIST SET
     ...
     UPDATE_INSTITUTION = decode (l_UPDATE_INSTITUTION_n,0,update_institution_in, NVL (update_institution_in, UPDATE_INSTITUTION))
          WHERE
             ID_DOC_MACRO_PROF = id_doc_macro_prof_in AND
...
PROCEDURE upd (
   ...
   l_UPDATE_TIME_n NUMBER(1);
   l_UPDATE_INSTITUTION_n NUMBER(1);
      id_doc_macro_prof_in DOC_MACRO_PROF_HIST.ID_DOC_MACRO_PROF%TYPE;
      dt_doc_macro_hist_in DOC_MACRO_PROF_HIST.DT_DOC_MACRO_HIST%TYPE;
   BEGIN
...
l_sql := 'UPDATE DOC_MACRO_PROF_HIST SET '
     || ' ID_DOC_MACRO = decode (' || l_ID_DOC_MACRO_n || ',0,:id_doc_macro_in, NVL (:id_doc_macro_in, ID_DOC_MACRO)) '|| ','
     || ' ID_PROFESSIONAL = decode (' || l_ID_PROFESSIONAL_n || ',0,:id_professional_in, NVL (:id_professional_in, ID_PROFESSIONAL)) '|| ','
     || ' FLG_STATUS = decode (' || l_FLG_STATUS_n || ',0,:flg_status_in, NVL (:flg_status_in, FLG_STATUS)) '|| ','
     || ' DT_CREATION = decode (' || l_DT_CREATION_n || ',0,:dt_creation_in, NVL (:dt_creation_in, DT_CREATION)) '|| ','
...
PROCEDURE upd (
   ...
   BEGIN
      upd (
         ...
         id_doc_macro_prof_in => rec_in.ID_DOC_MACRO_PROF,
   ...
   IS
      ...
      l_ID_DOC_MACRO_PROF ID_DOC_MACRO_PROF_cc;
   ...
   BEGIN
      FOR i IN col_in.FIRST .. col_in.LAST loop
         ...
         l_ID_DOC_MACRO_PROF(i) := col_in(i).ID_DOC_MACRO_PROF;
      ...
      THEN
         ...
         FORALL i IN col_in.FIRST .. col_in.LAST
            UPDATE DOC_MACRO_PROF_HIST SET
             ...
             WHERE
                ID_DOC_MACRO_PROF = l_ID_DOC_MACRO_PROF(i) AND
   ...
   BEGIN
      RETURN
         'BEGIN UPDATE DOC_MACRO_PROF_HIST
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE DOC_MACRO_PROF_HIST set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE DOC_MACRO_PROF_HIST set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
   ...
   IS
   BEGIN
      DELETE FROM DOC_MACRO_PROF_HIST
       WHERE
          ID_DOC_MACRO_PROF = id_doc_macro_prof_in AND
          DT_DOC_MACRO_HIST = dt_doc_macro_hist_in
       RETURNING ROWID BULK COLLECT INTO rows_out
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM DOC_MACRO_PROF_HIST
       WHERE ID_DOC_MACRO_PROF = id_doc_macro_prof_in
      RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM DOC_MACRO_PROF_HIST
       WHERE
          ID_DOC_MACRO_PROF = del_DCMPH_DCMP_FK.id_doc_macro_prof_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_DCMPH_DCMP_FK (
   ...
   BEGIN
      IF where_in IS NULL
      THEN
         RETURN 'DELETE FROM DOC_MACRO_PROF_HIST';
      ELSE
         RETURN
            'DELETE FROM DOC_MACRO_PROF_HIST WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   IS
   BEGIN
      doc_macro_prof_hist_inout.DT_DOC_MACRO_HIST := CURRENT_TIMESTAMP;
      doc_macro_prof_hist_inout.ID_DOC_MACRO_PROF := NULL;
      doc_macro_prof_hist_inout.ID_DOC_MACRO := NULL;
      doc_macro_prof_hist_inout.ID_PROFESSIONAL := NULL;
      doc_macro_prof_hist_inout.FLG_STATUS := NULL;
      doc_macro_prof_hist_inout.DT_CREATION := NULL;
      doc_macro_prof_hist_inout.CREATE_USER := NULL;
      doc_macro_prof_hist_inout.CREATE_TIME := NULL;
      doc_macro_prof_hist_inout.CREATE_INSTITUTION := NULL;
      doc_macro_prof_hist_inout.UPDATE_USER := NULL;
      doc_macro_prof_hist_inout.UPDATE_TIME := NULL;
      doc_macro_prof_hist_inout.UPDATE_INSTITUTION := NULL;
   END initrec;

   FUNCTION initrec RETURN DOC_MACRO_PROF_HIST%ROWTYPE
   IS
      l_doc_macro_prof_hist DOC_MACRO_PROF_HIST%ROWTYPE;
   BEGIN
      l_doc_macro_prof_hist.DT_DOC_MACRO_HIST := CURRENT_TIMESTAMP;
      RETURN l_doc_macro_prof_hist;
   END initrec;
   ...
   IS
        data DOC_MACRO_PROF_HIST_tc;
   BEGIN
        select * bulk collect into data from DOC_MACRO_PROF_HIST where rowid in (select /*+opt_estimate(table,t,scale_rows=0.0000001))*/ * from table(rows_in) t);
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
