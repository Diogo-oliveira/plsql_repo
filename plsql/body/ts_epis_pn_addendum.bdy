/*-- Last Change Revision: $Rev: 1811685 $*/
/*-- Last Change by: $Author: rui.mendonca $*/
/*-- Date of last change: $Date: 2017-12-15 16:07:18 +0000 (sex, 15 dez 2017) $*/
CREATE OR REPLACE PACKAGE BODY ts_epis_pn_addendum
/*
| Generated by or retrieved - DO NOT MODIFY!
| Created On: 2017-12-12 15:32:44
| Created By: ALERT
*/
 IS

    e_null_column_value EXCEPTION;
    PRAGMA EXCEPTION_INIT(e_null_column_value, -1400);

    e_existing_fky_reference EXCEPTION;
    PRAGMA EXCEPTION_INIT(e_existing_fky_reference, -2266);

    e_check_constraint_failure EXCEPTION;
    PRAGMA EXCEPTION_INIT(e_check_constraint_failure, -2290);

    e_no_parent_key EXCEPTION;
    PRAGMA EXCEPTION_INIT(e_no_parent_key, -2291);

    e_child_record_found EXCEPTION;
    PRAGMA EXCEPTION_INIT(e_child_record_found, -2292);

    e_forall_error EXCEPTION;
    PRAGMA EXCEPTION_INIT(e_forall_error, -24381);

    -- Defined for backward compatibilty.
    e_integ_constraint_failure EXCEPTION;
    PRAGMA EXCEPTION_INIT(e_integ_constraint_failure, -2291);

    FUNCTION next_key(sequence_in IN VARCHAR2 := NULL) RETURN epis_pn_addendum.id_epis_pn_addendum%TYPE IS
        retval epis_pn_addendum.id_epis_pn_addendum%TYPE;
    BEGIN
        IF sequence_in IS NULL
        THEN
            retval := seq_epis_pn_addendum.nextval;
        ELSE
            EXECUTE IMMEDIATE 'SELECT ' || sequence_in || '.NEXTVAL FROM dual'
                INTO retval;
        END IF;
        RETURN retval;
    END next_key;

    -- Insert one row, generating hidden primary key using a sequence
    PROCEDURE ins
    (
        id_epis_pn_in          IN epis_pn_addendum.id_epis_pn%TYPE DEFAULT NULL,
        flg_status_in          IN epis_pn_addendum.flg_status%TYPE DEFAULT 'D',
        id_professional_in     IN epis_pn_addendum.id_professional%TYPE DEFAULT NULL,
        dt_addendum_in         IN epis_pn_addendum.dt_addendum%TYPE DEFAULT NULL,
        pn_addendum_in         IN epis_pn_addendum.pn_addendum%TYPE DEFAULT NULL,
        dt_last_update_in      IN epis_pn_addendum.dt_last_update%TYPE DEFAULT NULL,
        id_prof_last_update_in IN epis_pn_addendum.id_prof_last_update%TYPE DEFAULT NULL,
        dt_signoff_in          IN epis_pn_addendum.dt_signoff%TYPE DEFAULT NULL,
        id_prof_signoff_in     IN epis_pn_addendum.id_prof_signoff%TYPE DEFAULT NULL,
        id_prof_cancel_in      IN epis_pn_addendum.id_prof_cancel%TYPE DEFAULT NULL,
        id_cancel_reason_in    IN epis_pn_addendum.id_cancel_reason%TYPE DEFAULT NULL,
        dt_cancel_in           IN epis_pn_addendum.dt_cancel%TYPE DEFAULT NULL,
        notes_cancel_in        IN epis_pn_addendum.notes_cancel%TYPE DEFAULT NULL,
        create_user_in         IN epis_pn_addendum.create_user%TYPE DEFAULT NULL,
        create_time_in         IN epis_pn_addendum.create_time%TYPE DEFAULT NULL,
        create_institution_in  IN epis_pn_addendum.create_institution%TYPE DEFAULT NULL,
        update_user_in         IN epis_pn_addendum.update_user%TYPE DEFAULT NULL,
        update_time_in         IN epis_pn_addendum.update_time%TYPE DEFAULT NULL,
        update_institution_in  IN epis_pn_addendum.update_institution%TYPE DEFAULT NULL,
        flg_type_in            IN epis_pn_addendum.flg_type%TYPE DEFAULT 'A',
        -- Pass false if you want errors to propagate out unhandled
        handle_error_in IN BOOLEAN := TRUE,
        rows_out        OUT table_varchar
    ) IS
        l_pky epis_pn_addendum.id_epis_pn_addendum%TYPE := next_key;
    BEGIN
        ins(id_epis_pn_addendum_in => l_pky,
            id_epis_pn_in          => id_epis_pn_in,
            flg_status_in          => flg_status_in,
            id_professional_in     => id_professional_in,
            dt_addendum_in         => dt_addendum_in,
            pn_addendum_in         => pn_addendum_in,
            dt_last_update_in      => dt_last_update_in,
            id_prof_last_update_in => id_prof_last_update_in,
            dt_signoff_in          => dt_signoff_in,
            id_prof_signoff_in     => id_prof_signoff_in,
            id_prof_cancel_in      => id_prof_cancel_in,
            id_cancel_reason_in    => id_cancel_reason_in,
            dt_cancel_in           => dt_cancel_in,
            notes_cancel_in        => notes_cancel_in,
            create_user_in         => create_user_in,
            create_time_in         => create_time_in,
            create_institution_in  => create_institution_in,
            update_user_in         => update_user_in,
            update_time_in         => update_time_in,
            update_institution_in  => update_institution_in,
            flg_type_in            => flg_type_in,
            handle_error_in        => handle_error_in,
            rows_out               => rows_out);
    END ins;

    -- Insert one row, generating hidden primary key using a sequence
    PROCEDURE ins
    (
        id_epis_pn_in          IN epis_pn_addendum.id_epis_pn%TYPE DEFAULT NULL,
        flg_status_in          IN epis_pn_addendum.flg_status%TYPE DEFAULT 'D',
        id_professional_in     IN epis_pn_addendum.id_professional%TYPE DEFAULT NULL,
        dt_addendum_in         IN epis_pn_addendum.dt_addendum%TYPE DEFAULT NULL,
        pn_addendum_in         IN epis_pn_addendum.pn_addendum%TYPE DEFAULT NULL,
        dt_last_update_in      IN epis_pn_addendum.dt_last_update%TYPE DEFAULT NULL,
        id_prof_last_update_in IN epis_pn_addendum.id_prof_last_update%TYPE DEFAULT NULL,
        dt_signoff_in          IN epis_pn_addendum.dt_signoff%TYPE DEFAULT NULL,
        id_prof_signoff_in     IN epis_pn_addendum.id_prof_signoff%TYPE DEFAULT NULL,
        id_prof_cancel_in      IN epis_pn_addendum.id_prof_cancel%TYPE DEFAULT NULL,
        id_cancel_reason_in    IN epis_pn_addendum.id_cancel_reason%TYPE DEFAULT NULL,
        dt_cancel_in           IN epis_pn_addendum.dt_cancel%TYPE DEFAULT NULL,
        notes_cancel_in        IN epis_pn_addendum.notes_cancel%TYPE DEFAULT NULL,
        create_user_in         IN epis_pn_addendum.create_user%TYPE DEFAULT NULL,
        create_time_in         IN epis_pn_addendum.create_time%TYPE DEFAULT NULL,
        create_institution_in  IN epis_pn_addendum.create_institution%TYPE DEFAULT NULL,
        update_user_in         IN epis_pn_addendum.update_user%TYPE DEFAULT NULL,
        update_time_in         IN epis_pn_addendum.update_time%TYPE DEFAULT NULL,
        update_institution_in  IN epis_pn_addendum.update_institution%TYPE DEFAULT NULL,
        flg_type_in            IN epis_pn_addendum.flg_type%TYPE DEFAULT 'A',
        -- Pass false if you want errors to propagate out unhandled
        handle_error_in IN BOOLEAN := TRUE
    ) IS
        rows_out table_varchar;
    BEGIN
        ins(id_epis_pn_in          => id_epis_pn_in,
            flg_status_in          => flg_status_in,
            id_professional_in     => id_professional_in,
            dt_addendum_in         => dt_addendum_in,
            pn_addendum_in         => pn_addendum_in,
            dt_last_update_in      => dt_last_update_in,
            id_prof_last_update_in => id_prof_last_update_in,
            dt_signoff_in          => dt_signoff_in,
            id_prof_signoff_in     => id_prof_signoff_in,
            id_prof_cancel_in      => id_prof_cancel_in,
            id_cancel_reason_in    => id_cancel_reason_in,
            dt_cancel_in           => dt_cancel_in,
            notes_cancel_in        => notes_cancel_in,
            create_user_in         => create_user_in,
            create_time_in         => create_time_in,
            create_institution_in  => create_institution_in,
            update_user_in         => update_user_in,
            update_time_in         => update_time_in,
            update_institution_in  => update_institution_in,
            flg_type_in            => flg_type_in,
            handle_error_in        => handle_error_in,
            rows_out               => rows_out);
    END ins;

    -- Insert one row, returning primary key generated by sequence
    PROCEDURE ins
    (
        id_epis_pn_in           IN epis_pn_addendum.id_epis_pn%TYPE DEFAULT NULL,
        flg_status_in           IN epis_pn_addendum.flg_status%TYPE DEFAULT 'D',
        id_professional_in      IN epis_pn_addendum.id_professional%TYPE DEFAULT NULL,
        dt_addendum_in          IN epis_pn_addendum.dt_addendum%TYPE DEFAULT NULL,
        pn_addendum_in          IN epis_pn_addendum.pn_addendum%TYPE DEFAULT NULL,
        dt_last_update_in       IN epis_pn_addendum.dt_last_update%TYPE DEFAULT NULL,
        id_prof_last_update_in  IN epis_pn_addendum.id_prof_last_update%TYPE DEFAULT NULL,
        dt_signoff_in           IN epis_pn_addendum.dt_signoff%TYPE DEFAULT NULL,
        id_prof_signoff_in      IN epis_pn_addendum.id_prof_signoff%TYPE DEFAULT NULL,
        id_prof_cancel_in       IN epis_pn_addendum.id_prof_cancel%TYPE DEFAULT NULL,
        id_cancel_reason_in     IN epis_pn_addendum.id_cancel_reason%TYPE DEFAULT NULL,
        dt_cancel_in            IN epis_pn_addendum.dt_cancel%TYPE DEFAULT NULL,
        notes_cancel_in         IN epis_pn_addendum.notes_cancel%TYPE DEFAULT NULL,
        create_user_in          IN epis_pn_addendum.create_user%TYPE DEFAULT NULL,
        create_time_in          IN epis_pn_addendum.create_time%TYPE DEFAULT NULL,
        create_institution_in   IN epis_pn_addendum.create_institution%TYPE DEFAULT NULL,
        update_user_in          IN epis_pn_addendum.update_user%TYPE DEFAULT NULL,
        update_time_in          IN epis_pn_addendum.update_time%TYPE DEFAULT NULL,
        update_institution_in   IN epis_pn_addendum.update_institution%TYPE DEFAULT NULL,
        flg_type_in             IN epis_pn_addendum.flg_type%TYPE DEFAULT 'A',
        id_epis_pn_addendum_out IN OUT epis_pn_addendum.id_epis_pn_addendum%TYPE,
        -- Pass false if you want errors to propagate out unhandled
        handle_error_in IN BOOLEAN := TRUE,
        rows_out        OUT table_varchar
    ) IS
        l_pky epis_pn_addendum.id_epis_pn_addendum%TYPE := next_key;
    BEGIN
        ins(id_epis_pn_addendum_in => l_pky,
            id_epis_pn_in          => id_epis_pn_in,
            flg_status_in          => flg_status_in,
            id_professional_in     => id_professional_in,
            dt_addendum_in         => dt_addendum_in,
            pn_addendum_in         => pn_addendum_in,
            dt_last_update_in      => dt_last_update_in,
            id_prof_last_update_in => id_prof_last_update_in,
            dt_signoff_in          => dt_signoff_in,
            id_prof_signoff_in     => id_prof_signoff_in,
            id_prof_cancel_in      => id_prof_cancel_in,
            id_cancel_reason_in    => id_cancel_reason_in,
            dt_cancel_in           => dt_cancel_in,
            notes_cancel_in        => notes_cancel_in,
            create_user_in         => create_user_in,
            create_time_in         => create_time_in,
            create_institution_in  => create_institution_in,
            update_user_in         => update_user_in,
            update_time_in         => update_time_in,
            update_institution_in  => update_institution_in,
            flg_type_in            => flg_type_in,
            handle_error_in        => handle_error_in,
            rows_out               => rows_out);
        id_epis_pn_addendum_out := l_pky;
    END ins;

    -- Insert one row, returning primary key generated by sequence
    PROCEDURE ins
    (
        id_epis_pn_in           IN epis_pn_addendum.id_epis_pn%TYPE DEFAULT NULL,
        flg_status_in           IN epis_pn_addendum.flg_status%TYPE DEFAULT 'D',
        id_professional_in      IN epis_pn_addendum.id_professional%TYPE DEFAULT NULL,
        dt_addendum_in          IN epis_pn_addendum.dt_addendum%TYPE DEFAULT NULL,
        pn_addendum_in          IN epis_pn_addendum.pn_addendum%TYPE DEFAULT NULL,
        dt_last_update_in       IN epis_pn_addendum.dt_last_update%TYPE DEFAULT NULL,
        id_prof_last_update_in  IN epis_pn_addendum.id_prof_last_update%TYPE DEFAULT NULL,
        dt_signoff_in           IN epis_pn_addendum.dt_signoff%TYPE DEFAULT NULL,
        id_prof_signoff_in      IN epis_pn_addendum.id_prof_signoff%TYPE DEFAULT NULL,
        id_prof_cancel_in       IN epis_pn_addendum.id_prof_cancel%TYPE DEFAULT NULL,
        id_cancel_reason_in     IN epis_pn_addendum.id_cancel_reason%TYPE DEFAULT NULL,
        dt_cancel_in            IN epis_pn_addendum.dt_cancel%TYPE DEFAULT NULL,
        notes_cancel_in         IN epis_pn_addendum.notes_cancel%TYPE DEFAULT NULL,
        create_user_in          IN epis_pn_addendum.create_user%TYPE DEFAULT NULL,
        create_time_in          IN epis_pn_addendum.create_time%TYPE DEFAULT NULL,
        create_institution_in   IN epis_pn_addendum.create_institution%TYPE DEFAULT NULL,
        update_user_in          IN epis_pn_addendum.update_user%TYPE DEFAULT NULL,
        update_time_in          IN epis_pn_addendum.update_time%TYPE DEFAULT NULL,
        update_institution_in   IN epis_pn_addendum.update_institution%TYPE DEFAULT NULL,
        flg_type_in             IN epis_pn_addendum.flg_type%TYPE DEFAULT 'A',
        id_epis_pn_addendum_out IN OUT epis_pn_addendum.id_epis_pn_addendum%TYPE,
        -- Pass false if you want errors to propagate out unhandled
        handle_error_in IN BOOLEAN := TRUE
    ) IS
        rows_out table_varchar;
    BEGIN
        ins(id_epis_pn_in           => id_epis_pn_in,
            flg_status_in           => flg_status_in,
            id_professional_in      => id_professional_in,
            dt_addendum_in          => dt_addendum_in,
            pn_addendum_in          => pn_addendum_in,
            dt_last_update_in       => dt_last_update_in,
            id_prof_last_update_in  => id_prof_last_update_in,
            dt_signoff_in           => dt_signoff_in,
            id_prof_signoff_in      => id_prof_signoff_in,
            id_prof_cancel_in       => id_prof_cancel_in,
            id_cancel_reason_in     => id_cancel_reason_in,
            dt_cancel_in            => dt_cancel_in,
            notes_cancel_in         => notes_cancel_in,
            create_user_in          => create_user_in,
            create_time_in          => create_time_in,
            create_institution_in   => create_institution_in,
            update_user_in          => update_user_in,
            update_time_in          => update_time_in,
            update_institution_in   => update_institution_in,
            flg_type_in             => flg_type_in,
            id_epis_pn_addendum_out => id_epis_pn_addendum_out,
            handle_error_in         => handle_error_in,
            rows_out                => rows_out);
    END ins;

    -- Insert one row with function, return generated primary key
    FUNCTION ins
    (
        id_epis_pn_in          IN epis_pn_addendum.id_epis_pn%TYPE DEFAULT NULL,
        flg_status_in          IN epis_pn_addendum.flg_status%TYPE DEFAULT 'D',
        id_professional_in     IN epis_pn_addendum.id_professional%TYPE DEFAULT NULL,
        dt_addendum_in         IN epis_pn_addendum.dt_addendum%TYPE DEFAULT NULL,
        pn_addendum_in         IN epis_pn_addendum.pn_addendum%TYPE DEFAULT NULL,
        dt_last_update_in      IN epis_pn_addendum.dt_last_update%TYPE DEFAULT NULL,
        id_prof_last_update_in IN epis_pn_addendum.id_prof_last_update%TYPE DEFAULT NULL,
        dt_signoff_in          IN epis_pn_addendum.dt_signoff%TYPE DEFAULT NULL,
        id_prof_signoff_in     IN epis_pn_addendum.id_prof_signoff%TYPE DEFAULT NULL,
        id_prof_cancel_in      IN epis_pn_addendum.id_prof_cancel%TYPE DEFAULT NULL,
        id_cancel_reason_in    IN epis_pn_addendum.id_cancel_reason%TYPE DEFAULT NULL,
        dt_cancel_in           IN epis_pn_addendum.dt_cancel%TYPE DEFAULT NULL,
        notes_cancel_in        IN epis_pn_addendum.notes_cancel%TYPE DEFAULT NULL,
        create_user_in         IN epis_pn_addendum.create_user%TYPE DEFAULT NULL,
        create_time_in         IN epis_pn_addendum.create_time%TYPE DEFAULT NULL,
        create_institution_in  IN epis_pn_addendum.create_institution%TYPE DEFAULT NULL,
        update_user_in         IN epis_pn_addendum.update_user%TYPE DEFAULT NULL,
        update_time_in         IN epis_pn_addendum.update_time%TYPE DEFAULT NULL,
        update_institution_in  IN epis_pn_addendum.update_institution%TYPE DEFAULT NULL,
        flg_type_in            IN epis_pn_addendum.flg_type%TYPE DEFAULT 'A',
        -- Pass false if you want errors to propagate out unhandled
        handle_error_in IN BOOLEAN := TRUE,
        rows_out        OUT table_varchar
    ) RETURN epis_pn_addendum.id_epis_pn_addendum%TYPE IS
        l_pky epis_pn_addendum.id_epis_pn_addendum%TYPE := next_key;
    BEGIN
        ins(id_epis_pn_addendum_in => l_pky,
            id_epis_pn_in          => id_epis_pn_in,
            flg_status_in          => flg_status_in,
            id_professional_in     => id_professional_in,
            dt_addendum_in         => dt_addendum_in,
            pn_addendum_in         => pn_addendum_in,
            dt_last_update_in      => dt_last_update_in,
            id_prof_last_update_in => id_prof_last_update_in,
            dt_signoff_in          => dt_signoff_in,
            id_prof_signoff_in     => id_prof_signoff_in,
            id_prof_cancel_in      => id_prof_cancel_in,
            id_cancel_reason_in    => id_cancel_reason_in,
            dt_cancel_in           => dt_cancel_in,
            notes_cancel_in        => notes_cancel_in,
            create_user_in         => create_user_in,
            create_time_in         => create_time_in,
            create_institution_in  => create_institution_in,
            update_user_in         => update_user_in,
            update_time_in         => update_time_in,
            update_institution_in  => update_institution_in,
            flg_type_in            => flg_type_in,
            handle_error_in        => handle_error_in,
            rows_out               => rows_out);
        RETURN l_pky;
    END ins;

    -- Insert one row with function, return generated primary key
    FUNCTION ins
    (
        id_epis_pn_in          IN epis_pn_addendum.id_epis_pn%TYPE DEFAULT NULL,
        flg_status_in          IN epis_pn_addendum.flg_status%TYPE DEFAULT 'D',
        id_professional_in     IN epis_pn_addendum.id_professional%TYPE DEFAULT NULL,
        dt_addendum_in         IN epis_pn_addendum.dt_addendum%TYPE DEFAULT NULL,
        pn_addendum_in         IN epis_pn_addendum.pn_addendum%TYPE DEFAULT NULL,
        dt_last_update_in      IN epis_pn_addendum.dt_last_update%TYPE DEFAULT NULL,
        id_prof_last_update_in IN epis_pn_addendum.id_prof_last_update%TYPE DEFAULT NULL,
        dt_signoff_in          IN epis_pn_addendum.dt_signoff%TYPE DEFAULT NULL,
        id_prof_signoff_in     IN epis_pn_addendum.id_prof_signoff%TYPE DEFAULT NULL,
        id_prof_cancel_in      IN epis_pn_addendum.id_prof_cancel%TYPE DEFAULT NULL,
        id_cancel_reason_in    IN epis_pn_addendum.id_cancel_reason%TYPE DEFAULT NULL,
        dt_cancel_in           IN epis_pn_addendum.dt_cancel%TYPE DEFAULT NULL,
        notes_cancel_in        IN epis_pn_addendum.notes_cancel%TYPE DEFAULT NULL,
        create_user_in         IN epis_pn_addendum.create_user%TYPE DEFAULT NULL,
        create_time_in         IN epis_pn_addendum.create_time%TYPE DEFAULT NULL,
        create_institution_in  IN epis_pn_addendum.create_institution%TYPE DEFAULT NULL,
        update_user_in         IN epis_pn_addendum.update_user%TYPE DEFAULT NULL,
        update_time_in         IN epis_pn_addendum.update_time%TYPE DEFAULT NULL,
        update_institution_in  IN epis_pn_addendum.update_institution%TYPE DEFAULT NULL,
        flg_type_in            IN epis_pn_addendum.flg_type%TYPE DEFAULT 'A',
        -- Pass false if you want errors to propagate out unhandled
        handle_error_in IN BOOLEAN := TRUE
    ) RETURN epis_pn_addendum.id_epis_pn_addendum%TYPE IS
        l_pky    epis_pn_addendum.id_epis_pn_addendum%TYPE := next_key;
        rows_out table_varchar;
    BEGIN
        ins(id_epis_pn_addendum_in => l_pky,
            id_epis_pn_in          => id_epis_pn_in,
            flg_status_in          => flg_status_in,
            id_professional_in     => id_professional_in,
            dt_addendum_in         => dt_addendum_in,
            pn_addendum_in         => pn_addendum_in,
            dt_last_update_in      => dt_last_update_in,
            id_prof_last_update_in => id_prof_last_update_in,
            dt_signoff_in          => dt_signoff_in,
            id_prof_signoff_in     => id_prof_signoff_in,
            id_prof_cancel_in      => id_prof_cancel_in,
            id_cancel_reason_in    => id_cancel_reason_in,
            dt_cancel_in           => dt_cancel_in,
            notes_cancel_in        => notes_cancel_in,
            create_user_in         => create_user_in,
            create_time_in         => create_time_in,
            create_institution_in  => create_institution_in,
            update_user_in         => update_user_in,
            update_time_in         => update_time_in,
            update_institution_in  => update_institution_in,
            flg_type_in            => flg_type_in,
            handle_error_in        => handle_error_in,
            rows_out               => rows_out);
        RETURN l_pky;
    END ins;

    PROCEDURE ins
    (
        id_epis_pn_addendum_in IN epis_pn_addendum.id_epis_pn_addendum%TYPE,
        id_epis_pn_in          IN epis_pn_addendum.id_epis_pn%TYPE DEFAULT NULL,
        flg_status_in          IN epis_pn_addendum.flg_status%TYPE DEFAULT 'D',
        id_professional_in     IN epis_pn_addendum.id_professional%TYPE DEFAULT NULL,
        dt_addendum_in         IN epis_pn_addendum.dt_addendum%TYPE DEFAULT NULL,
        pn_addendum_in         IN epis_pn_addendum.pn_addendum%TYPE DEFAULT NULL,
        dt_last_update_in      IN epis_pn_addendum.dt_last_update%TYPE DEFAULT NULL,
        id_prof_last_update_in IN epis_pn_addendum.id_prof_last_update%TYPE DEFAULT NULL,
        dt_signoff_in          IN epis_pn_addendum.dt_signoff%TYPE DEFAULT NULL,
        id_prof_signoff_in     IN epis_pn_addendum.id_prof_signoff%TYPE DEFAULT NULL,
        id_prof_cancel_in      IN epis_pn_addendum.id_prof_cancel%TYPE DEFAULT NULL,
        id_cancel_reason_in    IN epis_pn_addendum.id_cancel_reason%TYPE DEFAULT NULL,
        dt_cancel_in           IN epis_pn_addendum.dt_cancel%TYPE DEFAULT NULL,
        notes_cancel_in        IN epis_pn_addendum.notes_cancel%TYPE DEFAULT NULL,
        create_user_in         IN epis_pn_addendum.create_user%TYPE DEFAULT NULL,
        create_time_in         IN epis_pn_addendum.create_time%TYPE DEFAULT NULL,
        create_institution_in  IN epis_pn_addendum.create_institution%TYPE DEFAULT NULL,
        update_user_in         IN epis_pn_addendum.update_user%TYPE DEFAULT NULL,
        update_time_in         IN epis_pn_addendum.update_time%TYPE DEFAULT NULL,
        update_institution_in  IN epis_pn_addendum.update_institution%TYPE DEFAULT NULL,
        flg_type_in            IN epis_pn_addendum.flg_type%TYPE DEFAULT 'A',
        handle_error_in        IN BOOLEAN := TRUE,
        rows_out               OUT table_varchar
    ) IS
    BEGIN
        INSERT INTO epis_pn_addendum
            (id_epis_pn_addendum,
             id_epis_pn,
             flg_status,
             id_professional,
             dt_addendum,
             pn_addendum,
             dt_last_update,
             id_prof_last_update,
             dt_signoff,
             id_prof_signoff,
             id_prof_cancel,
             id_cancel_reason,
             dt_cancel,
             notes_cancel,
             create_user,
             create_time,
             create_institution,
             update_user,
             update_time,
             update_institution,
             flg_type)
        VALUES
            (id_epis_pn_addendum_in,
             id_epis_pn_in,
             flg_status_in,
             id_professional_in,
             dt_addendum_in,
             pn_addendum_in,
             dt_last_update_in,
             id_prof_last_update_in,
             dt_signoff_in,
             id_prof_signoff_in,
             id_prof_cancel_in,
             id_cancel_reason_in,
             dt_cancel_in,
             notes_cancel_in,
             create_user_in,
             create_time_in,
             create_institution_in,
             update_user_in,
             update_time_in,
             update_institution_in,
             flg_type_in)
        RETURNING ROWID BULK COLLECT INTO rows_out;
    
    EXCEPTION
        WHEN e_null_column_value
             OR e_existing_fky_reference
             OR e_check_constraint_failure
             OR e_no_parent_key
             OR e_child_record_found
             OR e_forall_error
             OR e_integ_constraint_failure THEN
            IF NOT handle_error_in
            THEN
                RAISE;
            ELSE
                pk_alertlog.log_info('DML error ignored: ' || SQLCODE || ': ' || SQLERRM);
            END IF;
        
    END ins;

    PROCEDURE ins
    (
        id_epis_pn_addendum_in IN epis_pn_addendum.id_epis_pn_addendum%TYPE,
        id_epis_pn_in          IN epis_pn_addendum.id_epis_pn%TYPE DEFAULT NULL,
        flg_status_in          IN epis_pn_addendum.flg_status%TYPE DEFAULT 'D',
        id_professional_in     IN epis_pn_addendum.id_professional%TYPE DEFAULT NULL,
        dt_addendum_in         IN epis_pn_addendum.dt_addendum%TYPE DEFAULT NULL,
        pn_addendum_in         IN epis_pn_addendum.pn_addendum%TYPE DEFAULT NULL,
        dt_last_update_in      IN epis_pn_addendum.dt_last_update%TYPE DEFAULT NULL,
        id_prof_last_update_in IN epis_pn_addendum.id_prof_last_update%TYPE DEFAULT NULL,
        dt_signoff_in          IN epis_pn_addendum.dt_signoff%TYPE DEFAULT NULL,
        id_prof_signoff_in     IN epis_pn_addendum.id_prof_signoff%TYPE DEFAULT NULL,
        id_prof_cancel_in      IN epis_pn_addendum.id_prof_cancel%TYPE DEFAULT NULL,
        id_cancel_reason_in    IN epis_pn_addendum.id_cancel_reason%TYPE DEFAULT NULL,
        dt_cancel_in           IN epis_pn_addendum.dt_cancel%TYPE DEFAULT NULL,
        notes_cancel_in        IN epis_pn_addendum.notes_cancel%TYPE DEFAULT NULL,
        create_user_in         IN epis_pn_addendum.create_user%TYPE DEFAULT NULL,
        create_time_in         IN epis_pn_addendum.create_time%TYPE DEFAULT NULL,
        create_institution_in  IN epis_pn_addendum.create_institution%TYPE DEFAULT NULL,
        update_user_in         IN epis_pn_addendum.update_user%TYPE DEFAULT NULL,
        update_time_in         IN epis_pn_addendum.update_time%TYPE DEFAULT NULL,
        update_institution_in  IN epis_pn_addendum.update_institution%TYPE DEFAULT NULL,
        flg_type_in            IN epis_pn_addendum.flg_type%TYPE DEFAULT 'A',
        handle_error_in        IN BOOLEAN := TRUE
    ) IS
        rows_out table_varchar;
    BEGIN
        ins(id_epis_pn_addendum_in => id_epis_pn_addendum_in,
            id_epis_pn_in          => id_epis_pn_in,
            flg_status_in          => flg_status_in,
            id_professional_in     => id_professional_in,
            dt_addendum_in         => dt_addendum_in,
            pn_addendum_in         => pn_addendum_in,
            dt_last_update_in      => dt_last_update_in,
            id_prof_last_update_in => id_prof_last_update_in,
            dt_signoff_in          => dt_signoff_in,
            id_prof_signoff_in     => id_prof_signoff_in,
            id_prof_cancel_in      => id_prof_cancel_in,
            id_cancel_reason_in    => id_cancel_reason_in,
            dt_cancel_in           => dt_cancel_in,
            notes_cancel_in        => notes_cancel_in,
            create_user_in         => create_user_in,
            create_time_in         => create_time_in,
            create_institution_in  => create_institution_in,
            update_user_in         => update_user_in,
            update_time_in         => update_time_in,
            update_institution_in  => update_institution_in,
            flg_type_in            => flg_type_in,
            handle_error_in        => handle_error_in,
            rows_out               => rows_out);
    END ins;

    PROCEDURE ins
    (
        rec_in          IN epis_pn_addendum%ROWTYPE,
        gen_pky_in      IN BOOLEAN DEFAULT FALSE,
        sequence_in     IN VARCHAR2 := NULL,
        handle_error_in IN BOOLEAN := TRUE,
        rows_out        OUT table_varchar
    ) IS
        l_rec epis_pn_addendum%ROWTYPE := rec_in;
    BEGIN
        IF gen_pky_in
        THEN
            l_rec.id_epis_pn_addendum := next_key(sequence_in);
        END IF;
    
        ins(id_epis_pn_addendum_in => l_rec.id_epis_pn_addendum,
            id_epis_pn_in          => l_rec.id_epis_pn,
            flg_status_in          => l_rec.flg_status,
            id_professional_in     => l_rec.id_professional,
            dt_addendum_in         => l_rec.dt_addendum,
            pn_addendum_in         => l_rec.pn_addendum,
            dt_last_update_in      => l_rec.dt_last_update,
            id_prof_last_update_in => l_rec.id_prof_last_update,
            dt_signoff_in          => l_rec.dt_signoff,
            id_prof_signoff_in     => l_rec.id_prof_signoff,
            id_prof_cancel_in      => l_rec.id_prof_cancel,
            id_cancel_reason_in    => l_rec.id_cancel_reason,
            dt_cancel_in           => l_rec.dt_cancel,
            notes_cancel_in        => l_rec.notes_cancel,
            create_user_in         => l_rec.create_user,
            create_time_in         => l_rec.create_time,
            create_institution_in  => l_rec.create_institution,
            update_user_in         => l_rec.update_user,
            update_time_in         => l_rec.update_time,
            update_institution_in  => l_rec.update_institution,
            flg_type_in            => l_rec.flg_type,
            handle_error_in        => handle_error_in,
            rows_out               => rows_out);
    END ins;

    PROCEDURE ins
    (
        rec_in          IN epis_pn_addendum%ROWTYPE,
        gen_pky_in      IN BOOLEAN DEFAULT FALSE,
        sequence_in     IN VARCHAR2 := NULL,
        handle_error_in IN BOOLEAN := TRUE
    ) IS
        rows_out table_varchar;
    BEGIN
        ins(rec_in          => rec_in,
            gen_pky_in      => gen_pky_in,
            sequence_in     => sequence_in,
            handle_error_in => handle_error_in,
            rows_out        => rows_out);
    END ins;

    -- Insert a collection of rows using FORALL; all primary key values
    -- must have already been generated, or are handled in triggers
    PROCEDURE ins
    (
        rows_in         IN epis_pn_addendum_tc,
        handle_error_in IN BOOLEAN := TRUE,
        rows_out        OUT table_varchar
    ) IS
    BEGIN
        IF rows_in.count = 0
        THEN
            NULL;
        ELSE
            FORALL indx IN rows_in.first .. rows_in.last SAVE EXCEPTIONS
                INSERT INTO epis_pn_addendum
                VALUES rows_in
                    (indx)
                RETURNING ROWID BULK COLLECT INTO rows_out;
        END IF;
    
    EXCEPTION
        WHEN e_forall_error THEN
            IF NOT handle_error_in
            THEN
                RAISE;
            ELSE
                FOR indx IN 1 .. SQL%bulk_exceptions.count
                LOOP
                    pk_alertlog.log_info('DML error ignored: ' || SQLCODE || ': ' || SQLERRM);
                END LOOP;
            END IF;
        WHEN OTHERS THEN
            IF NOT handle_error_in
            THEN
                RAISE;
            ELSE
                pk_alertlog.log_info('DML error ignored: ' || SQLCODE || ': ' || SQLERRM);
            END IF;
        
    END ins;

    -- Insert a collection of rows using FORALL; all primary key values
    -- must have already been generated, or are handled in triggers
    PROCEDURE ins
    (
        rows_in         IN epis_pn_addendum_tc,
        handle_error_in IN BOOLEAN := TRUE
    ) IS
        rows_out table_varchar;
    BEGIN
        ins(rows_in => rows_in, handle_error_in => handle_error_in, rows_out => rows_out);
    END ins;

    -- Update any/all columns by primary key. If you pass NULL, then
    -- the current column value is set to itself. If you need a more
    -- selected UPDATE then use one of the onecol procedures below.
    PROCEDURE upd
    (
        id_epis_pn_addendum_in  IN epis_pn_addendum.id_epis_pn_addendum%TYPE,
        id_epis_pn_in           IN epis_pn_addendum.id_epis_pn%TYPE DEFAULT NULL,
        id_epis_pn_nin          IN BOOLEAN := TRUE,
        flg_status_in           IN epis_pn_addendum.flg_status%TYPE DEFAULT NULL,
        flg_status_nin          IN BOOLEAN := TRUE,
        id_professional_in      IN epis_pn_addendum.id_professional%TYPE DEFAULT NULL,
        id_professional_nin     IN BOOLEAN := TRUE,
        dt_addendum_in          IN epis_pn_addendum.dt_addendum%TYPE DEFAULT NULL,
        dt_addendum_nin         IN BOOLEAN := TRUE,
        pn_addendum_in          IN epis_pn_addendum.pn_addendum%TYPE DEFAULT NULL,
        pn_addendum_nin         IN BOOLEAN := TRUE,
        dt_last_update_in       IN epis_pn_addendum.dt_last_update%TYPE DEFAULT NULL,
        dt_last_update_nin      IN BOOLEAN := TRUE,
        id_prof_last_update_in  IN epis_pn_addendum.id_prof_last_update%TYPE DEFAULT NULL,
        id_prof_last_update_nin IN BOOLEAN := TRUE,
        dt_signoff_in           IN epis_pn_addendum.dt_signoff%TYPE DEFAULT NULL,
        dt_signoff_nin          IN BOOLEAN := TRUE,
        id_prof_signoff_in      IN epis_pn_addendum.id_prof_signoff%TYPE DEFAULT NULL,
        id_prof_signoff_nin     IN BOOLEAN := TRUE,
        id_prof_cancel_in       IN epis_pn_addendum.id_prof_cancel%TYPE DEFAULT NULL,
        id_prof_cancel_nin      IN BOOLEAN := TRUE,
        id_cancel_reason_in     IN epis_pn_addendum.id_cancel_reason%TYPE DEFAULT NULL,
        id_cancel_reason_nin    IN BOOLEAN := TRUE,
        dt_cancel_in            IN epis_pn_addendum.dt_cancel%TYPE DEFAULT NULL,
        dt_cancel_nin           IN BOOLEAN := TRUE,
        notes_cancel_in         IN epis_pn_addendum.notes_cancel%TYPE DEFAULT NULL,
        notes_cancel_nin        IN BOOLEAN := TRUE,
        create_user_in          IN epis_pn_addendum.create_user%TYPE DEFAULT NULL,
        create_user_nin         IN BOOLEAN := TRUE,
        create_time_in          IN epis_pn_addendum.create_time%TYPE DEFAULT NULL,
        create_time_nin         IN BOOLEAN := TRUE,
        create_institution_in   IN epis_pn_addendum.create_institution%TYPE DEFAULT NULL,
        create_institution_nin  IN BOOLEAN := TRUE,
        update_user_in          IN epis_pn_addendum.update_user%TYPE DEFAULT NULL,
        update_user_nin         IN BOOLEAN := TRUE,
        update_time_in          IN epis_pn_addendum.update_time%TYPE DEFAULT NULL,
        update_time_nin         IN BOOLEAN := TRUE,
        update_institution_in   IN epis_pn_addendum.update_institution%TYPE DEFAULT NULL,
        update_institution_nin  IN BOOLEAN := TRUE,
        flg_type_in             IN epis_pn_addendum.flg_type%TYPE DEFAULT NULL,
        flg_type_nin            IN BOOLEAN := TRUE,
        handle_error_in         IN BOOLEAN := TRUE,
        rows_out                IN OUT table_varchar
    ) IS
        l_rows_out              table_varchar;
        l_id_epis_pn_n          NUMBER(1) := sys.diutil.bool_to_int(id_epis_pn_nin);
        l_flg_status_n          NUMBER(1) := sys.diutil.bool_to_int(flg_status_nin);
        l_id_professional_n     NUMBER(1) := sys.diutil.bool_to_int(id_professional_nin);
        l_dt_addendum_n         NUMBER(1) := sys.diutil.bool_to_int(dt_addendum_nin);
        l_pn_addendum_n         NUMBER(1) := sys.diutil.bool_to_int(pn_addendum_nin);
        l_dt_last_update_n      NUMBER(1) := sys.diutil.bool_to_int(dt_last_update_nin);
        l_id_prof_last_update_n NUMBER(1) := sys.diutil.bool_to_int(id_prof_last_update_nin);
        l_dt_signoff_n          NUMBER(1) := sys.diutil.bool_to_int(dt_signoff_nin);
        l_id_prof_signoff_n     NUMBER(1) := sys.diutil.bool_to_int(id_prof_signoff_nin);
        l_id_prof_cancel_n      NUMBER(1) := sys.diutil.bool_to_int(id_prof_cancel_nin);
        l_id_cancel_reason_n    NUMBER(1) := sys.diutil.bool_to_int(id_cancel_reason_nin);
        l_dt_cancel_n           NUMBER(1) := sys.diutil.bool_to_int(dt_cancel_nin);
        l_notes_cancel_n        NUMBER(1) := sys.diutil.bool_to_int(notes_cancel_nin);
        l_create_user_n         NUMBER(1) := sys.diutil.bool_to_int(create_user_nin);
        l_create_time_n         NUMBER(1) := sys.diutil.bool_to_int(create_time_nin);
        l_create_institution_n  NUMBER(1) := sys.diutil.bool_to_int(create_institution_nin);
        l_update_user_n         NUMBER(1) := sys.diutil.bool_to_int(update_user_nin);
        l_update_time_n         NUMBER(1) := sys.diutil.bool_to_int(update_time_nin);
        l_update_institution_n  NUMBER(1) := sys.diutil.bool_to_int(update_institution_nin);
        l_flg_type_n            NUMBER(1) := sys.diutil.bool_to_int(flg_type_nin);
    BEGIN
    
        UPDATE epis_pn_addendum
           SET id_epis_pn          = decode(l_id_epis_pn_n, 0, id_epis_pn_in, nvl(id_epis_pn_in, id_epis_pn)),
               flg_status          = decode(l_flg_status_n, 0, flg_status_in, nvl(flg_status_in, flg_status)),
               id_professional     = decode(l_id_professional_n,
                                            0,
                                            id_professional_in,
                                            nvl(id_professional_in, id_professional)),
               dt_addendum         = decode(l_dt_addendum_n, 0, dt_addendum_in, nvl(dt_addendum_in, dt_addendum)),
               pn_addendum         = decode(l_pn_addendum_n, 0, pn_addendum_in, nvl(pn_addendum_in, pn_addendum)),
               dt_last_update      = decode(l_dt_last_update_n,
                                            0,
                                            dt_last_update_in,
                                            nvl(dt_last_update_in, dt_last_update)),
               id_prof_last_update = decode(l_id_prof_last_update_n,
                                            0,
                                            id_prof_last_update_in,
                                            nvl(id_prof_last_update_in, id_prof_last_update)),
               dt_signoff          = decode(l_dt_signoff_n, 0, dt_signoff_in, nvl(dt_signoff_in, dt_signoff)),
               id_prof_signoff     = decode(l_id_prof_signoff_n,
                                            0,
                                            id_prof_signoff_in,
                                            nvl(id_prof_signoff_in, id_prof_signoff)),
               id_prof_cancel      = decode(l_id_prof_cancel_n,
                                            0,
                                            id_prof_cancel_in,
                                            nvl(id_prof_cancel_in, id_prof_cancel)),
               id_cancel_reason    = decode(l_id_cancel_reason_n,
                                            0,
                                            id_cancel_reason_in,
                                            nvl(id_cancel_reason_in, id_cancel_reason)),
               dt_cancel           = decode(l_dt_cancel_n, 0, dt_cancel_in, nvl(dt_cancel_in, dt_cancel)),
               notes_cancel        = decode(l_notes_cancel_n, 0, notes_cancel_in, nvl(notes_cancel_in, notes_cancel)),
               create_user         = decode(l_create_user_n, 0, create_user_in, nvl(create_user_in, create_user)),
               create_time         = decode(l_create_time_n, 0, create_time_in, nvl(create_time_in, create_time)),
               create_institution  = decode(l_create_institution_n,
                                            0,
                                            create_institution_in,
                                            nvl(create_institution_in, create_institution)),
               update_user         = decode(l_update_user_n, 0, update_user_in, nvl(update_user_in, update_user)),
               update_time         = decode(l_update_time_n, 0, update_time_in, nvl(update_time_in, update_time)),
               update_institution  = decode(l_update_institution_n,
                                            0,
                                            update_institution_in,
                                            nvl(update_institution_in, update_institution)),
               flg_type            = decode(l_flg_type_n, 0, flg_type_in, nvl(flg_type_in, flg_type))
         WHERE id_epis_pn_addendum = id_epis_pn_addendum_in
        RETURNING ROWID BULK COLLECT INTO l_rows_out;
        IF (rows_out IS NULL)
        THEN
            rows_out := table_varchar();
        END IF;
        rows_out := rows_out MULTISET UNION DISTINCT l_rows_out;
    
    EXCEPTION
        WHEN e_null_column_value
             OR e_existing_fky_reference
             OR e_check_constraint_failure
             OR e_no_parent_key
             OR e_child_record_found
             OR e_forall_error
             OR e_integ_constraint_failure THEN
            IF NOT handle_error_in
            THEN
                RAISE;
            ELSE
                pk_alertlog.log_info('DML error ignored: ' || SQLCODE || ': ' || SQLERRM);
            END IF;
        
    END upd;

    -- Update any/all columns by primary key. If you pass NULL, then
    -- the current column value is set to itself. If you need a more
    -- selected UPDATE then use one of the onecol procedures below.
    PROCEDURE upd
    (
        id_epis_pn_addendum_in  IN epis_pn_addendum.id_epis_pn_addendum%TYPE,
        id_epis_pn_in           IN epis_pn_addendum.id_epis_pn%TYPE DEFAULT NULL,
        id_epis_pn_nin          IN BOOLEAN := TRUE,
        flg_status_in           IN epis_pn_addendum.flg_status%TYPE DEFAULT NULL,
        flg_status_nin          IN BOOLEAN := TRUE,
        id_professional_in      IN epis_pn_addendum.id_professional%TYPE DEFAULT NULL,
        id_professional_nin     IN BOOLEAN := TRUE,
        dt_addendum_in          IN epis_pn_addendum.dt_addendum%TYPE DEFAULT NULL,
        dt_addendum_nin         IN BOOLEAN := TRUE,
        pn_addendum_in          IN epis_pn_addendum.pn_addendum%TYPE DEFAULT NULL,
        pn_addendum_nin         IN BOOLEAN := TRUE,
        dt_last_update_in       IN epis_pn_addendum.dt_last_update%TYPE DEFAULT NULL,
        dt_last_update_nin      IN BOOLEAN := TRUE,
        id_prof_last_update_in  IN epis_pn_addendum.id_prof_last_update%TYPE DEFAULT NULL,
        id_prof_last_update_nin IN BOOLEAN := TRUE,
        dt_signoff_in           IN epis_pn_addendum.dt_signoff%TYPE DEFAULT NULL,
        dt_signoff_nin          IN BOOLEAN := TRUE,
        id_prof_signoff_in      IN epis_pn_addendum.id_prof_signoff%TYPE DEFAULT NULL,
        id_prof_signoff_nin     IN BOOLEAN := TRUE,
        id_prof_cancel_in       IN epis_pn_addendum.id_prof_cancel%TYPE DEFAULT NULL,
        id_prof_cancel_nin      IN BOOLEAN := TRUE,
        id_cancel_reason_in     IN epis_pn_addendum.id_cancel_reason%TYPE DEFAULT NULL,
        id_cancel_reason_nin    IN BOOLEAN := TRUE,
        dt_cancel_in            IN epis_pn_addendum.dt_cancel%TYPE DEFAULT NULL,
        dt_cancel_nin           IN BOOLEAN := TRUE,
        notes_cancel_in         IN epis_pn_addendum.notes_cancel%TYPE DEFAULT NULL,
        notes_cancel_nin        IN BOOLEAN := TRUE,
        create_user_in          IN epis_pn_addendum.create_user%TYPE DEFAULT NULL,
        create_user_nin         IN BOOLEAN := TRUE,
        create_time_in          IN epis_pn_addendum.create_time%TYPE DEFAULT NULL,
        create_time_nin         IN BOOLEAN := TRUE,
        create_institution_in   IN epis_pn_addendum.create_institution%TYPE DEFAULT NULL,
        create_institution_nin  IN BOOLEAN := TRUE,
        update_user_in          IN epis_pn_addendum.update_user%TYPE DEFAULT NULL,
        update_user_nin         IN BOOLEAN := TRUE,
        update_time_in          IN epis_pn_addendum.update_time%TYPE DEFAULT NULL,
        update_time_nin         IN BOOLEAN := TRUE,
        update_institution_in   IN epis_pn_addendum.update_institution%TYPE DEFAULT NULL,
        update_institution_nin  IN BOOLEAN := TRUE,
        flg_type_in             IN epis_pn_addendum.flg_type%TYPE DEFAULT NULL,
        flg_type_nin            IN BOOLEAN := TRUE,
        handle_error_in         IN BOOLEAN := TRUE
    ) IS
        rows_out table_varchar;
    BEGIN
        upd(id_epis_pn_addendum_in  => id_epis_pn_addendum_in,
            id_epis_pn_in           => id_epis_pn_in,
            id_epis_pn_nin          => id_epis_pn_nin,
            flg_status_in           => flg_status_in,
            flg_status_nin          => flg_status_nin,
            id_professional_in      => id_professional_in,
            id_professional_nin     => id_professional_nin,
            dt_addendum_in          => dt_addendum_in,
            dt_addendum_nin         => dt_addendum_nin,
            pn_addendum_in          => pn_addendum_in,
            pn_addendum_nin         => pn_addendum_nin,
            dt_last_update_in       => dt_last_update_in,
            dt_last_update_nin      => dt_last_update_nin,
            id_prof_last_update_in  => id_prof_last_update_in,
            id_prof_last_update_nin => id_prof_last_update_nin,
            dt_signoff_in           => dt_signoff_in,
            dt_signoff_nin          => dt_signoff_nin,
            id_prof_signoff_in      => id_prof_signoff_in,
            id_prof_signoff_nin     => id_prof_signoff_nin,
            id_prof_cancel_in       => id_prof_cancel_in,
            id_prof_cancel_nin      => id_prof_cancel_nin,
            id_cancel_reason_in     => id_cancel_reason_in,
            id_cancel_reason_nin    => id_cancel_reason_nin,
            dt_cancel_in            => dt_cancel_in,
            dt_cancel_nin           => dt_cancel_nin,
            notes_cancel_in         => notes_cancel_in,
            notes_cancel_nin        => notes_cancel_nin,
            create_user_in          => create_user_in,
            create_user_nin         => create_user_nin,
            create_time_in          => create_time_in,
            create_time_nin         => create_time_nin,
            create_institution_in   => create_institution_in,
            create_institution_nin  => create_institution_nin,
            update_user_in          => update_user_in,
            update_user_nin         => update_user_nin,
            update_time_in          => update_time_in,
            update_time_nin         => update_time_nin,
            update_institution_in   => update_institution_in,
            update_institution_nin  => update_institution_nin,
            flg_type_in             => flg_type_in,
            flg_type_nin            => flg_type_nin,
            handle_error_in         => handle_error_in,
            rows_out                => rows_out);
    END upd;

    --Update any/all columns by dynamic WHERE
    -- If you pass NULL, then the current column value is set to itself
    PROCEDURE upd
    (
        id_epis_pn_in           IN epis_pn_addendum.id_epis_pn%TYPE DEFAULT NULL,
        id_epis_pn_nin          IN BOOLEAN := TRUE,
        flg_status_in           IN epis_pn_addendum.flg_status%TYPE DEFAULT NULL,
        flg_status_nin          IN BOOLEAN := TRUE,
        id_professional_in      IN epis_pn_addendum.id_professional%TYPE DEFAULT NULL,
        id_professional_nin     IN BOOLEAN := TRUE,
        dt_addendum_in          IN epis_pn_addendum.dt_addendum%TYPE DEFAULT NULL,
        dt_addendum_nin         IN BOOLEAN := TRUE,
        pn_addendum_in          IN epis_pn_addendum.pn_addendum%TYPE DEFAULT NULL,
        pn_addendum_nin         IN BOOLEAN := TRUE,
        dt_last_update_in       IN epis_pn_addendum.dt_last_update%TYPE DEFAULT NULL,
        dt_last_update_nin      IN BOOLEAN := TRUE,
        id_prof_last_update_in  IN epis_pn_addendum.id_prof_last_update%TYPE DEFAULT NULL,
        id_prof_last_update_nin IN BOOLEAN := TRUE,
        dt_signoff_in           IN epis_pn_addendum.dt_signoff%TYPE DEFAULT NULL,
        dt_signoff_nin          IN BOOLEAN := TRUE,
        id_prof_signoff_in      IN epis_pn_addendum.id_prof_signoff%TYPE DEFAULT NULL,
        id_prof_signoff_nin     IN BOOLEAN := TRUE,
        id_prof_cancel_in       IN epis_pn_addendum.id_prof_cancel%TYPE DEFAULT NULL,
        id_prof_cancel_nin      IN BOOLEAN := TRUE,
        id_cancel_reason_in     IN epis_pn_addendum.id_cancel_reason%TYPE DEFAULT NULL,
        id_cancel_reason_nin    IN BOOLEAN := TRUE,
        dt_cancel_in            IN epis_pn_addendum.dt_cancel%TYPE DEFAULT NULL,
        dt_cancel_nin           IN BOOLEAN := TRUE,
        notes_cancel_in         IN epis_pn_addendum.notes_cancel%TYPE DEFAULT NULL,
        notes_cancel_nin        IN BOOLEAN := TRUE,
        create_user_in          IN epis_pn_addendum.create_user%TYPE DEFAULT NULL,
        create_user_nin         IN BOOLEAN := TRUE,
        create_time_in          IN epis_pn_addendum.create_time%TYPE DEFAULT NULL,
        create_time_nin         IN BOOLEAN := TRUE,
        create_institution_in   IN epis_pn_addendum.create_institution%TYPE DEFAULT NULL,
        create_institution_nin  IN BOOLEAN := TRUE,
        update_user_in          IN epis_pn_addendum.update_user%TYPE DEFAULT NULL,
        update_user_nin         IN BOOLEAN := TRUE,
        update_time_in          IN epis_pn_addendum.update_time%TYPE DEFAULT NULL,
        update_time_nin         IN BOOLEAN := TRUE,
        update_institution_in   IN epis_pn_addendum.update_institution%TYPE DEFAULT NULL,
        update_institution_nin  IN BOOLEAN := TRUE,
        flg_type_in             IN epis_pn_addendum.flg_type%TYPE DEFAULT NULL,
        flg_type_nin            IN BOOLEAN := TRUE,
        where_in                IN VARCHAR2,
        handle_error_in         IN BOOLEAN := TRUE,
        rows_out                IN OUT table_varchar
    ) IS
        l_sql                   VARCHAR2(32767);
        l_rows_out              table_varchar;
        l_id_epis_pn_n          NUMBER(1) := sys.diutil.bool_to_int(id_epis_pn_nin);
        l_flg_status_n          NUMBER(1) := sys.diutil.bool_to_int(flg_status_nin);
        l_id_professional_n     NUMBER(1) := sys.diutil.bool_to_int(id_professional_nin);
        l_dt_addendum_n         NUMBER(1) := sys.diutil.bool_to_int(dt_addendum_nin);
        l_pn_addendum_n         NUMBER(1) := sys.diutil.bool_to_int(pn_addendum_nin);
        l_dt_last_update_n      NUMBER(1) := sys.diutil.bool_to_int(dt_last_update_nin);
        l_id_prof_last_update_n NUMBER(1) := sys.diutil.bool_to_int(id_prof_last_update_nin);
        l_dt_signoff_n          NUMBER(1) := sys.diutil.bool_to_int(dt_signoff_nin);
        l_id_prof_signoff_n     NUMBER(1) := sys.diutil.bool_to_int(id_prof_signoff_nin);
        l_id_prof_cancel_n      NUMBER(1) := sys.diutil.bool_to_int(id_prof_cancel_nin);
        l_id_cancel_reason_n    NUMBER(1) := sys.diutil.bool_to_int(id_cancel_reason_nin);
        l_dt_cancel_n           NUMBER(1) := sys.diutil.bool_to_int(dt_cancel_nin);
        l_notes_cancel_n        NUMBER(1) := sys.diutil.bool_to_int(notes_cancel_nin);
        l_create_user_n         NUMBER(1) := sys.diutil.bool_to_int(create_user_nin);
        l_create_time_n         NUMBER(1) := sys.diutil.bool_to_int(create_time_nin);
        l_create_institution_n  NUMBER(1) := sys.diutil.bool_to_int(create_institution_nin);
        l_update_user_n         NUMBER(1) := sys.diutil.bool_to_int(update_user_nin);
        l_update_time_n         NUMBER(1) := sys.diutil.bool_to_int(update_time_nin);
        l_update_institution_n  NUMBER(1) := sys.diutil.bool_to_int(update_institution_nin);
        l_flg_type_n            NUMBER(1) := sys.diutil.bool_to_int(flg_type_nin);
    BEGIN
        l_id_epis_pn_n          := sys.diutil.bool_to_int(id_epis_pn_nin);
        l_flg_status_n          := sys.diutil.bool_to_int(flg_status_nin);
        l_id_professional_n     := sys.diutil.bool_to_int(id_professional_nin);
        l_dt_addendum_n         := sys.diutil.bool_to_int(dt_addendum_nin);
        l_pn_addendum_n         := sys.diutil.bool_to_int(pn_addendum_nin);
        l_dt_last_update_n      := sys.diutil.bool_to_int(dt_last_update_nin);
        l_id_prof_last_update_n := sys.diutil.bool_to_int(id_prof_last_update_nin);
        l_dt_signoff_n          := sys.diutil.bool_to_int(dt_signoff_nin);
        l_id_prof_signoff_n     := sys.diutil.bool_to_int(id_prof_signoff_nin);
        l_id_prof_cancel_n      := sys.diutil.bool_to_int(id_prof_cancel_nin);
        l_id_cancel_reason_n    := sys.diutil.bool_to_int(id_cancel_reason_nin);
        l_dt_cancel_n           := sys.diutil.bool_to_int(dt_cancel_nin);
        l_notes_cancel_n        := sys.diutil.bool_to_int(notes_cancel_nin);
        l_create_user_n         := sys.diutil.bool_to_int(create_user_nin);
        l_create_time_n         := sys.diutil.bool_to_int(create_time_nin);
        l_create_institution_n  := sys.diutil.bool_to_int(create_institution_nin);
        l_update_user_n         := sys.diutil.bool_to_int(update_user_nin);
        l_update_time_n         := sys.diutil.bool_to_int(update_time_nin);
        l_update_institution_n  := sys.diutil.bool_to_int(update_institution_nin);
        l_flg_type_n            := sys.diutil.bool_to_int(flg_type_nin);
        l_sql                   := 'UPDATE epis_pn_addendum SET' || ' ID_EPIS_PN = decode (' || l_id_epis_pn_n ||
                                   ',0,:ID_EPIS_PN_in, NVL (:ID_EPIS_PN_in, ID_EPIS_PN)) ' || ',' ||
                                   ' FLG_STATUS = decode (' || l_flg_status_n ||
                                   ',0,:FLG_STATUS_in, NVL (:FLG_STATUS_in, FLG_STATUS)) ' || ',' ||
                                   ' ID_PROFESSIONAL = decode (' || l_id_professional_n ||
                                   ',0,:ID_PROFESSIONAL_in, NVL (:ID_PROFESSIONAL_in, ID_PROFESSIONAL)) ' || ',' ||
                                   ' DT_ADDENDUM = decode (' || l_dt_addendum_n ||
                                   ',0,:DT_ADDENDUM_in, NVL (:DT_ADDENDUM_in, DT_ADDENDUM)) ' || ',' ||
                                   ' PN_ADDENDUM = decode (' || l_pn_addendum_n ||
                                   ',0,:PN_ADDENDUM_in, NVL (:PN_ADDENDUM_in, PN_ADDENDUM)) ' || ',' ||
                                   ' DT_LAST_UPDATE = decode (' || l_dt_last_update_n ||
                                   ',0,:DT_LAST_UPDATE_in, NVL (:DT_LAST_UPDATE_in, DT_LAST_UPDATE)) ' || ',' ||
                                   ' ID_PROF_LAST_UPDATE = decode (' || l_id_prof_last_update_n ||
                                   ',0,:ID_PROF_LAST_UPDATE_in, NVL (:ID_PROF_LAST_UPDATE_in, ID_PROF_LAST_UPDATE)) ' || ',' ||
                                   ' DT_SIGNOFF = decode (' || l_dt_signoff_n ||
                                   ',0,:DT_SIGNOFF_in, NVL (:DT_SIGNOFF_in, DT_SIGNOFF)) ' || ',' ||
                                   ' ID_PROF_SIGNOFF = decode (' || l_id_prof_signoff_n ||
                                   ',0,:ID_PROF_SIGNOFF_in, NVL (:ID_PROF_SIGNOFF_in, ID_PROF_SIGNOFF)) ' || ',' ||
                                   ' ID_PROF_CANCEL = decode (' || l_id_prof_cancel_n ||
                                   ',0,:ID_PROF_CANCEL_in, NVL (:ID_PROF_CANCEL_in, ID_PROF_CANCEL)) ' || ',' ||
                                   ' ID_CANCEL_REASON = decode (' || l_id_cancel_reason_n ||
                                   ',0,:ID_CANCEL_REASON_in, NVL (:ID_CANCEL_REASON_in, ID_CANCEL_REASON)) ' || ',' ||
                                   ' DT_CANCEL = decode (' || l_dt_cancel_n ||
                                   ',0,:DT_CANCEL_in, NVL (:DT_CANCEL_in, DT_CANCEL)) ' || ',' ||
                                   ' NOTES_CANCEL = decode (' || l_notes_cancel_n ||
                                   ',0,:NOTES_CANCEL_in, NVL (:NOTES_CANCEL_in, NOTES_CANCEL)) ' || ',' ||
                                   ' CREATE_USER = decode (' || l_create_user_n ||
                                   ',0,:CREATE_USER_in, NVL (:CREATE_USER_in, CREATE_USER)) ' || ',' ||
                                   ' CREATE_TIME = decode (' || l_create_time_n ||
                                   ',0,:CREATE_TIME_in, NVL (:CREATE_TIME_in, CREATE_TIME)) ' || ',' ||
                                   ' CREATE_INSTITUTION = decode (' || l_create_institution_n ||
                                   ',0,:CREATE_INSTITUTION_in, NVL (:CREATE_INSTITUTION_in, CREATE_INSTITUTION)) ' || ',' ||
                                   ' UPDATE_USER = decode (' || l_update_user_n ||
                                   ',0,:UPDATE_USER_in, NVL (:UPDATE_USER_in, UPDATE_USER)) ' || ',' ||
                                   ' UPDATE_TIME = decode (' || l_update_time_n ||
                                   ',0,:UPDATE_TIME_in, NVL (:UPDATE_TIME_in, UPDATE_TIME)) ' || ',' ||
                                   ' UPDATE_INSTITUTION = decode (' || l_update_institution_n ||
                                   ',0,:UPDATE_INSTITUTION_in, NVL (:UPDATE_INSTITUTION_in, UPDATE_INSTITUTION)) ' || ',' ||
                                   ' FLG_TYPE = decode (' || l_flg_type_n ||
                                   ',0,:FLG_TYPE_in, NVL (:FLG_TYPE_in, FLG_TYPE)) ' || ' where ' ||
                                   nvl(where_in, '(1=1)') || ' RETURNING ROWID BULK COLLECT INTO :l_rows_out';
        EXECUTE IMMEDIATE 'BEGIN ' || l_sql || '; END;'
            USING IN id_epis_pn_in, flg_status_in, id_professional_in, dt_addendum_in, pn_addendum_in, dt_last_update_in, id_prof_last_update_in, dt_signoff_in, id_prof_signoff_in, id_prof_cancel_in, id_cancel_reason_in, dt_cancel_in, notes_cancel_in, create_user_in, create_time_in, create_institution_in, update_user_in, update_time_in, update_institution_in, flg_type_in, OUT l_rows_out;
        IF (rows_out IS NULL)
        THEN
            rows_out := table_varchar();
        END IF;
        rows_out := rows_out MULTISET UNION DISTINCT l_rows_out;
    
    EXCEPTION
        WHEN e_null_column_value
             OR e_existing_fky_reference
             OR e_check_constraint_failure
             OR e_no_parent_key
             OR e_child_record_found
             OR e_forall_error
             OR e_integ_constraint_failure THEN
            IF NOT handle_error_in
            THEN
                RAISE;
            ELSE
                pk_alertlog.log_info('DML error ignored: ' || SQLCODE || ': ' || SQLERRM);
            END IF;
        
    END upd;

    --Update any/all columns by dynamic WHERE
    -- If you pass NULL, then the current column value is set to itself
    PROCEDURE upd
    (
        id_epis_pn_in           IN epis_pn_addendum.id_epis_pn%TYPE DEFAULT NULL,
        id_epis_pn_nin          IN BOOLEAN := TRUE,
        flg_status_in           IN epis_pn_addendum.flg_status%TYPE DEFAULT NULL,
        flg_status_nin          IN BOOLEAN := TRUE,
        id_professional_in      IN epis_pn_addendum.id_professional%TYPE DEFAULT NULL,
        id_professional_nin     IN BOOLEAN := TRUE,
        dt_addendum_in          IN epis_pn_addendum.dt_addendum%TYPE DEFAULT NULL,
        dt_addendum_nin         IN BOOLEAN := TRUE,
        pn_addendum_in          IN epis_pn_addendum.pn_addendum%TYPE DEFAULT NULL,
        pn_addendum_nin         IN BOOLEAN := TRUE,
        dt_last_update_in       IN epis_pn_addendum.dt_last_update%TYPE DEFAULT NULL,
        dt_last_update_nin      IN BOOLEAN := TRUE,
        id_prof_last_update_in  IN epis_pn_addendum.id_prof_last_update%TYPE DEFAULT NULL,
        id_prof_last_update_nin IN BOOLEAN := TRUE,
        dt_signoff_in           IN epis_pn_addendum.dt_signoff%TYPE DEFAULT NULL,
        dt_signoff_nin          IN BOOLEAN := TRUE,
        id_prof_signoff_in      IN epis_pn_addendum.id_prof_signoff%TYPE DEFAULT NULL,
        id_prof_signoff_nin     IN BOOLEAN := TRUE,
        id_prof_cancel_in       IN epis_pn_addendum.id_prof_cancel%TYPE DEFAULT NULL,
        id_prof_cancel_nin      IN BOOLEAN := TRUE,
        id_cancel_reason_in     IN epis_pn_addendum.id_cancel_reason%TYPE DEFAULT NULL,
        id_cancel_reason_nin    IN BOOLEAN := TRUE,
        dt_cancel_in            IN epis_pn_addendum.dt_cancel%TYPE DEFAULT NULL,
        dt_cancel_nin           IN BOOLEAN := TRUE,
        notes_cancel_in         IN epis_pn_addendum.notes_cancel%TYPE DEFAULT NULL,
        notes_cancel_nin        IN BOOLEAN := TRUE,
        create_user_in          IN epis_pn_addendum.create_user%TYPE DEFAULT NULL,
        create_user_nin         IN BOOLEAN := TRUE,
        create_time_in          IN epis_pn_addendum.create_time%TYPE DEFAULT NULL,
        create_time_nin         IN BOOLEAN := TRUE,
        create_institution_in   IN epis_pn_addendum.create_institution%TYPE DEFAULT NULL,
        create_institution_nin  IN BOOLEAN := TRUE,
        update_user_in          IN epis_pn_addendum.update_user%TYPE DEFAULT NULL,
        update_user_nin         IN BOOLEAN := TRUE,
        update_time_in          IN epis_pn_addendum.update_time%TYPE DEFAULT NULL,
        update_time_nin         IN BOOLEAN := TRUE,
        update_institution_in   IN epis_pn_addendum.update_institution%TYPE DEFAULT NULL,
        update_institution_nin  IN BOOLEAN := TRUE,
        flg_type_in             IN epis_pn_addendum.flg_type%TYPE DEFAULT NULL,
        flg_type_nin            IN BOOLEAN := TRUE,
        where_in                IN VARCHAR2,
        handle_error_in         IN BOOLEAN := TRUE
    ) IS
        rows_out table_varchar;
    BEGIN
        upd(id_epis_pn_in           => id_epis_pn_in,
            id_epis_pn_nin          => id_epis_pn_nin,
            flg_status_in           => flg_status_in,
            flg_status_nin          => flg_status_nin,
            id_professional_in      => id_professional_in,
            id_professional_nin     => id_professional_nin,
            dt_addendum_in          => dt_addendum_in,
            dt_addendum_nin         => dt_addendum_nin,
            pn_addendum_in          => pn_addendum_in,
            pn_addendum_nin         => pn_addendum_nin,
            dt_last_update_in       => dt_last_update_in,
            dt_last_update_nin      => dt_last_update_nin,
            id_prof_last_update_in  => id_prof_last_update_in,
            id_prof_last_update_nin => id_prof_last_update_nin,
            dt_signoff_in           => dt_signoff_in,
            dt_signoff_nin          => dt_signoff_nin,
            id_prof_signoff_in      => id_prof_signoff_in,
            id_prof_signoff_nin     => id_prof_signoff_nin,
            id_prof_cancel_in       => id_prof_cancel_in,
            id_prof_cancel_nin      => id_prof_cancel_nin,
            id_cancel_reason_in     => id_cancel_reason_in,
            id_cancel_reason_nin    => id_cancel_reason_nin,
            dt_cancel_in            => dt_cancel_in,
            dt_cancel_nin           => dt_cancel_nin,
            notes_cancel_in         => notes_cancel_in,
            notes_cancel_nin        => notes_cancel_nin,
            create_user_in          => create_user_in,
            create_user_nin         => create_user_nin,
            create_time_in          => create_time_in,
            create_time_nin         => create_time_nin,
            create_institution_in   => create_institution_in,
            create_institution_nin  => create_institution_nin,
            update_user_in          => update_user_in,
            update_user_nin         => update_user_nin,
            update_time_in          => update_time_in,
            update_time_nin         => update_time_nin,
            update_institution_in   => update_institution_in,
            update_institution_nin  => update_institution_nin,
            flg_type_in             => flg_type_in,
            flg_type_nin            => flg_type_nin,
            where_in                => where_in,
            handle_error_in         => handle_error_in,
            rows_out                => rows_out);
    END upd;

    --Update/insert columns (with rows_out)
    PROCEDURE upd_ins
    (
        id_epis_pn_addendum_in IN epis_pn_addendum.id_epis_pn_addendum%TYPE,
        id_epis_pn_in          IN epis_pn_addendum.id_epis_pn%TYPE DEFAULT NULL,
        flg_status_in          IN epis_pn_addendum.flg_status%TYPE DEFAULT NULL,
        id_professional_in     IN epis_pn_addendum.id_professional%TYPE DEFAULT NULL,
        dt_addendum_in         IN epis_pn_addendum.dt_addendum%TYPE DEFAULT NULL,
        pn_addendum_in         IN epis_pn_addendum.pn_addendum%TYPE DEFAULT NULL,
        dt_last_update_in      IN epis_pn_addendum.dt_last_update%TYPE DEFAULT NULL,
        id_prof_last_update_in IN epis_pn_addendum.id_prof_last_update%TYPE DEFAULT NULL,
        dt_signoff_in          IN epis_pn_addendum.dt_signoff%TYPE DEFAULT NULL,
        id_prof_signoff_in     IN epis_pn_addendum.id_prof_signoff%TYPE DEFAULT NULL,
        id_prof_cancel_in      IN epis_pn_addendum.id_prof_cancel%TYPE DEFAULT NULL,
        id_cancel_reason_in    IN epis_pn_addendum.id_cancel_reason%TYPE DEFAULT NULL,
        dt_cancel_in           IN epis_pn_addendum.dt_cancel%TYPE DEFAULT NULL,
        notes_cancel_in        IN epis_pn_addendum.notes_cancel%TYPE DEFAULT NULL,
        create_user_in         IN epis_pn_addendum.create_user%TYPE DEFAULT NULL,
        create_time_in         IN epis_pn_addendum.create_time%TYPE DEFAULT NULL,
        create_institution_in  IN epis_pn_addendum.create_institution%TYPE DEFAULT NULL,
        update_user_in         IN epis_pn_addendum.update_user%TYPE DEFAULT NULL,
        update_time_in         IN epis_pn_addendum.update_time%TYPE DEFAULT NULL,
        update_institution_in  IN epis_pn_addendum.update_institution%TYPE DEFAULT NULL,
        flg_type_in            IN epis_pn_addendum.flg_type%TYPE DEFAULT NULL,
        handle_error_in        IN BOOLEAN := TRUE,
        rows_out               IN OUT table_varchar
    ) IS
    BEGIN
        upd(id_epis_pn_addendum_in => id_epis_pn_addendum_in,
            id_epis_pn_in          => id_epis_pn_in,
            flg_status_in          => flg_status_in,
            id_professional_in     => id_professional_in,
            dt_addendum_in         => dt_addendum_in,
            pn_addendum_in         => pn_addendum_in,
            dt_last_update_in      => dt_last_update_in,
            id_prof_last_update_in => id_prof_last_update_in,
            dt_signoff_in          => dt_signoff_in,
            id_prof_signoff_in     => id_prof_signoff_in,
            id_prof_cancel_in      => id_prof_cancel_in,
            id_cancel_reason_in    => id_cancel_reason_in,
            dt_cancel_in           => dt_cancel_in,
            notes_cancel_in        => notes_cancel_in,
            create_user_in         => create_user_in,
            create_time_in         => create_time_in,
            create_institution_in  => create_institution_in,
            update_user_in         => update_user_in,
            update_time_in         => update_time_in,
            update_institution_in  => update_institution_in,
            flg_type_in            => flg_type_in,
            handle_error_in        => handle_error_in,
            rows_out               => rows_out);
        IF SQL%ROWCOUNT = 0
        THEN
            ins(id_epis_pn_addendum_in => id_epis_pn_addendum_in,
                id_epis_pn_in          => id_epis_pn_in,
                flg_status_in          => flg_status_in,
                id_professional_in     => id_professional_in,
                dt_addendum_in         => dt_addendum_in,
                pn_addendum_in         => pn_addendum_in,
                dt_last_update_in      => dt_last_update_in,
                id_prof_last_update_in => id_prof_last_update_in,
                dt_signoff_in          => dt_signoff_in,
                id_prof_signoff_in     => id_prof_signoff_in,
                id_prof_cancel_in      => id_prof_cancel_in,
                id_cancel_reason_in    => id_cancel_reason_in,
                dt_cancel_in           => dt_cancel_in,
                notes_cancel_in        => notes_cancel_in,
                create_user_in         => create_user_in,
                create_time_in         => create_time_in,
                create_institution_in  => create_institution_in,
                update_user_in         => update_user_in,
                update_time_in         => update_time_in,
                update_institution_in  => update_institution_in,
                flg_type_in            => flg_type_in,
                handle_error_in        => handle_error_in,
                rows_out               => rows_out);
        END IF;
    END upd_ins;

    --Update/insert columns (without rows_out)
    PROCEDURE upd_ins
    (
        id_epis_pn_addendum_in IN epis_pn_addendum.id_epis_pn_addendum%TYPE,
        id_epis_pn_in          IN epis_pn_addendum.id_epis_pn%TYPE DEFAULT NULL,
        flg_status_in          IN epis_pn_addendum.flg_status%TYPE DEFAULT NULL,
        id_professional_in     IN epis_pn_addendum.id_professional%TYPE DEFAULT NULL,
        dt_addendum_in         IN epis_pn_addendum.dt_addendum%TYPE DEFAULT NULL,
        pn_addendum_in         IN epis_pn_addendum.pn_addendum%TYPE DEFAULT NULL,
        dt_last_update_in      IN epis_pn_addendum.dt_last_update%TYPE DEFAULT NULL,
        id_prof_last_update_in IN epis_pn_addendum.id_prof_last_update%TYPE DEFAULT NULL,
        dt_signoff_in          IN epis_pn_addendum.dt_signoff%TYPE DEFAULT NULL,
        id_prof_signoff_in     IN epis_pn_addendum.id_prof_signoff%TYPE DEFAULT NULL,
        id_prof_cancel_in      IN epis_pn_addendum.id_prof_cancel%TYPE DEFAULT NULL,
        id_cancel_reason_in    IN epis_pn_addendum.id_cancel_reason%TYPE DEFAULT NULL,
        dt_cancel_in           IN epis_pn_addendum.dt_cancel%TYPE DEFAULT NULL,
        notes_cancel_in        IN epis_pn_addendum.notes_cancel%TYPE DEFAULT NULL,
        create_user_in         IN epis_pn_addendum.create_user%TYPE DEFAULT NULL,
        create_time_in         IN epis_pn_addendum.create_time%TYPE DEFAULT NULL,
        create_institution_in  IN epis_pn_addendum.create_institution%TYPE DEFAULT NULL,
        update_user_in         IN epis_pn_addendum.update_user%TYPE DEFAULT NULL,
        update_time_in         IN epis_pn_addendum.update_time%TYPE DEFAULT NULL,
        update_institution_in  IN epis_pn_addendum.update_institution%TYPE DEFAULT NULL,
        flg_type_in            IN epis_pn_addendum.flg_type%TYPE DEFAULT NULL,
        handle_error_in        IN BOOLEAN := TRUE
    ) IS
        rows_out table_varchar;
    BEGIN
        upd_ins(id_epis_pn_addendum_in,
                id_epis_pn_in,
                flg_status_in,
                id_professional_in,
                dt_addendum_in,
                pn_addendum_in,
                dt_last_update_in,
                id_prof_last_update_in,
                dt_signoff_in,
                id_prof_signoff_in,
                id_prof_cancel_in,
                id_cancel_reason_in,
                dt_cancel_in,
                notes_cancel_in,
                create_user_in,
                create_time_in,
                create_institution_in,
                update_user_in,
                update_time_in,
                update_institution_in,
                flg_type_in,
                handle_error_in,
                rows_out);
    END upd_ins;

    --Update by record (with rows_out)
    PROCEDURE upd
    (
        rec_in          IN epis_pn_addendum%ROWTYPE,
        handle_error_in IN BOOLEAN := TRUE,
        rows_out        IN OUT table_varchar
    ) IS
    BEGIN
        upd(id_epis_pn_addendum_in => rec_in.id_epis_pn_addendum,
            id_epis_pn_in          => rec_in.id_epis_pn,
            flg_status_in          => rec_in.flg_status,
            id_professional_in     => rec_in.id_professional,
            dt_addendum_in         => rec_in.dt_addendum,
            pn_addendum_in         => rec_in.pn_addendum,
            dt_last_update_in      => rec_in.dt_last_update,
            id_prof_last_update_in => rec_in.id_prof_last_update,
            dt_signoff_in          => rec_in.dt_signoff,
            id_prof_signoff_in     => rec_in.id_prof_signoff,
            id_prof_cancel_in      => rec_in.id_prof_cancel,
            id_cancel_reason_in    => rec_in.id_cancel_reason,
            dt_cancel_in           => rec_in.dt_cancel,
            notes_cancel_in        => rec_in.notes_cancel,
            create_user_in         => rec_in.create_user,
            create_time_in         => rec_in.create_time,
            create_institution_in  => rec_in.create_institution,
            update_user_in         => rec_in.update_user,
            update_time_in         => rec_in.update_time,
            update_institution_in  => rec_in.update_institution,
            flg_type_in            => rec_in.flg_type,
            handle_error_in        => handle_error_in,
            rows_out               => rows_out);
    END upd;

    --Update by record (without rows_out)
    PROCEDURE upd
    (
        rec_in          IN epis_pn_addendum%ROWTYPE,
        handle_error_in IN BOOLEAN := TRUE
    ) IS
        rows_out table_varchar;
    BEGIN
        upd(id_epis_pn_addendum_in => rec_in.id_epis_pn_addendum,
            id_epis_pn_in          => rec_in.id_epis_pn,
            flg_status_in          => rec_in.flg_status,
            id_professional_in     => rec_in.id_professional,
            dt_addendum_in         => rec_in.dt_addendum,
            pn_addendum_in         => rec_in.pn_addendum,
            dt_last_update_in      => rec_in.dt_last_update,
            id_prof_last_update_in => rec_in.id_prof_last_update,
            dt_signoff_in          => rec_in.dt_signoff,
            id_prof_signoff_in     => rec_in.id_prof_signoff,
            id_prof_cancel_in      => rec_in.id_prof_cancel,
            id_cancel_reason_in    => rec_in.id_cancel_reason,
            dt_cancel_in           => rec_in.dt_cancel,
            notes_cancel_in        => rec_in.notes_cancel,
            create_user_in         => rec_in.create_user,
            create_time_in         => rec_in.create_time,
            create_institution_in  => rec_in.create_institution,
            update_user_in         => rec_in.update_user,
            update_time_in         => rec_in.update_time,
            update_institution_in  => rec_in.update_institution,
            flg_type_in            => rec_in.flg_type,
            handle_error_in        => handle_error_in,
            rows_out               => rows_out);
    END upd;

    --Update with collection (with rows_out)
    PROCEDURE upd
    (
        col_in            IN epis_pn_addendum_tc,
        ignore_if_null_in IN BOOLEAN := TRUE,
        handle_error_in   IN BOOLEAN := TRUE,
        rows_out          IN OUT table_varchar
    ) IS
        l_id_epis_pn_addendum id_epis_pn_addendum_cc;
        l_id_epis_pn          id_epis_pn_cc;
        l_flg_status          flg_status_cc;
        l_id_professional     id_professional_cc;
        l_dt_addendum         dt_addendum_cc;
        l_pn_addendum         pn_addendum_cc;
        l_dt_last_update      dt_last_update_cc;
        l_id_prof_last_update id_prof_last_update_cc;
        l_dt_signoff          dt_signoff_cc;
        l_id_prof_signoff     id_prof_signoff_cc;
        l_id_prof_cancel      id_prof_cancel_cc;
        l_id_cancel_reason    id_cancel_reason_cc;
        l_dt_cancel           dt_cancel_cc;
        l_notes_cancel        notes_cancel_cc;
        l_create_user         create_user_cc;
        l_create_time         create_time_cc;
        l_create_institution  create_institution_cc;
        l_update_user         update_user_cc;
        l_update_time         update_time_cc;
        l_update_institution  update_institution_cc;
        l_flg_type            flg_type_cc;
    BEGIN
        FOR i IN col_in.first .. col_in.last
        LOOP
            l_id_epis_pn_addendum(i) := col_in(i).id_epis_pn_addendum;
            l_id_epis_pn(i) := col_in(i).id_epis_pn;
            l_flg_status(i) := col_in(i).flg_status;
            l_id_professional(i) := col_in(i).id_professional;
            l_dt_addendum(i) := col_in(i).dt_addendum;
            l_pn_addendum(i) := col_in(i).pn_addendum;
            l_dt_last_update(i) := col_in(i).dt_last_update;
            l_id_prof_last_update(i) := col_in(i).id_prof_last_update;
            l_dt_signoff(i) := col_in(i).dt_signoff;
            l_id_prof_signoff(i) := col_in(i).id_prof_signoff;
            l_id_prof_cancel(i) := col_in(i).id_prof_cancel;
            l_id_cancel_reason(i) := col_in(i).id_cancel_reason;
            l_dt_cancel(i) := col_in(i).dt_cancel;
            l_notes_cancel(i) := col_in(i).notes_cancel;
            l_create_user(i) := col_in(i).create_user;
            l_create_time(i) := col_in(i).create_time;
            l_create_institution(i) := col_in(i).create_institution;
            l_update_user(i) := col_in(i).update_user;
            l_update_time(i) := col_in(i).update_time;
            l_update_institution(i) := col_in(i).update_institution;
            l_flg_type(i) := col_in(i).flg_type;
        END LOOP;
        IF nvl(ignore_if_null_in, FALSE)
        THEN
            -- Set any columns to their current values
            -- if incoming value is NULL.
            -- Put WHEN clause on column-level triggers!
            FORALL i IN col_in.first .. col_in.last
                UPDATE epis_pn_addendum
                   SET id_epis_pn          = nvl(l_id_epis_pn(i), id_epis_pn),
                       flg_status          = nvl(l_flg_status(i), flg_status),
                       id_professional     = nvl(l_id_professional(i), id_professional),
                       dt_addendum         = nvl(l_dt_addendum(i), dt_addendum),
                       pn_addendum         = nvl(l_pn_addendum(i), pn_addendum),
                       dt_last_update      = nvl(l_dt_last_update(i), dt_last_update),
                       id_prof_last_update = nvl(l_id_prof_last_update(i), id_prof_last_update),
                       dt_signoff          = nvl(l_dt_signoff(i), dt_signoff),
                       id_prof_signoff     = nvl(l_id_prof_signoff(i), id_prof_signoff),
                       id_prof_cancel      = nvl(l_id_prof_cancel(i), id_prof_cancel),
                       id_cancel_reason    = nvl(l_id_cancel_reason(i), id_cancel_reason),
                       dt_cancel           = nvl(l_dt_cancel(i), dt_cancel),
                       notes_cancel        = nvl(l_notes_cancel(i), notes_cancel),
                       create_user         = nvl(l_create_user(i), create_user),
                       create_time         = nvl(l_create_time(i), create_time),
                       create_institution  = nvl(l_create_institution(i), create_institution),
                       update_user         = nvl(l_update_user(i), update_user),
                       update_time         = nvl(l_update_time(i), update_time),
                       update_institution  = nvl(l_update_institution(i), update_institution),
                       flg_type            = nvl(l_flg_type(i), flg_type)
                 WHERE id_epis_pn_addendum = l_id_epis_pn_addendum(i)
                RETURNING ROWID BULK COLLECT INTO rows_out;
        ELSE
            FORALL i IN col_in.first .. col_in.last
                UPDATE epis_pn_addendum
                   SET id_epis_pn          = l_id_epis_pn(i),
                       flg_status          = l_flg_status(i),
                       id_professional     = l_id_professional(i),
                       dt_addendum         = l_dt_addendum(i),
                       pn_addendum         = l_pn_addendum(i),
                       dt_last_update      = l_dt_last_update(i),
                       id_prof_last_update = l_id_prof_last_update(i),
                       dt_signoff          = l_dt_signoff(i),
                       id_prof_signoff     = l_id_prof_signoff(i),
                       id_prof_cancel      = l_id_prof_cancel(i),
                       id_cancel_reason    = l_id_cancel_reason(i),
                       dt_cancel           = l_dt_cancel(i),
                       notes_cancel        = l_notes_cancel(i),
                       create_user         = l_create_user(i),
                       create_time         = l_create_time(i),
                       create_institution  = l_create_institution(i),
                       update_user         = l_update_user(i),
                       update_time         = l_update_time(i),
                       update_institution  = l_update_institution(i),
                       flg_type            = l_flg_type(i)
                 WHERE id_epis_pn_addendum = l_id_epis_pn_addendum(i)
                RETURNING ROWID BULK COLLECT INTO rows_out;
        END IF;
    
    EXCEPTION
        WHEN e_forall_error THEN
            IF NOT handle_error_in
            THEN
                RAISE;
            ELSE
                FOR indx IN 1 .. SQL%bulk_exceptions.count
                LOOP
                    pk_alertlog.log_info('DML error ignored: ' || SQLCODE || ': ' || SQLERRM);
                END LOOP;
            END IF;
        WHEN OTHERS THEN
            IF NOT handle_error_in
            THEN
                RAISE;
            ELSE
                pk_alertlog.log_info('DML error ignored: ' || SQLCODE || ': ' || SQLERRM);
            END IF;
        
    END upd;

    --Update with collection (without rows_out)
    PROCEDURE upd
    (
        col_in            IN epis_pn_addendum_tc,
        ignore_if_null_in IN BOOLEAN := TRUE,
        handle_error_in   IN BOOLEAN := TRUE
    ) IS
        rows_out table_varchar;
    BEGIN
        upd(col_in, ignore_if_null_in, handle_error_in, rows_out);
    END upd;

    --Dynamic update string
    FUNCTION dynupdstr
    (
        colname_in IN all_tab_columns.column_name%TYPE,
        where_in   IN VARCHAR2 := NULL
    ) RETURN VARCHAR2 IS
    BEGIN
        RETURN 'BEGIN UPDATE epis_pn_addendum
SET ' || colname_in || ' = :value
 WHERE ' || nvl(where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;';
    END dynupdstr;

    -- Use Native Dynamic SQL increment a single NUMBER column
    -- for all rows specified by the dynamic WHERE clause (with rows_out)
    PROCEDURE increment_onecol
    (
        colname_in         IN all_tab_columns.column_name%TYPE,
        where_in           IN VARCHAR2,
        increment_value_in IN NUMBER DEFAULT 1,
        handle_error_in    IN BOOLEAN := TRUE,
        rows_out           OUT table_varchar
    ) IS
    BEGIN
        EXECUTE IMMEDIATE 'BEGIN UPDATE epis_pn_addendum
SET ' || colname_in || '=' || colname_in || '+' || nvl(increment_value_in, 1) || ' WHERE ' ||
                          nvl(where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
            USING OUT rows_out;
    END increment_onecol;

    -- Use Native Dynamic SQL increment a single NUMBER column
    -- for all rows specified by the dynamic WHERE clause (without rows_out)
    PROCEDURE increment_onecol
    (
        colname_in         IN all_tab_columns.column_name%TYPE,
        where_in           IN VARCHAR2,
        increment_value_in IN NUMBER DEFAULT 1,
        handle_error_in    IN BOOLEAN := TRUE
    ) IS
        rows_out table_varchar;
    BEGIN
        EXECUTE IMMEDIATE 'UPDATE epis_pn_addendum
SET ' || colname_in || '=' || colname_in || '+' || nvl(increment_value_in, 1) || ' WHERE ' ||
                          nvl(where_in, '1=1');
    END increment_onecol;

    -- Delete one row by primary key
    PROCEDURE del
    (
        id_epis_pn_addendum_in IN epis_pn_addendum.id_epis_pn_addendum%TYPE,
        handle_error_in        IN BOOLEAN := TRUE,
        rows_out               OUT table_varchar
    ) IS
    BEGIN
        DELETE FROM epis_pn_addendum
         WHERE id_epis_pn_addendum = id_epis_pn_addendum_in
        RETURNING ROWID BULK COLLECT INTO rows_out;
    
    EXCEPTION
        WHEN e_null_column_value
             OR e_existing_fky_reference
             OR e_check_constraint_failure
             OR e_no_parent_key
             OR e_child_record_found
             OR e_forall_error
             OR e_integ_constraint_failure THEN
            IF NOT handle_error_in
            THEN
                RAISE;
            ELSE
                pk_alertlog.log_info('DML error ignored: ' || SQLCODE || ': ' || SQLERRM);
            END IF;
        
    END del;

    -- Delete one row by primary key
    PROCEDURE del
    (
        id_epis_pn_addendum_in IN epis_pn_addendum.id_epis_pn_addendum%TYPE,
        handle_error_in        IN BOOLEAN := TRUE
    ) IS
        rows_out table_varchar;
    BEGIN
        del(id_epis_pn_addendum_in => id_epis_pn_addendum_in, handle_error_in => handle_error_in, rows_out => rows_out);
    END del;

    -- Delete all rows for this EPMA_CRN_FK foreign key value
    PROCEDURE del_epma_crn_fk
    (
        id_cancel_reason_in IN epis_pn_addendum.id_cancel_reason%TYPE,
        handle_error_in     IN BOOLEAN := TRUE,
        rows_out            OUT table_varchar
    ) IS
    BEGIN
        DELETE FROM epis_pn_addendum
         WHERE id_cancel_reason = id_cancel_reason_in
        RETURNING ROWID BULK COLLECT INTO rows_out;
    
    EXCEPTION
        WHEN e_null_column_value
             OR e_existing_fky_reference
             OR e_check_constraint_failure
             OR e_no_parent_key
             OR e_child_record_found
             OR e_forall_error
             OR e_integ_constraint_failure THEN
            IF NOT handle_error_in
            THEN
                RAISE;
            ELSE
                pk_alertlog.log_info('DML error ignored: ' || SQLCODE || ': ' || SQLERRM);
            END IF;
        
    END del_epma_crn_fk;

    -- Delete all rows for this EPMA_EPN_FK foreign key value
    PROCEDURE del_epma_epn_fk
    (
        id_epis_pn_in   IN epis_pn_addendum.id_epis_pn%TYPE,
        handle_error_in IN BOOLEAN := TRUE,
        rows_out        OUT table_varchar
    ) IS
    BEGIN
        DELETE FROM epis_pn_addendum
         WHERE id_epis_pn = id_epis_pn_in
        RETURNING ROWID BULK COLLECT INTO rows_out;
    
    EXCEPTION
        WHEN e_null_column_value
             OR e_existing_fky_reference
             OR e_check_constraint_failure
             OR e_no_parent_key
             OR e_child_record_found
             OR e_forall_error
             OR e_integ_constraint_failure THEN
            IF NOT handle_error_in
            THEN
                RAISE;
            ELSE
                pk_alertlog.log_info('DML error ignored: ' || SQLCODE || ': ' || SQLERRM);
            END IF;
        
    END del_epma_epn_fk;

    -- Delete all rows for this EPMA_PROF_CANCEL_FK foreign key value
    PROCEDURE del_epma_prof_cancel_fk
    (
        id_prof_cancel_in IN epis_pn_addendum.id_prof_cancel%TYPE,
        handle_error_in   IN BOOLEAN := TRUE,
        rows_out          OUT table_varchar
    ) IS
    BEGIN
        DELETE FROM epis_pn_addendum
         WHERE id_prof_cancel = id_prof_cancel_in
        RETURNING ROWID BULK COLLECT INTO rows_out;
    
    EXCEPTION
        WHEN e_null_column_value
             OR e_existing_fky_reference
             OR e_check_constraint_failure
             OR e_no_parent_key
             OR e_child_record_found
             OR e_forall_error
             OR e_integ_constraint_failure THEN
            IF NOT handle_error_in
            THEN
                RAISE;
            ELSE
                pk_alertlog.log_info('DML error ignored: ' || SQLCODE || ': ' || SQLERRM);
            END IF;
        
    END del_epma_prof_cancel_fk;

    -- Delete all rows for this EPMA_PROF_FK foreign key value
    PROCEDURE del_epma_prof_fk
    (
        id_professional_in IN epis_pn_addendum.id_professional%TYPE,
        handle_error_in    IN BOOLEAN := TRUE,
        rows_out           OUT table_varchar
    ) IS
    BEGIN
        DELETE FROM epis_pn_addendum
         WHERE id_professional = id_professional_in
        RETURNING ROWID BULK COLLECT INTO rows_out;
    
    EXCEPTION
        WHEN e_null_column_value
             OR e_existing_fky_reference
             OR e_check_constraint_failure
             OR e_no_parent_key
             OR e_child_record_found
             OR e_forall_error
             OR e_integ_constraint_failure THEN
            IF NOT handle_error_in
            THEN
                RAISE;
            ELSE
                pk_alertlog.log_info('DML error ignored: ' || SQLCODE || ': ' || SQLERRM);
            END IF;
        
    END del_epma_prof_fk;

    -- Delete all rows for this EPMA_PROF_LAST_UPD_FK foreign key value
    PROCEDURE del_epma_prof_last_upd_fk
    (
        id_prof_last_update_in IN epis_pn_addendum.id_prof_last_update%TYPE,
        handle_error_in        IN BOOLEAN := TRUE,
        rows_out               OUT table_varchar
    ) IS
    BEGIN
        DELETE FROM epis_pn_addendum
         WHERE id_prof_last_update = id_prof_last_update_in
        RETURNING ROWID BULK COLLECT INTO rows_out;
    
    EXCEPTION
        WHEN e_null_column_value
             OR e_existing_fky_reference
             OR e_check_constraint_failure
             OR e_no_parent_key
             OR e_child_record_found
             OR e_forall_error
             OR e_integ_constraint_failure THEN
            IF NOT handle_error_in
            THEN
                RAISE;
            ELSE
                pk_alertlog.log_info('DML error ignored: ' || SQLCODE || ': ' || SQLERRM);
            END IF;
        
    END del_epma_prof_last_upd_fk;

    -- Delete all rows for this EPMA_PROF_SIGNOFF_FK foreign key value
    PROCEDURE del_epma_prof_signoff_fk
    (
        id_prof_signoff_in IN epis_pn_addendum.id_prof_signoff%TYPE,
        handle_error_in    IN BOOLEAN := TRUE,
        rows_out           OUT table_varchar
    ) IS
    BEGIN
        DELETE FROM epis_pn_addendum
         WHERE id_prof_signoff = id_prof_signoff_in
        RETURNING ROWID BULK COLLECT INTO rows_out;
    
    EXCEPTION
        WHEN e_null_column_value
             OR e_existing_fky_reference
             OR e_check_constraint_failure
             OR e_no_parent_key
             OR e_child_record_found
             OR e_forall_error
             OR e_integ_constraint_failure THEN
            IF NOT handle_error_in
            THEN
                RAISE;
            ELSE
                pk_alertlog.log_info('DML error ignored: ' || SQLCODE || ': ' || SQLERRM);
            END IF;
        
    END del_epma_prof_signoff_fk;

    -- Delete all rows for this EPMA_CRN_FK foreign key value
    PROCEDURE del_epma_crn_fk
    (
        id_cancel_reason_in IN epis_pn_addendum.id_cancel_reason%TYPE,
        handle_error_in     IN BOOLEAN := TRUE
    ) IS
        rows_out table_varchar;
    BEGIN
        del_epma_crn_fk(id_cancel_reason_in => id_cancel_reason_in,
                        handle_error_in     => handle_error_in,
                        rows_out            => rows_out);
    END del_epma_crn_fk;

    -- Delete all rows for this EPMA_EPN_FK foreign key value
    PROCEDURE del_epma_epn_fk
    (
        id_epis_pn_in   IN epis_pn_addendum.id_epis_pn%TYPE,
        handle_error_in IN BOOLEAN := TRUE
    ) IS
        rows_out table_varchar;
    BEGIN
        del_epma_epn_fk(id_epis_pn_in => id_epis_pn_in, handle_error_in => handle_error_in, rows_out => rows_out);
    END del_epma_epn_fk;

    -- Delete all rows for this EPMA_PROF_CANCEL_FK foreign key value
    PROCEDURE del_epma_prof_cancel_fk
    (
        id_prof_cancel_in IN epis_pn_addendum.id_prof_cancel%TYPE,
        handle_error_in   IN BOOLEAN := TRUE
    ) IS
        rows_out table_varchar;
    BEGIN
        del_epma_prof_cancel_fk(id_prof_cancel_in => id_prof_cancel_in,
                                handle_error_in   => handle_error_in,
                                rows_out          => rows_out);
    END del_epma_prof_cancel_fk;

    -- Delete all rows for this EPMA_PROF_FK foreign key value
    PROCEDURE del_epma_prof_fk
    (
        id_professional_in IN epis_pn_addendum.id_professional%TYPE,
        handle_error_in    IN BOOLEAN := TRUE
    ) IS
        rows_out table_varchar;
    BEGIN
        del_epma_prof_fk(id_professional_in => id_professional_in,
                         handle_error_in    => handle_error_in,
                         rows_out           => rows_out);
    END del_epma_prof_fk;

    -- Delete all rows for this EPMA_PROF_LAST_UPD_FK foreign key value
    PROCEDURE del_epma_prof_last_upd_fk
    (
        id_prof_last_update_in IN epis_pn_addendum.id_prof_last_update%TYPE,
        handle_error_in        IN BOOLEAN := TRUE
    ) IS
        rows_out table_varchar;
    BEGIN
        del_epma_prof_last_upd_fk(id_prof_last_update_in => id_prof_last_update_in,
                                  handle_error_in        => handle_error_in,
                                  rows_out               => rows_out);
    END del_epma_prof_last_upd_fk;

    -- Delete all rows for this EPMA_PROF_SIGNOFF_FK foreign key value
    PROCEDURE del_epma_prof_signoff_fk
    (
        id_prof_signoff_in IN epis_pn_addendum.id_prof_signoff%TYPE,
        handle_error_in    IN BOOLEAN := TRUE
    ) IS
        rows_out table_varchar;
    BEGIN
        del_epma_prof_signoff_fk(id_prof_signoff_in => id_prof_signoff_in,
                                 handle_error_in    => handle_error_in,
                                 rows_out           => rows_out);
    END del_epma_prof_signoff_fk;

    -- Deletions using dynamic SQL
    FUNCTION dyndelstr(where_in IN VARCHAR2) RETURN VARCHAR2 IS
    BEGIN
        IF where_in IS NULL
        THEN
            RETURN 'DELETE FROM epis_pn_addendum';
        ELSE
            RETURN 'DELETE FROM epis_pn_addendum WHERE ' || where_in;
        END IF;
    END dyndelstr;
    -- Delete all rows specified by dynamic WHERE clause
    PROCEDURE del_by
    (
        where_clause_in IN VARCHAR2,
        handle_error_in IN BOOLEAN := TRUE,
        rows_out        OUT table_varchar
    ) IS
    BEGIN
        EXECUTE IMMEDIATE 'BEGIN ' || dyndelstr(where_clause_in) ||
                          ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
            USING OUT rows_out;
    
    EXCEPTION
        WHEN e_null_column_value
             OR e_existing_fky_reference
             OR e_check_constraint_failure
             OR e_no_parent_key
             OR e_child_record_found
             OR e_forall_error
             OR e_integ_constraint_failure THEN
            IF NOT handle_error_in
            THEN
                RAISE;
            ELSE
                pk_alertlog.log_info('DML error ignored: ' || SQLCODE || ': ' || SQLERRM);
            END IF;
        
    END del_by;

    -- Delete all rows specified by dynamic WHERE clause
    PROCEDURE del_by
    (
        where_clause_in IN VARCHAR2,
        handle_error_in IN BOOLEAN := TRUE
    ) IS
    BEGIN
        EXECUTE IMMEDIATE dyndelstr(where_clause_in);
    
    EXCEPTION
        WHEN e_null_column_value
             OR e_existing_fky_reference
             OR e_check_constraint_failure
             OR e_no_parent_key
             OR e_child_record_found
             OR e_forall_error
             OR e_integ_constraint_failure THEN
            IF NOT handle_error_in
            THEN
                RAISE;
            ELSE
                pk_alertlog.log_info('DML error ignored: ' || SQLCODE || ': ' || SQLERRM);
            END IF;
        
    END del_by;

    -- Initialize a record with default values for columns in the table (prc)
    PROCEDURE initrec(epis_pn_addendum_inout IN OUT epis_pn_addendum%ROWTYPE) IS
    BEGIN
        epis_pn_addendum_inout.id_epis_pn_addendum := NULL;
        epis_pn_addendum_inout.id_epis_pn          := NULL;
        epis_pn_addendum_inout.flg_status          := NULL;
        epis_pn_addendum_inout.id_professional     := NULL;
        epis_pn_addendum_inout.dt_addendum         := NULL;
        epis_pn_addendum_inout.pn_addendum         := NULL;
        epis_pn_addendum_inout.dt_last_update      := NULL;
        epis_pn_addendum_inout.id_prof_last_update := NULL;
        epis_pn_addendum_inout.dt_signoff          := NULL;
        epis_pn_addendum_inout.id_prof_signoff     := NULL;
        epis_pn_addendum_inout.id_prof_cancel      := NULL;
        epis_pn_addendum_inout.id_cancel_reason    := NULL;
        epis_pn_addendum_inout.dt_cancel           := NULL;
        epis_pn_addendum_inout.notes_cancel        := NULL;
        epis_pn_addendum_inout.create_user         := NULL;
        epis_pn_addendum_inout.create_time         := NULL;
        epis_pn_addendum_inout.create_institution  := NULL;
        epis_pn_addendum_inout.update_user         := NULL;
        epis_pn_addendum_inout.update_time         := NULL;
        epis_pn_addendum_inout.update_institution  := NULL;
        epis_pn_addendum_inout.flg_type            := NULL;
    END initrec;

    -- Initialize a record with default values for columns in the table (fnc)
    FUNCTION initrec RETURN epis_pn_addendum%ROWTYPE IS
        l_epis_pn_addendum epis_pn_addendum%ROWTYPE;
    BEGIN
        RETURN l_epis_pn_addendum;
    END initrec;

    --get data from rowid
    FUNCTION get_data_rowid(rows_in IN table_varchar) RETURN epis_pn_addendum_tc IS
        data epis_pn_addendum_tc;
    BEGIN
        SELECT *
          BULK COLLECT
          INTO data
          FROM epis_pn_addendum
         WHERE ROWID IN (SELECT /*+opt_estimate(table,t,scale_rows=0.0000001))*/
                          *
                           FROM TABLE(rows_in) t);
        RETURN data;
    END get_data_rowid;

    --get data from rowid (pragma autonomous transacion)
    FUNCTION get_data_rowid_pat(rows_in IN table_varchar) RETURN epis_pn_addendum_tc IS
        data epis_pn_addendum_tc;
        PRAGMA AUTONOMOUS_TRANSACTION;
    BEGIN
        data := get_data_rowid(rows_in);
        COMMIT;
        RETURN data;
    
    EXCEPTION
        WHEN OTHERS THEN
            pk_alert_exceptions.raise_error(error_name_in => 'get_data_rowid_pat');
            ROLLBACK;
        
    END get_data_rowid_pat;

BEGIN
    NULL;
END ts_epis_pn_addendum;
/
