...
CREATE OR REPLACE PACKAGE BODY TS_CDR_CALL
/*
| Generated by or retrieved from QCGU - DO NOT MODIFY!
| QCGU - "Get it right, do it fast" - www.ToadWorld.com
| QCGU Universal ID: {657BD35E-2E48-4CD4-B14E-41420B8433B3}
| Created On: Fevereiro 28, 2012 17:44:9
| Created By: ALERT
*/
IS
      ...
      THEN
        ...
        BEGIN
           ...
           pk_alert_exceptions.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'CDR_CALL');
           IF l_name = 'CDRL_CDRL_FK'
           THEN
              -- Add a context value for each column
              pk_alert_exceptions.add_context (
                 err_instance_id_in => l_id
               , name_in => 'ID_CDR_CALL_PARENT'
               , value_in => id_cdr_call_parent_in);
           END IF;
           IF l_name = 'CDRL_EPIS_FK'
           THEN
              -- Add a context value for each column
              pk_alert_exceptions.add_context (
                 err_instance_id_in => l_id
           ...
           END IF;
           IF l_name = 'CDRL_PROF_FK'
           THEN
              -- Add a context value for each column
              pk_alert_exceptions.add_context (
                 err_instance_id_in => l_id
               , name_in => 'ID_PROF_CALL'
               , value_in => id_prof_call_in);
           END IF;
   ...
   BEGIN
     ...
     ins (
      ...
      id_cdr_call_parent_in => id_cdr_call_parent_in
   ...
   IS
      l_rec CDR_CALL%ROWTYPE := rec_in;
   BEGIN
      IF gen_pky_in THEN
         l_rec.ID_CDR_CALL := next_key (sequence_in);
      END IF;
      ins (
         id_cdr_call_in => l_rec.ID_CDR_CALL
         ,
         id_prof_call_in => l_rec.ID_PROF_CALL,
         dt_call_in => l_rec.DT_CALL,
         ...
         id_cdr_call_parent_in => l_rec.ID_CDR_CALL_PARENT
         ,handle_error_in => handle_error_in
         , rows_out => rows_out
         );
   END ins;
  ...
  ins (
   ...
   IS
     retval CDR_CALL.ID_CDR_CALL%TYPE;

   BEGIN
      IF sequence_in IS NULL
      THEN
         SELECT seq_CDR_CALL.NEXTVAL INTO retval FROM dual;
      ELSE
         EXECUTE IMMEDIATE
   ...
   EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
           ...
           ,name1_in => 'SEQUENCE'
           ,value1_in => NVL (sequence_in, 'seq_CDR_CALL')
           );
   END next_key;
   ...
   IS
        l_pky CDR_CALL.ID_CDR_CALL%TYPE := next_key;
   BEGIN
      ins (
         id_cdr_call_in => l_pky,
         id_prof_call_in => id_prof_call_in,
         dt_call_in => dt_call_in,
         id_episode_in => id_episode_in,
         id_patient_in => id_patient_in,
   ...
   BEGIN
      ...
      update_institution_in => update_institution_in,
      id_cdr_call_parent_in => id_cdr_call_parent_in,
      id_cdr_call_out => id_cdr_call_out
      ,handle_error_in => handle_error_in
      , rows_out => rows_out
   );
   END ins;
   ...
   BEGIN
      ...
      ELSE
         FORALL indx IN rows_in.FIRST .. rows_in.LAST
            SAVE EXCEPTIONS
            INSERT INTO CDR_CALL VALUES rows_in (indx) RETURNING ROWID BULK COLLECT INTO rows_out;
      END IF;
   EXCEPTION
     WHEN e_forall_error
     THEN
          ...
          BEGIN
             ...
             LOOP
                ...
                pk_alert_exceptions.add_context (
                  ...
                  ,value_in => rows_in(SQL%BULK_EXCEPTIONS (indx).ERROR_INDEX).ID_PROF_CALL
                  ,validate_in => FALSE
                );

                pk_alert_exceptions.add_context (
                   err_instance_id_in => l_err_instance_id
                  ,NAME_IN => 'DT_CALL _' || indx
                  ,value_in => rows_in(SQL%BULK_EXCEPTIONS (indx).ERROR_INDEX).DT_CALL
                  ,validate_in => FALSE
                );
                ...
                pk_alert_exceptions.add_context (
                   err_instance_id_in => l_err_instance_id
                  ,NAME_IN => 'ID_CDR_CALL_PARENT _' || indx
                  ,value_in => rows_in(SQL%BULK_EXCEPTIONS (indx).ERROR_INDEX).ID_CDR_CALL_PARENT
                  ,validate_in => FALSE
                );
...
PROCEDURE upd (
   ...
   l_ID_CDR_CALL_PARENT_n NUMBER(1);
   BEGIN

   l_ID_PROF_CALL_n := sys.diutil.bool_to_int(ID_PROF_CALL_nin);
   l_DT_CALL_n := sys.diutil.bool_to_int(DT_CALL_nin);
   ...
   l_ID_CDR_CALL_PARENT_n := sys.diutil.bool_to_int(ID_CDR_CALL_PARENT_nin);


         UPDATE CDR_CALL SET
     ID_PROF_CALL = decode (l_ID_PROF_CALL_n,0,id_prof_call_in, NVL (id_prof_call_in, ID_PROF_CALL)),
     DT_CALL = decode (l_DT_CALL_n,0,dt_call_in, NVL (dt_call_in, DT_CALL)),
     ...
     ID_CDR_CALL_PARENT = decode (l_ID_CDR_CALL_PARENT_n,0,id_cdr_call_parent_in, NVL (id_cdr_call_parent_in, ID_CDR_CALL_PARENT))
          WHERE
...
rows_out :=  rows_out MULTISET UNION DISTINCT l_rows_out;
   ...
   BEGIN
     upd (
      ...
      id_cdr_call_parent_in => id_cdr_call_parent_in,
      ID_CDR_CALL_PARENT_nin => ID_CDR_CALL_PARENT_nin,
     handle_error_in => handle_error_in
     , rows_out => rows_out
      );
   END upd;
...
PROCEDURE upd (
   ...
   l_UPDATE_INSTITUTION_n NUMBER(1);
   l_ID_CDR_CALL_PARENT_n NUMBER(1);
      id_cdr_call_in CDR_CALL.ID_CDR_CALL%TYPE;
   BEGIN
...
PROCEDURE upd (
   ...
   BEGIN
      upd (
         ...
         id_cdr_call_parent_in => rec_in.ID_CDR_CALL_PARENT
   ...
   BEGIN
      FOR i IN col_in.FIRST .. col_in.LAST loop
         ...
         l_ID_CDR_CALL_PARENT(i) := col_in(i).ID_CDR_CALL_PARENT;
      END LOOP;
      ...
      THEN
         ...
         FORALL i IN col_in.FIRST .. col_in.LAST
            UPDATE CDR_CALL SET
               ...
               DT_CALL = NVL (l_DT_CALL(i), DT_CALL),
               ...
               ID_CDR_CALL_PARENT = NVL (l_ID_CDR_CALL_PARENT(i), ID_CDR_CALL_PARENT)
             WHERE
      ...
      ELSE
         FORALL i IN col_in.FIRST .. col_in.LAST
            UPDATE CDR_CALL SET
               ...
               ID_CDR_CALL_PARENT = l_ID_CDR_CALL_PARENT(i)
             WHERE
                ID_CDR_CALL = l_ID_CDR_CALL(i)
        returning rowid bulk collect into rows_out  ;
      END IF;
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE CDR_CALL set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE CDR_CALL set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM CDR_CALL
       WHERE
          ID_CDR_CALL_PARENT = del_CDRL_CDRL_FK.id_cdr_call_parent_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_CDRL_CDRL_FK (
   ...
   BEGIN
      DELETE FROM CDR_CALL
       WHERE
          ID_EPISODE = del_CDRL_EPIS_FK.id_episode_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_CDRL_EPIS_FK (
   ...
   BEGIN
      DELETE FROM CDR_CALL
       WHERE
          ID_PATIENT = del_CDRL_PAT_FK.id_patient_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_CDRL_PAT_FK (
   ...
   BEGIN
      DELETE FROM CDR_CALL
       WHERE
          ID_PROF_CALL = del_CDRL_PROF_FK.id_prof_call_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_CDRL_PROF_FK (
   ...
   BEGIN
      IF where_in IS NULL
      THEN
         RETURN 'DELETE FROM CDR_CALL';
      ELSE
         RETURN
            'DELETE FROM CDR_CALL WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   IS
   BEGIN
      cdr_call_inout.ID_CDR_CALL := NULL;
      cdr_call_inout.ID_PROF_CALL := NULL;
      cdr_call_inout.DT_CALL := NULL;
      cdr_call_inout.ID_EPISODE := NULL;
      cdr_call_inout.ID_PATIENT := NULL;
      cdr_call_inout.CREATE_USER := NULL;
      cdr_call_inout.CREATE_TIME := NULL;
      cdr_call_inout.CREATE_INSTITUTION := NULL;
      cdr_call_inout.UPDATE_USER := NULL;
      cdr_call_inout.UPDATE_TIME := NULL;
      cdr_call_inout.UPDATE_INSTITUTION := NULL;
      cdr_call_inout.ID_CDR_CALL_PARENT := NULL;
   END initrec;

   FUNCTION initrec RETURN CDR_CALL%ROWTYPE
   IS
      l_cdr_call CDR_CALL%ROWTYPE;
   BEGIN
      RETURN l_cdr_call;
   END initrec;
   ...
   IS
        data CDR_CALL_tc;
   BEGIN
        select * bulk collect into data from CDR_CALL where rowid in (select /*+opt_estimate(table,t,scale_rows=0.0000001))*/ * from table(rows_in) t);
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
