/*-- Last Change Revision: $Rev: 2028339 $*/
/*-- Last Change by: $Author: mario.fernandes $*/
/*-- Date of last change: $Date: 2022-08-02 18:45:13 +0100 (ter, 02 ago 2022) $*/

CREATE OR REPLACE PACKAGE BODY TS_PROF_DEP_CLIN_SERV_HIST
/*
| Generated by or retrieved from QCGU - DO NOT MODIFY!
| QCGU - "Get it right, do it fast" - www.ToadWorld.com
| QCGU Universal ID: {657BD35E-2E48-4CD4-B14E-41420B8433B3}
| Created On: Novembro 19, 2008 14:34:58
| Created By: ALERT
*/
IS
   ...
   BEGIN
     ...
     INSERT INTO PROF_DEP_CLIN_SERV_HIST (
         ID_PROFESSIONAL,
         ID_DEP_CLIN_SERV,
         DT_BEGIN,
         DT_END,
        ...
        BEGIN
           ...
           ELSE
              pk_alert_exceptions.raise_error (
                    ...
                    ,value2_in => l_name
                    ,name3_in => 'TABLE_NAME'
                    ,value3_in => 'PROF_DEP_CLIN_SERV_HIST');
           END IF;
        END;
        END IF;
      ...
      THEN
        ...
        BEGIN
           ...
           pk_alert_exceptions.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'PROF_DEP_CLIN_SERV_HIST');
           IF l_name = 'PCSTH_DCS_FK'
           THEN
   ...
   IS
      l_rec PROF_DEP_CLIN_SERV_HIST%ROWTYPE := rec_in;
   BEGIN
      ins (
   ...
   BEGIN
      ...
      ELSE
         FORALL indx IN rows_in.FIRST .. rows_in.LAST
            SAVE EXCEPTIONS
            INSERT INTO PROF_DEP_CLIN_SERV_HIST VALUES rows_in (indx) RETURNING ROWID BULK COLLECT INTO rows_out;
      END IF;
   EXCEPTION
     WHEN e_forall_error
     THEN
     ...
     THEN
       ...
       pk_alert_exceptions.raise_error(
          error_name_in => 'FORALL-INSERT-FAILURE'
          ,name1_in => 'TABLE_NAME'
          ,value1_in => 'PROF_DEP_CLIN_SERV_HIST'
          ,name2_in => 'ROW_COUNT'
...
PROCEDURE upd (
   ...
   l_ID_INSTITUTION_n := sys.diutil.bool_to_int(ID_INSTITUTION_nin);
         ...
         UPDATE PROF_DEP_CLIN_SERV_HIST SET
...
PROCEDURE upd (
   ...
   l_FLG_DEFAULT_n NUMBER(1);
   l_ID_INSTITUTION_n NUMBER(1);
      id_professional_in PROF_DEP_CLIN_SERV_HIST.ID_PROFESSIONAL%TYPE;
      id_dep_clin_serv_in PROF_DEP_CLIN_SERV_HIST.ID_DEP_CLIN_SERV%TYPE;
      dt_begin_in PROF_DEP_CLIN_SERV_HIST.DT_BEGIN%TYPE;
   BEGIN
...
l_sql := 'UPDATE PROF_DEP_CLIN_SERV_HIST SET '
     || ' DT_END = decode (' || l_DT_END_n || ',0,:dt_end_in, NVL (:dt_end_in, DT_END)) '|| ','
     || ' FLG_DEFAULT = decode (' || l_FLG_DEFAULT_n || ',0,:flg_default_in, NVL (:flg_default_in, FLG_DEFAULT)) '|| ','
     || ' ID_INSTITUTION = decode (' || l_ID_INSTITUTION_n || ',0,:id_institution_in, NVL (:id_institution_in, ID_INSTITUTION)) '
...
PROCEDURE upd (
   ...
   BEGIN
      RETURN
         'BEGIN UPDATE PROF_DEP_CLIN_SERV_HIST
   ...
   BEGIN
      RETURN
         'UPDATE PROF_DEP_CLIN_SERV_HIST
             SET ' || colname_in || ' = :value
           WHERE ' || NVL (where_in, '1=1');
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE PROF_DEP_CLIN_SERV_HIST set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE PROF_DEP_CLIN_SERV_HIST set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
   ...
   IS
   BEGIN
      DELETE FROM PROF_DEP_CLIN_SERV_HIST
       WHERE
          ID_PROFESSIONAL = id_professional_in AND
          ID_DEP_CLIN_SERV = id_dep_clin_serv_in AND
          DT_BEGIN = dt_begin_in
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM PROF_DEP_CLIN_SERV_HIST
       WHERE
          ID_DEP_CLIN_SERV = del_PCSTH_DCS_FK.id_dep_clin_serv_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_PCSTH_DCS_FK (
   ...
   BEGIN
      DELETE FROM PROF_DEP_CLIN_SERV_HIST
       WHERE
          ID_INSTITUTION = del_PCSTH_INST_FK.id_institution_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_PCSTH_INST_FK (
   ...
   BEGIN
      DELETE FROM PROF_DEP_CLIN_SERV_HIST
       WHERE
          ID_PROFESSIONAL = del_PCSTH_PROF_FK.id_professional_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_PCSTH_PROF_FK (
   ...
   BEGIN
      IF where_in IS NULL
      THEN
         RETURN 'DELETE FROM PROF_DEP_CLIN_SERV_HIST';
      ELSE
         RETURN
            'DELETE FROM PROF_DEP_CLIN_SERV_HIST WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   IS
   BEGIN
      prof_dep_clin_serv_hist_inout.ID_PROFESSIONAL := NULL;
      prof_dep_clin_serv_hist_inout.ID_DEP_CLIN_SERV := NULL;
      prof_dep_clin_serv_hist_inout.DT_BEGIN := NULL;
      prof_dep_clin_serv_hist_inout.DT_END := NULL;
      prof_dep_clin_serv_hist_inout.FLG_DEFAULT := NULL;
      prof_dep_clin_serv_hist_inout.ID_INSTITUTION := NULL;
   END initrec;

   FUNCTION initrec RETURN PROF_DEP_CLIN_SERV_HIST%ROWTYPE
   IS
      l_prof_dep_clin_serv_hist PROF_DEP_CLIN_SERV_HIST%ROWTYPE;
   BEGIN
      RETURN l_prof_dep_clin_serv_hist;
   END initrec;
   ...
   IS
        data PROF_DEP_CLIN_SERV_HIST_tc;
   BEGIN
        select * bulk collect into data from PROF_DEP_CLIN_SERV_HIST where rowid in (select * from table(rows_in));
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
