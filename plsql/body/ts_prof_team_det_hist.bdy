/*-- Last Change Revision: $Rev: 2028346 $*/
/*-- Last Change by: $Author: mario.fernandes $*/
/*-- Date of last change: $Date: 2022-08-02 18:45:15 +0100 (ter, 02 ago 2022) $*/

CREATE OR REPLACE PACKAGE BODY TS_PROF_TEAM_DET_HIST
/*
| Generated by or retrieved from QCGU - DO NOT MODIFY!
| QCGU - "Get it right, do it fast" - www.ToadWorld.com
| QCGU Universal ID: {657BD35E-2E48-4CD4-B14E-41420B8433B3}
| Created On: July 5, 2009 17:34:55
| Created By: ALERT
*/
IS
   ...
   BEGIN
     ...
     INSERT INTO PROF_TEAM_DET_HIST (
         ID_PROF_TEAM_HIST,
         ID_PROF_TEAM,
         ID_PROFESSIONAL,
         ID_CATEGORY_SUB,
      ...
      THEN
        ...
        BEGIN
           ...
           pk_alert_exceptions.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'PROF_TEAM_DET_HIST');
           IF l_name = 'PRFT_DH_CATS_FK'
           THEN
           ...
           END IF;
           IF l_name = 'PRFT_DH_PROF_TEAMH_FK'
           THEN
              -- Add a context value for each column
              pk_alert_exceptions.add_context (
                 err_instance_id_in => l_id
               , name_in => 'ID_PROF_TEAM_HIST'
               , value_in => id_prof_team_hist_in);
           END IF;
           IF l_name = 'PRFT_DH_PROF_TEAM_FK'
           THEN
              -- Add a context value for each column
              pk_alert_exceptions.add_context (
                 err_instance_id_in => l_id
   ...
   IS
      l_rec PROF_TEAM_DET_HIST%ROWTYPE := rec_in;
   BEGIN
      ins (
   ...
   BEGIN
      ...
      ELSE
         FORALL indx IN rows_in.FIRST .. rows_in.LAST
            SAVE EXCEPTIONS
            INSERT INTO PROF_TEAM_DET_HIST VALUES rows_in (indx) RETURNING ROWID BULK COLLECT INTO rows_out;
      END IF;
   EXCEPTION
     WHEN e_forall_error
     THEN
       ...
       ELSE
          ...
          BEGIN
             ...
             LOOP
                ...
                pk_alert_exceptions.add_context (
                   err_instance_id_in => l_err_instance_id
                  ,NAME_IN => 'ID_PROF_TEAM_HIST _' || indx
                  ,value_in => rows_in(indx).ID_PROF_TEAM_HIST
                  ,validate_in => FALSE
                );
...
PROCEDURE upd (
   ...
   l_FLG_LEADER_n := sys.diutil.bool_to_int(FLG_LEADER_nin);
     ...
     FLG_LEADER = decode (l_FLG_LEADER_n,0,flg_leader_in, NVL (flg_leader_in, FLG_LEADER))
          WHERE
             ID_PROF_TEAM_HIST = id_prof_team_hist_in AND
...
PROCEDURE upd (
   ...
   l_UPDATE_INSTITUTION_n NUMBER(1);
   l_FLG_LEADER_n NUMBER(1);
      id_prof_team_hist_in PROF_TEAM_DET_HIST.ID_PROF_TEAM_HIST%TYPE;
      id_professional_in PROF_TEAM_DET_HIST.ID_PROFESSIONAL%TYPE;
   BEGIN
...
l_sql := 'UPDATE PROF_TEAM_DET_HIST SET '
     || ' ID_PROF_TEAM = decode (' || l_ID_PROF_TEAM_n || ',0,:id_prof_team_in, NVL (:id_prof_team_in, ID_PROF_TEAM)) '|| ','
     || ' ID_CATEGORY_SUB = decode (' || l_ID_CATEGORY_SUB_n || ',0,:id_category_sub_in, NVL (:id_category_sub_in, ID_CATEGORY_SUB)) '|| ','
     || ' FLG_AVAILABLE = decode (' || l_FLG_AVAILABLE_n || ',0,:flg_available_in, NVL (:flg_available_in, FLG_AVAILABLE)) '|| ','
     || ' FLG_STATUS = decode (' || l_FLG_STATUS_n || ',0,:flg_status_in, NVL (:flg_status_in, FLG_STATUS)) '|| ','
...
PROCEDURE upd (
      ...
      THEN
         ...
         FORALL i IN col_in.FIRST .. col_in.LAST
            UPDATE PROF_TEAM_DET_HIST SET
             ...
             WHERE
                ID_PROF_TEAM_HIST = l_ID_PROF_TEAM_HIST(i) AND
   ...
   BEGIN
      RETURN
         'BEGIN UPDATE PROF_TEAM_DET_HIST
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE PROF_TEAM_DET_HIST set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE PROF_TEAM_DET_HIST set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
   ...
   IS
   BEGIN
      DELETE FROM PROF_TEAM_DET_HIST
       WHERE
          ID_PROF_TEAM_HIST = id_prof_team_hist_in AND
          ID_PROFESSIONAL = id_professional_in
       RETURNING ROWID BULK COLLECT INTO rows_out
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM PROF_TEAM_DET_HIST
       WHERE ID_PROF_TEAM_HIST = id_prof_team_hist_in
      RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM PROF_TEAM_DET_HIST
       WHERE
          ID_CATEGORY_SUB = del_PRFT_DH_CATS_FK.id_category_sub_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_PRFT_DH_CATS_FK (
   ...
   BEGIN
      DELETE FROM PROF_TEAM_DET_HIST
       WHERE
          ID_PROFESSIONAL = del_PRFT_DH_PROF_FK.id_professional_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_PRFT_DH_PROF_FK (
   ...
   BEGIN
      DELETE FROM PROF_TEAM_DET_HIST
       WHERE
          ID_PROF_TEAM_HIST = del_PRFT_DH_PROF_TEAMH_FK.id_prof_team_hist_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_PRFT_DH_PROF_TEAMH_FK (
   ...
   BEGIN
      DELETE FROM PROF_TEAM_DET_HIST
       WHERE
          ID_PROF_TEAM = del_PRFT_DH_PROF_TEAM_FK.id_prof_team_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_PRFT_DH_PROF_TEAM_FK (
   ...
   BEGIN
      IF where_in IS NULL
      THEN
         RETURN 'DELETE FROM PROF_TEAM_DET_HIST';
      ELSE
         RETURN
            'DELETE FROM PROF_TEAM_DET_HIST WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   IS
   BEGIN
      prof_team_det_hist_inout.ID_PROF_TEAM_HIST := NULL;
      prof_team_det_hist_inout.ID_PROF_TEAM := NULL;
      prof_team_det_hist_inout.ID_PROFESSIONAL := NULL;
      prof_team_det_hist_inout.ID_CATEGORY_SUB := NULL;
      prof_team_det_hist_inout.FLG_AVAILABLE := NULL;
      prof_team_det_hist_inout.FLG_STATUS := NULL;
      prof_team_det_hist_inout.NOTES := NULL;
      prof_team_det_hist_inout.DT_BEGIN := NULL;
      prof_team_det_hist_inout.DT_END := NULL;
      prof_team_det_hist_inout.CREATE_USER := NULL;
      prof_team_det_hist_inout.CREATE_TIME := NULL;
      prof_team_det_hist_inout.CREATE_INSTITUTION := NULL;
      prof_team_det_hist_inout.UPDATE_USER := NULL;
      prof_team_det_hist_inout.UPDATE_TIME := NULL;
      prof_team_det_hist_inout.UPDATE_INSTITUTION := NULL;
      prof_team_det_hist_inout.FLG_LEADER := NULL;
   END initrec;

   FUNCTION initrec RETURN PROF_TEAM_DET_HIST%ROWTYPE
   IS
      l_prof_team_det_hist PROF_TEAM_DET_HIST%ROWTYPE;
   BEGIN
      RETURN l_prof_team_det_hist;
   END initrec;
   ...
   IS
        data PROF_TEAM_DET_HIST_tc;
   BEGIN
        select * bulk collect into data from PROF_TEAM_DET_HIST where rowid in (select * from table(rows_in));
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
