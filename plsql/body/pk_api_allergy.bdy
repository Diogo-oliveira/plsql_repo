/*-- Last Change Revision: $Rev: 2026658 $*/
/*-- Last Change by: $Author: mario.fernandes $*/
/*-- Date of last change: $Date: 2022-08-02 18:39:28 +0100 (ter, 02 ago 2022) $*/

CREATE OR REPLACE PACKAGE BODY pk_api_allergy IS
    ...
    */
    ...
    BEGIN
        ...
        g_error := 'PK_API_ALLERGY.GET_ALLERGY_LIST';
    
        RETURN pk_allergy.get_allergy_list(i_lang,
                                           i_prof,
                                           i_patient,
                                           i_episode,
                                           o_allergies,
                                           o_unawareness_active,
                                           o_unawareness_outdated,
                                           o_error);
    ...
    */
    ...
    BEGIN
        ...
        g_error := 'PK_API_ALLERGY.SET_ALLERGY/NORMAL';
    
        RETURN pk_allergy.set_allergy(i_lang                => i_lang,
                                      i_prof                => i_prof,
    ...
    */
    ...
    (
        ...
        l_record_pfh_origin VARCHAR2(1) := 'N';
    BEGIN
    
        g_error := 'PK_API_ALLERGY.SET_ALLERGY/NORMAL';
    
        RETURN pk_allergy.set_allergy_int(i_lang                   => i_lang,
                                          i_prof                   => i_prof,
                                          i_id_patient             => i_id_patient,
                                          i_id_episode             => i_id_episode,
                                          i_id_pat_allergy         => i_id_pat_allergy,
                                          ...
                                          i_dt_pat_allergy         => i_dt_pat_allergy,
                                          i_flg_cda_reconciliation => l_record_pfh_origin,
                                          o_id_pat_allergy         => o_id_pat_allergy,
                                          o_error                  => o_error);
    ...
    */
    ...
    BEGIN
        ...
        g_error := 'PK_API_ALLERGY.SET_ALLERGY/ARRAY';
    
        RETURN pk_allergy.set_allergy_array(i_lang                => i_lang,
                                            i_prof                => i_prof,
                                            i_id_patient          => i_id_patient,
                                            i_id_episode          => i_id_episode,
                                            i_id_pat_allergy      => i_id_pat_allergy,
    ...
    */
    ...
    (
        ...
        l_tbl_allergy          table_number := table_number();
        l_ttbl_symptom         table_table_number := table_table_number();
        l_tbl_symptom          table_number := table_number();
        l_tbl_allergy_severity table_number := table_number();
    
    BEGIN
    
        FOR i IN i_id_content_allergy.first .. i_id_content_allergy.last
        LOOP
            l_tbl_allergy.extend;
        
            IF i_id_content_allergy(i) IS NOT NULL
            THEN
            
                SELECT a.id_allergy
                  INTO l_tbl_allergy(i)
                  FROM allergy a
                 WHERE a.id_content = i_id_content_allergy(i)
                   AND a.flg_available = 'Y';
            
            ELSE
                l_tbl_allergy(i) := NULL;
            END IF;
        
        END LOOP;
    
        FOR i IN i_id_content_symptoms.first .. i_id_content_symptoms.last
        LOOP
        
            l_ttbl_symptom.extend;
            l_tbl_symptom := table_number();
        
            FOR j IN 1 .. i_id_content_symptoms(i).count
            LOOP
            
                IF i_id_content_symptoms(i) (j) IS NOT NULL
                THEN
                
                    l_tbl_symptom.extend;
                
                    SELECT a.id_allergy_symptoms
                      INTO l_tbl_symptom(j)
                      FROM allergy_symptoms a
                     WHERE a.id_content = i_id_content_symptoms(i) (j)
                       AND a.flg_status = 'A';
                
                ELSE
                    l_tbl_symptom(j) := NULL;
                END IF;
            
                l_ttbl_symptom(i) := l_tbl_symptom;
        ...
        FOR i IN i_id_content_allergy_severity.first .. i_id_content_allergy_severity.last
        LOOP
            l_tbl_allergy_severity.extend;
        
            IF i_id_content_allergy_severity(i) IS NOT NULL
            THEN
            
                SELECT a.id_allergy_severity
                  INTO l_tbl_allergy_severity(i)
                  FROM allergy_severity a
                 WHERE a.id_content = i_id_content_allergy_severity(i)
                   AND a.flg_status = 'A';
            ELSE
                l_tbl_allergy_severity(i) := NULL;
            END IF;
        
        END LOOP;
    
        g_error := 'PK_API_ALLERGY.SET_ALLERGY_INTF';
    
        RETURN pk_allergy.set_allergy_intf(i_lang                => i_lang,
                                           i_prof                => i_prof,
                                           i_id_patient          => i_id_patient,
                                           i_id_episode          => i_id_episode,
                                           i_id_pat_allergy      => i_id_pat_allergy,
                                           i_date_occur          => i_date_occur,
                                           i_id_allergy          => l_tbl_allergy,
                                           i_desc_allergy        => i_desc_allergy,
                                           i_notes               => i_notes,
                                           i_flg_status          => i_flg_status,
                                           i_flg_type            => i_flg_type,
                                           i_flg_aproved         => table_varchar(NULL),
                                           i_desc_aproved        => table_varchar(NULL),
                                           i_id_symptoms         => l_ttbl_symptom,
                                           i_id_allergy_severity => l_tbl_allergy_severity,
                                           i_flg_edit            => i_flg_edit,
                                           i_desc_edit           => i_edit_reason,
                                           i_cdr_call            => NULL,
                                           o_id_pat_allergy      => o_id_pat_allergy,
                                           o_error               => o_error);
    ...
    /**
    * This function cancels an array of allergies of a patient
    *
    * @param    IN  i_lang               Language ID
    * @param    IN  i_prof               Professional structure
    * @param    IN  i_id_pat_allergy     Array of id_pat_allergy
    ...
    */
    ...
    BEGIN
        g_error := 'PK_API_ALLERGY.CANCEL_ALLERGY_INTF';
    
        RETURN pk_allergy.cancel_allergy_intf(i_lang             => i_lang,
                                              i_prof             => i_prof,
                                              i_id_pat_allergy   => i_id_pat_allergy,
