...
IS
   ...
   BEGIN
     ...
     INSERT INTO EPIS_HIDRICS_BALANCE (
        ...
        BEGIN
           ...
           ELSE
              pk_alert_exceptions.raise_error (
                    ...
                    ,value2_in => l_name
                    ,name3_in => 'TABLE_NAME'
                    ,value3_in => 'EPIS_HIDRICS_BALANCE');
           END IF;
        END;
        END IF;
      ...
      THEN
        ...
        BEGIN
           ...
           pk_alert_exceptions.register_error (
             ...
             ,name3_in => 'TABLE_NAME', value3_in => 'EPIS_HIDRICS_BALANCE');
           IF l_name = 'EHBE_EHID_FK'
           THEN
   ...
   IS
      l_rec EPIS_HIDRICS_BALANCE%ROWTYPE := rec_in;
   BEGIN
      IF gen_pky_in THEN
         l_rec.ID_EPIS_HIDRICS_BALANCE := next_key (sequence_in);
      END IF;
      ins (
  ...
  ins (
   ...
   IS
     retval EPIS_HIDRICS_BALANCE.ID_EPIS_HIDRICS_BALANCE%TYPE;

   BEGIN
      IF sequence_in IS NULL
      THEN
         SELECT seq_EPIS_HIDRICS_BALANCE.NEXTVAL INTO retval FROM dual;
      ELSE
         EXECUTE IMMEDIATE
   ...
   EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
           ...
           ,name1_in => 'SEQUENCE'
           ,value1_in => NVL (sequence_in, 'seq_EPIS_HIDRICS_BALANCE')
           );
   END next_key;
   ...
   IS
        l_pky EPIS_HIDRICS_BALANCE.ID_EPIS_HIDRICS_BALANCE%TYPE := next_key;
   BEGIN
      ins (
         id_epis_hidrics_balance_in => l_pky,
         id_epis_hidrics_in => id_epis_hidrics_in,
         id_prof_close_in => id_prof_close_in,
         flg_status_in => flg_status_in,
         total_admin_in => total_admin_in,
      ...
      id_epis_hidrics_balance_out := l_pky;
   END ins;
   ...
   BEGIN
      ...
      flg_close_type_in => flg_close_type_in,
      total_times_in => total_times_in,
      id_epis_hidrics_balance_out => id_epis_hidrics_balance_out
      ,handle_error_in => handle_error_in
      , rows_out => rows_out
   );
   END ins;
   ...
   EXCEPTION
     ...
     THEN
       ...
       pk_alert_exceptions.raise_error(
          error_name_in => 'FORALL-INSERT-FAILURE'
          ,name1_in => 'TABLE_NAME'
          ,value1_in => 'EPIS_HIDRICS_BALANCE'
          ,name2_in => 'ROW_COUNT'
...
PROCEDURE upd (
   ...
   l_TOTAL_TIMES_n := sys.diutil.bool_to_int(TOTAL_TIMES_nin);
         ...
         UPDATE EPIS_HIDRICS_BALANCE SET
     ...
     TOTAL_TIMES = decode (l_TOTAL_TIMES_n,0,total_times_in, NVL (total_times_in, TOTAL_TIMES))
          WHERE
             ID_EPIS_HIDRICS_BALANCE = id_epis_hidrics_balance_in
...
PROCEDURE upd (
   ...
   l_FLG_CLOSE_TYPE_n NUMBER(1);
   l_TOTAL_TIMES_n NUMBER(1);
      id_epis_hidrics_balance_in EPIS_HIDRICS_BALANCE.ID_EPIS_HIDRICS_BALANCE%TYPE;
   BEGIN
...
l_sql := 'UPDATE EPIS_HIDRICS_BALANCE SET '
     || ' ID_EPIS_HIDRICS = decode (' || l_ID_EPIS_HIDRICS_n || ',0,:id_epis_hidrics_in, NVL (:id_epis_hidrics_in, ID_EPIS_HIDRICS)) '|| ','
     || ' ID_PROF_CLOSE = decode (' || l_ID_PROF_CLOSE_n || ',0,:id_prof_close_in, NVL (:id_prof_close_in, ID_PROF_CLOSE)) '|| ','
     || ' FLG_STATUS = decode (' || l_FLG_STATUS_n || ',0,:flg_status_in, NVL (:flg_status_in, FLG_STATUS)) '|| ','
     || ' TOTAL_ADMIN = decode (' || l_TOTAL_ADMIN_n || ',0,:total_admin_in, NVL (:total_admin_in, TOTAL_ADMIN)) '|| ','
...
PROCEDURE upd (
   ...
   BEGIN
      RETURN
         'BEGIN UPDATE EPIS_HIDRICS_BALANCE
  ...
  PROCEDURE increment_onecol (
   ...
   IS
   BEGIN
      EXECUTE IMMEDIATE 'BEGIN UPDATE EPIS_HIDRICS_BALANCE set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1') || ' RETURNING ROWID BULK COLLECT INTO :rows_out; END;'
      USING OUT rows_out;
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
   ...
   rows_out table_varchar;
   BEGIN
      EXECUTE IMMEDIATE 'UPDATE EPIS_HIDRICS_BALANCE set ' || colname_in || '=' || colname_in || ' + ' || nvl(increment_value_in,1) || ' WHERE ' || NVL (where_in, '1=1');
   EXCEPTION
      WHEN DUP_VAL_ON_INDEX
      THEN
        IF NOT handle_error_in THEN RAISE;
   ...
   IS
   BEGIN
      DELETE FROM EPIS_HIDRICS_BALANCE
       WHERE
          ID_EPIS_HIDRICS_BALANCE = id_epis_hidrics_balance_in
       RETURNING ROWID BULK COLLECT INTO rows_out
         ;
...
, rows_out => rows_out
   ...
   BEGIN
      DELETE FROM EPIS_HIDRICS_BALANCE
       WHERE
          ID_EPIS_HIDRICS = del_EHBE_EHID_FK.id_epis_hidrics_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_EHBE_EHID_FK (
   ...
   BEGIN
      DELETE FROM EPIS_HIDRICS_BALANCE
       WHERE
          ID_PROF_CLOSE = del_EHBE_PROF_FK.id_prof_close_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_EHBE_PROF_FK (
   ...
   BEGIN
      DELETE FROM EPIS_HIDRICS_BALANCE
       WHERE
          ID_UNIT_MEASURE = del_EHBE_UME_FK.id_unit_measure_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_EHBE_UME_FK (
   ...
   BEGIN
      DELETE FROM EPIS_HIDRICS_BALANCE
       WHERE
          ID_PROF_LAST_CHANGE = del_EHB_PROF_FK.id_prof_last_change_in
       RETURNING ROWID BULK COLLECT INTO rows_out;
   EXCEPTION
      WHEN e_check_constraint_failure
      THEN
...
del_EHB_PROF_FK (
   ...
   BEGIN
      IF where_in IS NULL
      THEN
         RETURN 'DELETE FROM EPIS_HIDRICS_BALANCE';
      ELSE
         RETURN
            'DELETE FROM EPIS_HIDRICS_BALANCE WHERE ' || where_in;
      END IF;
   END dyndelstr;
   ...
   IS
   BEGIN
      epis_hidrics_balance_inout.ID_EPIS_HIDRICS_BALANCE := NULL;
      epis_hidrics_balance_inout.ID_EPIS_HIDRICS := NULL;
      epis_hidrics_balance_inout.ID_PROF_CLOSE := NULL;
      epis_hidrics_balance_inout.FLG_STATUS := NULL;
      epis_hidrics_balance_inout.TOTAL_ADMIN := NULL;
      epis_hidrics_balance_inout.TOTAL_ELIM := NULL;
      epis_hidrics_balance_inout.ID_UNIT_MEASURE := NULL;
      epis_hidrics_balance_inout.DT_PREV_CLOSE_TSTZ := NULL;
      epis_hidrics_balance_inout.DT_CLOSE_BALANCE_TSTZ := NULL;
      epis_hidrics_balance_inout.DT_OPEN_TSTZ := NULL;
      epis_hidrics_balance_inout.DT_REGISTER_TSTZ := NULL;
      epis_hidrics_balance_inout.CREATE_USER := NULL;
      epis_hidrics_balance_inout.CREATE_TIME := NULL;
      epis_hidrics_balance_inout.CREATE_INSTITUTION := NULL;
      epis_hidrics_balance_inout.UPDATE_USER := NULL;
      epis_hidrics_balance_inout.UPDATE_TIME := NULL;
      epis_hidrics_balance_inout.UPDATE_INSTITUTION := NULL;
      epis_hidrics_balance_inout.ID_PROF_LAST_CHANGE := NULL;
      epis_hidrics_balance_inout.DT_EH_BALANCE := NULL;
      epis_hidrics_balance_inout.FLG_CLOSE_TYPE := NULL;
      epis_hidrics_balance_inout.TOTAL_TIMES := NULL;
   END initrec;
   ...
   IS
        data EPIS_HIDRICS_BALANCE_tc;
   BEGIN
        select * bulk collect into data from EPIS_HIDRICS_BALANCE where rowid in (select /*+opt_estimate(table,t,scale_rows=0.0000001))*/ * from table(rows_in) t);
        return data;
        EXCEPTION
      WHEN OTHERS THEN
        pk_alert_exceptions.raise_error (
