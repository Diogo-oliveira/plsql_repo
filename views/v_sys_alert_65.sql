CREATE OR REPLACE   VIEW V_SYS_ALERT_65 AS
SELECT /*+ opt_param('_OPTIMIZER_USE_FEEDBACK', 'FALSE') */
 ...
 WHERE v.id_sys_alert = 65
   ...
   AND ems.id_episode = v.id_episode
   AND d.id_episode = v.id_episode
   AND epr.id_episode = v.id_episode
   AND ems.flg_status = 'A'
   AND d.flg_status = 'A'
   AND epr.flg_type = 'D'
   AND epr.flg_status = 'F'
   AND epr.id_prof_comp = sys_context('ALERT_CONTEXT', 'i_prof')
   AND NOT EXISTS (SELECT 1
          FROM epis_mtos_param emp, mtos_param mpm, mtos_score mse
         WHERE emp.id_epis_mtos_score = ems.id_epis_mtos_score
           AND emp.id_mtos_param = mpm.id_mtos_param
           AND mpm.id_mtos_score = mse.id_mtos_score
           AND mpm.flg_fill_type = 'T'
           AND mse.flg_score_type = 'TRISS')
   AND NOT EXISTS (SELECT 1
