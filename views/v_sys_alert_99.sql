CREATE OR REPLACE   VIEW V_SYS_ALERT_99 AS
SELECT /*+ opt_param('_OPTIMIZER_USE_FEEDBACK', 'FALSE') */
 ...
 pk_alerts.get_alert_message(sys_context('ALERT_CONTEXT', 'i_lang'),
                             profissional(sys_context('ALERT_CONTEXT', 'i_prof'),
                                          ...
                                          sys_context('ALERT_CONTEXT', 'i_software')),
                             v.id_sys_alert,
                             pk_date_utils.get_elapsed_hours(sys_context('ALERT_CONTEXT', 'i_lang'),
                                                             pk_date_utils.add_to_ltstz(current_timestamp,
                                                                                        -to_number(pk_sysconfig.get_config('PATIENT_WAITING_X_HOUR',
 ...
 WHERE v.id_sys_alert = 99
   AND v.id_institution = sys_context('ALERT_CONTEXT', 'i_institution')
   AND v.flg_visible = 'Y'
   AND ((SELECT pk_utils.search_table_number(pk_hand_off_api.get_responsibles_id(sys_context('ALERT_CONTEXT', 'i_lang'),
                                                                                 profissional(sys_context('ALERT_CONTEXT',
                                                                                                          'i_prof'),
                                                                                              sys_context('ALERT_CONTEXT',
                                                                                                          'i_institution'),
           ...
           FROM dual) != -1 OR
       (SELECT pk_prof_utils.get_category(sys_context('ALERT_CONTEXT', 'i_lang'),
                                           profissional(sys_context('ALERT_CONTEXT', 'i_prof'),
                                                        sys_context('ALERT_CONTEXT', 'i_institution'),
                                                        sys_context('ALERT_CONTEXT', 'i_software')))
           FROM dual) = 'A')
   ...
   AND NOT EXISTS
 (SELECT 1
          FROM sys_alert_read re
         WHERE re.id_sys_alert_event = v.id_sys_alert_event
   ...
   AND current_timestamp >=
       v.dt_record +
       numtodsinterval((to_number(pk_sysconfig.get_config('PATIENT_WAITING_X_HOUR',
                                                          profissional(sys_context('ALERT_CONTEXT', 'i_prof'),
                                                                       sys_context('ALERT_CONTEXT', 'i_institution'),
                                                                       sys_context('ALERT_CONTEXT', 'i_software'))),
                                  'FM999999999999999999999999D9999999999',
                                  'NLS_NUMERIC_CHARACTERS= '', ''') / 24),
                       'DAY')
