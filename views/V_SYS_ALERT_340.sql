CREATE OR REPLACE VIEW V_SYS_ALERT_340 AS
...
join patient p          on p.id_patient  = vis.id_patient
join room r             on ei.id_room = r.id_room
join epis_prof_Resp epr on epr.id_epis_prof_resp = v.id_record and epr.id_prof_to = v.id_professional and e.id_episode = epr.id_episode
WHERE v.id_sys_alert = 340
...
AND NOT EXISTS (
    ...
    AND pk_alerts.get_config_flg_read(
          sys_context('ALERT_CONTEXT', 'i_lang'),
          profissional(sys_context('ALERT_CONTEXT', 'i_prof'),
                sys_context('ALERT_CONTEXT', 'i_institution'),
                sys_context('ALERT_CONTEXT', 'i_software')
                ),
