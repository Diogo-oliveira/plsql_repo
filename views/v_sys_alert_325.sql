CREATE OR REPLACE   VIEW V_SYS_ALERT_325 AS
with aux as
  (
  select
   xint.id_sys_alert
  ...
  , pk_message.get_message(xint.sys_lang, 'V_ALERT_M325')              sys_msg_ALERT_V00
  ...
  ,pk_hhc_core.is_coordinator(i_lang => xint.sys_lang, i_prof => xint.sys_lprof) sys_is_coordinator
  , 'R'  sys_hhc_requested
...
join epis_hhc_Req eh on eh.id_episode = e.id_episode
join visit v      on e.id_visit   = v.id_visit
join patient p    on p.id_patient = v.id_patient
left join room r     on r.id_room = ei.id_room
WHERE sae.id_sys_alert = 325
 AND sys_is_coordinator = 'Y'
 and eh.flg_status = aux.sys_hhc_requested
AND sae.id_institution = aux.sys_institution
and eh.id_epis_hhc_req = sae.id_record
AND NOT EXISTS (
  SELECT 1
