CREATE OR REPLACE VIEW v_daily_active_unpayed_01 AS
WITH sys_config AS
 (SELECT 1 rn,
         t.sys_lang sys_lang,
         ...
         CAST(pk_date_utils.trunc_insttimezone(t.sys_lprof,
                                               pk_date_utils.get_string_tstz(i_lang      => t.sys_lang,
                                                                             i_prof      => t.sys_lprof,
                                                                             i_timestamp => alert_context('l_dt_begin'),
                                                                             i_timezone  => '')) AS TIMESTAMP WITH LOCAL TIME ZONE) sys_dt_begin,
         CAST((pk_date_utils.trunc_insttimezone(t.sys_lprof,
                                                pk_date_utils.get_string_tstz(i_lang      => t.sys_lang,
                                                                              i_prof      => t.sys_lprof,
                                                                              i_timestamp => alert_context('l_dt_end'),
                                                                              i_timezone  => '') + INTERVAL '1' DAY)) AS
              TIMESTAMP WITH LOCAL TIME ZONE) sys_dt_end
...
SELECT sc.sys_lang             sys_lang,
 ...
 WHERE 0 = 0
   AND e.flg_status != 'C'
   AND sp.dt_target_tstz BETWEEN sc.sys_dt_begin AND sc.sys_dt_end
