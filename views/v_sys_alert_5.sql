CREATE OR REPLACE VIEW V_SYS_ALERT_5 AS
SELECT /*+ opt_param('_OPTIMIZER_USE_FEEDBACK', 'FALSE') */
 ...
 REPLACE(pk_message.get_message(sys_context('ALERT_CONTEXT', 'i_lang'), 'V_ALERT_M005'), '@1', v.replace1) message,
 NULL id_room,
 p.id_patient,
 ...
 id_prof_order
  FROM sys_alert_event v, episode e, patient p, epis_info ei, professional pf, room r
 WHERE v.id_sys_alert = 5
   AND v.id_institution = sys_context('ALERT_CONTEXT', 'i_institution')
   AND v.id_episode = e.id_episode
   AND e.id_patient = p.id_patient
   AND ((pf.id_professional = v.id_professional) OR
       (pf.id_professional = sys_context('ALERT_CONTEXT', 'i_prof') AND v.id_professional IS NULL))
   AND pk_date_utils.add_days_to_tstz(pk_date_utils.trunc_insttimezone(profissional(NULL, v.id_institution, NULL),
                                                                       current_timestamp,
                                                                       NULL),
                                      -pk_sysconfig.get_config('ALERT_EXPIRE_HARVEST_MOV',
                                                               v.id_institution,
                                                               v.id_software)) <
   ...
   AND v.dt_record <
       pk_date_utils.add_days_to_tstz(current_timestamp,
                                      - (pk_sysconfig.get_config('ALERT_HARVEST_MOV_TIMEOUT',
                                                                v.id_institution,
                                                                v.id_software) / (24 * 60)))
   AND EXISTS (SELECT 1
          FROM sys_alert_prof sap
         WHERE sap.id_sys_alert = 5
