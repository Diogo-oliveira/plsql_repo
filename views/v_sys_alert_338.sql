CREATE OR REPLACE VIEW v_sys_alert_338 AS
SELECT /*+ opt_param('_optimizer_use_feedback', 'false') */
 ...
 pk_message.get_message(sys_context('ALERT_CONTEXT', 'i_lang'), 'V_ALERT_M338') message,
 NULL id_room,
 p.id_patient,
 ...
 id_prof_order
  FROM sys_alert_event v, episode e, epis_info ei, patient p
 WHERE v.id_sys_alert = 338
   AND v.id_institution = sys_context('ALERT_CONTEXT', 'i_institution')
   AND v.flg_visible = 'Y'
   AND v.dt_record < current_timestamp
   AND NOT EXISTS (SELECT 1
          FROM epis_documentation ed
         WHERE ed.id_episode = v.id_episode
           AND ed.dt_creation_tstz > v.dt_record
           AND EXISTS (SELECT 1
                  FROM doc_area da
                 WHERE da.id_parent_doc_area = 36160
                   AND da.id_doc_area = ed.id_doc_area))
