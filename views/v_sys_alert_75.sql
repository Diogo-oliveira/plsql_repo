CREATE OR REPLACE VIEW v_sys_alert_75 AS
SELECT /*+ opt_param('_OPTIMIZER_USE_FEEDBACK', 'FALSE') */
 ...
 REPLACE(pk_message.get_message(sys_context('ALERT_CONTEXT', 'i_lang'), 'V_ALERT_M111'), '@1', v.replace1) message,
 NULL id_room,
 p.id_patient,
 ...
 WHERE v.id_sys_alert = 75
   ...
   AND v.flg_visible = 'Y'
      /* AND pk_date_utils.add_days_to_tstz(pk_date_utils.trunc_insttimezone(profissional(NULL, v.id_institution, NULL),
                                                                      current_timestamp,
                                                                      NULL),
                                     -pk_sysconfig.get_config('ALERT_EXPIRE_DRUG_REJECT',
                                                              v.id_institution,
                                                              v.id_software)) <=
   ...
   AND v.dt_record <
       pk_date_utils.add_days_to_tstz(current_timestamp,
                                      - (pk_sysconfig.get_config('ALERT_DRUG_REJECT_TIMEOUT1',
                                                                v.id_institution,
                                                                v.id_software) / (24 * 60)))
