CREATE OR REPLACE VIEW v_cmt_other_exam_available_s AS
WITH tmp AS
 (SELECT /*+ materialized */
   *
    FROM (SELECT DISTINCT id_cnt_other_exam,
            ...
            FROM (SELECT id_cnt_other_exam, code_exam, id_exam
                    FROM (SELECT e.id_content id_cnt_other_exam, e.code_exam, e.id_exam
                            FROM exam e
                  ...
                  UNION
                  SELECT id_cnt_other_exam, code_exam, id_exam
                    FROM (SELECT e.id_content id_cnt_other_exam, e.code_exam, e.id_exam
                            FROM exam e
                           WHERE e.flg_available = 'Y'
                             AND e.flg_type = 'E'
                  ...
                  MINUS
                  SELECT id_content id_cnt_other_exam, e.code_exam, e.id_exam
                    FROM exam e
                    JOIN v_cmt_other_exam_available avlb
                      ON e.id_content = avlb.id_cnt_other_exam) a
...
SELECT desc_other_exam,
       ...
       desc_room,
       id_room
  FROM (SELECT DISTINCT tmp.desc_translation  desc_other_exam,
          ...
          FROM exam e
          JOIN tmp tmp
            ON tmp.id_cnt_other_exam = e.id_content)
 ORDER BY 1;
