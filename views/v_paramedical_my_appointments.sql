CREATE OR REPLACE VIEW V_PARAMEDICAL_MY_APPOINTMENTS AS
SELECT s.id_schedule,
 ...
 WHERE sp.dt_target_tstz BETWEEN CAST((SELECT pk_date_utils.get_string_tstz(i_lang      => alert_context('l_lang'),
   ...
   AND ((sp.flg_state <> 'B' AND e.flg_ehr <> 'S') OR (sp.flg_state <> 'M'))
	 AND (NOT(e.flg_status = 'I' AND e.flg_ehr = 'S'))
   AND (nvl(ei.id_professional, spo.id_professional) = sys_context('ALERT_CONTEXT', 'l_prof_id') OR
       (pk_prof_follow.get_follow_episode_by_me(profissional(sys_context('ALERT_CONTEXT', 'l_prof_id'),
                                                              sys_context('ALERT_CONTEXT', 'l_prof_institution'),
                                                              sys_context('ALERT_CONTEXT', 'l_prof_software')),
                                                 ei.id_episode,
                                                 s.id_schedule) = 'Y') OR
       sys_context('ALERT_CONTEXT', 'l_prof_id') IN
