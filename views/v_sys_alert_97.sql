CREATE OR REPLACE   VIEW V_SYS_ALERT_97 AS
SELECT /*+ opt_param('_OPTIMIZER_USE_FEEDBACK', 'FALSE') */
 ...
 WHERE v.id_sys_alert = 97
   AND v.id_institution = sys_context('ALERT_CONTEXT', 'i_institution')
   AND v.flg_visible = 'Y'
   AND ((pk_inp_hidrics.check_prof_requested(sys_context('ALERT_CONTEXT', 'i_lang'),
                                             profissional(sys_context('ALERT_CONTEXT', 'i_prof'),
                                                          sys_context('ALERT_CONTEXT', 'i_institution'),
                                                          sys_context('ALERT_CONTEXT', 'i_software')),
                                             v.id_record) = 1) OR
       ((SELECT pk_patient.get_prof_resp(sys_context('ALERT_CONTEXT', 'i_lang'),
                                          profissional(sys_context('ALERT_CONTEXT', 'i_prof'),
                                                       sys_context('ALERT_CONTEXT', 'i_institution'),
                                                       sys_context('ALERT_CONTEXT', 'i_software')),
                                          e.id_patient,
