CREATE OR REPLACE VIEW V_SYS_ALERT_336 AS
WITH aux_sql AS
 (SELECT xint.id_sys_alert,
         ...
         pk_message.get_message(xint.sys_lang, 'BLOOD_PRODUCTS_T168') sys_msg_alert_bp_t168,
         pk_date_utils.trunc_insttimezone(xint.sys_lprof_i, current_timestamp, NULL) sys_dt_current,
         pk_sysconfig.get_config('BLOOD_TIME_LIMIT_POPUP_SHOW', xint.sys_institution, xint.sys_software) sys_cfg_limit_popup_show,
         pk_sysconfig.get_config('BLOOD_TRANSFUSION_TIME_LIMIT', xint.sys_institution, xint.sys_software) sys_cfg_transfusion_time_limit,
         nvl(to_number(pk_sysconfig.get_config('BLOOD_TRANSFUSION_TIME_LIMIT', xint.sys_institution, xint.sys_software)),
             0) / (24 * 60) sys_transfusion_time_limit_day
    FROM (SELECT 336 id_sys_alert,
...
SELECT /*+ opt_param('_OPTIMIZER_USE_FEEDBACK', 'FALSE') */
 ...
 pk_date_utils.to_char_insttimezone(profissional(aux_sql.sys_id_prof, aux_sql.sys_institution, aux_sql.sys_software),
                                    pk_date_utils.add_days_to_tstz(bpe.dt_bp_execution_tstz,
                                                                   sys_transfusion_time_limit_day),
                                    'YYYYMMDDHH24MISS') dt_req,
 decode(nvl(instr(pk_date_utils.get_elapsed_sysdate_tsz(aux_sql.sys_lang,
                                                        pk_date_utils.add_days_to_tstz(bpe.dt_bp_execution_tstz,
                                                                                       sys_transfusion_time_limit_day)),
                  ':'),
            0),
        0,
        pk_date_utils.get_elapsed_sysdate_tsz(aux_sql.sys_lang,
                                              pk_date_utils.add_days_to_tstz(bpe.dt_bp_execution_tstz,
                                                                             sys_transfusion_time_limit_day)),
        pk_date_utils.get_elapsed_sysdate_tsz(aux_sql.sys_lang,
                                              pk_date_utils.add_days_to_tstz(bpe.dt_bp_execution_tstz,
                                                                             sys_transfusion_time_limit_day)) || ' ' ||
        pk_message.get_message(aux_sql.sys_lang, 'ALERT_LIST_T003')) TIME,
 aux_sql.sys_msg_alert_bp_t168 || ' (' || (SELECT pk_translation.get_translation(aux_sql.sys_lang, ht.code_hemo_type)
                                             FROM dual) || ')' message,
 ei.id_room,
 sae.id_patient,
 ...
 WHERE sae.id_sys_alert = 336
   ...
   AND aux_sql.sys_cfg_limit_popup_show = 'Y'
   AND pk_date_utils.get_elapsed_minutes_abs_tsz(bpe.dt_bp_execution_tstz) > aux_sql.sys_cfg_transfusion_time_limit
   ...
   AND bpd.flg_status IN ('RS', 'RC', 'RW', 'WT', 'OT', 'CT', 'RT', 'O', 'H', 'DF')
