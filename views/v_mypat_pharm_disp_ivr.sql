CREATE OR REPLACE VIEW V_MYPAT_PHARM_DISP_IVR AS
SELECT z.id_episode,
       z.id_visit,
       z.id_patient,
       z.flg_status_e,
  ...
  FROM (SELECT epis.id_episode,
               ...
               gt.drug_presc,
               gt.drug_req,
               --nvl(vpd.dt_last_update, vpd.dt_create) pha_date,
        ...
        -----
          JOIN v_pha_dispense vpd
           ...
           AND vpd.flg_prepared_iv_room = 'Y'
           AND vpd.id_status IN
          ...
          JOIN presc p
            ON (p.id_presc = vpd.id_task AND p.id_workflow IN (13, 20, 15) AND p.id_status NOT IN (62, 76, 77, 70) AND
               p.id_workflow IN
