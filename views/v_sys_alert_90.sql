CREATE OR REPLACE   VIEW V_SYS_ALERT_90 AS
SELECT /*+ opt_param('_OPTIMIZER_USE_FEEDBACK', 'FALSE') */
 ...
 id_prof_order
  ...
  JOIN epis_complication ec
    ON ec.id_epis_complication = v.id_record
  JOIN sys_alert sa
 ...
 WHERE v.id_sys_alert = 90
   AND v.id_institution = sys_context('ALERT_CONTEXT', 'i_institution')
   AND v.flg_visible = 'Y'
   AND ec.flg_status_req = 'R' --Requested
   AND ((current_timestamp -
       to_number(pk_sysconfig.get_config('EPIS_COMP_TIME_LIMIT',
                                           sys_context('ALERT_CONTEXT', 'i_institution'),
                                           sys_context('ALERT_CONTEXT', 'i_software'))) / 1440) <= v.dt_record OR
       v.dt_record IS NULL)
   AND EXISTS (SELECT 1
          FROM professional pf
         WHERE pf.id_professional = sys_context('ALERT_CONTEXT', 'i_prof'))
