CREATE OR REPLACE FORCE VIEW ALERT.V_CMT_DIET_CATALOGUE_S AS
SELECT DISTINCT desc_diet,
  ...
  FROM (SELECT (SELECT pk_translation.get_translation(sys_context('ALERT_CONTEXT', 'ID_LANGUAGE'), a.code_diet)
               ...
               a.id_diet
          FROM diet a
          JOIN TABLE(pk_translation.get_search_translation(sys_context('ALERT_CONTEXT', 'ID_LANGUAGE'), sys_context('ALERT_CONTEXT', 'SEARCH_TEXT'), 'DIET.CODE_DIET')) t
            ON t.code_translation = a.code_diet
          LEFT JOIN diet b
