CREATE OR REPLACE FORCE VIEW ALERT.V_CMT_LAB_TEST_CLIN_QUEST AS
WITH temp AS
 (SELECT /*+ MATERIALIZED */
  DISTINCT b.id_analysis, b.id_sample_type, e.id_exam_cat
...
SELECT "DESC_LAB_TEST_SAMPLE",
       ...
       "FLG_EXTERIOR",
       "ID_UNIT_MEASURE"
  FROM (SELECT tt.desc_translation   AS desc_lab_test_sample,
               c.id_content          AS id_cnt_lab_test_sample_type,
               ttt.desc_translation  AS desc_clinical_question,
               tttt.desc_translation AS desc_response,
               qr.id_content         AS id_cnt_question_response,
               a.flg_type,
               a.flg_mandatory,
               a.rank,
               a.flg_time,
               ...
               a.flg_exterior,
               a.id_unit_measure
          FROM analysis_questionnaire a
          JOIN questionnaire b
            ON b.id_questionnaire = a.id_questionnaire
           AND b.flg_available = 'Y'
          JOIN questionnaire_response qr
            ON qr.id_questionnaire = a.id_questionnaire
           AND qr.id_response = a.id_response
          JOIN analysis_sample_type c
            ON c.id_analysis = a.id_analysis
           AND c.id_sample_type = a.id_sample_type
           AND c.flg_available = 'Y'
          JOIN temp tmp
            ON c.id_analysis = tmp.id_analysis
           AND c.id_sample_type = tmp.id_sample_type
          JOIN response d
            ON d.id_response = a.id_response
           AND d.flg_available = 'Y'
          JOIN v_cmt_translation_ast tt
            ON tt.code_translation = c.code_analysis_sample_type
 ...
 WHERE desc_lab_test_sample IS NOT NULL
