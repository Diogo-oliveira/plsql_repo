CREATE OR REPLACE FORCE VIEW ALERT.V_CMT_PROCEDURE_AVAILABLE_S AS
WITH tmp AS
 (SELECT /*+ MATERIALIZED */
   *
    FROM (SELECT DISTINCT id_cnt_procedure,
            ...
            FROM (SELECT id_cnt_procedure, code_intervention, id_intervention
                    FROM (SELECT i.id_content id_cnt_procedure, i.code_intervention, i.id_intervention
                            FROM intervention i
                  ...
                  UNION
                  SELECT id_cnt_procedure, code_intervention, id_intervention
                    FROM (SELECT i.id_content id_cnt_procedure, i.code_intervention, i.id_intervention
                            FROM intervention i
                           WHERE i.flg_status = 'A'
                             AND i.flg_category_type = 'P'
                  ...
                  MINUS
                  SELECT id_content AS id_cnt_procedure, i.code_intervention, i.id_intervention
                    FROM intervention i
                    JOIN v_cmt_procedure_available avlb
                      ON i.id_content = avlb.id_cnt_procedure) a
...
SELECT desc_procedure,
       ...
       flg_priority
  FROM (SELECT DISTINCT tmp.desc_translation AS desc_procedure,
          ...
          FROM intervention i
          JOIN tmp tmp
            ON tmp.id_cnt_procedure = i.id_content
