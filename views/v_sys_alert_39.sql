CREATE OR REPLACE VIEW V_SYS_ALERT_39 AS
SELECT /*+ opt_param('_OPTIMIZER_USE_FEEDBACK', 'FALSE') */
 ...
 NULL id_schedule,
 decode(x.flg_type, 'E', 11, 10) id_sys_shortcut,
 ...
 id_prof_order
  FROM sys_alert_event v,
       ...
       room r,
       (SELECT DISTINCT e.*
          FROM exam e, exam_cat_dcs ecdcs
         WHERE e.id_exam_cat = ecdcs.id_exam_cat
           AND EXISTS (SELECT 1
                  FROM prof_dep_clin_serv pdcs
                 WHERE pdcs.id_professional = sys_context('ALERT_CONTEXT', 'i_prof')
                   AND pdcs.flg_status = 'S'
                   AND pdcs.id_institution = sys_context('ALERT_CONTEXT', 'i_institution')
                   AND pdcs.id_dep_clin_serv = ecdcs.id_dep_clin_serv)) x
 WHERE v.id_sys_alert = 39
