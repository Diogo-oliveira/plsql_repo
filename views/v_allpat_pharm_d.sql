CREATE OR REPLACE VIEW V_ALLPAT_PHARM_D AS
SELECT id_episode,
       id_visit,
       id_patient,
       flg_status_e,
  ...
  FROM (SELECT epis.id_episode,
          ...
          JOIN v_pha_dispense phd
            ON phd.id_episode = epis.id_episode
           AND phd.id_patient = epis.id_patient
          ...
          JOIN presc p
            ON (p.id_presc = phd.id_task AND p.id_workflow IN (13, 20, 15) AND p.id_status NOT IN (62, 76, 77, 70) AND
               p.id_workflow IN
