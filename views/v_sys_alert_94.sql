CREATE OR REPLACE VIEW V_SYS_ALERT_94 AS
SELECT /*+ opt_param('_OPTIMIZER_USE_FEEDBACK', 'FALSE') */
 ...
 REPLACE(pk_message.get_message(sys_context('ALERT_CONTEXT', 'i_lang'), 'V_ALERT_M094'), '@1', v.replace1) message,
 ei.id_room,
 v.id_patient,
 ...
 WHERE v.id_sys_alert = 94
   ...
   AND ot.id_category =
       pk_prof_utils.get_id_category(sys_context('ALERT_CONTEXT', 'i_lang'),
                                     profissional(sys_context('ALERT_CONTEXT', 'i_prof'),
                                                  sys_context('ALERT_CONTEXT', 'i_institution'),
                                                  sys_context('ALERT_CONTEXT', 'i_software')))
   AND v.dt_record >
       current_timestamp - pk_sysconfig.get_config('FOLLOW_UP_REQUEST_NEEDS_ACCEPTANCE_ALERT_TIMEOUT',
                                                   sys_context('ALERT_CONTEXT', 'i_institution'),
                                                   sys_context('ALERT_CONTEXT', 'i_software')) / 24
