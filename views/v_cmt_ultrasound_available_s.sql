CREATE OR REPLACE VIEW v_cmt_ultrasound_available_s AS
WITH tmp AS
 (SELECT /*+ materialized */
   *
    FROM (SELECT DISTINCT id_cnt_ultrasound,
            ...
            FROM (SELECT id_cnt_ultrasound, code_exam, id_exam
                    FROM (SELECT e.id_content id_cnt_ultrasound, e.code_exam, e.id_exam
                            FROM exam e
                  ...
                  UNION
                  SELECT id_cnt_ultrasound, code_exam, id_exam
                    FROM (SELECT e.id_content id_cnt_ultrasound, e.code_exam, e.id_exam
                            FROM exam e
                           WHERE e.flg_available = 'Y'
                             AND e.flg_type = 'I'
                  ...
                  MINUS
                  SELECT id_content  id_cnt_ultrasound, e.code_exam, e.id_exam
                    FROM exam e
                    JOIN v_cmt_ultrasound_available avlb
                      ON e.id_content = avlb.id_cnt_ultrasound) a
...
SELECT desc_ultrasound,
       ...
       flg_priority,
       nvl(flg_bypass_validation, 'N') flg_bypass_validation,
       desc_room,
       id_room
  FROM (SELECT DISTINCT tmp.desc_translation   desc_ultrasound,
                        tmp.desc_alias         desc_alias,
                        tmp.id_cnt_ultrasound,
                        NULL                   rank,
                        NULL                   flg_execute,
                        ...
                        NULL                   flg_chargeable,
                        NULL                   flg_priority,
                        NULL                   flg_bypass_validation,
                        NULL                   desc_room,
                        NULL                   id_room
          FROM exam e
          JOIN tmp tmp
            ON tmp.id_cnt_ultrasound = e.id_content)
 ORDER BY 1;
