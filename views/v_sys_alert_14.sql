CREATE OR REPLACE VIEW v_sys_alert_14 AS
SELECT /*+ opt_param('_OPTIMIZER_USE_FEEDBACK', 'FALSE') */
 ...
 REPLACE(REPLACE(pk_message.get_message(sys_context('ALERT_CONTEXT', 'i_lang'), 'V_ALERT_M014'), '@1', v.replace1),
         '@2',
         v.replace2) message,
 NULL id_room,
 e.id_patient,
 ...
 WHERE v.id_sys_alert = 14
   ...
   AND pk_date_utils.add_days_to_tstz(pk_date_utils.trunc_insttimezone(profissional(NULL, v.id_institution, NULL),
                                                                       current_timestamp,
                                                                       NULL),
                                      -pk_sysconfig.get_config('ALERT_EXPIRE_DRUG_MOV', v.id_institution, v.id_software)) <
   ...
   AND v.dt_record <
       pk_date_utils.add_days_to_tstz(current_timestamp,
                                      - (pk_sysconfig.get_config('ALERT_DRUG_MOV_TIMEOUT',
                                                                v.id_institution,
                                                                v.id_software) / (24 * 60)))
   AND v.id_episode = e.id_episode
   AND e.id_patient = p.id_patient
   AND (v.id_professional = sys_context('ALERT_CONTEXT', 'i_prof') OR v.id_professional IS NULL)
   AND EXISTS (SELECT 1
