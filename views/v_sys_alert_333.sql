CREATE OR REPLACE   VIEW V_SYS_ALERT_333 AS
WITH aux_sql AS
 (SELECT 333 id_sys_alert,
         pk_message.get_message(sys_lang, 'SYS_ALERT_333_T001') sys_msg,
         pk_message.get_message(sys_lang, 'ALERT_LIST_T003') sys_list_t003,
         pk_hhc_core.is_coordinator(i_lang => sys_lang, i_prof => sys_lprof) sys_is_coordinator,
         'V' k_sch_status_pending_app,
         312 k_hhc_software,
		 99 sys_hhc_process,
         x01.sys_lprof,
         x01.sys_lang,
...
SELECT /*+ opt_param('_OPTIMIZER_USE_FEEDBACK', 'FALSE') */
 ...
 id_prof_order
  ...
  JOIN episode e  ON e.id_episode = ei.id_episode
  join episode hc on hc.id_episode = e.id_prev_episode
  JOIN visit vis
 ...
 WHERE 0 = 0
   ...
   and hc.id_epis_type = aux.sys_hhc_process
   AND v.id_software = aux.sys_software
   AND v.flg_visible = 'Y'
   AND s.dt_begin_tstz > aux.dt_limit
   AND s.flg_status = aux.k_sch_status_pending_app
   AND e.flg_ehr = 'S'
   AND aux.sys_is_coordinator = 'Y'
