CREATE OR REPLACE VIEW V_SYS_ALERT_44 AS
   ...
   AND NOT EXISTS (SELECT 1
         ...
         WHERE sar.id_sys_alert_event = v.id_sys_alert_event
           ...
           AND pk_alerts.get_config_flg_read(sys_context('ALERT_CONTEXT', 'i_lang'),
                                             ...
                                             v.id_sys_alert,
                                             NULL) = 'N')
   AND ((v.id_clinical_service IS NOT NULL --Devolve o alerta para o prof que requitou ou todos os não médicos do serviço do episódio
       AND (pf.id_professional IN (SELECT DISTINCT pdcs.id_professional
                                      ...
                                      WHERE dcs.id_clinical_service = v.id_clinical_service
                                        ...
                                        AND pc.id_institution = v.id_institution
                                        AND c.id_category = pc.id_category
                                        AND c.flg_type != 'D') OR v.id_professional = pf.id_professional)) OR
