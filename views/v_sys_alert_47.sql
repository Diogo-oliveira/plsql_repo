CREATE OR REPLACE   VIEW V_SYS_ALERT_47 AS
SELECT /*+ opt_param('_OPTIMIZER_USE_FEEDBACK', 'FALSE') */
 ...
 decode(tci.obs_time,
        0,
        REPLACE((SELECT pk_message.get_message(sys_context('ALERT_CONTEXT', 'i_lang'), 'V_ALERT_M046')
                  FROM dual),
                '@1',
                (SELECT pk_translation.get_translation(sys_context('ALERT_CONTEXT', 'i_lang'), tco.code_triage_color)
                   FROM dual)),
        pk_alerts.get_alert_message(sys_context('ALERT_CONTEXT', 'i_lang'),
                                    profissional(sys_context('ALERT_CONTEXT', 'i_prof'),
                                                 ...
                                                 sys_context('ALERT_CONTEXT', 'i_software')),
                                    v.id_sys_alert,
                                    (SELECT pk_translation.get_translation(sys_context('ALERT_CONTEXT', 'i_lang'),
                                                                           tco.code_triage_color)
                                       FROM dual),
                                    tci.obs_time)) message,
 NULL id_room,
 e.id_patient,
 ...
 id_prof_order
  FROM sys_alert_event v, episode e, epis_info ei, patient p, room r, triage_color tco, triage_color_time_inst tci
 WHERE v.id_sys_alert = 47
   AND v.id_institution = sys_context('ALERT_CONTEXT', 'i_institution')
   AND v.flg_visible = 'Y'
   AND e.flg_ehr = 'N'
   AND pk_date_utils.add_days_to_tstz(pk_date_utils.trunc_insttimezone(profissional(NULL, v.id_institution, NULL),
                                                                       current_timestamp,
                                                                       NULL),
                                      -pk_sysconfig.get_config('ALERT_EXPIRE_TRIAGE', v.id_institution, v.id_software)) <
       pk_date_utils.trunc_insttimezone(profissional(NULL, v.id_institution, NULL), v.dt_record, NULL)
   AND v.dt_record < pk_date_utils.add_days_to_tstz(current_timestamp, - (tci.obs_time / (24 * 60)))
 ...
 (SELECT 1
         ...
         WHERE sar.id_sys_alert_event = v.id_sys_alert_event
           ...
           AND pk_alerts.get_config_flg_read(sys_context('ALERT_CONTEXT', 'i_lang'),
                                             ...
                                             v.id_sys_alert,
                                             NULL) = 'N')
   AND tco.id_triage_color = ei.id_triage_color
   AND tco.id_triage_color = tci.id_triage_color(+)
