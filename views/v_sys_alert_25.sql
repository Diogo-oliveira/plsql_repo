CREATE OR REPLACE VIEW V_SYS_ALERT_25 AS
SELECT /*+ opt_param('_OPTIMIZER_USE_FEEDBACK', 'FALSE') */
 ...
 REPLACE(REPLACE(pk_message.get_message(sys_context('ALERT_CONTEXT', 'i_lang'), 'V_ALERT_M008'), '@1', v.replace1),
         '@2',
         v.replace2) message,
 NULL id_room,
 e.id_patient,
 ...
 WHERE v.id_sys_alert = 25
   ...
   AND pk_date_utils.add_days_to_tstz(pk_date_utils.trunc_insttimezone(profissional(NULL, v.id_institution, NULL),
                                                                       current_timestamp,
                                                                       NULL),
                                      -pk_sysconfig.get_config('ALERT_EXPIRE_IVFLUIDS', v.id_institution, v.id_software)) <
   ...
   AND v.dt_record <
       pk_date_utils.add_days_to_tstz(current_timestamp,
                                      - (pk_sysconfig.get_config('ALERT_IVFLUIDS_TIMEOUT',
                                                                v.id_institution,
                                                                v.id_software) / (24 * 60)))
   ...
   AND NOT EXISTS (SELECT 1
         ...
         WHERE sar.id_sys_alert_event = v.id_sys_alert_event
           ...
           AND pk_alerts.get_config_flg_read(sys_context('ALERT_CONTEXT', 'i_lang'),
                                             ...
                                             v.id_sys_alert,
                                             NULL) = 'N')
   AND ((v.id_clinical_service IS NOT NULL --Devolve o alerta para o prof que requitou ou todos os elementos do serviço do episódio
       AND (pf.id_professional IN (SELECT DISTINCT pdcs.id_professional
                                     ...
                                     --                  AND c.flg_type != 'D' --Devolve também para todos os médicos do serviço do episódio
                                     ) OR v.id_professional = pf.id_professional)) OR
       (v.id_clinical_service IS NULL AND v.id_professional = pf.id_professional))
