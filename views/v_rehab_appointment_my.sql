CREATE OR REPLACE VIEW v_rehab_appointment_my AS
SELECT s.id_schedule,
 ...
 WHERE s.flg_sch_type = sys_context('ALERT_CONTEXT', 'l_flg_sch_type_cr')
   ...
   AND pk_grid.get_schedule_real_state(sp.flg_state, e.flg_ehr) != 'M'
   AND s.flg_status != 'C'
   AND s.id_instit_requested = sys_context('ALERT_CONTEXT', 'l_prof_institution')
   AND scr.id_professional = sys_context('ALERT_CONTEXT', 'l_prof_id')
