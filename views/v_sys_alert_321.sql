CREATE OR REPLACE VIEW V_SYS_ALERT_321 AS
SELECT /*+ opt_param('_OPTIMIZER_USE_FEEDBACK', 'FALSE') */
 ...
 decode(nvl(instr((SELECT pk_date_utils.get_elapsed_sysdate_tsz(sys_context('ALERT_CONTEXT', 'i_lang'), sae.dt_record)
        ...
        (SELECT pk_date_utils.get_elapsed_sysdate_tsz(sys_context('ALERT_CONTEXT', 'i_lang'), sae.dt_record)
           FROM dual) || ' ' || (SELECT pk_message.get_message(sys_context('ALERT_CONTEXT', 'i_lang'), 'ALERT_LIST_T003')
                                   FROM dual)) TIME,
 REPLACE((SELECT pk_message.get_message(sys_context('ALERT_CONTEXT', 'i_lang'), 'PRESC_ERROR_ALERT_T001')
           FROM dual),
         '@1',
         sae.replace1) message,
 ei.id_room,
 sae.id_patient,
 ...
 (SELECT pk_fast_track.get_fast_track_icon(sys_context('ALERT_CONTEXT', 'i_lang'),
                                           ...
                                           ei.id_triage_color,
                                           NULL,
                                           pk_transfer_institution.check_epis_transfer(ei.id_episode))
 ...
 id_prof_order
  FROM (SELECT *
         ...
         WHERE id_sys_alert = 321
           AND id_institution = sys_context('ALERT_CONTEXT', 'i_institution')
           AND EXISTS (SELECT 1
                  FROM v_episode_act vea
                 WHERE id_episode = vea.id_episode)) sae
 ...
 WHERE NOT EXISTS (SELECT 1
   ...
   AND (ei.id_professional = sys_context('ALERT_CONTEXT', 'i_prof') OR
       sae.id_professional = sys_context('ALERT_CONTEXT', 'i_prof'))
