...
-- CHANGE DATE: 08/07/2009 17:39
-- CHANGE REASON: [ALERT-32069] Making appointments
CREATE OR REPLACE TRIGGER B_IU_SCD_AUDIT 
BEFORE INSERT OR UPDATE ON SCH_COMBI_DETAIL
FOR EACH ROW
DECLARE
...
EXCEPTION
    WHEN OTHERS THEN
        PK_ALERTLOG.log_error('B_IU_SCD_AUDIT-'||sqlerrm);
END B_IU_SCD_AUDIT;
