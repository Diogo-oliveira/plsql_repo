...
CREATE OR REPLACE TRIGGER B_IU_SR_POS_SCHED_HIST_AUDIT
BEFORE INSERT OR UPDATE ON ALERT.SR_POS_SCHEDULE_HIST
FOR EACH ROW
BEGIN
...
EXCEPTION
    WHEN OTHERS THEN
        alertlog.pk_alertlog.log_error('B_IU_SR_POS_SCHED_HIST_AUDIT-' || SQLERRM);
END b_iu_sr_pos_sched_hist_audit;

drop TRIGGER b_iu_sr_pos_sched_hist_audit;
/
